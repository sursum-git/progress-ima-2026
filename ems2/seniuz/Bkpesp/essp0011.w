...
*******************************************************************************/
{include/i-prgvrs.i ESSP0011 2.04.00.000}
...
DEFINE VARIABLE h_q01es001 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es001 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-consim /* Amostras por Fam¡lia */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Amostras por Fam¡lia */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es001.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es001 ).
       RUN set-position IN h_v01es001 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 14.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es001.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es001.w,
                     ProgVaPara = esgo/g01es001.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es001 ).
       RUN set-position IN h_q01es001 ( 1.50 , 61.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.13 , 7.72 ) */

       /* Links to SmartViewer h_v01es001. */
       RUN add-link IN adm-broker-hdl ( h_q01es001 , 'Record':U , h_v01es001 ).

       /* Links to SmartQuery h_q01es001. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es001 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es001 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es001 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es001 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0011" "2.04.00.000"}
