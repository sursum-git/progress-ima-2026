...
&Scoped-Define ENABLED-FIELDS teste-cgoma.fm-codigo teste-cgoma.data-teste ~
teste-cgoma.num-seq 
...
teste-cgoma.data-teste teste-cgoma.num-seq 
...
DEFINE FRAME d-vapara
     teste-cgoma.fm-codigo AT ROW 1.17 COL 15.29 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .88
     fi-desc-familia AT ROW 1.17 COL 26 COLON-ALIGNED NO-LABEL
     teste-cgoma.data-teste AT ROW 2.17 COL 15.29 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     teste-cgoma.num-seq AT ROW 3.17 COL 15.29 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 2.29 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Tabela de Carga Goma"
...
ON GO OF FRAME d-vapara /* V  Para Tabela de Carga Goma */
DO:
  find teste-cgoma no-lock 
      where teste-cgoma.fm-codigo  = input frame {&frame-name} teste-cgoma.fm-codigo  and
            teste-cgoma.data-teste = input frame {&frame-name} teste-cgoma.data-teste and 
            teste-cgoma.num-seq    = input frame {&frame-name} teste-cgoma.num-seq  no-error.
  if  not avail teste-cgoma then do:
      {utp/ut-table.i espec teste-cgoma 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(teste-cgoma).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Tabela de Carga Goma */
DO:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL teste-cgoma.fm-codigo d-vapara
ON ENTRY OF teste-cgoma.fm-codigo IN FRAME d-vapara /* Familia */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL teste-cgoma.fm-codigo d-vapara
ON LEAVE OF teste-cgoma.fm-codigo IN FRAME d-vapara /* Familia */
DO:
  FIND familia WHERE familia.fm-codigo = INPUT FRAME {&FRAME-NAME} teste-cgoma.fm-codigo NO-LOCK NO-ERROR.
  IF AVAIL familia THEN
     ASSIGN fi-desc-familia:SCREEN-VALUE IN FRAME {&FRAME-NAME} = familia.descricao.
  ELSE DO:
     MESSAGE "Familia inv lida." VIEW-AS ALERT-BOX.                              
     return no-apply. 
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-desc-familia 
      WITH FRAME d-vapara.
  IF AVAILABLE teste-cgoma THEN 
    DISPLAY teste-cgoma.fm-codigo teste-cgoma.data-teste teste-cgoma.num-seq 
      WITH FRAME d-vapara.
  ENABLE teste-cgoma.fm-codigo teste-cgoma.data-teste teste-cgoma.num-seq bt-ok 
