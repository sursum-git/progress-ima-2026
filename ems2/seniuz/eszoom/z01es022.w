...
*******************************************************************************/
{include/i-prgvrs.i Z01ES022 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS bt-implantar bt-ok bt-cancela bt-ajuda ~
rt-button 
...
DEFINE VARIABLE h_b01es022 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es022 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b03es022 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b04es022 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 14.92 COL 1.57
     bt-ok AT ROW 16.29 COL 3
     bt-cancela AT ROW 16.29 COL 14
     bt-ajuda AT ROW 16.29 COL 79
     rt-button AT ROW 16.04 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Movimentos de Manuten‡Æo"
         HEIGHT             = 16.5
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Movimentos de Manuten‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Movimentos de Manuten‡Æo */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'N£mero OS|Setor|Data|M quina' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       ...
       RUN add-link IN adm-broker-hdl ( h_b02es022 , 'State':U , h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es022.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es022 ).
       RUN set-position IN h_b01es022 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 87.00 ) */

       /* Links to SmartBrowser h_b01es022. */
       RUN add-link IN adm-broker-hdl ( h_b01es022 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es022.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es022 ).
       RUN set-position IN h_b02es022 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 87.14 ) */

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03es022.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03es022 ).
       RUN set-position IN h_b03es022 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 87.14 ) */

       /* Links to SmartBrowser h_b03es022. */
       RUN add-link IN adm-broker-hdl ( h_b03es022 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 3 */

    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b04es022.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b04es022 ).
       RUN set-position IN h_b04es022 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 87.00 ) */

       /* Links to SmartBrowser h_b04es022. */
       RUN add-link IN adm-broker-hdl ( h_b04es022 , 'State':U , THIS-PROCEDURE ).
