...
&global-define ProgramName bcx0003a
/************************************************************/
...
DEF VAR DE-QT-TOTAL       AS DEC  DECIMALS 4  NO-UNDO.
DEF VAR DE-QT-lido        AS DEC  DECIMALS 4  NO-UNDO.
...
DEF VAR de-qt-at          AS DEC FORMAT "999999.9999" NO-UNDO INIT 0.
DEF VAR de-qt-pd          AS DEC FORMAT "999999.9999" NO-UNDO INIT 0.
...
DEF STREAM e-arquivo.
DEF STREAM s-bat.

/***************************************** Frames Inicio ******************************************/
/* Definicao da Frame01 ---                                 */
&global-define Frame01Name   Frame01
&global-define Frame01Defs   'EMBARCAR'                At Row 01 Col 01          ~
...
&global-define Frame02Defs   'EMBARCAR'           At Row 01 Col 01          ~
                             'EMBARQUE:'          At Row 02 Col 01          ~
...
Procedure GravaCamposFrame02:
    ...
    IF LENGTH(c-item) <> 11 THEN DO.
       FIND ob-etiqueta WHERE
            ob-etiqueta.cod-estabel = '5' AND
            ob-etiqueta.num-etiqueta = INT(SUBSTR(c-item,1,9)) NO-LOCK NO-ERROR.
       IF AVAIL ob-etiqueta THEN
          ASSIGN c-item = ob-etiqueta.progressivo.
    END.
    ...
    FIND bc-etiqueta WHERE 
         bc-etiqueta.progressivo = c-item SHARE-LOCK NO-ERROR.
    ASSIGN bc-etiqueta.cod-estado = 2
           bc-etiqueta.cdd-embarq = ttwork.num-aviso-embarque
           bc-etiqueta.nr-embarque = ttwork.num-aviso-embarque.
...
/*          MESSAGE "Embarque: " TT-CAMPO.INTEIRO VIEW-AS ALERT-BOX. */
        END.

        IF  TT-CAMPO.CAMPO = "NR-RESUMO" THEN DO:
            ASSIGN I-NR-RESUMO = TT-CAMPO.INTEIRO.
/*             MESSAGE "Resumo: " TT-CAMPO.INTEIRO VIEW-AS ALERT-BOX. */
        END.
    END.
    ...
    FOR EACH TT-CAMPO:
         ...
         AND TT-CAMPO.INTEIRO    > 2 /* IMPRESSA */
         AND tt-campo.inteiro    <> 7 
         THEN DO:
    ...
    IF  DE-QT-TOTAL = 0 THEN DO:
        Assign vLogErro = YES
        C-MSG = "ITEM (" + ttwork.cod-item + ") DA ETIQUETA (" + c-item + ") NAO ENCONTRADO NO EMBARQUE".
    ...
    ASSIGN de-qt-at = ttwork.qtd-embarcada + DE-QT-LIDO
           de-qt-pd = DE-QT-TOTAL.
...
PROCEDURE pi-ler-coletor.
   ...
   ASSIGN c-arq-coletor = SESSION:TEMP-DIRECTORY + 
                          "ROM-COLETOR" + SUBSTRING(C-EMBARQUE-RESUMO,1,6) + ".TXT"
          c-arq-saida = SESSION:TEMP-DIRECTORY + '\ROM-SAIDA' +
...
PROCEDURE pi-ler-arquivo.
    ...
    INPUT STREAM e-arquivo FROM VALUE(c-arq-texto) NO-ECHO.

    REPEAT.                 
        SET STREAM e-arquivo c-id.
