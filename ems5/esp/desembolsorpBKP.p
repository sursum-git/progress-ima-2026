/**************************************************************
Programa: DesembolsorpBKP.p
Objetivo: Buscar as contas contabeis para analise de gastos a 
partir dos movimentos de caixa e banco, base bkp.
...
FOR EACH ems5bkp.movto_cta_corren 
    ...
    FIND FIRST ems5bkp.cta_corren OF ems5bkp.movto_cta_corren
        NO-LOCK NO-ERROR.
/*     IF NOT AVAIL ems5bkp.cta_corren THEN DO:                                                                                                       */
/*        RUN incluirLog IN hLog('agrupamento Geral','fim movto_cta_corren=====EMPRESA DA CONTA CORRENTE DIFERENTE DA CORRENTE===================='). */
/*        NEXT.                                                                                                                                       */
/*     END.                                                                                                                                           */
    CASE ems5bkp.movto_cta_corren.cod_modul_dtsul:
        WHEN 'acr'  THEN DO:
           FOR EACH ems5bkp.movto_tit_acr OF ems5bkp.movto_cta_corren NO-LOCK:
               FIND FIRST ems5bkp.tit_acr OF ems5bkp.movto_tit_acr NO-LOCK NO-ERROR.                                                                                                               
               FIND FIRST emitente WHERE                                                                                                                                           
                    tit_acr.cdn_cliente =  emitente.cod-emitente NO-LOCK NO-ERROR. 
/*                MESSAGE AVAIL tit_acr                  */
/*                    VIEW-AS ALERT-BOX INFO BUTTONS OK. */
               FOR EACH ems5bkp.aprop_ctbl_acr OF ems5bkp.movto_tit_acr                                                                                                                               
...
/*                        VIEW-AS ALERT-BOX INFO BUTTONS OK. */
        ...
        END.
        WHEN 'apb'  THEN DO:
           FOR EACH ems5bkp.movto_tit_ap OF movto_cta_corren
               NO-LOCK:
               RUN incluirLog IN hLog('agrupamento Geral','==inicio movto_tit_ap========================================').
               FIND FIRST ems5bkp.tit_ap OF movto_tit_ap NO-LOCK NO-ERROR. 
               ...
               IF (AVAIL tit_ap AND tit_ap.ind_origin_tit_ap <> "REC" /*origem recebimento*/ ) OR NOT AVAIL tit_Ap  THEN DO:
                  ...
                  ELSE DO:
                     ...
                     FOR EACH ems5bkp.antecip_pef_pend OF movto_tit_ap  NO-LOCK:
                         FIND FIRST ems5bkp.fornecedor OF ems5bkp.antecip_pef_pend NO-LOCK NO-ERROR.
                         IF AVAIL ems5bkp.fornecedor THEN
                         FIND FIRST ems5bkp.grp_fornec where
                               ems5bkp.grp_fornec.cod_grp_fornec = fornecedor.cod_grp_fornec NO-LOCK NO-ERROR.
                         FOR EACH ems5bkp.aprop_ctbl_pend_ap OF antecip_pef_pend NO-LOCK:
                             CREATE tt.                                                                                                                                                   
                             ASSIGN 
                             tt.cod_modulo        = movto_cta_corren.cod_modul_dtsul
                             tt.cod_empresa       = aprop_ctbl_pend_ap.cod_empresa                                                                                                             
                             tt.cod_estab         = aprop_ctbl_pend_ap.cod_estab                                                                                                               
                             tt.cod_emitente      = string(ems5bkp.antecip_pef_pend.cdn_fornecedor) 
                             tt.desc_emitente     =  IF AVAIL ems5bkp.fornecedor THEN ems5bkp.fornecedor.nom_pessoa  ELSE ''                                                                           
                             tt.data              =  movto_cta_corren.dat_transacao                                                                                                        
                  ...
                  END.
               END.
 /**********************bloco comentado, pois, no backup n∆o existem titulos integrados pelo recebimento*****************************************************/
/*                ELSE DO:                                                                                                                                  */
...
/************************************************************************************************************************************************************/
        ...
        END.
        WHEN 'cmg'  THEN DO:
            FIND FIRST ems5bkp.tip_trans_cx OF movto_cta_corren NO-LOCK NO-ERROR.
            FOR EACH ems5bkp.aprop_ctbl_cmg OF movto_cta_corren
...
DEFINE INPUT  PARAMETER p_val_movto_ap                  LIKE ems5bkp.bf_movto_tit_ap_03.val_movto_ap                    NO-UNDO.
...
FIND ems5bkp.bf_movto_tit_ap_03 
    WHERE ROWID(bf_movto_tit_ap_03) = RowidMovtoTitAp NO-LOCK NO-ERROR.
FIND FIRST ems5bkp.bf_tit_ap_05 OF bf_movto_tit_ap_03 NO-LOCK NO-ERROR.
FIND FIRST ems5bkp.fornecedor                                                                                                                                              
    WHERE ems5bkp.tit_ap.cdn_fornecedor = fornecedor.cdn_fornecedor NO-LOCK NO-ERROR. 
IF AVAIL fornecedor THEN
   FIND FIRST ems5bkp.grp_fornec where
    grp_fornec.cod_grp_fornec = fornecedor.cod_grp_fornec NO-LOCK NO-ERROR.

FOR EACH ems5bkp.aprop_ctbl_ap OF bf_movto_tit_ap_03 NO-LOCK                                                                                                                   
    ...
    IF ems5bkp.aprop_ctbl_ap.cod_indic_econ = 'dolar' THEN DO:
        RUN incluirLog IN hLog('agrupamento Geral','nivel 2 - achou emitente?' + string(AVAIL ems5bkp.fornecedor)).
        FIND FIRST ems5bkp.val_aprop_ctbl_ap OF aprop_ctbl_ap
...
DEFINE INPUT  PARAMETER pDataContaCorrente              LIKE ems5bkp.movto_cta_corren.dat_transacao                     NO-UNDO.
DEFINE INPUT  PARAMETER p_ind_fluxo_movto_cta_corren    LIKE ems5bkp.movto_cta_corren.ind_fluxo_movto_cta_corren        NO-UNDO.
DEFINE INPUT  PARAMETER p_val_movto_ap                  LIKE ems5bkp.movto_tit_ap.val_movto_ap                          NO-UNDO.
DEFINE INPUT  PARAMETER p_val_juros                     LIKE ems5bkp.movto_tit_ap.val_juros                             NO-UNDO.
DEFINE INPUT  PARAMETER p_val_desconto                  LIKE ems5bkp.movto_tit_ap.val_desconto                          NO-UNDO.
DEFINE INPUT  PARAMETER p_cod_cta_corren                LIKE ems5bkp.movto_cta_corren.cod_cta_corren                    NO-UNDO.
DEFINE INPUT  PARAMETER p_num_seq_movto_cta_corren      LIKE ems5bkp.movto_cta_corren.num_seq_movto_cta_corren          NO-UNDO.
DEFINE INPUT  PARAMETER p_num_id_movto_cta_corren       LIKE ems5bkp.movto_cta_corren.num_id_movto_cta_corren           NO-UNDO.
...
FIND FIRST ems5bkp.fornecedor                                                                                                                                              
     WHERE bf_tit_ap_01.cdn_fornecedor = fornecedor.cdn_fornecedor NO-LOCK NO-ERROR. 
IF AVAIL ems5bkp.fornecedor THEN
FIND FIRST ems5bkp.grp_fornec where
     grp_fornec.cod_grp_fornec = fornecedor.cod_grp_fornec NO-LOCK NO-ERROR.
...
FOR EACH ems5bkp.aprop_ctbl_ap OF bf_movto_tit_ap_01 NO-LOCK                                                                                                                   
     WHERE  (p_ind_fluxo_movto_cta_corren = 'ent' AND aprop_ctbl_ap.ind_natur_lancto_ctbl = 'cr')                                                  
      OR  (p_ind_fluxo_movto_cta_corren   = 'sai' AND aprop_ctbl_ap.ind_natur_lancto_ctbl = 'db') :
...
DEFINE INPUT  PARAMETER pDataContaCorrente              LIKE ems5bkp.movto_cta_corren.dat_transacao                     NO-UNDO.
DEFINE INPUT  PARAMETER p_ind_fluxo_movto_cta_corren    LIKE ems5bkp.movto_cta_corren.ind_fluxo_movto_cta_corren        NO-UNDO.
DEFINE INPUT  PARAMETER p_val_movto_ap                  LIKE ems5bkp.bf_movto_tit_ap_02.val_movto_ap                    NO-UNDO.
DEFINE INPUT  PARAMETER p_cod_cta_corren                LIKE ems5bkp.movto_cta_corren.cod_cta_corren                    NO-UNDO.
DEFINE INPUT  PARAMETER p_num_seq_movto_cta_corren      LIKE ems5bkp.movto_cta_corren.num_seq_movto_cta_corren          NO-UNDO.
DEFINE INPUT  PARAMETER p_num_id_movto_cta_corren       LIKE ems5bkp.movto_cta_corren.num_id_movto_cta_corren           NO-UNDO.
...
FOR EACH bf_movto_tit_ap_02 NO-LOCK 
    ...
    FIND FIRST ems5bkp.fornecedor                                                                                                                                              
        WHERE bf_tit_ap_04.cdn_fornecedor = fornecedor.cdn_fornecedor NO-LOCK NO-ERROR. 
    IF AVAIL fornecedor THEN
       FIND FIRST ems5bkp.grp_fornec where
        grp_fornec.cod_grp_fornec = fornecedor.cod_grp_fornec NO-LOCK NO-ERROR.
    ...
    ELSE DO:
       ...
       FOR EACH ttApropRateio 
           ...
           tt.cod_emitente      = IF AVAIL bf_tit_ap_04 THEN string(ems5bkp.tit_ap.cdn_fornecedor) ELSE '0'                                                                          
           tt.desc_emitente     = IF AVAIL ems5bkp.fornecedor THEN ems5bkp.fornecedor.nom_pessoa  ELSE ''                                                                                   
           tt.data              = pDataContaCorrente                                                                                                          
           tt.conta_contabil    = ttApropRateio.conta                                                                                                           
