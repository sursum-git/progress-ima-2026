...
*******************************************************************************/
{include/i-prgvrs.i essp0026 2.04.00.000}
...
DEFINE VARIABLE h_v01in172 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btn-parametro 
     IMAGE-UP FILE "image/im-param2.bmp":U
     LABEL "Button 1" 
     SIZE 4 BY 1.17 TOOLTIP "Sele‡Æo do intervalo de C¢digos de itens.".
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Itens - Dados Complementares"
         HEIGHT             = 7.96
         WIDTH              = 90
         MAX-HEIGHT         = 20.71
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadsi2 /* Itens - Dados Complementares */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Itens - Dados Complementares */
DO:
...
DO:
  RUN esdlg/d01in172.w.
  RUN adm-open-query-cases IN h_q01in172.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01in172.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01in172 ).
       RUN set-position IN h_v01in172 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01in172.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = inzoom/z01in172.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01in172 ).
       RUN set-position IN h_q01in172 ( 1.50 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01in172. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v01in172 ).
       RUN add-link IN adm-broker-hdl ( h_q01in172 , 'Record':U , h_v01in172 ).

       /* Links to SmartQuery h_q01in172. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_q01in172 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01in172 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0026" "2.04.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF gr-item <> ? THEN
     RUN pi-reposiciona-query IN h_q01in172 (INPUT gr-item).
  ELSE
     RUN dispatch IN h_q01in172 ( INPUT "get-first":U).
