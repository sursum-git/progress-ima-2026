...
&Scoped-Define ENABLED-OBJECTS fi-it-codigo fi-cod-refer bt-ok bt-cancela ~
bt-ajuda RECT-1 rt-button 
...
DEFINE VARIABLE fi-cod-refer AS CHARACTER FORMAT "X(10)":U 
     LABEL "Referencia" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     fi-it-codigo AT ROW 1.5 COL 12.29 COLON-ALIGNED
     fi-desc-item AT ROW 1.5 COL 26.57 COLON-ALIGNED NO-LABEL
     fi-cod-refer AT ROW 2.46 COL 12.29 COLON-ALIGNED
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 58
     RECT-1 AT ROW 1.17 COL 1.43
     rt-button AT ROW 4.25 COL 1
     SPACE(0.13) SKIP(0.09)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V  Para Referencia Item"
...
ON GO OF FRAME d-vapara /* V  Para Referencia Item */
DO:
  find ref-item where
       ref-item.it-codigo = INPUT FRAME {&FRAME-NAME} fi-it-codigo AND
       ref-item.cod-refer = INPUT FRAME {&FRAME-NAME} fi-cod-refer NO-ERROR.

  IF NOT AVAIL ref-item THEN DO:
      {utp/ut-table.i mgcad ref-item 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(ref-item).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Referencia Item */
DO:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-it-codigo d-vapara
ON LEAVE OF fi-it-codigo IN FRAME d-vapara /* Item */
DO:
  FIND ITEM WHERE
...
------------------------------------------------------------------------------*/
  DISPLAY fi-it-codigo fi-desc-item fi-cod-refer 
      WITH FRAME d-vapara.
  ENABLE fi-it-codigo fi-cod-refer bt-ok bt-cancela bt-ajuda RECT-1 rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
