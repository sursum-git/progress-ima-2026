...
DEFINE  INPUT PARAMETER p-row-di159 AS ROWID.
DEFINE  INPUT PARAMETER prePedido   AS CHARACTER   NO-UNDO.
...
ASSIGN infPedido.id = i-id
       ...
       infPedido.prePedido = prePedido
       infPedido.destinatario = ped-venda.nome-abrev
       infPedido.cnpjTransportador = transporte.cgc
       infPedido.observacoes = 'Separar pe‡as Maiores'.
...
FOR EACH ped-item OF ped-venda WHERE 
    ...
    IF lCodigoProdUnificado THEN DO:
       ASSIGN  infItens.produto = ped-item.it-codigo + "-" +  ped-item.cod-refer
               infItens.lote    = ''.        
    END.
    ELSE DO:
...
PROCEDURE pi-chama-api.
    ...
    RUN lisa/enviarAlteracaoPedVenda.p (INPUT h-dataset,
