...
*******************************************************************************/
{include/i-prgvrs.i ESSP0127 2.04.00.000}
...
DEFINE VARIABLE h_q01un128 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01un128 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Programa no Menu"
         HEIGHT             = 10.75
         WIDTH              = 89.14
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-consim /* Consulta Programa no Menu */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Programa no Menu */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01un128.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01un128 ).
       RUN set-position IN h_v01un128 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01un128.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01un128.w,
                     ProgVaPara = esgo/g01un128.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01un128 ).
       RUN set-position IN h_q01un128 ( 1.25 , 59.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01un128. */
       RUN add-link IN adm-broker-hdl ( h_q01un128 , 'Record':U , h_v01un128 ).

       /* Links to SmartQuery h_q01un128. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01un128 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01un128 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01un128 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01un128 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0127" "2.04.00.000"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-busca-qry w-consim 
PROCEDURE pi-busca-qry :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEF OUTPUT PARAMETER h-query AS HANDLE.
    ASSIGN h-query = h_q01un128.
END PROCEDURE.
