...
&Scoped-Define ENABLED-FIELDS teste-amos.fm-codigo teste-amos.data-teste ~
teste-amos.num-seq 
...
DEFINE FRAME d-vapara
     teste-amos.fm-codigo AT ROW 1.17 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .88
     fi-desc-familia AT ROW 1.17 COL 27.29 COLON-ALIGNED NO-LABEL
     teste-amos.data-teste AT ROW 2.17 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     teste-amos.num-seq AT ROW 3.17 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 2.29 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Testes de Amostras"
...
ON GO OF FRAME d-vapara /* V  Para Testes de Amostras */
DO:
  find teste-amos no-lock 
      where teste-amos.fm-codigo  = input frame {&frame-name} teste-amos.fm-codigo
        ...
        AND teste-amos.num-seq    = INPUT FRAME {&FRAME-NAME} teste-amos.num-seq
       no-error.
  if  not avail teste-amos then do:
      {utp/ut-table.i espec teste-amos 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(teste-amos).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Testes de Amostras */
DO:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL teste-amos.fm-codigo d-vapara
ON LEAVE OF teste-amos.fm-codigo IN FRAME d-vapara /* Familia */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY fi-desc-familia 
      WITH FRAME d-vapara.
  IF AVAILABLE teste-amos THEN 
    DISPLAY teste-amos.fm-codigo teste-amos.data-teste teste-amos.num-seq 
      WITH FRAME d-vapara.
  ENABLE teste-amos.fm-codigo teste-amos.data-teste teste-amos.num-seq bt-ok 
