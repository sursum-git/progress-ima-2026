...
*******************************************************************************/
{include/i-prgvrs.i ESSP0145 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-corte 
...
DEFINE VARIABLE h_q03es049 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v04es049 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-corte 
     IMAGE-UP FILE "image/imt-tes.bmp":U
     IMAGE-INSENSITIVE FILE "image/iit-tes.bmp":U
     LABEL "Button 1" 
     SIZE 5.72 BY 1.25 TOOLTIP "Retira Corte para Amostra".
...
DEFINE FRAME f-cad
     bt-corte AT ROW 1.33 COL 45.86
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Cortes de Amostra"
         HEIGHT             = 16.17
         WIDTH              = 90
         MAX-HEIGHT         = 28.13
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsi2 /* Manuten‡Æo Cortes de Amostra */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Manuten‡Æo Cortes de Amostra */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-corte w-cadsi2
ON CHOOSE OF bt-corte IN FRAME f-cad /* Button 1 */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v04es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v04es049 ).
       RUN set-position IN h_v04es049 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 14.25 , 88.14 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q03es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z02es049.w,
                     ProgVaPara = esgo/g01es049.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q03es049 ).
       RUN set-position IN h_q03es049 ( 1.25 , 58.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-cadsi2. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v04es049. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v04es049 ).
       RUN add-link IN adm-broker-hdl ( h_q03es049 , 'Record':U , h_v04es049 ).
       RUN add-link IN adm-broker-hdl ( h_v04es049 , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q03es049. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q03es049 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q03es049 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             bt-corte:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v04es049 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0145" "2.04.00.000"}
  ...
  IF gr-ob-etiqueta <> ? THEN
     RUN pi-reposiciona-query IN h_q03es049 (INPUT gr-ob-etiqueta).
  ELSE
     RUN dispatch IN h_q03es049 ( INPUT "get-first":U).
...
-------------------------------------------------------------*/
  ...
  IF p-state = 'enable-button' THEN DO.
     ASSIGN bt-corte:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
     bt-corte:LOAD-IMAGE("image/imt-tes.bmp").

     RUN enable-modifica IN h_p-cadsi2 (INPUT NO).
  END.

  IF p-state = 'disable-button' THEN DO.
     ASSIGN bt-corte:SENSITIVE = NO.
     bt-corte:LOAD-IMAGE("image/iit-tes.bmp").
