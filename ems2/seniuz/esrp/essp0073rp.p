...
{utp/ut-liter.i Relatorio_de_Defeitos_por_Tipo_Defeito * r}
...
for each mov-est-acbd
   WHERE mov-est-acbd.cod-estabel  >= tt-param.fi-ini-cod-estabel
     ...
     AND mov-est-acbd.num-lote     >= tt-param.fi-ini-num-lote
     AND mov-est-acbd.num-lote     <= tt-param.fi-fin-num-lote
   ...
   no-lock,
   FIRST mov-est-acbm WHERE mov-est-acbm.cod-estabel = mov-est-acbd.cod-estabel
...
for each mov-est-acbd
   ...
   run pi-acompanhar in h-acomp (input "Tipo Defeito: " + mov-est-acbd.cod-tipo-def + 
                                       " Defeito: " + mov-est-acbd.cod-defeito  + 
                                       " Item: " + mov-est-acbd.it-codigo).
   ...
   if first-of(mov-est-acbd.cod-tipo-def) then do:
      ...
      for each b-mov-est-acbd
          WHERE b-mov-est-acbd.cod-estabel >= tt-param.fi-ini-cod-estabel
            ...
            AND b-mov-est-acbd.num-lote    >= tt-param.fi-ini-num-lote
            AND b-mov-est-acbd.num-lote    <= tt-param.fi-fin-num-lote
            ...
            AND NOT CAN-FIND(tt-excluir-ob WHERE tt-excluir-ob.num-lote = b-mov-est-acbd.num-lote)
          no-lock,
          FIRST b-mov-est-acbm WHERE b-mov-est-acbm.cod-estabel = b-mov-est-acbd.cod-estabel
          ...
          if b-mov-est-acbd.classific = "Rg" then
             assign de-tot-reg-tip = de-tot-reg-tip +
                                     b-mov-est-acbd.qtd-defeit.
          else
          if b-mov-est-acbd.classific = "Rt" then
             assign de-tot-ret-tip = de-tot-ret-tip +
                                     b-mov-est-acbd.qtd-defeit.
          else
             assign de-tot-ld-tip = de-tot-ld-tip +
                                    b-mov-est-acbd.qtd-defeit.
      end.
   ...
   if last-of(mov-est-acbd.cod-defeito) then do: 
      find defeito where defeito.cod-tipo-def = mov-est-acbd.cod-tipo-def
   ...
   if last-of(mov-est-acbd.cod-tipo-def) then do:
      assign de-tot-def-tip = de-tot-reg-tip + de-tot-ld-tip + de-tot-ret-tip.
