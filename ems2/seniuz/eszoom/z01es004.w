...
DEFINE VARIABLE h_b01es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b03es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b04es004 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Controles de SAC"
         HEIGHT             = 16.54
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Controles de SAC */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Controles de SAC */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Fam¡lia|Data|Cliente|Representante' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es004.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es004 ).
       RUN set-position IN h_b01es004 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.72 ) */

       /* Links to SmartBrowser h_b01es004. */
       RUN add-link IN adm-broker-hdl ( h_b01es004 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es004.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es004 ).
       RUN set-position IN h_b02es004 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.72 ) */

       /* Links to SmartBrowser h_b02es004. */
       RUN add-link IN adm-broker-hdl ( h_b02es004 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03es004.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03es004 ).
       RUN set-position IN h_b03es004 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.72 ) */

       /* Links to SmartBrowser h_b03es004. */
       RUN add-link IN adm-broker-hdl ( h_b03es004 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 3 */

    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b04es004.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b04es004 ).
       RUN set-position IN h_b04es004 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.72 ) */

       /* Links to SmartBrowser h_b04es004. */
       RUN add-link IN adm-broker-hdl ( h_b04es004 , 'State':U , THIS-PROCEDURE ).
