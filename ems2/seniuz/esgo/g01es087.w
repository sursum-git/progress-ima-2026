...
&Scoped-Define ENABLED-OBJECTS rt-button c_rua c_doca bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_rua c_doca 
...
DEFINE VARIABLE c_doca AS CHARACTER FORMAT "x(3)":U 
     LABEL "Doca" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 NO-UNDO.

DEFINE VARIABLE c_rua AS CHARACTER FORMAT "x(3)":U 
     LABEL "Rua" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 NO-UNDO.
...
DEFINE FRAME d-vapara
     c_rua AT ROW 1.42 COL 30 COLON-ALIGNED WIDGET-ID 2
     c_doca AT ROW 2.42 COL 30 COLON-ALIGNED WIDGET-ID 4
...
DO:
FIND ob-localiz NO-LOCK
    WHERE ob-localiz.rua EQ INPUT FRAME {&FRAME-NAME} c_rua
      AND ob-localiz.doca EQ INPUT FRAME {&FRAME-NAME} c_doca NO-ERROR.

IF NOT AVAILABLE ob-localiz THEN DO:
    {utp/ut-table.i espec ob-localiz 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(ob-localiz).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY c_rua c_doca 
      WITH FRAME d-vapara.
  ENABLE rt-button c_rua c_doca bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
