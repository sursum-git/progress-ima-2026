...
DEFINE VARIABLE h_b01es030 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es030 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa Tabela de Carga Goma"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa Tabela de Carga Goma */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa Tabela de Carga Goma */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'C¢digo|Data' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es030.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es030 ).
       RUN set-position IN h_b01es030 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 88.00 ) */

       /* Links to SmartBrowser h_b01es030. */
       RUN add-link IN adm-broker-hdl ( h_b01es030 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es030.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es030 ).
       RUN set-position IN h_b02es030 ( 2.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.00 ) */

       /* Links to SmartBrowser h_b02es030. */
       RUN add-link IN adm-broker-hdl ( h_b02es030 , 'State':U , THIS-PROCEDURE ).
