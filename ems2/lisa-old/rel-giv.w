...
DEFINE VARIABLE tg-conf-etq AS LOGICAL INITIAL no 
     LABEL "Alteraáao e Cortes de Etiquetas" 
     VIEW-AS TOGGLE-BOX
     SIZE 29 BY .83 NO-UNDO.

DEFINE VARIABLE tg-env-isf AS LOGICAL INITIAL no 
     LABEL "Solicitar Separaá∆o" 
     VIEW-AS TOGGLE-BOX
     SIZE 21 BY .83 NO-UNDO.

DEFINE VARIABLE tg-env-nfs AS LOGICAL INITIAL no 
     LABEL "Envia Nota de Venda (Aprovaá∆o)" 
     VIEW-AS TOGGLE-BOX
     SIZE 29 BY .83 NO-UNDO.

DEFINE VARIABLE tg-nf-retorno AS LOGICAL INITIAL no 
     LABEL "Nota Retorno LISA" 
     VIEW-AS TOGGLE-BOX
     SIZE 29 BY .83 NO-UNDO.

DEFINE VARIABLE tg-nota-remessa AS LOGICAL INITIAL no 
     LABEL "Nota de Remessa LISA" 
     VIEW-AS TOGGLE-BOX
     SIZE 21 BY .83 NO-UNDO.

DEFINE VARIABLE tg-pck-list AS LOGICAL INITIAL no 
     LABEL "Packing List" 
     VIEW-AS TOGGLE-BOX
     SIZE 11.29 BY .83 NO-UNDO.

DEFINE VARIABLE tg-ret-isf AS LOGICAL INITIAL no 
     LABEL "Retorno ISF (Criar Reserva)" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-sel
     tg-nota-remessa AT ROW 3 COL 18 WIDGET-ID 14
     tg-pck-list AT ROW 3.75 COL 18 WIDGET-ID 16
     tg-env-isf AT ROW 5 COL 18 WIDGET-ID 6
     tg-ret-isf AT ROW 5.75 COL 18 WIDGET-ID 4
     tg-env-nfs AT ROW 6.5 COL 18 WIDGET-ID 12
     tg-nf-retorno AT ROW 7.75 COL 18 WIDGET-ID 18
     tg-conf-etq AT ROW 9 COL 18 WIDGET-ID 22
     " Processar" VIEW-AS TEXT
          SIZE 8 BY .54 AT ROW 2 COL 16 WIDGET-ID 20
     RECT-100 AT ROW 2.25 COL 15 WIDGET-ID 10
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Integra Dados com Sistema LISA"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-relat /* Integra Dados com Sistema LISA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Integra Dados com Sistema LISA */
DO:
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY tg-nota-remessa tg-pck-list tg-env-isf tg-ret-isf tg-env-nfs 
          tg-nf-retorno tg-conf-etq 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE RECT-100 tg-nota-remessa tg-pck-list tg-env-isf tg-ret-isf tg-env-nfs 
         tg-nf-retorno tg-conf-etq 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    ASSIGN tt-param.l-nota-entrada = INPUT FRAME f-pg-sel tg-nota-remessa
           tt-param.l-pck-list = INPUT FRAME f-pg-sel tg-pck-list
           tt-param.l-env-isf = INPUT FRAME f-pg-sel tg-env-isf
           tt-param.l-ret-isf = INPUT FRAME f-pg-sel tg-ret-isf
           tt-param.l-env-nfs-venda = INPUT FRAME f-pg-sel tg-env-nfs
           tt-param.l-nota-retorno = INPUT FRAME f-pg-sel tg-nf-retorno
           tt-param.l-conf-etiquetas = INPUT FRAME f-pg-sel tg-conf-etq.
    ...
    {include/i-rprun.i lisa/rel-giv-rp.p}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-trata-tab w-relat 
PROCEDURE pi-trata-tab :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    IF c-tab-lisa = '' THEN RETURN.

    DO WITH FRAME f-pg-sel.
    END.

    ASSIGN tg-nota-remessa:SENSITIVE = NO
           tg-pck-list:SENSITIVE = NO
           tg-env-isf:SENSITIVE = NO
           tg-ret-isf:SENSITIVE = NO
           tg-env-nfs:SENSITIVE = NO
           tg-nf-retorno:SENSITIVE = NO
           tg-conf-etq:SENSITIVE = NO.

    CASE c-tab-lisa:
        WHEN 'NotaRemessa'      THEN ASSIGN tg-nota-remessa:SENSITIVE = YES
                                            tg-nota-remessa:SCREEN-VALUE = 'YES'.
        WHEN 'PackingList'      THEN ASSIGN tg-pck-list:SENSITIVE = YES
                                            tg-pck-list:SCREEN-VALUE = 'YES'.
        WHEN 'ISF'              THEN ASSIGN tg-env-isf:SENSITIVE = YES
                                            tg-env-isf:SCREEN-VALUE = 'YES'.
        WHEN 'RetornoISF'       THEN ASSIGN tg-ret-isf:SENSITIVE = YES
                                            tg-ret-isf:SCREEN-VALUE = 'YES'.
        WHEN 'RemessaNotaVenda' THEN ASSIGN tg-env-nfs:SENSITIVE = YES
                                            tg-env-nfs:SCREEN-VALUE = 'YES'.
        WHEN 'NotaRetorno'      THEN ASSIGN tg-nf-retorno:SENSITIVE = YES
                                            tg-nf-retorno:SCREEN-VALUE = 'YES'.
        WHEN 'ConfEtiquetas'    THEN ASSIGN tg-conf-etq:SENSITIVE = YES
                                            tg-conf-etq:SCREEN-VALUE = 'YES'.
