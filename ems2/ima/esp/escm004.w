...
DEFINE VARIABLE h_q01cm008 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01cm008 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 89.43 BY 7.88 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manutená∆o Descontos da Comiss∆o"
         HEIGHT             = 7.88
         WIDTH              = 89.43
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manutená∆o Descontos da Comiss∆o */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manutená∆o Descontos da Comiss∆o */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01cm008.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01cm008 ).
       RUN set-position IN h_v01cm008 ( 2.75 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 6.08 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01cm008.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = M:\EMS206\especificos\eszoom\z01cm008.w,
                     ProgVaPara = M:\EMS206\especificos\esgo\g01cm008.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01cm008 ).
       RUN set-position IN h_q01cm008 ( 1.25 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01cm008. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01cm008 ).
       RUN add-link IN adm-broker-hdl ( h_q01cm008 , 'Record':U , h_v01cm008 ).

       /* Links to SmartQuery h_q01cm008. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01cm008 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01cm008 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01cm008 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCM004" "2.06.00.000"}
