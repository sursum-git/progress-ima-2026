...
*******************************************************************************/
{include/i-prgvrs.i B04AD098 2.04.00.000}
...
emitente.cod-emitente emitente.nome-abrev emitente.cgc 
...
&Scoped-define QUERY-STRING-br-table FOR EACH emitente WHERE ~{&KEY-PHRASE} ~
 ...
 AND emitente.nome-emit >= fi-ini-nome-emit ~
 AND emitente.nome-emit <= fi-fin-nome-emit NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS fi-ini-nome-emit fi-fin-nome-emit ~
bt-confirma br-table IMAGE-1 IMAGE-2 
&Scoped-Define DISPLAYED-OBJECTS fi-ini-nome-emit fi-fin-nome-emit 
...
DEFINE VARIABLE fi-fin-nome-emit AS CHARACTER FORMAT "X(40)" INITIAL "ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 35 BY .88 TOOLTIP "Nome do emitente final."
     FONT 1 NO-UNDO.

DEFINE VARIABLE fi-ini-nome-emit AS CHARACTER FORMAT "X(40)" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     fi-ini-nome-emit AT ROW 1.17 COL 2.86 HELP
          "Nome Completo do Emitente" NO-LABEL
     fi-fin-nome-emit AT ROW 1.17 COL 42.43 COLON-ALIGNED HELP
          "Nome Completo do Emitente" NO-LABEL
...
/* SETTINGS FOR FILL-IN fi-ini-nome-emit IN FRAME F-Main
   ALIGN-L                                                              */
...
/* Query rebuild information for BROWSE br-table
     ...
     _OrdList          = "mgcad.emitente.nome-emit|yes,mgcad.emitente.cod-emitente|yes"
     _Where[1]         = "mgcad.emitente.identific <> 2
 AND mgcad.emitente.cod-rep >= i-cod-rep-ini
 AND mgcad.emitente.cod-rep <= i-cod-rep-fin
 AND mgcad.emitente.nome-emit >= fi-ini-nome-emit
 AND mgcad.emitente.nome-emit <= fi-fin-nome-emit"
...
DO:
  assign input frame {&frame-name} fi-ini-nome-emit fi-fin-nome-emit.
