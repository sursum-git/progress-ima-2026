...
DEFINE VARIABLE h_b01es029 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es029 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Testes de Amostras"
         HEIGHT             = 17
         WIDTH              = 89.43
         MAX-HEIGHT         = 28.92
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Testes de Amostras */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Testes de Amostras */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es029.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es029 ).
       RUN set-position IN h_b01es029 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.00 ) */

       /* Links to SmartBrowser h_b01es029. */
       RUN add-link IN adm-broker-hdl ( h_b01es029 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es029.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es029 ).
       RUN set-position IN h_b02es029 ( 2.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.50 , 88.00 ) */

       /* Links to SmartBrowser h_b02es029. */
       RUN add-link IN adm-broker-hdl ( h_b02es029 , 'State':U , THIS-PROCEDURE ).
