...
DEFINE VARIABLE extDesc        AS CHARACTER EXTENT 500  NO-UNDO FORMAT  'x(100)'.
DEFINE VARIABLE extNome        AS CHARACTER EXTENT 500  NO-UNDO FORMAT  'x(100)'.
DEFINE VARIABLE extTipo        AS CHARACTER EXTENT 500  NO-UNDO FORMAT  'x(20)' .
DEFINE VARIABLE extformat      AS CHARACTER EXTENT 500  NO-UNDO FORMAT  'x(50)' .
DEFINE VARIABLE extExtent      AS INTEGER     NO-UNDO EXTENT 500.
...
REPEAT iDB = 1 TO NUM-DBS:
    ...
    CREATE BUFFER bhFile  FOR TABLE cBanco + '._file' NO-ERROR.
    CREATE BUFFER bhField FOR TABLE cBanco + '._field' NO-ERROR.
    /*hQueryMD:SET-BUFFERS(bhFile:HANDLE).*/
    hQueryMD:ADD-BUFFER(bhFile) NO-ERROR.
    hQueryMD:ADD-BUFFER(bhField) NO-ERROR.
    ...
    hQueryMD:QUERY-OPEN NO-ERROR. 
    IF ERROR-STATUS:ERROR THEN DO:
        MESSAGE 'verifique a digita‡Æo dos campos e tabelas.'
            VIEW-AS ALERT-BOX INFO BUTTONS OK.
        RETURN NO-APPLY.
    END.
    REPEAT:
      hQueryMD:GET-NEXT() NO-ERROR.
      ...
      IF(bhField:BUFFER-FIELD('_extent'):BUFFER-VALUE() > 0) THEN DO:
        DO i = 1 TO bhField:BUFFER-FIELD('_field-name'):EXTENT:
           ASSIGN i = i + 1.
           CREATE ttCampos.
           ASSIGN ttCampos.labelCampo = RIGHT-trim(LEFT-TRIM(bhField:BUFFER-FIELD('_label'):BUFFER-VALUE()))
                  ttCampos.nomeCampo  = RIGHT-trim(left-trim(bhField:BUFFER-FIELD('_field-name'):BUFFER-VALUE() + "-" + bhField:BUFFER-FIELD('_extent'):BUFFER-VALUE()))
                  ttCampos.tipo       = right-trim(left-trim(bhField:BUFFER-FIELD('_data-type'):BUFFER-VALUE()))
                  ttCampos.formato    = RIGHT-TRIM(left-trim(bhField:BUFFER-FIELD('_format'):BUFFER-VALUE()))
                  ttCampos.iExtent    = bhField:BUFFER-FIELD('_Extent'):BUFFER-VALUE() 
                  ttCampos.registro   = i NO-ERROR .
           IF ttCampos.labelCampo = ? THEN
              ASSIGN ttCampos.labelCampo = '?'.
        END.
      END.
      ELSE DO :
          CREATE ttCampos.
          ASSIGN ttCampos.labelCampo = RIGHT-trim(LEFT-TRIM(bhField:BUFFER-FIELD('_label'):BUFFER-VALUE()))
                 ttCampos.nomeCampo  = RIGHT-trim(left-trim(bhField:BUFFER-FIELD('_field-name'):BUFFER-VALUE() ))
                 ttCampos.tipo       = right-trim(left-trim(bhField:BUFFER-FIELD('_data-type'):BUFFER-VALUE()))
                 ttCampos.formato    =  RIGHT-TRIM(left-trim(bhField:BUFFER-FIELD('_format'):BUFFER-VALUE()))
                 ttCampos.registro   = i NO-ERROR.
      END.
...
REPEAT:
    ...
    FOR EACH ttCampos:
        ...
        IF ttCampos.tipo = 'character' THEN
           RUN piRetirarQuebra(ttValores.conteudo,OUTPUT ttValores.conteudo).
...
FOR EACH ttCampos:
    ...
    ASSIGN j = j + 1
           extDesc[j]     =   ttCampos.labelCampo      
           extNome[j]     =   ttCampos.nomeCampo       
           extTipo[j]     =   ttCampos.tipo            
           extformat[j]   =   ttCampos.formato         
           extExtent[j]   =   ttCampos.iExtent .
...
/*
EXPORT DELIMITER "|" extDesc.
EXPORT DELIMITER "|" extNome.
EXPORT DELIMITER "|" extTipo.
EXPORT DELIMITER "|" extformat.
EXPORT DELIMITER "|" extExtent.
*/
        
    
    
    
    /*cLinhaDescCampo    SKIP
...
FOR EACH ttValores BREAK BY ttValores.regValor :
    PUT  right-trim(left-trim(ttValores.conteudo))  "|".
...
OUTPUT TO c:\temp\ttValores.txt NO-CONVERT .
  FOR EACH ttValores:
      EXPORT DELIMITER "|" ttValores.
  END.
OUTPUT CLOSE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE piRetirarQuebra w-livre 
PROCEDURE piRetirarQuebra :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT  PARAMETER cEntrada AS CHARACTER   NO-UNDO FORMAT 'x(1000)'.
DEFINE OUTPUT PARAMETER cSaida   AS CHARACTER   NO-UNDO FORMAT 'x(1000)'.
DEFINE VARIABLE c AS CHARACTER   NO-UNDO FORMAT 'x(1)'.

DEFINE VARIABLE i AS INTEGER     NO-UNDO.
REPEAT i = 1 TO LENGTH(cEntrada):
   CASE SUBSTR(cEntrada,i,1):
       WHEN CHR(10) THEN
           ASSIGN c = '*' .
       WHEN CHR(13) THEN
           ASSIGN c = '*' .
       OTHERWISE 
           ASSIGN c = SUBSTR(cEntrada,i,1).
   END CASE.
   ASSIGN cSaida = cSaida + c.
END.
