...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-liquida w-digita _FREEFORM
  QUERY br-liquida NO-LOCK DISPLAY
      ...
      tt-liquida-ima.preco-item     COLUMN-LABEL "Preáo OutLet"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
ON MOUSE-SELECT-DBLCLICK OF br-liquida IN FRAME F-Main /* Outlet IMA */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br-liquida w-digita
ON ROW-DISPLAY OF br-liquida IN FRAME F-Main /* Outlet IMA */
DO:
   ASSIGN i-font = ?.
   IF tt-liquida-ima.marca THEN
      ASSIGN i-font = 6.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL br-liquida w-digita
ON VALUE-CHANGED OF br-liquida IN FRAME F-Main /* Outlet IMA */
DO:
...
DO:
    ...
    ELSE
       MESSAGE 'Ser∆o gerados os Dados com o Layout' SKIP(1)
               ' Item|Descriá∆o|Referància|Un|Preáo|Qtde Atual|PreáoTAB|PreáoOutLet'
...
DO:
  ...
  IF AVAIL tt-liquida-ima THEN DO.
     MESSAGE 'Existem Itens em Liquidaá∆o sem o Percentual de Desconto...' SKIP
             'Verifique...'
         VIEW-AS ALERT-BOX INFO BUTTONS OK.
  ...
  FOR EACH tt-liquida-ima WHERE
      ...
      FOR EACH liquida-ima WHERE
               ...
               liquida-ima.tipo-tab    = 1 AND // Outlet
      ...
      CASE tt-liquida-ima.acao.
          WHEN 'I' THEN DO.
             ...
             CREATE liquida-ima.
             ASSIGN liquida-ima.tipo-tab      = 1  // Outlet
...
------------------------------------------------------------------------------*/
    ...
    */
    ...
    ASSIGN chWorkSheet:cells(1,6):VALUE = "Preco OUTLET".
        ...
        FOR EACH saldo-estoq WHERE
            ...
            ELSE DO.
               ...
               ASSIGN tt-estoque.cod-estabel   = saldo-estoq.cod-estabel
                      ...
                      //tt-estoque.dt-ult-compra = item.dt-ult-compra.
            END.
        END.
    END.
