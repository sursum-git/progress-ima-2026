...
*******************************************************************************/
{include/i-prgvrs.i ESFT0035 2.04.00.000}
...
DEF NEW GLOBAL SHARED VAR h-v01ad229 AS HANDLE.
DEF NEW GLOBAL SHARED VAR h-b06ad098 AS HANDLE.
DEF NEW GLOBAL SHARED VAR h-b07ad098 AS HANDLE.
...
DEFINE VARIABLE h_b06ad098 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b07ad098 AS HANDLE NO-UNDO.
...
DEFINE BUTTON btn-parametro 
     ...
     LABEL "Button 1" 
     SIZE 4 BY 1.17 TOOLTIP "Seleá∆o do intervalo de ÷tens de Pedidos.".
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Faturamento de Representante"
         COLUMN             = 25.29
         ROW                = 7.63
         HEIGHT             = 17
         WIDTH              = 90
...
ON END-ERROR OF w-concom /* Consulta Faturamento de Representante */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Consulta Faturamento de Representante */
DO:
...
DO:
  RUN esdlg/d01di135.p.
  SESSION:SET-WAIT-STATE("general":U).
  RUN local-display-fields IN h_v01ad229. 
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v01ad229 ).
       RUN set-position IN h_v01ad229 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.29 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Por Cliente|Por Produto' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b06ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b06ad098 ).
       RUN set-position IN h_b06ad098 ( 6.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.08 , 87.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adqry/q01ad229.r':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Key-Name = ,
                     ProgPesquisa = adzoom/z01ad229.w,
                     ProgVaPara = adgo/g01ad229.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01ad229 ).
       RUN set-position IN h_q01ad229 ( 1.50 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartQuery h_q01ad229. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01ad229 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b07ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b07ad098 ).
       RUN set-position IN h_b07ad098 ( 6.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.08 , 87.72 ) */
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(h_v01ad229) THEN
     ASSIGN h-v01ad229 = h_v01ad229.
  
  IF VALID-HANDLE(h_b06ad098) THEN
     ASSIGN h-b06ad098 = h_b06ad098.

  IF VALID-HANDLE(h_b07ad098) THEN
     ASSIGN h-b07ad098 = h_b07ad098.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESFT0035" "2.04.00.000"}

  {include/i-inifld.i} 

  ASSIGN fi-cod-rep = ?.  /* Invalida o Representante para n∆o iniciar executando */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-view w-concom 
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute AFTER standard behavior.    */
  APPLY 'choose' TO btn-parametro IN FRAME {&FRAME-NAME}.
END PROCEDURE.
