...
*******************************************************************************/
{include/i-prgvrs.i impd026 2.04.00.001}
...
DEF NEW SHARED VAR h-query  AS HANDLE NO-UNDO.
DEF NEW SHARED VAR c-row-v01im026 AS CHAR NO-UNDO.
...
DEFINE VARIABLE h_b02im026 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadpai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01im026 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01im026 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-atualiza 
     IMAGE-UP FILE "image/im-chck3.bmp":U
     LABEL "Atualiza" 
     SIZE 4 BY 1.13 TOOLTIP "Acione esse bot∆o para atualizar todos os preáos da tabela de preáos original".
...
DEFINE FRAME f-cad
     bt-atualiza AT ROW 1.21 COL 44
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manutená∆o Tabela de Preáos"
         HEIGHT             = 18.42
         WIDTH              = 93.43
         MAX-HEIGHT         = 28.21
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manutená∆o Tabela de Preáos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manutená∆o Tabela de Preáos */
DO:
...
DO:
     ...
     run pi-inicializar in h-acomp(input "Atualizando Preáo").

     FOR EACH tt-preco-item WHERE 
              tt-preco-item.log-2 = YES SHARE-LOCK:

         FIND FIRST item WHERE 
                    item.it-codigo = tt-preco-item.it-codigo NO-LOCK NO-ERROR.

         IF AVAIL item THEN DO:
            FIND FIRST tb-preco WHERE 
                       tb-preco.nr-tabpre = tt-preco-item.nr-tabpre NO-LOCK NO-ERROR.
            IF AVAIL tb-preco THEN
               ASSIGN tt-preco-item.char-1    = item.desc-item
                      tt-preco-item.situacao  = tb-preco.situacao
                      tt-preco-item.dt-inival = tb-preco.dt-inival.
         END.

         FOR EACH ref-item WHERE 
                  ref-item.it-codigo = tt-preco-item.it-codigo NO-LOCK:
             RUN pi-acompanhar in h-acomp(INPUT "Atualizando Precos " + tt-preco-item.nr-tabpre + " " +  tt-preco-item.it-codigo + " " + ref-item.cod-refer ).
             ...
             END.
             ASSIGN preco-item.situacao   = tt-preco-item.situacao
         ...
         END.
         ASSIGN tt-preco-item.log-2 = NO.
     END.
     RUN pi-finalizar in h-acomp.  

    /*RUN pi-le-tabela IN h_v01im026 (INPUT 'rowid').

    IF RETURN-VALUE <> ''
    THEN DO:
         ASSIGN c-row-v01im026 = RETURN-VALUE.

         FIND FIRST tb-preco WHERE ROWID(tb-preco) = to-rowid(c-row-v01im026) NO-LOCK NO-ERROR. 
         IF AVAIL tb-preco  THEN
             MESSAGE "tb" tb-preco.nr-tabpre VIEW-AS ALERT-BOX.
        
    END.*/
/*
      IF AVAIL tb-preco THEN
          MESSAGE " avail" VIEW-AS ALERT-BOX.*/
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imvwr/v01im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01im026 ).
       RUN set-position IN h_v01im026 ( 2.75 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.63 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Preáos' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 7.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.75 , 92.57 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imqry/q01im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = imzoom\z01im026.w,
                     ProgVaPara = imgo\g01im026.w,
                     ProgIncMod = imp\impd026a.w,
                     Implantar = no':U ,
             OUTPUT h_q01im026 ).
       RUN set-position IN h_q01im026 ( 1.00 , 58.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v01im026. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_v01im026 ).
       RUN add-link IN adm-broker-hdl ( h_q01im026 , 'Record':U , h_v01im026 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'tableio':U , h_v01im026 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01im026. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_q01im026 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01im026 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01im026 ).
       RUN add-link IN adm-broker-hdl ( h_q01im026 , 'State':U , THIS-PROCEDURE ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadpai , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01im026 ,
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = imp\impd026b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b02im026 ).
       RUN set-position IN h_b02im026 ( 9.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 89.00 ) */

       /* Links to BrowserCadastro2 h_b02im026. */
       RUN add-link IN adm-broker-hdl ( h_q01im026 , 'Record':U , h_b02im026 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02im026 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "impd026" "2.04.00.001"}

  ASSIGN h-query  = h_q01im026.
