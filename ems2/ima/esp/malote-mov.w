...
*******************************************************************************/
{include/i-prgvrs.i malote-mov 2.00.04.000}
...
DEFINE VARIABLE h_q01malote-mov AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01malote-mov AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadsim /* Movimento de Malote */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Movimento de Malote */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01malote-mov.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01malote-mov ).
       RUN set-position IN h_v01malote-mov ( 2.79 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01malote-mov.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01malote-mov.w,
                     ProgVaPara = ,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01malote-mov ).
       RUN set-position IN h_q01malote-mov ( 1.00 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01malote-mov. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01malote-mov ).
       RUN add-link IN adm-broker-hdl ( h_q01malote-mov , 'Record':U , h_v01malote-mov ).

       /* Links to SmartQuery h_q01malote-mov. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01malote-mov ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01malote-mov ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01malote-mov ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01malote-mov ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Movimento de Malote" "2.00.04.000"}
