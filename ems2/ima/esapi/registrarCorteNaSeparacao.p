...
AND   ttJson.tag     = "id"  no-error.
//se n∆o encontrar Ç porque n∆o tem corte
IF NOT AVAIL ttjson THEN RETURN 'ok'.
...
FOR EACH ttReservas:
    FOR EACH ttJson
        ...
        CASE ttJson.tag:
            ...
            DO:
                ASSIGN ttReservas.numRoloOrigem = INT(ttJson.valor).
    ...
    IF ttReservas.numEtqOrigemCorte  <> '' THEN DO: 
       ...
       RUN esapi/cortarEtqLisa.p(
            ...
            INPUT c-pre-pedido,
            INPUT  hBoMsg,
            /*INPUT ttreservas.it-Codigo,
            INPUT ttReservas.cod-refer,
            INPUT ttReservas.nr-container, 
            INPUT ttReservas.numRoloOrigem ,*/
            OUTPUT cErro).      
        IF cErro <> '' THEN
        DO:
