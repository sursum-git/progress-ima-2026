...
def var pdf_inc_ObjectSequence      as int no-undo.
...
procedure pdf_load_external: /* PRIVATE */
  ...
  def input param pdfStream  as char no-undo.

  def var cContent           as char no-undo.

  def VARIABLE L_count AS INTEGER NO-UNDO.
  def VARIABLE L_start AS INTEGER NO-UNDO.
  def VARIABLE L_end   AS INTEGER NO-UNDO.

  def VARIABLE L_EncryptMem  AS MEMPTR NO-UNDO.
  def VARIABLE L_EncryptKey  AS MEMPTR NO-UNDO.

  def VARIABLE mHolder AS MEMPTR  NO-UNDO.
  def VARIABLE mTotal  AS MEMPTR  NO-UNDO.
  def VARIABLE mTemp   AS MEMPTR  NO-UNDO.
  def VARIABLE mFile   AS MEMPTR  NO-UNDO.
  def VARIABLE vDest   AS MEMPTR NO-UNDO.

  def VARIABLE vDestLen  AS INTEGER NO-UNDO.
  def VARIABLE iPointer  AS INTEGER NO-UNDO.
  def VARIABLE iFileSize AS INTEGER NO-UNDO.
  def VARIABLE iMemSize  AS INTEGER NO-UNDO.

  def VARIABLE L_fontobj AS CHARACTER NO-UNDO.

  def VARIABLE L_Stat    AS INTEGER NO-UNDO.
  ...
  FOR EACH TT_pdf_external WHERE 
      TT_pdf_external.obj_stream = pdfStream:
      ...
      if  iFileSize < 31000 then do:
         ...
         for each TT_Resource where 
                  TT_Resource.obj_stream = pdfStream and 
                  TT_Resource.res_type   = "Xobject" and 
                  TT_Resource.res_old   <> "":
    ...
    /* This portion handles content replacement where the total size of the
      ...
      do while true:
        ...
        for each TT_Resource where 
          ...
          if  "~/" + TT_resource.pdf_id + string(TT_resource.page_id ) = TT_pdf_external.ext_tag then 
          do:
              cContent = replace(cContent, TT_resource.res_old, TT_resource.res_text).
          end.
...
procedure pdf_open_PDF:
  define input param pdfStream  AS CHARACTER NO-UNDO.
  define input param pdfName    AS CHARACTER NO-UNDO.
  define input param pdfID      AS CHARACTER NO-UNDO.

  find first TT_pdf_ext where 
             TT_pdf_ext.obj_stream = pdfStream and 
...
procedure pdf_use_PDF_page:
  def INPUT PARAMETER pdfStream  AS CHARACTER NO-UNDO.
  def INPUT PARAMETER pdfID      AS CHARACTER NO-UNDO.
  def INPUT PARAMETER pdfPage    AS INTEGER   NO-UNDO.

  def VARIABLE L_Page AS INTEGER NO-UNDO.
  
  def BUFFER B_TT_pdf_page FOR TT_pdf_page.

  IF  NOT CAN-FIND(FIRST TT_pdf_stream
                  WHERE TT_pdf_stream.obj_stream = pdfStream NO-LOCK) THEN 
  DO:
  ...
  find first B_TT_pdf_page where 
             B_TT_pdf_page.obj_Stream = pdfStream and 
             B_TT_pdf_page.page_nbr   = L_Page 
             no-error.
...
PROCEDURE LoadExternalFonts: /* PRIVATE */
  ...
  def VARIABLE l_text  AS CHARACTER NO-UNDO.

  def VARIABLE l_Obj         AS INTEGER NO-UNDO.
  def VARIABLE l_Seq         AS INTEGER NO-UNDO.
  def VARIABLE l_Descendant  AS INTEGER NO-UNDO.
  def VARIABLE l_Descriptor  AS INTEGER NO-UNDO.
  def VARIABLE l_Enc         AS INTEGER NO-UNDO.

  def VARIABLE L_EncryptMem  AS MEMPTR NO-UNDO.
  def VARIABLE L_EncryptKey  AS MEMPTR NO-UNDO.

  def VARIABLE l_FoundDescendant   AS LOGICAL NO-UNDO.
  def VARIABLE l_FoundDescriptor  AS LOGICAL NO-UNDO.
  def VARIABLE L_FoundEnc         AS LOGICAL NO-UNDO.

  def VARIABLE file-content  AS MEMPTR NO-UNDO.

  def BUFFER B_TT_Font FOR TT_Font.
  ...
  for each TT_Resource where 
           ...
           THEN DO:
               ...
               find first TT_font where 
                          TT_Font.obj_stream = pdfStream       and 
                          TT_Font.obj_id = TT_Resource.res_obj and 
                          TT_Font.gen_id = TT_Resource.res_gen no-lock no-error.
...
/*               message SESSION:TEMP-DIR + TT_Resource.pdf_id + "-" 
