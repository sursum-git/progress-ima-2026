...
DEF INPUT PARAMETER p-nome-abrev LIKE loc-entr.nome-abrev.
...
ASSIGN c-arq-log = "p:\integra-loc-entrega" + STRING(TODAY,"99-99-9999") + ".txt".

/* Main Block */
OUTPUT TO VALUE(c-arq-log) APPEND.
   PUT UNFORMATTED "INTEGRANDO LOCAL DE ENTREGA DO EMITENTE: " 
       p-nome-abrev
...
IF h-server:CONNECTED() THEN DO.
   ...
   FOR EACH loc-entr WHERE
            loc-entr.nome-abrev = p-nome-abrev NO-LOCK.
       CREATE tt-loc-entr.
   ...
   CASE param-global.empresa-prin:
       WHEN '1' THEN DO.
          /* Cria Local de Entrega na MED OFICIAL */
          ...
          RUN esapi/importa-loc-entr.p ON SERVER h-server (INPUT TABLE tt-loc-entr).
          RUN esapi/disconecta-base.p ON SERVER h-server.
       ...
       END.
       WHEN '5' THEN DO.
           /* Cria Local de Entrega na IMA OFICIAL */
   ...
   /* Cria Local de Entrega na IMA BKP */
   ...
   /* Cria Local de Entrega na MED BKP */
   ...
   OUTPUT TO VALUE(c-arq-log) APPEND.
      PUT UNFORMATTED "TERMINO INTEGRAÄ«O DO LOCAL DE ENTREGA DO EMITENTE: " 
          p-nome-abrev
          "  " 
          NOW
          SKIP.
