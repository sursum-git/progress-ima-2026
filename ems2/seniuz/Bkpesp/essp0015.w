...
*******************************************************************************/
{include/i-prgvrs.i ESSP0015 2.04.00.000}
...
ON END-ERROR OF w-consim /* Consulta Defeitos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Defeitos */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es009 ).
       RUN set-position IN h_v01es009 ( 3.00 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'ProgPesquisa = ,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01es009 ).
       RUN set-position IN h_q01es009 ( 1.50 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es009. */
       RUN add-link IN adm-broker-hdl ( h_q01es009 , 'Record':U , h_v01es009 ).

       /* Links to SmartQuery h_q01es009. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es009 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es009 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es009 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0015" "2.04.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .
  IF v-row-defeito <> ? THEN
     RUN pi-reposiciona-query IN h_q01es009 (INPUT v-row-defeito).
  ELSE
     RUN dispatch IN h_q01es009 (INPUT "get-first":U).
