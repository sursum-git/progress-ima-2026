...
DEFINE VARIABLE h_q04im026 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v04im026 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 7.92 COL 2 HELP
          "Salva e sai"
     bt-save AT ROW 7.92 COL 12.43 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 7.92 COL 23 HELP
          "Cancela"
     bt-ajuda AT ROW 7.92 COL 72
     rt-button AT ROW 7.75 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Pre‡o Item"
         HEIGHT             = 8.42
         WIDTH              = 83.29
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Pre‡o Item */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Pre‡o Item */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imvwr/v04im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v04im026 ).
       RUN set-position IN h_v04im026 ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.50 , 83.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imqry/q04im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q04im026 ).
       RUN set-position IN h_q04im026 ( 7.25 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v04im026. */
       RUN add-link IN adm-broker-hdl ( h_q04im026 , 'Record':U , h_v04im026 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'tableio':U , h_v04im026 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v04im026 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "impd026B" "2.04.00.001"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  run pi-atualiza-parent in h_v04im026 (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q04im026 (input v-row-table).
