...
DEFINE VARIABLE h_v02es037 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 4.33 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 4.33 COL 13.57 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 4.33 COL 24.57 HELP
          "Cancela"
     bt-ajuda AT ROW 4.33 COL 80.57
     rt-button AT ROW 4.08 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-incmdp ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Tipos de Defeito"
         HEIGHT             = 4.54
         WIDTH              = 90.43
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-incmdp /* Inclui/Modifica Tipos de Defeito */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-incmdp w-incmdp
ON WINDOW-CLOSE OF w-incmdp /* Inclui/Modifica Tipos de Defeito */
DO:
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'ProgPesquisa = ,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01es037 ).
       RUN set-position IN h_q01es037 ( 1.00 , 72.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_v02es037 ).
       RUN set-position IN h_v02es037 ( 1.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.75 , 89.00 ) */

       /* Links to SmartViewer h_v02es037. */
       RUN add-link IN adm-broker-hdl ( h_q01es037 , 'Record':U , h_v02es037 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_v02es037 ).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q01es037 (INPUT v-row-table). 
