...
DEFINE VARIABLE h_q01es009 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es009 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 5.29 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 5.29 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 5.29 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 5.29 COL 70.72
     rt-button AT ROW 5.04 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Defeitos"
         HEIGHT             = 5.5
         WIDTH              = 80.43
         MAX-HEIGHT         = 28.92
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Defeitos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Defeitos */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es009.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es009 ).
       RUN set-position IN h_v01es009 ( 1.00 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.92 , 80.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es009.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q01es009 ).
       RUN set-position IN h_q01es009 ( 1.00 , 69.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es009. */
       RUN add-link IN adm-broker-hdl ( h_q01es009 , 'Record':U , h_v01es009 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_v01es009 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es009 ,
...
------------------------------------------------------------------------------*/
  ...
  run pi-atualiza-parent in h_v01es009 (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q01es009 (input v-row-table).
