...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 55 BY 15.75
         FONT 1.
...
DEFINE FRAME DEFAULT-FRAME
     ...
     fi-usr-all AT ROW 19.25 COL 27 COLON-ALIGNED WIDGET-ID 54
     fi-usr-sup AT ROW 20.25 COL 27 COLON-ALIGNED WIDGET-ID 64
     fi-usr-idle AT ROW 20.25 COL 46.43 COLON-ALIGNED WIDGET-ID 70
     fi-usr-run AT ROW 21.25 COL 27 COLON-ALIGNED WIDGET-ID 68
     fi-usr-out AT ROW 21.25 COL 46.43 COLON-ALIGNED WIDGET-ID 66
...
DO:
  CASE tt-usr-connect.marca.
      WHEN 'i' OR WHEN 'r' THEN DO.   /* Idle ou Run */
          ...
          RUN pi-disconnecta-usr (INPUT tt-usr-connect.marca,
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-14 RECT-15 RECT-16 RECT-17 RECT-18 bt-exit tg-run tg-idle tg-out 
         tg-sup br-usr-connect br-usr fi-exit bt-super bt-mata-usr sl-time 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-disconecta-usr C-Win 
PROCEDURE pi-disconecta-usr :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-usr WHERE
        ...
        RUN seniuz/t70c.p tt-usr.banco p-marca STRING(tt-usr.usr).
    END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
    ...
    REPEAT i-ct = 1 TO NUM-DBS:
        RUN seniuz/t70a.p (INPUT-OUTPUT TABLE tt-usr) LDBNAME(i-ct).
    END. 
