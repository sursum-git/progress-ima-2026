...
*******************************************************************************/
{include/i-prgvrs.i ESSP0037 2.04.00.000}
...
DEFINE VAR cod-ssist-ini LIKE ssist-mec.codigo.
DEFINE VAR cod-ssist-fin LIKE ssist-mec.codigo.
...
DEFINE VARIABLE h_q01es027 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es027 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Sub-sistemas da Mecƒnica"
         HEIGHT             = 4.5
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-consim /* Sub-sistemas da Mecƒnica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Sub-sistemas da Mecƒnica */
DO:
...
DO:
  RUN esdlg/d01es027.w (OUTPUT cod-ssist-ini, OUTPUT cod-ssist-fin, OUTPUT arq-saida).

  IF arq-saida <> "" THEN DO:
     RUN pi-gera-excel (INPUT cod-ssist-ini, INPUT cod-ssist-fin, INPUT arq-saida).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es027.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es027 ).
       RUN set-position IN h_v01es027 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es027.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es027.w,
                     ProgVaPara = esgo/g01es027.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es027 ).
       RUN set-position IN h_q01es027 ( 1.50 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es027. */
       RUN add-link IN adm-broker-hdl ( h_q01es027 , 'Record':U , h_v01es027 ).

       /* Links to SmartQuery h_q01es027. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es027 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es027 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es027 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es027 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0037" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER p-cod-ssist-ini LIKE ssist-mec.codigo.
    DEF INPUT PARAMETER p-cod-ssist-fin LIKE ssist-mec.codigo.
    ...
    ASSIGN c-Lin = c-empresa + " - " + " TABELA DE SUB-SISTEMAS DA MEC¶NICA - DATA: " + STRING(TODAY) +
                                       " - " + STRING(TIME,"HH:MM").
    ...
    DDE EXECUTE i-canal COMMAND '[select(~"C1:C2~")]'.
    ...
    FOR EACH ssist-mec WHERE ssist-mec.codigo >= p-cod-ssist-ini
                         AND ssist-mec.codigo <= p-cod-ssist-fin
                       NO-LOCK:
    
        DDE SEND i-canal SOURCE ssist-mec.codigo    ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
        DDE SEND i-canal SOURCE ssist-mec.descricao ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
