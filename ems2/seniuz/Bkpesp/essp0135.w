...
*******************************************************************************/
{include/i-prgvrs.i ESSP0135 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-deleta 
...
DEFINE VARIABLE h_q04es049 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es049 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-deleta 
     IMAGE-UP FILE "image/im-era.bmp":U
     LABEL "Button 1" 
     SIZE 4 BY 1.21 TOOLTIP "Elimina Etiqueta".
...
DEFINE FRAME f-cad
     bt-deleta AT ROW 1.33 COL 48 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Etiquetas de Tecidos"
         HEIGHT             = 14.67
         WIDTH              = 89.57
         MAX-HEIGHT         = 20.71
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-cadsi2 /* Manuten‡Æo Etiquetas de Tecidos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Manuten‡Æo Etiquetas de Tecidos */
DO:
...
&Scoped-define SELF-NAME bt-deleta
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-deleta w-cadsi2
ON CHOOSE OF bt-deleta IN FRAME f-cad /* Button 1 */
DO:
   MESSAGE 'Deseja Realmente Eliminar essa Etiqueta ?' 
       VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE l-confirma AS LOGICAL.
   IF l-confirma THEN DO.
      RUN pi-deleta IN h_v01es049.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es049 ).
       RUN set-position IN h_v01es049 ( 2.79 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 12.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q04es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es049.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q04es049 ).
       RUN set-position IN h_q04es049 ( 1.50 , 57.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es049. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v01es049 ).
       RUN add-link IN adm-broker-hdl ( h_q04es049 , 'Record':U , h_v01es049 ).

       /* Links to SmartQuery h_q04es049. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q04es049 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q04es049 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-deleta:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsi2 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             bt-deleta:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es049 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0135" "2.04.00.000"}
