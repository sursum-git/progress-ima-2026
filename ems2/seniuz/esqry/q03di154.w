...
*******************************************************************************/
{include/i-prgvrs.i Q03DI154 2.04.00.000}
...
DEFINE NEW GLOBAL SHARED VAR fi-ini-cod-refer  LIKE ped-item.cod-refer.
DEFINE NEW GLOBAL SHARED VAR fi-fin-cod-refer  LIKE ped-item.cod-refer.
...
&Scoped-define QUERY-STRING-Query-Main FOR EACH ped-item      WHERE ped-item.it-codigo  =  fi-it-codigo        AND ped-item.cod-refer  >= fi-ini-cod-refer        AND ped-item.cod-refer  <= fi-fin-cod-refer        AND (ped-item.cod-sit-item < 3 OR ped-item.cod-sit-item = 5) NO-LOCK, ~
            EACH ped-venda OF ped-item      WHERE ped-venda.dt-implant >= fi-ini-dt-implant        AND ped-venda.dt-implant <= fi-fin-dt-implant        AND ped-venda.dt-entrega >= fi-ini-dt-entrega        AND ped-venda.dt-entrega <= fi-fin-dt-entrega        AND (ped-venda.cod-sit-ped = 1 AND to-abe = YES OR             ped-venda.cod-sit-ped = 2 AND to-atp = YES OR             ped-venda.cod-sit-ped = 3 AND to-att = YES OR             ped-venda.cod-sit-ped = 4 AND to-pen = YES OR             ped-venda.cod-sit-ped = 5 AND to-sus = YES OR             ped-venda.cod-sit-ped = 6 AND to-can = YES OR             ped-item.cod-sit-item = 7 AND to-bal = YES) NO-LOCK, ~
            EACH ped-item-ext OF ped-item      WHERE substr(espec.ped-item-ext.acondicionamento, ~
      1, ~
      4) >= fi-ini-acond        AND substr(espec.ped-item-ext.acondicionamento, ~
      1, ~
      4) <= fi-fin-acond        and SUBSTR(ped-item-ext.lote, ~
      2, ~
      1) = "P"        AND ped-item-ext.reservado = FALSE NO-LOCK      BY IF substr(fi-ini-cod-refer, ~
      7, ~
      1) = "0" THEN ped-item.it-codigo                                               ELSE ped-item.it-codigo + substr(ped-item.cod-refer, ~
      3, ~
      4)      BY ped-venda.dt-entrega      BY ped-item.nr-pedcli      BY ped-item.nr-sequencia      BY ped-venda.nome-transp
&Scoped-define OPEN-QUERY-Query-Main OPEN QUERY {&SELF-NAME} FOR EACH ped-item      WHERE ped-item.it-codigo  =  fi-it-codigo        AND ped-item.cod-refer  >= fi-ini-cod-refer        AND ped-item.cod-refer  <= fi-fin-cod-refer        AND (ped-item.cod-sit-item < 3 OR ped-item.cod-sit-item = 5) NO-LOCK, ~
            EACH ped-venda OF ped-item      WHERE ped-venda.dt-implant >= fi-ini-dt-implant        AND ped-venda.dt-implant <= fi-fin-dt-implant        AND ped-venda.dt-entrega >= fi-ini-dt-entrega        AND ped-venda.dt-entrega <= fi-fin-dt-entrega        AND (ped-venda.cod-sit-ped = 1 AND to-abe = YES OR             ped-venda.cod-sit-ped = 2 AND to-atp = YES OR             ped-venda.cod-sit-ped = 3 AND to-att = YES OR             ped-venda.cod-sit-ped = 4 AND to-pen = YES OR             ped-venda.cod-sit-ped = 5 AND to-sus = YES OR             ped-venda.cod-sit-ped = 6 AND to-can = YES OR             ped-item.cod-sit-item = 7 AND to-bal = YES) NO-LOCK, ~
            EACH ped-item-ext OF ped-item      WHERE substr(espec.ped-item-ext.acondicionamento, ~
      1, ~
      4) >= fi-ini-acond        AND substr(espec.ped-item-ext.acondicionamento, ~
...
/* Query rebuild information for QUERY Query-Main
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH ped-item
     WHERE ped-item.it-codigo  =  fi-it-codigo
       AND ped-item.cod-refer  >= fi-ini-cod-refer
       AND ped-item.cod-refer  <= fi-fin-cod-refer
       AND (ped-item.cod-sit-item < 3 OR ped-item.cod-sit-item = 5) NO-LOCK,
     EACH ped-venda OF ped-item
     WHERE ped-venda.dt-implant >= fi-ini-dt-implant
       AND ped-venda.dt-implant <= fi-fin-dt-implant
     ...
     WHERE substr(espec.ped-item-ext.acondicionamento,1,4) >= fi-ini-acond
       AND substr(espec.ped-item-ext.acondicionamento,1,4) <= fi-fin-acond
       and SUBSTR(ped-item-ext.lote,2,1) = "P"
       AND ped-item-ext.reservado = FALSE NO-LOCK
     BY IF substr(fi-ini-cod-refer,7,1) = "0" THEN ped-item.it-codigo
                                              ELSE ped-item.it-codigo + substr(ped-item.cod-refer,3,4)
     ...
     _Options          = "NO-LOCK"
     _TblOptList       = ",,"
     _OrdList          = "mgmov.ped-item.it-codigo|yes,mgmov.ped-item.cod-refer|yes,mgmov.ped-item.dt-entrega|yes,mgmov.ped-item.nr-pedcli|yes,mgmov.ped-venda.nome-transp|yes"
     _Where[1]         = "mgmov.ped-item.it-codigo = fi-it-codigo
 AND mgmov.ped-item.cod-refer >= fi-ini-cod-refer
 and mgmov.ped-item.cod-refer <= fi-fin-cod-refer
 and mgmov.ped-item.dt-entrega >= fi-ini-dt-entrega
 and mgmov.ped-item.dt-entrega <= fi-fin-dt-entrega
 AND (mgmov.ped-item.cod-sit-item = 1 and to-abe = yes or
     ...
     _JoinCode[2]      = "mgmov.ped-item.it-codigo >= fi-ini-it-codigo
 AND mgmov.ped-item.it-codigo <= fi-fin-it-codigo
 AND mgmov.ped-item.cod-refer >= fi-ini-cod-refer
 AND mgmov.ped-item.cod-refer <= fi-fin-cod-refer
 and mgmov.ped-item.dt-entrega >= fi-ini-dt-entrega
 and mgmov.ped-item.dt-entrega <= fi-fin-dt-entrega
 AND (mgmov.ped-item.cod-sit-item = 1 and to-abe = yes or
      ...
      mgmov.ped-item.cod-sit-item = 6 and to-can = yes or
      mgmov.ped-item.cod-sit-item = 7 and to-bal = yes)"
     _Where[2]         = "mgmov.ped-venda.dt-implant >= fi-ini-dt-implant
 AND mgmov.ped-venda.dt-implant <= fi-fin-dt-implant"
     _Where[3]         = "substr(espec.ped-item-ext.acondicionamento,1,4) >= fi-ini-acond
 AND substr(espec.ped-item-ext.acondicionamento,1,4) <= fi-fin-acond 
 and SUBSTR(ped-item-ext.lote,2,1) = ""P""
 AND espec.ped-item-ext.reservado = FALSE"
