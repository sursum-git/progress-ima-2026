...
DEFINE VARIABLE h_q01es073 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es073 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 10 COL 2 HELP
          "Salva e sai"
     bt-save AT ROW 10 COL 12.43 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 10 COL 23 HELP
          "Cancela"
     bt-ajuda AT ROW 10 COL 70.14
     rt-button AT ROW 9.75 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Etiquetas"
         HEIGHT             = 10.38
         WIDTH              = 80.43
         MAX-HEIGHT         = 26.75
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Etiquetas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Etiquetas */
DO:
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_v01es073 ).
       RUN set-position IN h_v01es073 ( 1.25 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.25 , 80.14 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es073.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q01es073 ).
       RUN set-position IN h_q01es073 ( 10.08 , 51.00 ) NO-ERROR.
       /* Size in UIB:  ( 0.92 , 7.72 ) */

       /* Links to SmartViewer h_v01es073. */
       RUN add-link IN adm-broker-hdl ( h_q01es073 , 'Record':U , h_v01es073 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TABLEIO':U , h_v01es073 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es073 ,
...
------------------------------------------------------------------------------*/
  ...
  run pi-atualiza-parent in h_v01es073 (input v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q01es073 (input v-row-table).
