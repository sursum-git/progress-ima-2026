...
*******************************************************************************/
{include/i-prgvrs.i ESSP0152A 2.04.00.000}
...
DEFINE VARIABLE h_v02es072 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 10.5 COL 2.14 HELP
          "Salva e sai"
     bt-save AT ROW 10.5 COL 13.14 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 10.5 COL 24.14 HELP
          "Cancela"
     bt-ajuda AT ROW 10.5 COL 78
     rt-button AT ROW 10.25 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-incmdp ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Transforma‡äes"
         HEIGHT             = 10.83
         WIDTH              = 88.29
         MAX-HEIGHT         = 28.96
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-incmdp /* Inclui/Modifica Transforma‡äes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-incmdp w-incmdp
ON WINDOW-CLOSE OF w-incmdp /* Inclui/Modifica Transforma‡äes */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02es072.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02es072 ).
       RUN set-position IN h_v02es072 ( 1.00 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q01es072 ).
       RUN set-position IN h_q01es072 ( 10.29 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02es072. */
       RUN add-link IN adm-broker-hdl ( h_q01es072 , 'Record':U , h_v02es072 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_v02es072 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02es072 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0152A" "2.04.00.000"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q01es072 (INPUT v-row-table). 
