...
*******************************************************************************/
{include/i-prgvrs.i Z01ES057 2.04.00.000}
...
DEFINE VARIABLE h_b02es057 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b03es057 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b04es057 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Entradas de Caminhäes"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Entradas de Caminhäes */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Entradas de Caminhäes */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Placa|Data Entrada|N£mero CDR' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es057.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es057 ).
       RUN set-position IN h_b02es057 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.17 , 87.29 ) */

       /* Links to SmartBrowser h_b02es057. */
       RUN add-link IN adm-broker-hdl ( h_b02es057 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03es057.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03es057 ).
       RUN set-position IN h_b03es057 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.17 , 87.29 ) */

       /* Links to SmartBrowser h_b03es057. */
       RUN add-link IN adm-broker-hdl ( h_b03es057 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b04es057.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b04es057 ).
       RUN set-position IN h_b04es057 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.17 , 87.29 ) */

       /* Links to SmartBrowser h_b04es057. */
       RUN add-link IN adm-broker-hdl ( h_b04es057 , 'State':U , THIS-PROCEDURE ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01ES057" "2.04.00.000"}
