...
DEF BUFFER b-tbs_preco      FOR tbs_preco.
...
DEF VAR hBoTranspCli        AS HANDLE NO-UNDO.
...
DEF VAR iTransp             AS INT.
DEF VAR idTranspEstab       AS INT.
...
DEF VAR lAtuTransp          AS LOGICAL     NO-UNDO.
...
DEF VAR de-preco-medio AS DECIMAL.
...
DEF VAR percComisClass             AS DEC.
                                
DEF VAR h-acomp            AS HANDLE NO-UNDO.

DEF VAR h-objeto AS HANDLE.
DEFINE VARIABLE isitComFrete             AS INTEGER     NO-UNDO.
DEFINE VARIABLE logFreteNegAnt           AS LOGICAL     NO-UNDO.
DEFINE VARIABLE lModificouFreteNegociado AS LOGICAL     NO-UNDO.

DEFINE VARIABLE idClasseComisValid AS INTEGER     NO-UNDO INIT 999999.
/*
...
&Scoped-Define ENABLED-OBJECTS tg_comis_neg fi-vlr-frete cb-tp-pedido ~
fi-cliente cb-fin-venda fi-ped-repres fi-cod-cond-pag fi-nome-transp ~
fi-nome-tr-red cb-tp-frete cb-aprov-ped fi-cod-rota fi-data-base ~
fi-cod-estabel ed-obs br-ped-item bt-inc fi-nr-pedido bt-sair ~
bt-preco-frete bt-log bt-cons-cliente-tri fi-observ-nf tg_vl_frete_negoc ~
RECT-1 RECT-2 RECT-3 RECT-4 RECT-7 
...
fi-cod-estabel ed-obs fi-denominacao tg-nao-aprovar tg-bloqueio ~
...
fi-tot-desconto fi-tot-vlr-abe fi-reserva fi-perc-comis-vend fi-cidade-cli ~
fi-uf fi-sit-preco fi-cidade-cli-tri fi-uf-tri fi-sit-cred fi-observ-nf ~
tg_vl_frete_negoc 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-4 tg-oc-cliente tg_comis_neg fi-vlr-frete fi-vendedor ~
fi-ped-repres fi-moeda fi-cod-cond-pag cb-tp-preco cb-tab-preco ~
cb-tp-entrega cb-prioridade fi-nome-transp fi-nome-tr-red cb-tp-frete ~
fi-cod-rota cb-origem ed-obs bt-msg bt-copia-it-ped bt-inc bt-can ~
bt-preco-frete fi-observ-nf tg_vl_frete_negoc 
...
DEFINE BUTTON bt-preco-frete 
     ...
     LABEL "" 
     SIZE 4 BY 1 TOOLTIP "Calcula Valor Frete".
...
DEFINE VARIABLE cb-preposto AS CHARACTER FORMAT "X(30)":U 
     LABEL "Preposto" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     DROP-DOWN-LIST
...
DEFINE VARIABLE tg_vl_frete_negoc AS LOGICAL INITIAL no 
     LABEL "Negoc." 
     VIEW-AS TOGGLE-BOX
     SIZE 7.86 BY .71 TOOLTIP "Comiss∆o negociada?" NO-UNDO.
...
DEFINE FRAME F-Main
     tg-oc-cliente AT ROW 13 COL 85 WIDGET-ID 104
     tg_comis_neg AT ROW 6.25 COL 31 WIDGET-ID 88
     ...
     cb-tab-preco AT ROW 8.25 COL 89 COLON-ALIGNED WIDGET-ID 24
     cb-tp-entrega AT ROW 9.21 COL 12 COLON-ALIGNED
     fi-dt-entrega AT ROW 9.21 COL 37.57 COLON-ALIGNED
     ...
     fi-nome-tr-red AT ROW 10.21 COL 37.72 COLON-ALIGNED
     cb-tp-frete AT ROW 10.21 COL 59.29 COLON-ALIGNED
     cb-aprov-ped AT ROW 10.21 COL 89 COLON-ALIGNED WIDGET-ID 102
     fi-cod-rota AT ROW 11.21 COL 37.57 COLON-ALIGNED WIDGET-ID 2
     fi-data-base AT ROW 11.21 COL 89 COLON-ALIGNED
     cb-origem AT ROW 11.21 COL 59.29 COLON-ALIGNED WIDGET-ID 70
     ...
     tg-nao-aprovar AT ROW 13.75 COL 85 WIDGET-ID 42
     tg-bloqueio AT ROW 14.5 COL 85
...
DEFINE FRAME F-Main
     ...
     fi-desc-rota AT ROW 11.21 COL 41.86 COLON-ALIGNED NO-LABEL WIDGET-ID 4 NO-TAB-STOP 
     ...
     tg_vl_frete_negoc AT ROW 11.29 COL 27.14 WIDGET-ID 108
...
/* SETTINGS FOR TOGGLE-BOX tg_comis_neg IN FRAME F-Main
   4                                                                    */
/* SETTINGS FOR TOGGLE-BOX tg_vl_frete_negoc IN FRAME F-Main
   4                                                                    */
...
DO:
   IF emitente.ind-cre-cli = 5 THEN DO.   // Somente a vista
   ...
   IF p-acao <> 'Consultar' THEN DO.
      ...
      IF  cb-tipo-pagto:SCREEN-VALUE <> 'Cart∆o de CrÇdito' THEN DO:
...
DO:
   ...
   IF cb-tp-frete:SCREEN-VALUE <> 'FOB Total' THEN DO:
      RUN calcularFreteDestacado.
   END.
   
   
    IF INPUT FRAME {&FRAME-NAME} fi-vlr-frete > 0 THEN  DO:
...
DO:
    ...
    //ASSIGN fi-vlr-frete:SCREEN-VALUE = STRING(INPUT FRAME {&FRAME-NAME} fi-tot-vlr-ped * 3 / 100).
...
DO:
   CASE SELF:SCREEN-VALUE.
       WHEN 'PI' THEN DO.
            ...
            ASSIGN tg-nao-aprovar:SCREEN-VALUE = 'YES'.
       END.
       WHEN 'PE' THEN DO.
            ...
            ASSIGN fi-nr-container:SCREEN-VALUE = ""
...
DO:
   IF cb-tab-preco:SCREEN-VALUE <> cb-tab-preco THEN DO.  // ENTERED N«O FUNCIONA EM COMBO-BOX
      ...
      FOR EACH tt-itens-ped NO-LOCK.
          ...
          ASSIGN tt-itens-ped.num-id-preco = c-id-preco.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cb-tab-preco w-window
ON VALUE-CHANGED OF cb-tab-preco IN FRAME F-Main /* Tab. Preáo Ref. */
DO:
 

 FIND tbs_preco WHERE
       tbs_preco.tb_preco_id = INTEGER(INPUT FRAME {&FRAME-NAME} cb-tab-preco) 
       NO-LOCK NO-ERROR.
    
 RUN pi-validar-troca-tb-preco.
 IF RETURN-VALUE = 'nok' THEN DO:
     ASSIGN cb-tab-preco:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(ped-venda-ext.tb_preco_id) .
    MESSAGE 'Existem um ou mais itens sem preáo cadastrado para a tabela: ' + tbs_preco.descricao
        VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    APPLY 'entry' TO SELF.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cb-tp-frete w-window
ON VALUE-CHANGED OF cb-tp-frete IN FRAME F-Main /* Tipo Frete */
DO:

  DEFINE VARIABLE codCliRemessa AS INT   NO-UNDO.
    
  IF cb-tp-frete:SCREEN-VALUE <> 'FOB TOTAL'  THEN DO:
    IF fi-cliente-tri:SCREEN-VALUE <> '' THEN DO:
    RUN setNomeAbrev IN hboEmitente(INPUT fi-cliente-tri:SCREEN-VALUE).
    RUN getCodEmitente IN hboEmitente(OUTPUT codCliRemessa). 
    RUN pi-buscar-transp(INPUT codCliRemessa,cb-tp-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
    END.
    ELSE DO:
        /*MESSAGE 'Deseja buscar a transportadora do cliente?' UPDATE logBuscar AS LOGICAL
          VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO.
         IF logBuscar THEN DO:*/
         RUN pi-buscar-transp(INPUT emitente.cod-emitente,cb-tp-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
         //END.
    END.
     
  END.
  ELSE DO:
    ASSIGN fi-nome-transp:SCREEN-VALUE = '' 
           fi-nome-tr-red:SCREEN-VALUE = ''.
  END.

   ASSIGN fi-vlr-frete:SENSITIVE = NO
          bt-preco-frete:SENSITIVE = NO.
   IF SELF:SCREEN-VALUE = 'Cif Destaque NF' THEN
      ASSIGN fi-vlr-frete:SENSITIVE = YES
             bt-preco-frete:SENSITIVE = YES.
   ...
   */
   ...
   IF AVAIL b-estabelec OR
      ...
      IF AVAIL estabelec THEN
         ASSIGN fi-natur-oper:SCREEN-VALUE = '62207'.

      IF cb-tp-pedido:SCREEN-VALUE = 'Amostra' THEN DO:

          IF fi-cod-estabel:SCREEN-VALUE = '5' THEN DO:
             ...
             IF NOT emitente.contrib-icms THEN DO.
              ...
              IF AVAIL b-estabelec AND
                 b-estabelec.estado = 'ES' THEN            
                 ASSIGN fi-natur-oper:SCREEN-VALUE = '5927m'. 
              
          END.
          ELSE IF fi-cod-estabel:SCREEN-VALUE = '505' THEN DO:
...
DO:
   ...
   END.
   RUN pi-buscar-transp(INPUT b-emitente.cod-emitente,cb-tp-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
END.
...
DO:
   ...
   /*IF c-lst-preposto <> '' THEN DO.
       ASSIGN cb-preposto:SENSITIVE = YES.

       &if "{&FNC_MULTI_IDIOMA}" = "Yes" &then
          run utp/ut-lstit.p (input-output c-lst-preposto).
          ASSIGN cb-preposto:LIST-ITEM-PAIRS = c-lst-preposto.
       &else
          ASSIGN cb-preposto:LIST-ITEMS = c-lst-preposto.
       &endif
   END.*/
   IF SELF:SCREEN-VALUE IN FRAME {&FRAME-NAME} <> fi-Cod-rep THEN DO:
      ...
      FOR EACH user-web WHERE
         ...
         ASSIGN c-lst-preposto = IF c-lst-preposto = ""
                                 THEN user-web.login + "," + user-web.login
                                 ELSE c-lst-preposto + "," + user-web.login + "," + user-web.login.
      END.
      IF c-lst-preposto <> ''  THEN DO:
          ASSIGN cb-preposto:VISIBLE  = TRUE
                 cb-preposto:SENSITIVE = TRUE.
          /*IF c-seg-usuario = 'super' THEN
             MESSAGE c-lst-preposto
              VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
          ASSIGN cb-preposto:LIST-ITEM-PAIRS =  c-lst-preposto.
       END.                                                    
       ELSE DO:
          ASSIGN cb-preposto:VISIBLE  = FALSE
                 cb-preposto:SCREEN-VALUE = ''.
...
DO:
   ...
   /*
   IF p-acao = "Modificar" AND 
      ped-venda-ext.l-etiqueta AND 
      INPUT FRAME {&FRAME-NAME} fi-dt-entrega ENTERED THEN DO.

      MESSAGE 'Pedido em Separaá∆o, Data de Entrega n∆o podera ser Alterada...'
          VIEW-AS ALERT-BOX ERROR BUTTONS OK.

      ASSIGN SELF:SCREEN-VALUE = STRING(ped-venda.dt-entrega).
      APPLY 'ENTRY' TO SELF.
   END.
   */
END.
...
&Scoped-define SELF-NAME tg_vl_frete_negoc
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg_vl_frete_negoc w-window
ON VALUE-CHANGED OF tg_vl_frete_negoc IN FRAME F-Main /* Negoc. */
DO:
  RUN tratarVlFrete(NO).  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE calcularFreteDestacado w-window 
PROCEDURE calcularFreteDestacado :
DEFINE VARIABLE iCli                  AS INTEGER     NO-UNDO.
DEFINE VARIABLE iCliTri               AS INTEGER     NO-UNDO.
DEFINE VARIABLE logFreteDestacado     AS LOGICAL     NO-UNDO.
DEFINE BUFFER bfEmit FOR emitente.
FIND bfEmit NO-LOCK
    WHERE bfEmit.nome-abrev = fi-cliente:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-ERROR.
ASSIGN iCli = IF AVAIL bfEmit THEN bfEmit.cod-emitente ELSE 0.
FIND bfEmit NO-LOCK
    WHERE bfEmit.nome-abrev = fi-cliente-tri:SCREEN-VALUE IN FRAME {&FRAME-NAME} NO-ERROR.
ASSIGN iCliTri = IF AVAIL bfEmit THEN bfEmit.cod-emitente ELSE 0.

RUN esapi/calcFreteDestacado.p(int(fi-nr-pedido:SCREEN-VALUE IN FRAME {&FRAME-NAME}),
                               fi-cod-estabel:SCREEN-VALUE IN FRAME {&FRAME-NAME},
                               cb-tp-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME},
                               iCli,
                               iCliTri,
                               DECIMAL(fi-tot-vlr-ped:SCREEN-VALUE IN FRAME {&FRAME-NAME}),
                               OUTPUT vlFreteDestacado,
                               OUTPUT logFreteDestacado
                               ).



IF INPUT FRAME {&FRAME-NAME} tg_vl_frete_negoc  <> tg_vl_frete_negoc THEN 
    //gera log somente na alteraá∆o da negociaá∆o de frete
   RUN esapi/cria-log-pedvenda.p (INPUT fi-nr-pedido:SCREEN-VALUE,
                                  INPUT fi-cliente:SCREEN-VALUE,
                                  INPUT "Valor de Frete Calculado: " + STRING(vlFreteDestacado) ,
                                  INPUT NO).

IF INPUT FRAME {&FRAME-NAME} tg_vl_frete_negoc  THEN DO:
IF INPUT FRAME {&FRAME-NAME} tg_vl_frete_negoc  <> tg_vl_frete_negoc THEN 
   //gera log somente na alteraá∆o da negociaá∆o de frete
   RUN esapi/cria-log-pedvenda.p (INPUT fi-nr-pedido:SCREEN-VALUE,
                                  INPUT fi-cliente:SCREEN-VALUE,
                                  INPUT "Valor de Frete Negociado: " + fi-vlr-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME} ,
                                  INPUT NO).
END.   
ELSE DO:
   IF logFreteDestacado THEN DO:
      ASSIGN  cb-tp-frete:SCREEN-VALUE IN FRAME {&frame-name} = 'Cif Destaque NF'
           fi-vlr-frete:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(vlFreteDestacado) 
           .                                                                           
   END.
...
------------------------------------------------------------------------------*/
  DISPLAY tg-oc-cliente tg_comis_neg fi-vlr-frete fi-dt-implant cb-tp-pedido 
          ...
          ed-obs fi-denominacao tg-nao-aprovar tg-bloqueio fi-nome-estabel 
          fi-desc-moeda fi-completo fi-nr-pedido fi-tot-qtd-ped fi-tot-qtd-res 
          fi-tot-qtd-fat fi-tot-vlr-ped fi-desc-cond-pag fi-tot-desconto 
          fi-tot-vlr-abe fi-reserva fi-perc-comis-vend fi-cidade-cli fi-uf 
          fi-sit-preco fi-cidade-cli-tri fi-uf-tri fi-sit-cred fi-observ-nf 
          tg_vl_frete_negoc 
      WITH FRAME F-Main IN WINDOW w-window.
  ENABLE tg_comis_neg fi-vlr-frete cb-tp-pedido fi-cliente cb-fin-venda 
         fi-ped-repres fi-cod-cond-pag fi-nome-transp fi-nome-tr-red 
         cb-tp-frete cb-aprov-ped fi-cod-rota fi-data-base fi-cod-estabel 
         ed-obs br-ped-item bt-inc fi-nr-pedido bt-sair bt-preco-frete bt-log 
         bt-cons-cliente-tri fi-observ-nf tg_vl_frete_negoc RECT-1 RECT-2 
         RECT-3 RECT-4 RECT-7 
      WITH FRAME F-Main IN WINDOW w-window.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE finalizarBos w-window 
...
------------------------------------------------------------------------------*/
  IF valid-handle(h-bonat001) THEN
      delete object h-bonat001 NO-ERROR.

   
   IF VALID-HANDLE(hBoEmitente) THEN DO:
      RUN finalizarBos IN hBoEmitente.
      delete object hBoEmitente NO-ERROR.
   END.
   
   IF VALID-HANDLE(hBoRepres) THEN DO:
      RUN finalizarBos IN hBoRepres.
      delete object hBoRepres NO-ERROR.
   END.
      

   IF VALID-HANDLE(hBoCondPagtoPed) THEN
      delete object hBoCondPagtoPed NO-ERROR.
      
      
   IF VALID-HANDLE(h-bo-preco-item) THEN DO:
     RUN finalizarBos IN h-bo-preco-item.
     delete object h-bo-preco-item NO-ERROR.
   END.
     
   IF VALID-HANDLE(hBoTranspCli) THEN DO:
      RUN finalizar IN hBoTranspCli.
      delete object hBoTranspCli NO-ERROR.             
   END. 
   
      
   IF VALID-HANDLE(hBoHistAvalPedVenda) THEN DO:
      RUN finalizarBos IN hBoHistAvalPedVenda.
      delete object hBoHistAvalPedVenda NO-ERROR.             
   END.  
   ...
   IF VALID-HANDLE(hBoPedVenda) THEN DO:
      ...
      delete object hBoPedVenda NO-ERROR.
   END.
...
------------------------------------------------------------------------------*/
  ...
  IF NOT VALID-HANDLE(hBoTranspCli) THEN DO:
  ...
  &if "{&FNC_MULTI_IDIOMA}" = "Yes" &then
      run utp/ut-lstit.p (input-output c-lst-prioridade).
      ASSIGN cb-prioridade:LIST-ITEM-PAIRS = c-lst-prioridade.
  &else
      ASSIGN cb-prioridade:LIST-ITEMS = c-lst-prioridade.
  &endif
  
  ASSIGN cb-prioridade:SCREEN-VALUE = ENTRY(1,c-lst-prioridade).
  */
  ASSIGN cb-prioridade:SCREEN-VALUE = '10'.
  ...
  CASE p-acao:
      ...
      WHEN 'Modificar' THEN DO.
          ...
          ELSE DO.
             ...
             IF ped-venda-ext.tp-frete = 'Cif Destaque NF' THEN
                ASSIGN fi-vlr-frete:SENSITIVE = YES
                       bt-preco-frete:SENSITIVE = YES.
          ...
          IF ped-venda-ext.l-etiqueta THEN  /* Pedido em Separaá∆o */
             ASSIGN bt-copia-it-ped:SENSITIVE = NO.
      END.
...
------------------------------------------------------------------------------*/
    ...
    DEFINE VARIABLE l-divide-comis      AS LOGICAL.     
    DEFINE VARIABLE de-perc-comis-vend  AS DECIMAL.
    DEFINE VARIABLE de-perc-comis-rep   AS DECIMAL.
    DEFINE VARIABLE idClasseComis       AS INTEGER     NO-UNDO.
    DEFINE VARIABLE percComisClassC     AS DECIMAL     NO-UNDO.
    ...
    IF p-campanha = '' AND  // N∆o Exste campnha para PI
       cb-ext-tp-pedido = 'PI' AND fi-nr-container <> 0 THEN 
    ...
    RUN getIdClasseComis   IN h-bo-preco-item(INT(p-controlePreco),OUTPUT idClasseComis).
    
    IF  NUM-RESULTS("br-ped-item":U) = 0 THEN DO:
        ASSIGN idClasseComisValid = idClasseComis.
    
       RUN getPercClasseComis IN h-bo-preco-item(int(idClasseComis),OUTPUT percComisClassC).
       IF idClasseComisValid > 0 THEN
       DO:
           fi-perc-comis-rep:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(percComisClassC).
       END.
                   
                    
    END.
    ELSE DO:
       IF idClasseComis <> idClasseComisValid THEN  DO:       
          ASSIGN c-msg-erro = 'Classe De Comiss∆o do Item Corrente difere dos demais itens do pedido'.     
          RETURN 'nok'.
       END.    
    END.  
...
------------------------------------------------------------------------------*/
    ...
    IF VALID-HANDLE(h-boPrecosItemRef) THEN DO:
       RUN finalizarBos IN h-boPrecosItemRef.
       delete object h-boPrecosItemRef.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-buscar-transp w-window 
PROCEDURE pi-buscar-transp :
DEFINE INPUT PARAMETER pCliente         AS INTEGER     NO-UNDO.
DEFINE INPUT PARAMETER pCodFrete        AS CHAR     NO-UNDO.

RUN setProp IN hboTranspCli('codEstab',0,fi-cod-estabel:SCREEN-VALUE IN FRAME {&FRAME-NAME}).
RUN setProp IN hboTranspCli('codCliente',0, string(pCliente)).
RUN setProp IN hboTranspCli('codTpFrete',0, pCodFrete).
RUN exec    IN hboTranspCli.
RUN getTransportadora IN hboTranspCli(OUTPUT iTransp, OUTPUT idTranspEstab).
/*MESSAGE 'cliente:' pCliente
        
    VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/


FIND transporte WHERE
    transporte.cod-transp = iTransp NO-LOCK NO-ERROR.

IF AVAIL transporte THEN
  ASSIGN fi-nome-transp:SCREEN-VALUE = transporte.nome-abrev.

ASSIGN lAtuTransp = YES .
       ...
       CASE ENTRY(i-ct,c-results).
            ...
            WHEN 'nat-operacao' THEN DO.
                 RUN esapi/cria-log-pedvenda.p (INPUT tt-ped-venda.nr-pedcli,
                                                ...
                                                INPUT NO).
            END.
            WHEN 'cod-portador' THEN DO.
                 RUN esapi/cria-log-pedvenda.p (INPUT tt-ped-venda.nr-pedcli,
                                                INPUT tt-ped-venda.nome-abrev,
                                                INPUT "Alterado Portador, para: " + STRING(tt-ped-venda.cod-portador),
                                                INPUT NO).
            END.
...
------------------------------------------------------------------------------*/
   IF AVAIL ped-venda THEN DO.
      ...
      IF c-Lst-preposto = '' then
         ASSIGN cb-preposto:VISIBLE IN FRAME {&FRAME-NAME}  = FALSE.
      ELSE
         ASSIGN cb-preposto:LIST-ITEM-PAIRS  = c-lst-preposto.  
      /*IF c-lst-preposto <> '' THEN DO.
          &if "{&FNC_MULTI_IDIOMA}" = "Yes" &then
...
------------------------------------------------------------------------------*/
    ...
    IF cb-tp-pedido:SCREEN-VALUE = 'Amostra' THEN DO.
       ...
       IF AVAIL b-estabelec AND
          b-estabelec.estado = 'ES' THEN DO.
         
          RUN esapi/calcula-preco.p (INPUT fi-cod-estabel:SCREEN-VALUE,
                                     INPUT ITEM.it-codigo,
                                     OUTPUT de-preco-medio). 
          ASSIGN tt-itens-ped.vl-preori:SCREEN-VALUE IN BROWSE {&browse-name} = STRING(de-preco-medio).
       END.                          
    END.          
...
------------------------------------------------------------------------------*/
    IF p-acao = 'Modificar' THEN DO.
       IF STRING(ped-venda-ext.tb_preco_id) <> INPUT FRAME {&FRAME-NAME} cb-tab-preco THEN DO.
          FIND b-tbs_preco WHERE
               b-tbs_preco.tb_preco_id = ped-venda-ext.tb_preco_id NO-LOCK NO-ERROR.
          FIND tbs_preco WHERE
               tbs_preco.tb_preco_id = INTEGER(INPUT FRAME {&FRAME-NAME} cb-tab-preco) 
               NO-LOCK NO-ERROR.
          ...
          RUN esapi/cria-log-pedvenda.p (INPUT ped-venda.nr-pedcli,
                                         INPUT ped-venda.nome-abrev,
                                         INPUT "Alterado Tabela de Preáo, de: " + b-tbs_preco.descricao + " para: " + tbs_preco.descricao,
                                         INPUT NO).
       END.
        ...
        CASE tt-itens-ped.tp-acao:
            WHEN 'Incluir' THEN DO.
               ...
               ASSIGN ped-item-ext.cod-estabel   = tt-itens-ped.cod-estabel  
                ...
                IF INTEGER(INPUT FRAME {&FRAME-NAME} cb-tp-preco) = 2  THEN
...
------------------------------------------------------------------------------*/
    ...
    DO TRANSACTION:
        ...
        ELSE
           ASSIGN tt-ped-venda.cod-portador = 9.
        ...
        // Informaá‰es Complementares do Pedido
        FIND ped-venda-ext WHERE
        ...
        END.
        ASSIGN logFreteNegAnt = ped-venda-ext.log_vl_frete_negoc.

        ASSIGN ped-venda-ext.preposto = INPUT FRAME {&FRAME-NAME} cb-preposto
               ped-venda-ext.tp-pedido = INPUT FRAME {&FRAME-NAME} cb-tp-pedido
               ped-venda-ext.LOG_vl_frete_negoc = INPUT FRAME {&FRAME-NAME} tg_vl_frete_negoc 
               ped-venda-ext.vl_frete_calc      = vlFreteDestacado .

        RUN pi-logs.

        ASSIGN ped-venda-ext.tb_preco_id = INTEGER(INPUT FRAME {&FRAME-NAME} cb-tab-preco)
        ...
        IF (ped-venda.des-pct-desconto-inform <> "" AND ped-venda.ind-sit-desconto <> 2) OR
           ...
           RUN esapi/cria-log-pedvenda.p (INPUT tt-ped-venda.nr-pedcli,
                                          ...
                                          INPUT NO).
        END.      
        RUN tratarModFreteNeg.

        RUN validarFrete. //foi retirado daqui por n∆o caber mais   
        ...
        IF VALID-HANDLE(h-bodi018) THEN
           delete object h-bodi018.
        IF VALID-HANDLE(h-bodi157) THEN
           delete object h-bodi157.
    END.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH ped-item OF ped-venda NO-LOCK,
        ...
        ASSIGN tt-itens-ped.bloqueio-fat = ped-item-ext.bloqueio-fat
               ...
               tt-itens-ped.num-id-preco = STRING(ped-item-ext.cod_controle_preco) + "|" + 
                                           ped-item-ext.num-id-liquida
               /*
...
------------------------------------------------------------------------------*/
    ...
    IF //tg_comis_neg OR
       cb-tab-preco <> '1' OR
       fi-perc-comis-rep <> repres.comis-direta THEN
...
------------------------------------------------------------------------------*/
   ...
   ELSE DO.
       /* Busca Estoque */ 
       ASSIGN var-glb-cod-estab-ini  = p-cod-estabel
              var-glb-cod-estab-fim  = p-cod-estabel
              var-glb-refer-ini      = p-cod-refer
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-validar-troca-tb-preco w-window 
PROCEDURE pi-validar-troca-tb-preco :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
FOR EACH tt-itens-ped:
    IF tt-itens-ped.cod-sit-item  = 1 THEN DO:
        run pi-busca-preco(tt-itens-ped.it-codigo,tt-itens-ped.cod-refer,'',output precoReal, OUTPUT precoDolar, OUTPUT id).
        IF id = '0' OR id = '' THEN DO:
           RETURN "NOK".
        END.
    END.
END.
...
------------------------------------------------------------------------------*/
    ...
    IF p-acao = "Modificar" AND 
       ...
       ASSIGN fi-dt-entrega:SCREEN-VALUE = STRING(ped-venda.dt-entrega).
       APPLY 'ENTRY' TO fi-dt-entrega.
       RETURN 'ADM-ERROR'.
    END.
    */
    ...
    */
    ...
    IF emitente.ind-cre-cli = 4 // Cliente Suspenso
       THEN DO.
    ...
    IF cb-tp-frete:SCREEN-VALUE = 'Cif Destaque NF' AND
       INPUT FRAME {&FRAME-NAME} fi-vlr-frete = 0 THEN DO.
       MESSAGE 'Valor do Frete deve ser Informado para Cif Destaque NF'
              VIEW-AS ALERT-BOX ERROR BUTTONS OK.
       APPLY 'ENTRY' TO fi-vlr-frete.
       RETURN 'ADM-ERROR'.
    END.
    ...
    IF INTEGER(INPUT FRAME {&FRAME-NAME} cb-tp-preco) = 2 AND
       INTEGER(INPUT FRAME {&FRAME-NAME} cb-tab-preco) = 0 THEN DO.
    ...
    IF AVAIL tt-itens-ped AND 
       INTEGER(INPUT FRAME {&FRAME-NAME} cb-tp-preco) = 2 THEN DO.
       MESSAGE 'Pedido com Inconsistencia....' SKIP
               'Existem Itens sem ID do Preáo' SKIP
...
------------------------------------------------------------------------------*/
   ...
   ELSE DO.
       ...
       ASSIGN c-cod-depos = IF INPUT FRAME {&FRAME-NAME} fi-cod-estabel = '505'
                            THEN 'ITA' ELSE 'ARM'.
       ...
       FOR EACH tt-itens-ped WHERE
           ...
           CREATE tt-ref-item.
           ASSIGN tt-ref-item.cod-refer   = tt-itens-ped.cod-refer
                  tt-ref-item.cod-depos   = c-cod-depos
                  tt-ref-item.preco-un    = tt-itens-ped.vl-preuni
                  tt-ref-item.qt-pedida   = tt-itens-ped.qt-pedida
                  tt-ref-item.vl-tot-ref  = tt-itens-ped.vl-preuni * tt-itens-ped.qt-pedida.
       END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE tratarModFreteNeg w-window 
PROCEDURE tratarModFreteNeg :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
ASSIGN lModificouFreteNegociado = logFreteNegAnt
               <> LOGICAL(tg_vl_frete_negoc:SCREEN-VALUE IN FRAME {&FRAME-NAME})
               AND LOGICAL(tg_vl_frete_negoc:SCREEN-VALUE IN FRAME {&FRAME-NAME}) = YES 
               .
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE tratarVlFrete w-window 
PROCEDURE tratarVlFrete :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
IF pInicio THEN DO:
   IF AVAIL ped-venda-ext THEN DO:
      ASSIGN tg_vl_frete_negoc:screen-value IN FRAME {&FRAME-NAME} = STRING(ped-venda-ext.log_vl_frete_negoc)
              .
...
ASSIGN fi-vlr-frete:SENSITIVE IN FRAME {&FRAME-NAME} = tg_vl_frete_negoc:SCREEN-VALUE = 'yes'.


IF p-acao = 'consultar' THEN
   ASSIGN fi-vlr-frete:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
END PROCEDURE.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE validarFrete w-window 
PROCEDURE validarFrete :
/* Valida Frete */
IF p-acao = 'Incluir' OR 
   ...
   THEN DO.
    ...
    IF lModificouFreteNegociado THEN DO:
       ASSIGN ped-venda.cod-sit-com = 1.
       RUN esapi/cria-log-pedvenda.p (INPUT ped-venda.nr-pedcli,
                                         INPUT ped-venda.nome-abrev,
                                         INPUT "Pedido Com Frete negociado. Colocado em Aprovaá∆o de frete.",
                                         INPUT NO).
