...
*******************************************************************************/
{include/i-prgvrs.i G01ES055 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS mp-tipo.codigo 
&Scoped-define ENABLED-TABLES mp-tipo
&Scoped-define FIRST-ENABLED-TABLE mp-tipo
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS mp-tipo.codigo 
&Scoped-define DISPLAYED-TABLES mp-tipo
&Scoped-define FIRST-DISPLAYED-TABLE mp-tipo
...
DEFINE FRAME d-vapara
     mp-tipo.codigo AT ROW 1.5 COL 23 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 4 BY .88 TOOLTIP "C¢digo do Tipo de Algod∆o."
...
ON GO OF FRAME d-vapara /* V† Para Tipos de Algod∆o */
DO:
  find mp-tipo no-lock 
      where mp-tipo.codigo = input frame {&frame-name} mp-tipo.codigo no-error.
  if  not avail mp-tipo then do:
      {utp/ut-table.i espec mp-tipo 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mp-tipo).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Tipos de Algod∆o */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE mp-tipo THEN 
    DISPLAY mp-tipo.codigo 
      WITH FRAME d-vapara.
  ENABLE mp-tipo.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
