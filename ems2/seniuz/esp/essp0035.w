...
*******************************************************************************/
{include/i-prgvrs.i ESSP0035 2.04.00.000}
...
DEFINE VAR cod-setor-ini LIKE set-mec.codigo.
DEFINE VAR cod-setor-fin LIKE set-mec.codigo.
...
DEFINE VARIABLE h_q01es026 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es026 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-consim /* Setores da Mecƒnica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Setores da Mecƒnica */
DO:
...
DO:
  RUN esdlg/d01es026.w (OUTPUT cod-setor-ini, OUTPUT cod-setor-fin, OUTPUT arq-saida).

  IF arq-saida <> "" THEN DO:
     RUN pi-gera-excel (INPUT cod-setor-ini, INPUT cod-setor-fin, INPUT arq-saida).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es026 ).
       RUN set-position IN h_v01es026 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es026.w,
                     ProgVaPara = esgo/g01es026.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es026 ).
       RUN set-position IN h_q01es026 ( 1.50 , 67.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es026. */
       RUN add-link IN adm-broker-hdl ( h_q01es026 , 'Record':U , h_v01es026 ).

       /* Links to SmartQuery h_q01es026. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es026 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es026 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es026 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es026 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0035" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER p-cod-setor-ini LIKE set-mec.codigo.
    DEF INPUT PARAMETER p-cod-setor-fin LIKE set-mec.codigo.
    ...
    ASSIGN c-Lin = c-empresa + " - " + " TABELA DE SETORES DA MEC¶NICA - DATA: " + STRING(TODAY) +
                                       " - " + STRING(TIME,"HH:MM").
    ...
    FOR EACH set-mec WHERE set-mec.codigo >= p-cod-setor-ini
                       AND set-mec.codigo <= p-cod-setor-fin
                     NO-LOCK:
    
        DDE SEND i-canal SOURCE set-mec.codigo    ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
        DDE SEND i-canal SOURCE set-mec.descricao ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
