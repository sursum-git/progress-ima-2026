...
&Scoped-Define ENABLED-FIELDS tipo-def.cod-tipo-def 
&Scoped-define ENABLED-TABLES tipo-def
&Scoped-define FIRST-ENABLED-TABLE tipo-def
&Scoped-Define ENABLED-OBJECTS fi-desc-tipo-def bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-FIELDS tipo-def.cod-tipo-def 
&Scoped-define DISPLAYED-TABLES tipo-def
&Scoped-define FIRST-DISPLAYED-TABLE tipo-def
&Scoped-Define DISPLAYED-OBJECTS fi-desc-tipo-def 
...
DEFINE FRAME d-vapara
     tipo-def.cod-tipo-def AT ROW 1.17 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 3.14 BY .88
     fi-desc-tipo-def AT ROW 1.17 COL 12.29 COLON-ALIGNED NO-LABEL
     bt-ok AT ROW 2.46 COL 2.14
     bt-cancela AT ROW 2.46 COL 13
     bt-ajuda AT ROW 2.46 COL 58
     rt-button AT ROW 2.21 COL 1
     SPACE(0.13) SKIP(0.07)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "V  Para Tipos de Defeitos de Fabrica‡Æo"
...
ON GO OF FRAME d-vapara /* V  Para Tipos de Defeitos de Fabrica‡Æo */
DO:
  find tipo-def no-lock 
      where tipo-def.cod-tipo-def = input frame {&frame-name} tipo-def.cod-tipo-def no-error.
  if  not avail tipo-def then do:
      {utp/ut-table.i espec tipo-def 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(tipo-def).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Tipos de Defeitos de Fabrica‡Æo */
DO:  
...
&Scoped-define SELF-NAME tipo-def.cod-tipo-def
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tipo-def.cod-tipo-def d-vapara
ON LEAVE OF tipo-def.cod-tipo-def IN FRAME d-vapara /* Codigo */
DO:
  FIND tipo-def WHERE tipo-def.cod-tipo-def = INPUT FRAME {&FRAME-NAME} tipo-def.cod-tipo-def
                NO-LOCK NO-ERROR.
  IF AVAIL tipo-def THEN
     ASSIGN fi-desc-tipo-def:SCREEN-VALUE IN FRAME {&FRAME-NAME} = tipo-def.descricao.
  ELSE DO:
     MESSAGE "Tipo Defeito inv lido." VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-desc-tipo-def 
      WITH FRAME d-vapara.
  IF AVAILABLE tipo-def THEN 
    DISPLAY tipo-def.cod-tipo-def 
      WITH FRAME d-vapara.
  ENABLE tipo-def.cod-tipo-def fi-desc-tipo-def bt-ok bt-cancela bt-ajuda 
         rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
