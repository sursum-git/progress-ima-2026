...
&Scoped-Define ENABLED-OBJECTS rt-button btn_param 
...
DEFINE VARIABLE h_q01es047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es047 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btn_param 
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo de OB's */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo de OB's */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es047 ).
       RUN set-position IN h_v01es047 ( 2.67 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 16.50 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es047.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01es047 ).
       RUN set-position IN h_q01es047 ( 1.50 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es047. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es047 ).
       RUN add-link IN adm-broker-hdl ( h_q01es047 , 'Record':U , h_v01es047 ).

       /* Links to SmartQuery h_q01es047. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es047 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             btn_param:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-cadsim ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es047 ,
             btn_param:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
   ...
   {utp/ut9000.i "ESSP0100" "2.04.00.000"}
