...
DEFINE VARIABLE h_b01es003 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es003 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Composi‡äes de Itens"
         HEIGHT             = 16.5
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Composi‡äes de Itens */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Composi‡äes de Itens */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es003.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es003 ).
       RUN set-position IN h_b01es003 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.00 ) */

       /* Links to SmartBrowser h_b01es003. */
       RUN add-link IN adm-broker-hdl ( h_b01es003 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es003.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es003 ).
       RUN set-position IN h_b02es003 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.00 ) */

       /* Links to SmartBrowser h_b02es003. */
       RUN add-link IN adm-broker-hdl ( h_b02es003 , 'State':U , THIS-PROCEDURE ).
