...
*******************************************************************************/
{include/i-prgvrs.i ESSP0008 2.04.00.000}
...
DEFINE VARIABLE h_q01es041 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es041 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadsim /* Testes de ASTM */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Testes de ASTM */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es041.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es041 ).
       RUN set-position IN h_v01es041 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es041.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom\z01es041.w,
                     ProgVaPara = esgo\g01es041.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es041 ).
       RUN set-position IN h_q01es041 ( 1.75 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es041. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es041 ).
       RUN add-link IN adm-broker-hdl ( h_q01es041 , 'Record':U , h_v01es041 ).

       /* Links to SmartQuery h_q01es041. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es041 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es041 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es041 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es041 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0008" "2.04.00.000"}
