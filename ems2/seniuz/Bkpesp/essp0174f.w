...
&Scoped-define FIELDS-IN-QUERY-br-pedidos tt-fat.tipo tt-fat.nr-nota-fis tt-fat.dt-emis-nota tt-fat.dt-saida tt-fat.nr-pedcli tt-fat.nome-ab-cli tt-fat.no-ab-reppri tt-fat.qtde tt-fat.valor   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-pedidos   
&Scoped-define SELF-NAME br-pedidos
&Scoped-define OPEN-QUERY-br-pedidos RUN pi-total. OPEN QUERY {&SELF-NAME} FOR EACH tt-fat  NO-LOCK                               BY tt-fat.dt-emis-nota                               BY tt-fat.nr-nota-fis.
&Scoped-define TABLES-IN-QUERY-br-pedidos tt-fat
&Scoped-define FIRST-TABLE-IN-QUERY-br-pedidos tt-fat
...
tt-fat.nr-nota-fis  COLUMN-LABEL "N.Fiscal"  FORMAT "x(9)" 
tt-fat.dt-emis-nota COLUMN-LABEL "Dt.Emissao"    WIDTH 9.15  COLUMN-BGCOLOR 8
...
tt-fat.valor        COLUMN-LABEL "Valores Faturados"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME D-Dialog
     ...
     fi-tot-qtd-nor AT ROW 13.08 COL 65.29 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-nor AT ROW 13.13 COL 78 COLON-ALIGNED NO-LABEL
     bt-detalhe AT ROW 13.21 COL 2.57
     bt-imprime AT ROW 13.21 COL 16.14
     bt-vapara AT ROW 13.21 COL 29.57
     fi-tot-qtd-ljs AT ROW 14.08 COL 65.29 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ljs AT ROW 14.08 COL 78 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-ind AT ROW 15.08 COL 65.29 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ind AT ROW 15.08 COL 78 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-out AT ROW 16.08 COL 65.29 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-out AT ROW 16.08 COL 78 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-geral AT ROW 17.08 COL 65.29 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-geral AT ROW 17.08 COL 78 COLON-ALIGNED NO-LABEL
     bt-ok AT ROW 18.75 COL 2.72
     bt-cancela AT ROW 18.75 COL 13.72
     bt-ajuda AT ROW 18.75 COL 82.29
     rt-buttom AT ROW 18.5 COL 1.72
     "Normal:" VIEW-AS TEXT
          SIZE 6 BY .88 AT ROW 13 COL 61.14
     "Outros:" VIEW-AS TEXT
          SIZE 5.57 BY .88 AT ROW 16 COL 61.57
     "Lojas:" VIEW-AS TEXT
          SIZE 4.57 BY .88 AT ROW 14 COL 62.29
     "Industrializaá∆o:" VIEW-AS TEXT
          SIZE 11 BY .88 AT ROW 15 COL 55.57
     "Total Geral:" VIEW-AS TEXT
          SIZE 8 BY .88 AT ROW 17 COL 58.57
     SPACE(28.71) SKIP(2.32)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "Detalha Pedidos do Periodo - ESSP0174F"
...
/* Query rebuild information for BROWSE br-pedidos
     _START_FREEFORM
RUN pi-total.
OPEN QUERY {&SELF-NAME} FOR EACH tt-fat  NO-LOCK
                              BY tt-fat.dt-emis-nota
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Detalha Pedidos do Periodo - ESSP0174F */
DO:  
...
DO:
    ...
    IF c-nr-pedcli <> ? THEN DO:
       FIND tt-fat WHERE
            tt-fat.nr-pedcli = c-nr-pedcli NO-LOCK NO-ERROR. 
       ...
       h-query:REPOSITION-TO-ROWID(ROWID(tt-fat)) NO-ERROR.
...
------------------------------------------------------------------------------*/
  ...
  IF p-tp-relat = 1 THEN
     ASSIGN  br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DOS PEDIDOS FATURADOS DO PERIODO: " + 
                                                       SUBSTR(p-periodo,1,2) + "/" + SUBSTR(p-periodo,3,4).
  ELSE
     ASSIGN  br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DOS PEDIDOS FATURADOS DO EXERCICIO: " + 
...
------------------------------------------------------------------------------*/
    ...
    IF p-tp-relat = 1 THEN DO:
       PUT "RELATORIO DE FATURAMENTO DO PERIODO " AT 36.
       PUT SUBSTR(p-periodo,1,2) + "/" + substr(p-periodo,3,4) AT 72 SKIP(1).
    END.
    ELSE DO:
       PUT "RELATORIO DE FATURAMENTO DO EXERCICIO " AT 36.
       PUT SUBSTR(p-periodo,3,4) AT 72 SKIP(1).
    END.
...
------------------------------------------------------------------------------*/
 ...
 CASE i-saida:
     ...
     WHEN 3 THEN DO.
         ASSIGN c-saida = SESSION:TEMP-DIRECTORY + "essp0174f.tmp".
         OUTPUT TO VALUE(c-saida).
     END.
 END CASE.
