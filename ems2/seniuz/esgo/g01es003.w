...
&Scoped-Define ENABLED-FIELDS composi.cod-composi 
&Scoped-define ENABLED-TABLES composi
&Scoped-define FIRST-ENABLED-TABLE composi
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS composi.cod-composi composi.descricao 
&Scoped-define DISPLAYED-TABLES composi
&Scoped-define FIRST-DISPLAYED-TABLE composi
...
DEFINE FRAME d-vapara
     composi.cod-composi AT ROW 1.17 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 5.14 BY .88
     composi.descricao AT ROW 1.17 COL 14.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 46.14 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Composi‡Æo de Item"
...
/* SETTINGS FOR FILL-IN composi.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para Composi‡Æo de Item */
DO:
  find composi no-lock 
      where composi.cod-composi = input frame {&frame-name} composi.cod-composi no-error.
  if  not avail composi then do:
      {utp/ut-table.i espec composi 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(composi).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Composi‡Æo de Item */
DO:  
...
&Scoped-define SELF-NAME composi.cod-composi
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL composi.cod-composi d-vapara
ON LEAVE OF composi.cod-composi IN FRAME d-vapara /* Codigo */
DO:
  FIND composi WHERE composi.cod-composi = INPUT FRAME {&FRAME-NAME} composi.cod-composi 
               NO-LOCK NO-ERROR.
  IF AVAIL composi THEN
     ASSIGN composi.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = composi.descricao.
  ELSE DO:
     MESSAGE "Composi‡Æo inv lida." VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE composi THEN 
    DISPLAY composi.cod-composi composi.descricao 
      WITH FRAME d-vapara.
  ENABLE composi.cod-composi bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
