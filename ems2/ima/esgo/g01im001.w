...
&Scoped-Define ENABLED-OBJECTS rt-button c_cod-param bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS c_cod-param 
...
DEFINE VARIABLE c_cod-param AS CHARACTER FORMAT "x(50)":U 
     LABEL "Parƒmetro" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     c_cod-param AT ROW 2 COL 9 COLON-ALIGNED WIDGET-ID 2
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         FONT 1
         TITLE "V  Para Parƒmetros Gerais"
...
ON GO OF FRAME d-vapara /* V  Para Parƒmetros Gerais */
DO:
FIND im-param NO-LOCK
    WHERE im-param.cod-param EQ INPUT FRAME {&FRAME-NAME} c_cod-param NO-ERROR.

IF NOT AVAILABLE im-param THEN DO:
    {utp/ut-table.i espec im-param 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(im-param).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Parƒmetros Gerais */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY c_cod-param 
      WITH FRAME d-vapara.
  ENABLE rt-button c_cod-param bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
