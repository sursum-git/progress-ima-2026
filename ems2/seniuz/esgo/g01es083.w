...
*******************************************************************************/
{include/i-prgvrs.i G01ES083 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS fi-num-etiqueta bt-ok bt-cancela bt-ajuda ~
rt-button 
...
DEFINE VARIABLE fi-num-etiqueta AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     ...
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 TOOLTIP "N£mero da etiqueta gerada na montagem da peáa." NO-UNDO.
...
DEFINE FRAME d-vapara
     fi-num-etiqueta AT ROW 1.17 COL 30 COLON-ALIGNED
     bt-ok AT ROW 2.63 COL 2.14
     bt-cancela AT ROW 2.63 COL 13
     bt-ajuda AT ROW 2.63 COL 58
     rt-button AT ROW 2.38 COL 1
     SPACE(0.13) SKIP(0.11)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V† Para Etiqueta da Sala de Pano Cru"
...
ON GO OF FRAME d-vapara /* V† Para Etiqueta da Sala de Pano Cru */
DO:
FIND etiq-pano-cru NO-LOCK
    WHERE etiq-pano-cru.num-etiqueta EQ INPUT FRAME {&FRAME-NAME} fi-num-etiqueta NO-ERROR.

IF NOT AVAILABLE etiq-pano-cru THEN DO:
    {utp/ut-table.i espec etiq-pano-cru 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(etiq-pano-cru).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Etiqueta da Sala de Pano Cru */
DO:  
...
ON LEAVE OF fi-num-etiqueta IN FRAME d-vapara /* Numero Etiqueta */
DO:
   FIND etiq-pano-cru WHERE etiq-pano-cru.num-etiqueta = INPUT FRAME {&FRAME-NAME} fi-num-etiqueta NO-LOCK NO-ERROR.
   IF NOT AVAIL etiq-pano-cru THEN DO.
      MESSAGE "Etiqueta n∆o Cadastrada..." VIEW-AS ALERT-BOX.
      APPLY 'entry' TO SELF.
      RETURN NO-APPLY.
   END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-num-etiqueta 
      WITH FRAME d-vapara.
  ENABLE fi-num-etiqueta bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ES083" "2.04.00.000"}
