...
DEFINE BUTTON bt-arq-excel 
     IMAGE-UP FILE "image/im-open3.bmp":U
     LABEL "" 
     SIZE 4 BY 1 TOOLTIP "Caminho/Nome do arquivo excel.".
...
DEFINE VARIABLE tg-gerar-excel AS LOGICAL INITIAL no 
     ...
     VIEW-AS TOGGLE-BOX
     SIZE 17.14 BY .88 TOOLTIP "Imprimir ParÉmetros/Seleá∆o ao final do relat¢rio."
     FONT 1 NO-UNDO.
...
DEFINE FRAME f-pg-sel
     ...
     IMAGE-4 AT ROW 3.54 COL 34.29
     IMAGE-5 AT ROW 4.54 COL 34.29
...
DEFINE FRAME f-pg-par
     ...
     to-imp-param AT ROW 6.58 COL 28
     tg-tp-pedido AT ROW 2.21 COL 25.29
          ...
          "Dispon°vel para classificaá∆o/indicaá∆o pr¢pria do usu†rio" NO-LABEL
     fi-arq-excel AT ROW 9.33 COL 30 COLON-ALIGNED
     tg-gerar-excel AT ROW 9.33 COL 7.57
     bt-arq-excel AT ROW 9.33 COL 68.14 HELP
          "Caminho/Nome do arquivo excel."
     RECT-27 AT ROW 6 COL 4
     RECT-28 AT ROW 1.5 COL 4
     RECT-29 AT ROW 8.79 COL 4
     "Pedidos do Tipo:" VIEW-AS TEXT
          SIZE 12 BY .75 AT ROW 3.21 COL 13.29
     "Planilha para Excel" VIEW-AS TEXT
          SIZE 13.57 BY .54 AT ROW 8.5 COL 30.86
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 77.14 BY 10.5
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Resumo do Faturamento em M/R$"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 28.63
         MAX-WIDTH          = 146.29
...
/* SETTINGS FOR FRAME f-pg-par
   Custom                                                               */
/* SETTINGS FOR BUTTON bt-arq-excel IN FRAME f-pg-par
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-relat /* Resumo do Faturamento em M/R$ */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Resumo do Faturamento em M/R$ */
DO:
...
&Scoped-define SELF-NAME bt-arq-excel
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-arq-excel w-relat
ON CHOOSE OF bt-arq-excel IN FRAME f-pg-par
DO:
   SYSTEM-DIALOG GET-FILE fi-arq-excel
   ...
   IF OKpressed = TRUE THEN DO:
      IF fi-arq-excel <> "" AND (NOT fi-arq-excel MATCHES "*.xls") THEN
         ASSIGN fi-arq-excel:SCREEN-VALUE = fi-arq-excel + ".xls".
      ELSE
         ASSIGN fi-arq-excel:SCREEN-VALUE = fi-arq-excel.
   END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-arq-excel w-relat
ON LEAVE OF bt-arq-excel IN FRAME f-pg-par
DO:
   IF NOT(INPUT FRAME {&FRAME-NAME} fi-arq-excel MATCHES "*.xls*") THEN DO:
      MESSAGE "Nome do arquivo deve terminar em .XLS."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      ASSIGN fi-arq-excel:SCREEN-VALUE IN FRAME f-pg-par = SESSION:TEMP-DIRECTORY + "res-fatur.xls".
      APPLY 'entry' TO fi-arq-excel IN FRAME {&FRAME-NAME}.
      RETURN NO-APPLY.
   END.
END.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY tg-tp-normal to-imp-param tg-tp-pedido fi-tp-pedido-1 fi-tp-pedido-2 
          fi-tp-pedido-3 fi-tp-pedido-4 fi-tp-pedido-5 fi-arq-excel 
          tg-gerar-excel 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE to-imp-param tg-tp-pedido tg-gerar-excel RECT-27 RECT-28 RECT-29 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ...
  ENABLE fi-cod-estabel fi-ini-dt-emissao fi-fin-dt-emissao fi-ini-it-codigo 
         fi-fin-it-codigo fi-ini-ge-codigo fi-fin-ge-codigo IMAGE-1 IMAGE-2 
         IMAGE-4 IMAGE-5 IMAGE-7 IMAGE-8 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    {include/i-rprun.i esrp/esft0020rp.p}
