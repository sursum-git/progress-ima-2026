...
DEFINE INPUT PARAMETER TABLE FOR tt-nfe.  
...
DEF VAR c-nome-abrev-rep AS CHAR.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-work D-Dialog _FREEFORM
  QUERY br-work NO-LOCK DISPLAY
      ...
      tt-work.nome-ab-cli   COLUMN-LABEL "Cliente"   WIDTH 23  
/*       tt-work.nr-pedcli     COLUMN-LABEL "Pedido" WIDTH 6 */
      tt-work.no-ab-reppri  COLUMN-LABEL "Representante" WIDTH 23
      tt-work.qtd-nota      COLUMN-LABEL "Quantidade"   WIDTH 13
      tt-work.vl-tot-nota   COLUMN-LABEL "Valor NF"    WIDTH 13
/*       tt-work.nro-docto     COLUMN-LABEL "NF Ent." WIDTH 6 */
/*       tt-work.qtd-devol     COLUMN-LABEL "Qtd Ent."        */
/*       tt-work.vlr-devol     COLUMN-LABEL "Valor Ent."      */
/*       tt-work.dt-devol      COLUMN-LABEL "Dt."             */
/*       tt-work.motivo        COLUMN-LABEL "Motivo"          */
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 93.29 BY 11.92
         FONT 1
         TITLE "NFE" ROW-HEIGHT-CHARS .46.
...
DEFINE FRAME D-Dialog
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         FONT 1
         TITLE "NFE de Mercadorias - ESSP0191E"
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* NFE de Mercadorias - ESSP0191E */
DO:  
...
ON VALUE-CHANGED OF br-work IN FRAME D-Dialog /* NFE */
DO:
...
DO:
  IF AVAIL tt-work THEN DO:
      FIND docum-est WHERE
           docum-est.nro-docto   = tt-work.nr-nota-fis AND
           docum-est.cod-estabel = tt-work.cod-estabel       AND
           docum-est.serie-docto = tt-work.serie NO-LOCK NO-ERROR.
     IF AVAIL docum-est THEN DO:
        ASSIGN gr-nota-fiscal = ROWID(docum-est).
...
------------------------------------------------------------------------------*/
  ...
  /* {utp/ut9000.i "ESSP0191E" "2.04.00.000"} */ 
...
------------------------------------------------------------------------------*/
 ...
 PUT "NFE DE MERCADORIAS NO PERIODO DE: " + 
      p-dt-faturadas-ini + " A " + p-dt-faturadas-fin FORMAT "x(100)" AT 39 SKIP(1).                          

 PUT "N.Fiscal Dt.Emissao Cliente      Pedido Representante Quantidade Valor NF    " AT 1.  
 PUT "-------- ---------- ------------ ------ ------------- ---------- ----------- " AT 1.
...
/*         ACCUMULATE b-tt-work.vlr-dev     (TOTAL). */
    ...
    IF (ACCUM TOTAL b-tt-work.qtd-nota) > 0 THEN DO:
       ...
       PUT ACCUM TOTAL b-tt-work.vl-tot-nota FORMAT ">>>>,>>9.99"  AT  66 SKIP.
...
/*
 ASSIGN  br-work:TITLE IN FRAME {&FRAME-NAME} = "NFE DE MERCADORIAS NO PERIODO DE: " +
                                                p-dt-faturadas-ini + " A " + 
                                               p-dt-faturadas-fin.
/*                                                  */
/*   FOR EACH tt-nfe.                               */
/*           MESSAGE tt-nfe.serie-docto SKIP        */
/*                   tt-nfe.nro-docto   SKIP        */
/*                   tt-nfe.cod-emitente SKIP       */
/*                   tt-nfe.nat-operacao SKIP       */
/*                   tt-nfe.qtd-devol    SKIP       */
/*                   tt-nfe.vlr-devol    SKIP       */
/*                                                  */
...
/*   END.                                           */
 ...
 FOR EACH tt-nfe NO-LOCK.
     FIND docum-est WHERE
/*           docum-est.serie-docto  = tt-nfe.serie-docto  AND */
          docum-est.nro-docto    = tt-nfe.nro-docto    AND
          docum-est.cod-emitente = tt-nfe.cod-emitente AND
          docum-est.nat-operacao = tt-nfe.nat-operacao NO-LOCK NO-ERROR.
...
/*      FOR EACH item-doc-est OF docum-est NO-LOCK. */
         ...
         RUN pi-acompanhar IN h-acomp (INPUT "Data: "    + STRING(docum-est.dt-trans) +
                                            " Nota Fiscal: " + docum-est.nro-docto).
         FIND docum-est WHERE
/*               docum-est.cod-estabel  = docum-est.cod-estabel   AND */
/*               docum-est.serie        = item-doc-est.serie-comp AND */
              docum-est.nr-nota-fis  = docum-est.nro-docto   NO-LOCK NO-ERROR.
/*          IF AVAIL docum-est THEN DO:                           */
/*             FOR EACH it-nota-fisc OF docum-est NO-LOCK.        */
/*                  ACCUMULATE it-nota-fisc.qt-faturada[1] (TOTAL). */
/*              END.                                                */
/*          END.                                                    */
/*          FIND tt-work WHERE                                             */
/*               tt-work.nr-nota-fis = docum-est.nr-nota-fis AND         */
/*               tt-work.nro-docto   = tt-nfe.nro-docto  NO-LOCK NO-ERROR. */
/*          IF NOT AVAIL tt-work THEN DO:                                  */
             ...
             IF AVAIL docum-est THEN  DO: 
               CREATE tt-work.
               ASSIGN  tt-work.dt-emis-nota = ?.
               ASSIGN tt-work.nr-nota-fis  = docum-est.nr-nota-fis 
                      tt-work.cod-estabel  = docum-est.cod-estabel
                      tt-work.serie        = docum-est.serie
                      tt-work.dt-emis-nota = docum-est.dt-emis-nota  
                      tt-work.nome-ab-cli  = docum-est.nome-ab-cli   
                      tt-work.nr-pedcli    = docum-est.nr-pedcli     
                      tt-work.no-ab-reppri = docum-est.no-ab-reppri
                      tt-work.qtd-nota     = tt-nfe.qtd-devol.
                      tt-work.vl-tot-nota  = tt-nfe.vlr-devol. 

/*                       tt-work.qtd-nota     = ACCUM TOTAL it-nota-fisc.qt-faturada[1] */
/*                       tt-work.vl-tot-nota  = docum-est.vl-tot-nota.                */
                IF docum-est.despesa-nota <> 0 THEN.  /* Frete */
    /*                ASSIGN tt-work.vlr-devol = docum-est.despesa-nota               */
    /*                       fi-tot-vlr        = fi-tot-vlr + docum-est.despesa-nota. */
                   ...
                   ASSIGN fi-tot-vlr        = fi-tot-vlr + tt-nfe.vlr-devol
                          fi-tot-qtd        = fi-tot-qtd + tt-nfe.qtd-devol.
    /*              ASSIGN fi-tot-vlr        = fi-tot-vlr + docum-est.vl-tot-nota                  */
    /*                     fi-tot-qtd        = fi-tot-qtd + ACCUM TOTAL it-nota-fisc.qt-faturada[1]. */
...
/*          ASSIGN tt-work.qtd-devol = tt-work.qtd-devol + item-doc-est.quantidade      */
/*                 tt-work.vlr-devol = tt-work.vlr-devol + item-doc-est.preco-total[1]  */
/*                 fi-tot-qtd        = fi-tot-qtd        + item-doc-est.quantidade      */
/*                 fi-tot-vlr        = fi-tot-vlr        + item-doc-est.preco-total[1]. */
/*       END. */
 END. 
*/

 FOR EACH tt-nfe NO-LOCK
        BREAK BY tt-nfe.cod-rep.

     FIND repres WHERE
          repres.cod-rep = tt-nfe.cod-rep NO-LOCK NO-ERROR.
     IF AVAIL repres THEN DO.
        ASSIGN c-nome-abrev-rep = repres.nome-abrev.
     END.


      RUN pi-acompanhar IN h-acomp (INPUT "Nota Fiscal : "    +  tt-nfe.nr-nota-fis +
                                                " Repres.: " + STRING(tt-nfe.cod-rep, "99999")).

     FIND docum-est WHERE
          docum-est.cod-estabel = tt-nfe.cod-estabel AND
          docum-est.serie-docto = tt-nfe.serie       AND
          docum-est.nro-docto   = tt-nfe.nr-nota-fis NO-LOCK NO-ERROR.
     IF AVAIL docum-est THEN DO.
            ...
            FIND tt-work WHERE
                 tt-work.nr-nota-fis = docum-est.nro-docto AND
                 tt-work.cod-estabel = docum-est.cod-estabel AND
                 tt-work.serie       = docum-est.serie-docto      
                 NO-LOCK NO-ERROR.
            IF NOT AVAIL tt-work THEN DO:
               CREATE tt-work.
               ASSIGN tt-work.nr-nota-fis  = docum-est.nro-docto 
                      tt-work.cod-estabel  = docum-est.cod-estabel 
                      tt-work.serie        = docum-est.serie-docto       
                      tt-work.dt-emis-nota = docum-est.dt-emissao
                      tt-work.nome-ab-cli  = c-nome-abrev-rep    
                      tt-work.no-ab-reppri = c-nome-abrev-rep     
                      tt-work.vl-tot-nota  = docum-est.tot-valor
                      tt-work.qtd-nota     = ACCUM TOTAL item-doc-est.quantidade.       
            END.
            IF AVAIL tt-work THEN DO.
               ASSIGN fi-tot-qtd        = fi-tot-qtd  + tt-work.qtd-nota    
                      fi-tot-vlr        = fi-tot-vlr  + tt-work.vl-tot-nota.
            END.
 ...
 END.
            ...
            /* IF LAST-OF(tt-nfe.cod-rep) THEN DO:
               

               CREATE tt-work.
               ASSIGN tt-work.cod-rep      = tt-nfe.cod-rep
                      tt-work.no-ab-reppri = c-nome-abrev-rep
