...
07/2023 - acrescimo do tipo de atividade do cliente triangular no critÇrio de busca da regra de natureza de operaá∆o.
04/2024 - mudanáa do procedimento retornarTipoAtividadeCliente para buscar pela ordem crescente do tipo de atividade
(1-primaria/2-secundaria) antes aparentemente era(1-primaria/0-secundaria) - Tadeu
************************************************************************************************************************/
//{utp/ut-glob.i}
...
{esp/util.i}
{esp/params.i}
{esp/getCaminhoSistOper.i}
{esapi/desenharLogNatOperacao.i}
...
PROCEDURE retornarTipoAtividadeCliente:
    ...
    FOR EACH emitente_cnae NO-LOCK
        ...
        AND emitente_cnae.eliminado = NO
         BY emitente_cnae.cod_tipo_cnae . // retirado o Desc
...
RUN esporadicos/log02.p PERSISTENT SET  hLog02.
...
DEFINE VARIABLE iTipoAtividadeTriang    AS INTEGER     NO-UNDO.
...
DEFINE VARIABLE iEmitenteTriang         AS INTEGER     NO-UNDO.
DEFINE VARIABLE codCnae                 AS CHAR       NO-UNDO.
DEFINE VARIABLE codCnaeTriang           AS CHAR       NO-UNDO.
...
DEFINE VARIABLE cDescTipoAtivTriang     AS CHARACTER   NO-UNDO.
...
DEFINE VARIABLE caminho                 AS CHARACTER   NO-UNDO.



ASSIGN cArquivoLog =  getDirERP() +  getCaminhoSistOper('\especificos\logs\regra_nat_oper\log_nat_operacao-' + c-seg-usuario + "_" +  STRING(TIME) + '.html').
RUN limparErros.
...
FIND FIRST bfEmitente
    WHERE bfEmitente.nome-abrev = pNomeAbrevTriang NO-LOCK NO-ERROR. 
IF AVAIL bfEmitente THEN
   ASSIGN iEmitenteTriang = bfEmitente.cod-Emitente .
...
IF AVAIL emitente THEN DO:
    ASSIGN cEstado       = emitente.estado
           lOpcaoSimples = IF SUBSTR(emitente.char-1,133,1) = 'S' THEN YES ELSE NO.
   ...
   RUN retornarTipoAtividadeCliente(iEmitenteTriang, OUTPUT iTipoAtividadeTriang, OUTPUT codCnaeTriang).
   RUN buscarDescTipoAtividade(INPUT iTipoAtividade ,OUTPUT cDescTipoAtiv ).
   RUN buscarDescTipoAtividade(INPUT iTipoAtividadeTriang ,OUTPUT cDescTipoAtivTriang ).
   ...
   RUN inserirTTCab('finalidade_venda',cDescFinVenda,"Finalidad de Venda").               
   RUN inserirTTCab('estab',pCodEstab,"Estabelecimento").               
   RUN inserirTTCab('cliente',pCliente,"Cliente").               
   RUN inserirTTCab('estado',cEstado,"Estado").               
   RUN inserirTTCab('contribuinte',cLabelContrib,"Contribuinte").               
   RUN inserirTTCab('suframa',cLabelSuframa,"Suframa").               
   RUN inserirTTCab('tipo_atividade_cliente',cDescTipoAtiv,"Tipo de Atividade Cliente").               
   RUN inserirTTCab('cnae_cliente',cDescTipoAtiv,"Codigo CNAE do Cliente").               
   RUN inserirTTCab('enquadramento_ttd',getDescrEnquadTTD(cEnquadTTD),"Enquadramento TTD").  
...
IF pNomeAbrevTriang <> '' THEN DO:
   ...
   ASSIGN     cLabelContrib     =  IF lContrIcmsTriang THEN "SIM"    ELSE "NAO"
              ...
              .
   RUN inserirTTCab('cliente_triang',pNomeAbrevTriang,"Cliente Triangular").                 
   RUN inserirTTCab('contrib_triang',cLabelContrib,"Contribuinte Triangular").                 
   RUN inserirTTCab('relac_estado'  ,cLabelRelacEstado,"Relaá∆o Estado").                 
   RUN inserirTTCab('suframa'       ,cLabelSuframa,"Suframa").
...
IF AVAIL emitente THEN DO:
   ...
   IF  iTipoAtividade = 0 THEN DO:
        ...
        RUN inserirTTCab('tipo_atividade_zerada'       ,"SIM","Tipo Atividade Zerada").
       IF codCnae <> '' THEN DO:
          RUN criarMensagem('erro',"Cliente com CNAE:" + codCnae + " - sem Tipo de Atividade").
          RUN inserirTTCab('com_cnae_sem_ativ', codCnae ,"Tipo Atividade Zerada com CNAE").
       END.
          
       ELSE DO:
          RUN criarMensagem('erro',"Cliente sem CNAE cadastrado").
          RUN inserirTTCab('sem_cnae_sem_ativ'       ,'SIM',"Tipo Atividade Zerada e sem CNAE").
       END.
          
       
   END.
   ELSE DO:
        RUN inserirTTCab('tipo_atividade_zerada'       ,"NAO","Tipo Atividade Zerada").
           ...
           IF lNext = NO THEN DO:
              IF lVendaTriangular = YES THEN DO:
                  ...
                  FOR EACH bfParamNatOperacao NO-LOCK
                     ...
                     // desconsidera o registro que tem o tipo de atividade informada e diferente  do cliente triangular
                     // parte-se do principio que os clientes triangulares tem os cnaes cadastrados corretamente
                     IF  bfParamNatOperacao.cod_tipo_atividade <> 0 AND iTipoAtividadeTriang <> bfParamNatOperacao.cod_tipo_atividade THEN NEXT.
...
IF VALID-HANDLE(hlog02) THEN DO:
   DELETE OBJECT hLog02 NO-ERROR.
