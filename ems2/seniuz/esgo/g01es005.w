...
&Scoped-Define ENABLED-FIELDS cota-fam.fm-codigo 
...
&Scoped-Define DISPLAYED-FIELDS cota-fam.fm-codigo 
...
DEFINE FRAME d-vapara
     cota-fam.fm-codigo AT ROW 1.17 COL 10 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .88
     fi-desc-familia AT ROW 1.17 COL 20.29 COLON-ALIGNED NO-LABEL
     bt-ok AT ROW 2.54 COL 2.14
     bt-cancela AT ROW 2.54 COL 13
     bt-ajuda AT ROW 2.54 COL 58
     rt-button AT ROW 2.29 COL 1
     SPACE(0.13) SKIP(0.16)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "V† Para Cotas de Vendas por Fam°lia"
...
ON GO OF FRAME d-vapara /* V† Para Cotas de Vendas por Fam°lia */
DO:
  find cota-fam no-lock 
      where cota-fam.fm-codigo = input frame {&frame-name} cota-fam.fm-codigo no-error.
  if  not avail cota-fam then do:
      {utp/ut-table.i espec cota-fam 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(cota-fam).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Cotas de Vendas por Fam°lia */
DO:  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cota-fam.fm-codigo d-vapara
ON LEAVE OF cota-fam.fm-codigo IN FRAME d-vapara /* Familia */
DO:
  ...
  IF AVAIL familia THEN DO:
     ...
     FIND cota-fam WHERE cota-fam.fm-codigo = familia.fm-codigo NO-LOCK NO-ERROR.
     IF NOT AVAIL cota-fam THEN DO:
        MESSAGE "Fam°lia ainda n∆o tem informaá‰es de Cotas de Vendas." VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
     END.
  END.
  ELSE DO:
...
------------------------------------------------------------------------------*/
  DISPLAY fi-desc-familia 
      WITH FRAME d-vapara.
  IF AVAILABLE cota-fam THEN 
    DISPLAY cota-fam.fm-codigo 
      WITH FRAME d-vapara.
  ENABLE cota-fam.fm-codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
