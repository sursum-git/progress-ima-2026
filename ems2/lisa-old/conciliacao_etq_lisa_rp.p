...
ASSIGN cPlanilhaExcel = 'excel/analiseEtqLisa.xlsx'.
IF SEARCH(cPlanilhaExcel) <> ? THEN DO:
   OS-COMMAND SILENT VALUE('start excel -t ' + SEARCH(cPlanilhaExcel) ).
...
FOR EACH ttEtq
    ...
    IF AVAIL ttEtqMed THEN DO:
       ...
       RUN esapi/atuLocalizEtq('505',ttEtqMed.idEtq,ttEtq.localiz).
    END.
