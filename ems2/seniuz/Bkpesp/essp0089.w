...
*******************************************************************************/
{include/i-prgvrs.i ESSP0056 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS btn-importa rt-button 
...
DEFINE VARIABLE h_q01es012 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es012 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     btn-importa AT ROW 1.21 COL 59.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Invent rio de Ötens Acabados"
         HEIGHT             = 10.71
         WIDTH              = 89.14
         MAX-HEIGHT         = 27.79
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Invent rio de Ötens Acabados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Invent rio de Ötens Acabados */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-importa w-cadsim
ON CHOOSE OF btn-importa IN FRAME f-cad /* Button 1 */
DO:
  ASSIGN g-tp-dados = "INVENTARIO".
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es012.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es012 ).
       RUN set-position IN h_v01es012 ( 2.75 , 1.72 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es012.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es012.w,
                     ProgVaPara = esgo/g01es012.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es012 ).
       RUN set-position IN h_q01es012 ( 1.50 , 68.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es012. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es012 ).
       RUN add-link IN adm-broker-hdl ( h_q01es012 , 'Record':U , h_v01es012 ).

       /* Links to SmartQuery h_q01es012. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es012 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es012 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es012 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es012 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0089" "2.04.00.000"}
  ...
  RUN enable-inclui IN h_p-cadsim (INPUT NO).
