...
*******************************************************************************/
{include/i-prgvrs.i ESPD0010 2.04.00.000}
...
DEFINE VARIABLE h_b01di159 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02di135 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b03di154 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01ad098 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01ad098 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Clientes com Pedidos/Faturamento"
         HEIGHT             = 17
         WIDTH              = 91.29
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 91.72
...
ON END-ERROR OF w-concom /* Clientes com Pedidos/Faturamento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Clientes com Pedidos/Faturamento */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 75.14 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01ad098 ).
       RUN set-position IN h_v01ad098 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.75 , 89.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Pedidos|Itens Pedido|Notas Pedido' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.58 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.33 , 90.43 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = adzoom/z02ad098.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01ad098 ).
       RUN set-position IN h_q01ad098 ( 1.50 , 67.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01ad098. */
       RUN add-link IN adm-broker-hdl ( h_q01ad098 , 'Record':U , h_v01ad098 ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01di159 ).
       RUN set-position IN h_b01di159 ( 9.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 89.57 ) */

       /* Links to SmartBrowser h_b01di159. */
       RUN add-link IN adm-broker-hdl ( h_q01ad098 , 'Record':U , h_b01di159 ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03di154 ).
       RUN set-position IN h_b03di154 ( 9.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.00 , 89.57 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1') NO-ERROR.

       /* Links to BrowseDigitacao h_b03di154. */
       RUN add-link IN adm-broker-hdl ( h_b01di159 , 'Record':U , h_b03di154 ).

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02di135.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02di135 ).
       RUN set-position IN h_b02di135 ( 9.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.92 , 89.57 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('1') NO-ERROR.

       /* Links to SmartBrowser h_b02di135. */
       RUN add-link IN adm-broker-hdl ( h_b01di159 , 'Record':U , h_b02di135 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESPD0010" "2.04.00.000"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-primeira-pagina w-concom 
PROCEDURE pi-primeira-pagina :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
