...
*******************************************************************************/
{include/i-prgvrs.i Malote 2.00.04.000}
...
DEFINE VARIABLE h_q01malote-4 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01malote AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 90 BY 12.04 WIDGET-ID 100.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01malote.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01malote ).
       RUN set-position IN h_v01malote ( 2.92 , 1.43 ) NO-ERROR.
       /* Size in UIB:  ( 9.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01malote.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01malote.w,
                     ProgVaPara = esgo/g01malote.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01malote-4 ).
       RUN set-position IN h_q01malote-4 ( 1.25 , 61.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01malote. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01malote ).
       RUN add-link IN adm-broker-hdl ( h_q01malote-4 , 'Record':U , h_v01malote ).

       /* Links to SmartQuery h_q01malote-4. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01malote-4 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01malote-4 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01malote-4 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01malote ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Manuten‡Æo de Malote" "2.00.04.000"}
