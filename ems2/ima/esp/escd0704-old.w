...
/* Essa Procedure foi definida como Fun‡Æo pela Totvs na include/i-fun.i 
...
DEFINE FRAME f-cad
    ...
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         ...
         AT COL 1 ROW 1
         SIZE 89.72 BY 9.71 WIDGET-ID 100.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Dados Complementares do Cliente"
         HEIGHT             = 9.71
         WIDTH              = 89.72
         MAX-HEIGHT         = 40.5
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF w-cadsi2 /* Dados Complementares do Cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Dados Complementares do Cliente */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v01ad098 ).
       RUN set-position IN h_v01ad098 ( 2.79 , 1.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q01ad098 ).
       RUN set-position IN h_q01ad098 ( 1.33 , 60.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01ad098. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v01ad098 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01ad098 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCD0704" "2.06.00.001"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-disable-close w-cadsi2 
PROCEDURE pi-disable-close :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
...
DEFINE VARIABLE vhWinHdl  AS WIDGET-HANDLE       NO-UNDO.
  ...
  /* Note, if this is run from within a dialog-box the HWND of the frame is needed and there is no need 
     to call the following function (getParent).  The only stipulation is that the dialog frame MUST 
     be visible in order to retrieve the HWND. */
  IF hFrame:TYPE EQ "DIALOG-BOX" THEN
    ASSIGN hFrame:VISIBLE = TRUE
           hFrame:HIDDEN  = FALSE
           viWinHWND      = hFrame:HWND.

  /* Comentado e alterado para Executar a FUN€ÇO da TOTVS
  IF viWinHWND = 0 THEN
    RUN GetParent({&WINDOW-NAME}:HWND,
                  OUTPUT viWinHWND).
  */
  IF viWinHWND = 0 THEN
     ASSIGN viWinHWND = GetParent ( {&WINDOW-NAME}:HWND ). 
