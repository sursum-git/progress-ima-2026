...
*******************************************************************************/
{include/i-prgvrs.i ESSP0060 2.04.00.000}
...
DEFINE NEW GLOBAL SHARED VAR gr-ref-item     AS ROWID NO-UNDO.
...
&Scoped-Define ENABLED-OBJECTS btn-parametro to-codigo-ean rt-button 
&Scoped-Define DISPLAYED-OBJECTS to-programacao to-codigo-ean 
...
DEFINE VARIABLE h_q01in375 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01in375 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btn-parametro 
     IMAGE-UP FILE "image/im-param2.bmp":U
     LABEL "Button 1" 
     SIZE 4 BY 1.17 TOOLTIP "Sele‡Æo do intervalo de C¢digos de Fam¡lias.".
...
DEFINE VARIABLE to-codigo-ean AS LOGICAL INITIAL no 
     LABEL "EAN" 
     VIEW-AS TOGGLE-BOX
     SIZE 5.72 BY 1 TOOLTIP "Indica que o tipo de manuten‡Æo a ser feita ‚ C¢digo EAN."
     BGCOLOR 14 FGCOLOR 1 FONT 1 NO-UNDO.

DEFINE VARIABLE to-programacao AS LOGICAL INITIAL no 
     LABEL "PCP" 
     VIEW-AS TOGGLE-BOX
     SIZE 5.72 BY 1 TOOLTIP "Indica que o tipo de manuten‡Æo a ser feita ‚ Programa‡Æo de Produ‡Æo."
...
DEFINE FRAME f-cad
     btn-parametro AT ROW 1.42 COL 44.43
     to-programacao AT ROW 1.5 COL 49.29
     to-codigo-ean AT ROW 1.5 COL 55.72
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Ötens/Referˆncias - Dados Complementares"
         HEIGHT             = 11.71
         WIDTH              = 90
         MAX-HEIGHT         = 20.71
         MAX-WIDTH          = 114.29
...
/* SETTINGS FOR FRAME f-cad
   L-To-R                                                               */
/* SETTINGS FOR TOGGLE-BOX to-programacao IN FRAME f-cad
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-cadsi2 /* Ötens/Referˆncias - Dados Complementares */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Ötens/Referˆncias - Dados Complementares */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-parametro w-cadsi2
ON CHOOSE OF btn-parametro IN FRAME f-cad /* Button 1 */
DO:
  RUN esdlg/d01in375.w.
  RUN adm-open-query-cases IN h_q01in375.
...
&Scoped-define SELF-NAME to-codigo-ean
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL to-codigo-ean w-cadsi2
ON VALUE-CHANGED OF to-codigo-ean IN FRAME f-cad /* EAN */
DO:
   ASSIGN l-codigo-ean  = INPUT FRAME {&frame-name} to-codigo-ean.
   IF l-codigo-ean THEN
      ASSIGN l-programacao = NOT l-codigo-ean
             to-programacao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = string(NOT l-codigo-ean).
END.
...
&Scoped-define SELF-NAME to-programacao
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL to-programacao w-cadsi2
ON VALUE-CHANGED OF to-programacao IN FRAME f-cad /* PCP */
DO:
   ASSIGN l-programacao = INPUT FRAME {&frame-name} to-programacao.
   IF l-programacao THEN
      ASSIGN l-codigo-ean  = NOT l-programacao
             to-codigo-ean:SCREEN-VALUE IN FRAME {&FRAME-NAME} = string(NOT l-programacao).
END.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01in375.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01in375 ).
       RUN set-position IN h_v01in375 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01in375.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01in375.w,
                     ProgVaPara = esgo/g01in375.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01in375 ).
       RUN set-position IN h_q01in375 ( 1.25 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01in375. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v01in375 ).
       RUN add-link IN adm-broker-hdl ( h_q01in375 , 'Record':U , h_v01in375 ).

       /* Links to SmartQuery h_q01in375. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01in375 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01in375 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01in375 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01in375 ,
             to-codigo-ean:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  DISPLAY to-programacao to-codigo-ean 
      WITH FRAME f-cad IN WINDOW w-cadsi2.
  ENABLE btn-parametro to-codigo-ean rt-button 
      WITH FRAME f-cad IN WINDOW w-cadsi2.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-create-objects w-cadsi2 
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(THIS-PROCEDURE) THEN
     ASSIGN h-w-cadsi2 = THIS-PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0060" "2.04.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF gr-ref-item <> ? THEN
     RUN pi-reposiciona-query IN h_q01in375 (INPUT gr-ref-item).
  ELSE
     RUN dispatch IN h_q01in375 ( INPUT "get-first":U).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-desabilita w-cadsi2 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    ASSIGN btn-parametro:SENSITIVE IN FRAME {&FRAME-NAME}  = NO
           to-programacao:SENSITIVE IN FRAME {&FRAME-NAME} = NO
           to-codigo-ean:SENSITIVE IN FRAME {&FRAME-NAME}  = NO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-habilita w-cadsi2 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    ASSIGN btn-parametro:SENSITIVE IN FRAME {&FRAME-NAME} = YES
           /*to-programacao:SENSITIVE IN FRAME {&FRAME-NAME} = YES */
           to-codigo-ean:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
END PROCEDURE.
