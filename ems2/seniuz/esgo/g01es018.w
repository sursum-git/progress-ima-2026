...
&Scoped-Define ENABLED-FIELDS maq-benef.codigo 
&Scoped-define ENABLED-TABLES maq-benef
&Scoped-define FIRST-ENABLED-TABLE maq-benef
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS maq-benef.codigo maq-benef.descricao 
&Scoped-define DISPLAYED-TABLES maq-benef
&Scoped-define FIRST-DISPLAYED-TABLE maq-benef
...
DEFINE FRAME d-vapara
     maq-benef.codigo AT ROW 1.17 COL 12 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9.14 BY .88
     maq-benef.descricao AT ROW 1.17 COL 21.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 21.14 BY .88
     bt-ok AT ROW 2.54 COL 2.14
     bt-cancela AT ROW 2.54 COL 13
     bt-ajuda AT ROW 2.54 COL 58
     rt-button AT ROW 2.29 COL 1
     SPACE(0.13) SKIP(0.07)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         TITLE "V  Para M quinas do Beneficiamento"
...
/* SETTINGS FOR FILL-IN maq-benef.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para M quinas do Beneficiamento */
DO:
  find maq-benef no-lock 
      where maq-benef.codigo = input frame {&frame-name} maq-benef.codigo no-error.
  if  not avail maq-benef then do:
      {utp/ut-table.i espec maq-benef 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(maq-benef).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para M quinas do Beneficiamento */
DO:  
...
&Scoped-define SELF-NAME maq-benef.codigo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL maq-benef.codigo d-vapara
ON LEAVE OF maq-benef.codigo IN FRAME d-vapara /* Codigo */
DO:
  FIND maq-benef WHERE maq-benef.codigo = INPUT FRAME {&FRAME-NAME} maq-benef.codigo 
                 NO-LOCK NO-ERROR.
  IF AVAIL maq-benef THEN
     ASSIGN maq-benef.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = maq-benef.descricao.
  ELSE DO:
...
------------------------------------------------------------------------------*/
  IF AVAILABLE maq-benef THEN 
    DISPLAY maq-benef.codigo maq-benef.descricao 
      WITH FRAME d-vapara.
  ENABLE maq-benef.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
