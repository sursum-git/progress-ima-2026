...
*******************************************************************************/
{include/i-prgvrs.i ESSP0178 2.04.00.000}
...
DEFINE VAR num-etiqueta-ini LIKE etiq-pano-cru.num-etiqueta.
DEFINE VAR num-etiqueta-fin LIKE etiq-pano-cru.num-etiqueta.
DEFINE VAR data-trans-ini   LIKE etiq-pano-cru.data-trans.
DEFINE VAR data-trans-fin   LIKE etiq-pano-cru.data-trans.
DEFINE VAR classificacao    AS INTEGER.
...
DEFINE VARIABLE h_q01es083 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es083 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     IMAGE-UP FILE "image/excel.bmp":U
     LABEL "Button 1" 
     SIZE 5 BY 1.13 TOOLTIP "Gerar uma lista de Etiquetas para uma planilha Excel.".
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Etiquetas de Pano Cru"
         HEIGHT             = 5.96
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-consim /* Consulta Etiquetas de Pano Cru */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Etiquetas de Pano Cru */
DO:
...
DO:
  RUN esdlg/d01es083.w (OUTPUT num-etiqueta-ini, 
                        OUTPUT num-etiqueta-fin, 
                        OUTPUT data-trans-ini,
                        OUTPUT data-trans-fin,
                        OUTPUT classificacao,
                        OUTPUT arq-saida).

  IF arq-saida <> "" THEN DO:
     RUN pi-gera-excel (INPUT num-etiqueta-ini,
                        INPUT num-etiqueta-fin,
                        INPUT data-trans-ini,  
                        INPUT data-trans-fin,
                        INPUT classificacao,
                        INPUT arq-saida).      
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es083.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es083 ).
       RUN set-position IN h_v01es083 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es083.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es083.w,
                     ProgVaPara = esgo/g01es083.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es083 ).
       RUN set-position IN h_q01es083 ( 1.50 , 50.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es083. */
       RUN add-link IN adm-broker-hdl ( h_q01es083 , 'Record':U , h_v01es083 ).

       /* Links to SmartQuery h_q01es083. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es083 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es083 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es083 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es083 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0178" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER p-num-etiqueta-ini LIKE etiq-pano-cru.num-etiqueta.
    DEF INPUT PARAMETER p-num-etiqueta-fin LIKE etiq-pano-cru.num-etiqueta.
    DEF INPUT PARAMETER p-data-trans-ini   LIKE etiq-pano-cru.data-trans.
    DEF INPUT PARAMETER p-data-trans-fin   LIKE etiq-pano-cru.data-trans.
    DEF INPUT PARAMETER p-classificacao    AS INTEGER.
    ...
    ASSIGN c-Lin = c-empresa + " - " + " ETIQUETAS DE PANO CRU - DATA: " + STRING(TODAY).
    DDE SEND i-canal SOURCE c-Lin ITEM "L1C1".
    
    IF p-classificacao = 1 THEN DO:
       DDE SEND i-canal SOURCE "NUM.ETIQUETA" ITEM "L3C1". 
       DDE SEND i-canal SOURCE "DATA-TRANS"   ITEM "L3C2".
    END.
    ELSE DO:
       DDE SEND i-canal SOURCE "DATA-TRANS"   ITEM "L3C1".
       DDE SEND i-canal SOURCE "NUM.ETIQUETA" ITEM "L3C2". 
    END.
    DDE SEND i-canal SOURCE "NUMEROS DOS TEARES" ITEM "L3C3".
    
    IF p-classificacao = 1 THEN DO:
       DDE EXECUTE i-canal COMMAND "[select(~"C1~")]". 
       DDE EXECUTE sys     COMMAND "[column.width(12.57)]".
    ...
    DDE EXECUTE sys     COMMAND "[column.width(58)]". 
    
    DDE EXECUTE i-canal COMMAND '[select("L1C1:L5C4")]'.
    ...
    FOR EACH etiq-pano-cru WHERE etiq-pano-cru.num-etiqueta >= p-num-etiqueta-ini
                             AND etiq-pano-cru.num-etiqueta <= p-num-etiqueta-fin
                             AND etiq-pano-cru.data-trans   >= p-data-trans-ini
                             AND etiq-pano-cru.data-trans   <= p-data-trans-fin
                           NO-LOCK
        BY IF p-classificacao = 1 THEN STRING(etiq-pano-cru.num-etiqueta)
                                  ELSE STRING(etiq-pano-cru.data-trans) +
                                       STRING(etiq-pano-cru.num-etiqueta):
    
        IF p-classificacao = 1 THEN DO:
           DDE SEND i-canal SOURCE STRING(etiq-pano-cru.num-etiqueta) ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
           DDE SEND i-canal SOURCE STRING(etiq-pano-cru.data-trans)   ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
        END.
        ELSE DO:
           DDE SEND i-canal SOURCE STRING(etiq-pano-cru.data-trans)   ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
           DDE SEND i-canal SOURCE STRING(etiq-pano-cru.num-etiqueta) ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
        END.
        DDE SEND i-canal SOURCE etiq-pano-cru.num-teares ITEM "L" + TRIM(STRING(i-Lin)) + "C3".
