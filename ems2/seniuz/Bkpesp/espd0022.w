...
*******************************************************************************/
{include/i-prgvrs.i ESPD0022 2.04.00.000}
...
DEF NEW GLOBAL SHARED VAR h-b02di159 AS HANDLE.
...
DEFINE VARIABLE h_b02di159 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q02ad098 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Clientes do Representante"
         HEIGHT             = 16
         WIDTH              = 91.43
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 92
...
ON END-ERROR OF w-consim /* Consulta Clientes do Representante */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Clientes do Representante */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02ad098 ).
       RUN set-position IN h_v02ad098 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.75 , 89.72 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02di159 ).
       RUN set-position IN h_b02di159 ( 8.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.08 , 89.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q02ad098.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01ad098.w,
                     ProgVaPara = esgo/g01ad098.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q02ad098 ).
       RUN set-position IN h_q02ad098 ( 1.50 , 61.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02ad098. */
       RUN add-link IN adm-broker-hdl ( h_q02ad098 , 'Record':U , h_v02ad098 ).

       /* Links to SmartBrowser h_b02di159. */
       RUN add-link IN adm-broker-hdl ( h_q02ad098 , 'Record':U , h_b02di159 ).

       /* Links to SmartQuery h_q02ad098. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q02ad098 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q02ad098 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q02ad098 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02ad098 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02di159 ,
             h_v02ad098 , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(h_b02di159) THEN
     ASSIGN h-b02di159 = h_b02di159.
END PROCEDURE.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESPD0022" "2.04.00.000"}
