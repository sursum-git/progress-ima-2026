...
PROCEDURE getLogsUltTransPorChave:
   ...
   RUN setChave IN hBoTransacoes(pChave).    //nÆo entendi para que serve
...
PROCEDURE getLogsPorChave:
     ...
     /*MESSAGE iCalculo SKIP
             cListaIdTransacoes SKIP
         VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
     RUN limparTT.
     FOR EACH logs NO-LOCK
         WHERE lookup(string(logs.transacao_id),cListaIdTransacoes,',') > 0
         AND   logs.calculo_id = iCalculo :
         /*MESSAGE 'entrei'
             VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
         CREATE ttLogs.
