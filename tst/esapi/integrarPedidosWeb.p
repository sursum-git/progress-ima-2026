...
/************************************************************************************************** 
** Programa: integrarPedidosWeb.p
** Modulo..: Controle do Portal
** Objetivo: Analisa pedidos do portal, se os mesmos estiverem
**           status OK, a importaá∆o ser† efetuada. Com base no espd001rp com adaptaá‰es
** Autor...: Tadeu - 11/2025
***************************************************************************************************/
...
{esp/params.i}
{esp/util.i}
DEFINE INPUT  PARAMETER pAcompanha   AS LOGICAL     NO-UNDO.
DEFINE INPUT  PARAMETER pEnviarEmail AS LOGICAL     NO-UNDO.
DEFINE INPUT  PARAMETER pNrPedWeb    AS INTEGER     NO-UNDO.
...
DEFINE VARIABLE iNrPedIni AS INTEGER     NO-UNDO.
DEFINE VARIABLE iNrPedFim AS INTEGER     NO-UNDO.
...
DEFINE VARIABLE cErrosAcum  AS CHARACTER   NO-UNDO.
...
DEFINE VARIABLE nrPedidoCorrente AS INTEGER     NO-UNDO. 
...
//DEF VAR pEnviarEmail       AS LOGICAL     NO-UNDO INIT YES.
...
OUTPUT TO VALUE(SESSION:TEMP-DIRECTORY  + 'V11.TXT').
...
RUN setDados IN hBologtxt('usuario','api').
...
RUN setDados IN hBologtxt('descricao',"Base " + c-base + 'Processamento via API' ).
RUN gerarLogs IN hBoLogtxt.

// avaliaá∆o de hor†rio e dia para envio de e-mails de erro.
//RUN getAvapEnviarEmail. retirado em 08/2023
...
RUN piAcomp(INPUT 'Corrigindo situaá‰es os pedidos web').
    
RUN corrigirSitPedsWebAproGer IN hBoPedsWeb.


RUN piAcomp(INPUT "Inicio - Vencendo Pedidos...." ).  
RUN vencerPedidos.         
RUN piAcomp(INPUT "Final - Vencendo Pedidos...." ).  
...
/*       RUN pi-envia-email-erro(eAsysexcpError:GetMessage(1)).                                      */
/*       UNDO, THROW  eAsysexcpError .                                                               */
/*    END CATCH.                                                                                     */
...
FUNCTION getTipoFreteERP RETURNS CHAR(numTipoFrete AS INT):
    ...
    CASE numTipoFrete.
        ...
        WHEN 4 THEN ASSIGN cRetorno = "Cif Destaque NF".
    END CASE.
...
PROCEDURE pi-integrarPedidos:
    ...
    RUN setDados IN hBologtxt('descricao','Envia Email?' +  'SIM' ).
    RUN gerarLogs IN hBoLogtxt.
    IF pNrPedWeb > 0 THEN DO:
       ASSIGN iNrPedIni = pNrPedWeb
              iNrPedFim = pNrPedWeb.                            
    END.
    ELSE DO:
       ASSIGN iNrPedIni = 0
    ...
    RUN incrSeqDados IN hBologTxt. 
    RUN setDados IN hBologtxt('descricao','Param Pedido:' + IF STRING(pNrPedWeb) = '0' THEN "TODOS" ELSE STRING(pNrPedWeb) ).
    RUN gerarLogs IN hBoLogtxt.
    
    // 1-Em Digitaá∆o   2-Efetivado   3-Cancelado   4-Integrado   5-Rejeitado   6-Vencido  7-Aprovado
    FOR EACH peds_web 
        WHERE peds_web.ind_sit_ped_web = 2 
        AND   peds_web.ped_web_id >= iNrPedIni
        AND   peds_web.ped_web_id <= iNrPedFim  
        NO-LOCK.
        ASSIGN nrPedidoCorrente = peds_web.ped_web_id .     
        RUN incrSeqDados IN hBologTxt. 
        RUN setDados IN hBologtxt('data_hora',NOW).
        RUN setDados IN hBologtxt('nr_pedido',  nrPedidoCorrente).
        RUN setDados IN hBologtxt('acao','INICIO').
        ...
        IF NOT AVAIL itens_ped_web THEN DO.
           ASSIGN c-mens-erro = "Pedido WEB Efetiviado sem Itens, Imposs°vel importar Pedido... " + CHR(10) +
                  ...
                  cSever      = 'ERRO' .                  
           RUN incrValor(INPUT-OUTPUT cErrosAcum, c-mens-erro, CHR(13) ).           
           RUN enviarErro(c-mens-erro). 
        ...
        IF NOT AVAIL repres THEN DO: 
           RUN incluirLogPed('validarRepresPed',
                             ...
                             'ERRO').
           RUN finalizarLogPed.
           RUN incrValor(INPUT-OUTPUT cErrosAcum, 'Pedido:' + string(nrPedidoCorrente) + ' - Representante NAO encontrado:' + STRING(i-cod-rep) ,CHR(13)).
           NEXT.
        END.
        ELSE DO:
           RUN incluirLogPed('validarRepresPed',
                             'Representante encontrado:' + string(i-cod-rep)  ,
                             'INF').
        END.
        ...
        IF i-cod-vend <> 0 THEN DO.
           ...
           IF NOT AVAIL b-repres THEN DO: 
              RUN incluirLogPed('validarRepresPed',
                                ...
                                'ERRO').
              RUN finalizarLogPed.
              RUN incrValor(INPUT-OUTPUT cErrosAcum, 'Pedido:' + string(nrPedidoCorrente) + ' - Vendedor NAO encontrado:' + STRING(i-cod-vend) ,CHR(13)).
              NEXT.
           END.
           ELSE DO:
              FIND b-repres WHERE
        ...
        RUN piAcomp(INPUT "Pedido: " + STRING(peds_web.ped_web_id) ).
        ...
        */   
        ...
        IF NOT peds_web.log_operac_triang THEN DO.
           ...
           IF lOkNatOper = NO  THEN DO:
              ASSIGN c-mens-erro = 'SEM Operacao Triangular - Retornou Erro ao buscar a natureza de operacao -> pi-calc-natur-oper-> emitente:' + STRING(emitente.cod-emit)
                                  + ' ->' + c-erro-nat.
              RUN rejeitarPedido(c-mens-erro,'buscarNatOperacao'). 
              RUN incrValor(INPUT-OUTPUT cErrosAcum,'Pedido:' + STRING(nrPedidoCorrente) + c-mens-erro,CHR(13)).                                  
              NEXT.
           END.
           ELSE DO:
        ...
        IF l-erro = YES THEN DO.
           ...
           RUN incrValor(INPUT-OUTPUT cErrosAcum,cDescLog, CHR(13)).
           NEXT. 
        ...
        IF NOT AVAIL cond-pagto       AND 
           ...
           IF AVAIL tt-cond-ped THEN DO.
              ...
              RUN piAcomp (INPUT "Criando Condiá∆o Especial de Pagamento...").
    ...
    RUN setDados IN hBologtxt('descricao','Envia Email?' + IF pEnviarEmail THEN 'SIM' ELSE 'NAO').
    /*IF cErrosAcum <> '' THEN
    DO:
        UNDO, THROW NEW PROGRESS.lang.apperror(cErrosAcum,999)   .
    END.
...
PROCEDURE pi-envia-email-erro.
   ...
   RUN esapi/esapi002.p (INPUT getEmailErroIntegrPedPortal(),                         /* e-mail remetente */
                        INPUT c-destinatario,                      /* e-mail destinat†rio */
                        INPUT "Rejeiá∆o de PEDIDO - " + string(nrPedidoCorrente)  + STRING(TODAY,"99/99/9999") + " " + STRING(TIME,"HH:MM:SS"), /* Assunto */
                        INPUT c-mensagem,                          /* Mensagem */
                        INPUT "",                                  /* Anexo */
                        INPUT NOT pAcompanha).                                /* Mostra Erros */ 
   RUN incluirLogPed('pi-envia-email-erro','Enviado o e-mail pela api esapi/esapi002 com a mensagem:'
                         + c-mensagem,'INF').
                                
   IF pAcompanha = NO THEN DO:
...
PROCEDURE pi-envia-email-pedido.
   ...
   //ASSIGN c-destinatario = "tadeu.parreiras@gmail.com". 
...
PROCEDURE rejeitarPedido:
    ...
    RUN enviarErro(cMsg,pAcompanha,YES).
...
PROCEDURE piAcomp:
    ...
    IF pAcompanha THEN DO:
        RUN pi-acompanhar IN h-acomp(INPUT pMsg).
