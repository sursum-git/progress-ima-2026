...
*******************************************************************************/
{include/i-prgvrs.i ESSP0019 2.04.00.000}
...
DEFINE VARIABLE h_q01es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02es004 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v03es004 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadcom /* Controles de SAC */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadcom w-cadcom
ON WINDOW-CLOSE OF w-cadcom /* Controles de SAC */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-navega ).
       RUN set-position IN h_p-navega ( 1.33 , 2.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-cadsim ).
       RUN set-position IN h_p-cadsim ( 1.33 , 27.72 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 74.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es004.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es004 ).
       RUN set-position IN h_v01es004 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.63 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Complementos|Observa‡äes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.71 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 88.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es004.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es004.w,
                     ProgVaPara = esgo/g01es004.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es004 ).
       RUN set-position IN h_q01es004 ( 1.50 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es004. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es004 ).
       RUN add-link IN adm-broker-hdl ( h_q01es004 , 'Record':U , h_v01es004 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01es004. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es004 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es004 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es004 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es004 ,
             h_p-exihel , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02es004.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02es004 ).
       RUN set-position IN h_v02es004 ( 10.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.75 , 86.00 ) */

       /* Links to SmartViewer h_v02es004. */
       RUN add-link IN adm-broker-hdl ( h_q01es004 , 'Record':U , h_v02es004 ).
       RUN add-link IN adm-broker-hdl ( h_v01es004 , 'GROUP-ASSIGN':U , h_v02es004 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02es004 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v03es004.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v03es004 ).
       RUN set-position IN h_v03es004 ( 10.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.00 , 85.00 ) */

       /* Links to SmartViewer h_v03es004. */
       RUN add-link IN adm-broker-hdl ( h_q01es004 , 'Record':U , h_v03es004 ).
       RUN add-link IN adm-broker-hdl ( h_v01es004 , 'GROUP-ASSIGN':U , h_v03es004 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v03es004 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0021" "2.04.00.000"}
