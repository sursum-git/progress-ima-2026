...
*******************************************************************************/
{include/i-prgvrs.i ESSP0125 2.04.00.000}
...
DEFINE VARIABLE h_q01es065 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es065 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo de Cortes Comerciais"
         HEIGHT             = 10.92
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo de Cortes Comerciais */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo de Cortes Comerciais */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es065.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es065 ).
       RUN set-position IN h_v01es065 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es065.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es065.w,
                     ProgVaPara = esgo/g01es065.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es065 ).
       RUN set-position IN h_q01es065 ( 1.50 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es065. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es065 ).
       RUN add-link IN adm-broker-hdl ( h_q01es065 , 'Record':U , h_v01es065 ).

       /* Links to SmartQuery h_q01es065. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es065 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es065 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es065 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es065 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0125" "2.04.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF gr-corte-comerc <> ? THEN
     RUN pi-reposiciona-query IN h_q01es065 (INPUT gr-corte-comerc).
  ELSE
     RUN dispatch IN h_q01es065 ( INPUT "get-first":U).
