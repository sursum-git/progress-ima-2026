...
DEF VAR de-tot-retorno    AS DECIMAL.
DEF VAR de-qt-retorno     AS DEC.
...
bt-tra-def bt-fat-aut ed-obs bt-monitor bt-imp-danfe fi-cgc 
...
DEFINE BUTTON bt-tra-def AUTO-GO 
     IMAGE-UP FILE "image/im-undo.bmp":U
     LABEL "" 
     SIZE 7 BY 1.33 TOOLTIP "Retorno do Dep. Fechado"
     BGCOLOR 8 FONT 10.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-pedidos w-livre _FREEFORM
  QUERY br-pedidos NO-LOCK DISPLAY
      ...
      tt-ped-venda.vl-reservado    COLUMN-LABEL "Vlr NF Faturar"  WIDTH 10 FORMAT ">>>,>>>,>>9.99" 
      tt-ped-venda.vl-desconto     COLUMN-LABEL "Vlr Desconto"    WIDTH 10 FORMAT ">>>,>>9.99" 
      tt-ped-venda.vl-tot-ped      COLUMN-LABEL "Vlr TOTAL"       WIDTH 10 FORMAT ">>>,>>>,>>9.99" 
...
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS MULTIPLE SIZE 107 BY 14.25
         FONT 1
         TITLE "Pedidos Reservados" ROW-HEIGHT-CHARS .58.
...
DEFINE FRAME f-cad
     ...
     bt-tra-def AT ROW 15.46 COL 111
...
DO:
   IF tg-faturaveis:SCREEN-VALUE = 'yes' OR 
      ...
      ASSIGN bt-tra-def:SENSITIVE = NO
      ...
      IF AVAIL tt-ped-venda THEN DO.
         ...
         IF tt-ped-venda.restricao = 0 THEN DO.
            IF tt-ped-venda.dep-fechado = YES THEN
               ASSIGN bt-tra-def:SENSITIVE = YES
...
&Scoped-define SELF-NAME bt-tra-def
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-tra-def w-livre
ON CHOOSE OF bt-tra-def IN FRAME f-cad
DO:
   FIND ped-venda WHERE
        ped-venda.nr-pedcli = tt-ped-venda.nr-pedcli AND
        ped-venda.nome-abrev = tt-ped-venda.nome-abrev NO-LOCK NO-ERROR.
   ...
   FIND emitente WHERE
        emitente.cod-emit = estabelec.cod-emit NO-LOCK NO-ERROR.

   ASSIGN de-tot-retorno = 0.
   FOR EACH ped-item-rom WHERE
      ...
      IF AVAIL ob-etiqueta THEN
         ASSIGN de-tot-retorno = de-tot-retorno + ob-etiqueta.quantidade.
   END.
   IF de-tot-retorno = 0 THEN DO.
      MESSAGE 'Etiquetas j† foram Retornadas por Outro Usu†rio...'
   ...
   MESSAGE 'Deseja Retornar ' STRING(de-tot-retorno,">>>,>>9.99") 'm do Dep¢sito Fechado ? ' 
       VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE l-conf-retorno AS LOGICAL.

   IF NOT l-conf-retorno THEN
      RETURN NO-APPLY.
   ...
   IF RETURN-VALUE = "ADM-ERROR" THEN DO.
      MESSAGE 'Erro ao Gerar Nota Autom†tica de Transferància do Dep¢sito DEF' SKIP(1)
              'Deseja Gerar Espelho para Nota Manual ?'
          VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE l-conf-espelho AS LOGICAL.
      IF NOT l-conf-espelho THEN
         RETURN NO-APPLY.
   ...
   /*
   MESSAGE 'Gerada a NF ' nota-fiscal.nr-nota-fis SKIP(2)
            'Favor Proceder com a entrada no Recebimento (RE1001)'
       VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
   */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-1 RECT-2 rt-button bt-param bt-imprime tg-imp-danfe tg-faturaveis 
         ...
         bt-sintegra bt-modifica bt-consulta bt-pre-nota bt-log bt-tra-def 
...
------------------------------------------------------------------------------*/
    ...
    IF ped-repre.cod-classif <> '' THEN   // COMISSAO
       ASSIGN tt-ped-venda.restricao = 12.

    IF (tt-ped-venda.des-pct-desconto-inform <> "" AND   // DESCONTO
        tt-ped-venda.ind-sit-desconto <> 2) THEN 
        ASSIGN tt-ped-venda.restricao = 13.

    IF tt-ped-venda.cod-sit-preco <> 2 THEN    // PRECO
       ASSIGN tt-ped-venda.restricao = 14.

    IF tt-ped-venda.cod-sit-com <> 2 THEN    // FRETE
       ASSIGN tt-ped-venda.restricao = 17.

    IF tt-ped-venda.cod-sit-aval < 2 OR     
       tt-ped-venda.cod-sit-aval > 3 THEN  // CREDITO
       ASSIGN tt-ped-venda.restricao = 1.

    IF tt-ped-venda.restricao <> 0 THEN NEXT.
    ...
    IF AVAIL ped-venda-ext THEN DO.
        ...
        IF (ped-venda-ext.tp-pedido = '∑ Vista' OR l-avista) AND
        ...
        IF ped-venda-ext.tp-pedido <> '∑ Vista' AND
           ...
           IF natur-oper.emite-duplic THEN DO.
              IF l-valor-max-nf AND
                 ...
                 (tt-ped-venda.vl-reservado + tt-ped-venda.val-frete) > ped-venda-ext.fat-max-nf THEN DO. 
                 ASSIGN tt-ped-venda.restricao = 5.
                 NEXT.
              END.
        
              IF l-valor-min-nf AND  /* Valor M°nimo NF */
                 ((tt-ped-venda.vl-reservado + tt-ped-venda.val-frete) < (param-dis.vl-min-nf) OR
                  (tt-ped-venda.vl-aberto > 0 AND
                  (tt-ped-venda.vl-aberto + tt-ped-venda.val-frete) < (param-dis.vl-min-nf))) THEN DO. 
              ...
              IF l-valor-min-dup AND /* Valor M°nimo DUPLICATA */
                 ...
                 (((tt-ped-venda.vl-reservado + tt-ped-venda.val-frete) / tt-ped-venda.num-parcelas) < param-dis.vl-min-dup OR
                  (tt-ped-venda.vl-aberto > 0 AND
                  ((tt-ped-venda.vl-aberto + tt-ped-venda.val-frete) / tt-ped-venda.num-parcelas) < param-dis.vl-min-dup) ) THEN DO.
       ...
       /* IF ped-venda-ext.tb_preco_id = 2 THEN DO.  // Rubi
           FIND ped-venda WHERE
                ped-venda.nr-pedcli = tt-ped-venda.nr-pedcli AND
                ped-venda.nome-abrev = tt-ped-venda.nome-abrev NO-LOCK NO-ERROR.
           FIND ped-repre OF ped-venda WHERE 
                ped-repre.nome-ab-rep = tt-ped-venda.no-ab-reppri NO-LOCK NO-ERROR.
           FIND repres WHERE
                repres.nome-abrev = ped-repre.nome-ab-rep NO-LOCK NO-ERROR.

           IF AVAIL ped-repre AND 
              ped-venda-ext.l_comis_neg = NO AND
              ped-repre.perc-comis > (repres.comis-direta / 2) THEN DO.
              ASSIGN tt-ped-venda.restricao = 12.
              NEXT.
           END.
        END. */
        ...
        IF (LOOKUP(ped-venda-ext.tp-pedido,"Bonificaá∆o,Doaá∆o") > 0 AND
            ...
            ASSIGN tt-ped-venda.restricao = 15.

        IF ped-venda-ext.l-nao-aprovar THEN   // Nao aprovar
           ASSIGN tt-ped-venda.restricao = 16.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-espelho-nf w-livre 
PROCEDURE pi-espelho-nf :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    ASSIGN c-arq-saida = SESSION:TEMP-DIRECTORY + "espelho-nf.txt".

    OUTPUT TO VALUE(c-arq-saida).

    PUT "ESPELHO PARA NOTA FISCAL DE RETORNO" SKIP
        FILL("-",40) FORMAT "x(50)"  SKIP
        SKIP(1) 
        "Pedido de Venda: " ped-venda.nr-pedcli SKIP
        "Natureza de Operaá∆o: 51209M" SKIP
        SKIP(1)
        "Item       Descriá∆o                       Referencia  Quantidade" SKIP
        "---------  ------------------------------  ----------  ----------" SKIP.
    ...
    FOR EACH ped-item OF ped-venda NO-LOCK.
        ...
        PUT ped-item.it-codigo   AT 1   FORMAT "x(10)" 
            ITEM.desc-item       AT 12  FORMAT "x(30)" 
            ped-item.cod-refer   AT 44  FORMAT "x(8)"   
            de-qt-retorno        AT 56
            SKIP.
    END.
    OUTPUT CLOSE.
    ...
    ASSIGN tt-ped-venda.dep-fechado = NO.
    ...
    FOR EACH ped-item-rom WHERE
        ...
        FIND ob-etiqueta WHERE
             ...
             SHARE-LOCK NO-ERROR.
        IF AVAIL ob-etiqueta THEN
           ASSIGN ob-etiqueta.cod-depos = 'ARM'
                  ob-etiqueta.cod-estab = '5'.
    END.
...
------------------------------------------------------------------------------*/
    ...
    FIND estabelec WHERE
         estabelec.cod-estabel = tt-ped-venda.cod-estabel NO-LOCK NO-ERROR.
    ...
    ASSIGN c-seg-usuario     = c-seg-usuario            /* Usu†rio                    */
           ...
           c-serie           = estabelec.serie          /* SÇrie das notas            */
    ...
    FOR EACH tt-notas-geradas NO-LOCK:
        ...
        IF nota-fiscal.nr-pedcli <> "" THEN DO.  // Nota Triangular n∆o tem o Pedido
           FIND ped-venda WHERE
           ...
           IF AVAIL ped-venda THEN DO.
              ...
              IF tt-ped-venda.cod-estab = '505' THEN DO.
                 ASSIGN c-chave = tt-ped-venda.nr-pedcli + "|" + 
                 ...
                 IF AVAIL lisa-integra THEN
                    ASSIGN lisa-integra.acao = 'APROVAR'.

                 ASSIGN c-chave = tt-ped-venda.cod-estabel + "|" + 
                                  tt-ped-venda.serie + "|" +       
                                  tt-ped-venda.nr-nota-fis.        

                 ASSIGN c-tabela = "RemessaNotaVenda".
                 FIND lisa-integra WHERE
                      lisa-integra.cod-trans = c-tabela AND
                      lisa-integra.chave = c-chave SHARE-LOCK NO-ERROR.

                 IF NOT AVAIL lisa-integra THEN DO.
                    CREATE lisa-integra.
                    ASSIGN lisa-integra.cod-trans = c-tabela
                           lisa-integra.chave = c-chave
                           lisa-integra.ind-situacao = 1  // Pendente
    ...
    IF AVAIL tt-notas-geradas THEN
       RUN esapi/cria-log-pedvenda.p (INPUT tt-ped-venda.nr-pedcli,
                                      INPUT tt-ped-venda.nome-abrev,
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-nota-retorno w-livre 
PROCEDURE pi-nota-retorno :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    RUN esapi/cria-nota-ft4003-ret.p (INPUT ped-venda.nr-pedcli,
                                      OUTPUT TABLE tt-notas-geradas).
    ...
    // Se chegar aqui Ç porque a nota foi gerada....
    ASSIGN tt-ped-venda.dep-fechado = NO.
    FOR EACH ped-item-rom WHERE
             ped-item-rom.nome-abrev = ped-venda.nome-abrev AND
             ped-item-rom.nr-pedcli = ped-venda.nr-pedcli  NO-LOCK.
        FIND ob-etiqueta WHERE
             ...
             SHARE-LOCK NO-ERROR.
        IF AVAIL ob-etiqueta THEN
           ASSIGN ob-etiqueta.cod-estab = '5'
...
------------------------------------------------------------------------------*/
   ...
   FOR EACH ped-item-res WHERE
      ...
      FIRST ped-item-ext WHERE
       ...
       FOR EACH ped-item-rom WHERE
           ...
           IF ob-etiqueta.cod-depos = 'DEF' THEN
              ASSIGN tt-ped-venda.dep-fechado = YES.
       END.
   END.
   ...
   FOR EACH tt-ped-venda.
       ...
       IF AVAIL cond-pagto THEN  DO.
          ...
          IF cond-pagto.cod-vencto = 2 OR
             cond-pagto.cod-vencto = 3 THEN /* A Vista e Antecipado */
             ASSIGN l-avista = YES.
       END.
       ELSE DO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-recebimento w-livre 
...
------------------------------------------------------------------------------*/
     ...
     CASE tt-ped-venda.restricao.
         WHEN 1  THEN ASSIGN c-restricao = "CrÇdito n∆o Aprovado".
         ...
         WHEN 12 THEN ASSIGN c-restricao = "Comiss∆o Inv†lida".
         WHEN 13 THEN ASSIGN c-restricao = "DESCONTO n∆o Aprovado".
         WHEN 14 THEN ASSIGN c-restricao = "PREÄO n∆o Aprovado".
         WHEN 15 THEN ASSIGN c-restricao = "Tipo de Pedido N«O Aprovado".
         WHEN 16 THEN ASSIGN c-restricao = "N«O APROVAR".
         WHEN 17 THEN ASSIGN c-restricao = "FRETE n∆o Aprovado".
