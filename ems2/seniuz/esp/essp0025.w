...
*******************************************************************************/
{include/i-prgvrs.i ESSP0025 2.04.00.000}
...
DEFINE VARIABLE h_q01es005 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es005 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Cotas de Vendas por Fam¡lia"
         HEIGHT             = 13.5
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-consim /* Cotas de Vendas por Fam¡lia */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Cotas de Vendas por Fam¡lia */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es005 ).
       RUN set-position IN h_v01es005 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es005.w,
                     ProgVaPara = esgo/g01es005.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es005 ).
       RUN set-position IN h_q01es005 ( 1.25 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es005. */
       RUN add-link IN adm-broker-hdl ( h_q01es005 , 'Record':U , h_v01es005 ).

       /* Links to SmartQuery h_q01es005. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es005 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es005 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es005 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es005 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0025" "2.04.00.000"}
