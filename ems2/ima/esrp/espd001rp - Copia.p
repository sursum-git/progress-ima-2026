...
DEFINE VARIABLE codParam    AS CHARACTER   NO-UNDO FORMAT 'x(50)'.
DEFINE VARIABLE cDesclog    AS CHARACTER   NO-UNDO FORMAT 'x(500)'.
DEFINE VARIABLE cDesclog2   AS CHARACTER   NO-UNDO FORMAT 'x(500)'.
DEFINE VARIABLE cSever      AS CHARACTER   NO-UNDO.
DEFINE VARIABLE cSever1     AS CHARACTER   NO-UNDO.
DEFINE VARIABLE cAdcLogPed  AS CHARACTER   NO-UNDO INIT " Adicionado ao Log do Pedido de venda".
DEFINE VARIABLE cPastaLog   AS CHARACTER   NO-UNDO.
DEFINE VARIABLE lOkNatOper  AS LOGICAL     NO-UNDO.
DEFINE VARIABLE lOkComisRep AS LOGICAL     NO-UNDO.
DEFINE VARIABLE lEnviarEmail AS LOGICAL    NO-UNDO.
...
ASSIGN cPastaLog = IF c-base = 'base-pro' THEN  't:\especificos\logs'
                                    ELSE 't:\tst\especificos\logs'.
...
FUNCTION  getTpPagto RETURNS CHAR (numFormaPagto AS INT):
    ...
    END CASE.
    RUN incluirLogPed('setTpPagto','Forma de Pagto Portal WEb =' + string(numformaPagto)
                      + ', tp.pagto ->' + cRetorno
                      ,'INF').
    RETURN cRetorno.
END FUNCTION .
...
PROCEDURE pi-cria-repres.
    ...
    IF NOT AVAIL ped-repre THEN DO.
       RUN incluirLogPed('pi-cri-repres','N∆o encontrou registro na ped-repre, vai tentar criar a comiss∆o - %:' + 
                       string(peds_web.perc_comis) + '-pedido web:' + STRING(peds_web.ped_web_id),'INF').
       ...
       ELSE DO:
         FIND ped-repre WHERE
          ...
          RUN incluirLogPed('pi-cria-repres','Comiss∆o foi criada sem erro com o %:' + 
                            IF AVAIL ped-repre THEN string(ped-repre.perc-comis) ELSE '0','INF').
...
FOR EACH peds_web WHERE
         peds_web.ind_sit_ped_web = 1 NO-LOCK.
    ...
    FIND im-param WHERE 
         im-param.cod-param = codParam NO-LOCK NO-ERROR.

    RUN incrSeqDados IN hBologTxt.
    RUN setDados IN hBologtxt('data_hora',NOW).
    RUN setDados IN hBologtxt('nr_pedido',peds_web.ped_web_id).
    RUN setDados IN hBologtxt('acao','avaliarVenctoPedido').

    ASSIGN cDescLog = "Tipo Pedido:" + peds_web.cod_tipo_pedido + " - buscou parametro:" + codParam .
    ...
    END.
    ELSE DO:
        ASSIGN cDescLog = cDescLog + " - valor -> " + im-param.val-param .
        RUN setDados IN hBologtxt('severidade','INF').
    END.
    ...
    IF peds_web.dt_hr_venc <> ? THEN DO.
       IF da-dt-now > peds_web.dt_hr_venc THEN DO:

          RUN setRowid IN hBoPedsWeb(ROWID(peds_web)).
          RUN setIndsitPedweb IN hBoPedsWeb(6). // Vencido
          ASSIGN cDescLog = 'pedido setado como vencido e com data/hora de vencimento anterior a hora inicial de processando:' 
                                    + string(peds_web.dt_hr_venc,'99/99/9999 hh:mm:ss.sss ') .
    ...
    END.
    ELSE DO.
       IF ADD-INTERVAL(da-dt-now,- INT(im-param.val-param),"minutes") > peds_web.dt_hr_registro THEN DO:
          RUN setRowid IN hBoPedsWeb(ROWID(peds_web)).
          RUN setIndsitPedweb IN hBoPedsWeb(6). // Vencido
...
FOR EACH peds_web WHERE
    ...
    IF NOT AVAIL itens_ped_web THEN DO.
       ...
       RUN incluirLogPed('validarPedidoSemItens',cDescLog2,'INF').
       RUN setRowid IN hBoPedsWeb(ROWID(peds_web)).
       RUN setIndsitPedweb IN hBoPedsWeb(5). 
       RUN setDescrRejeicao IN hBoPedsWeb(c-mens-erro).
       RUN incluirLogPed('validarPedidoSemItens',cDescLog,cSever).
       RUN finalizarLogPed.
       NEXT.
    END.
    ELSE DO:
    ...
    IF peds_web.nr_pedido_erp <> 0 THEN DO.
       ...
       IF AVAIL ped-venda THEN DO.
          ...
          RUN incluirLogPed('validarExistPedERP','Pedido web atualizado para Integrado por encontrar Pedido ERP:' + string(peds_web.nr_pedido_erp)    
                            ,'AVISO').
          RUN finalizarLogPed.
          NEXT.
       END.
    ...
    IF AVAIL ped-venda-ext THEN DO.
       ...
       RUN incluirLogPed('validarExistPedEXT','Pedido EXT encontrado e APAGADO' ,'AVISO').
    END.
    ELSE DO:
       RUN incluirLogPed('validarExistPedEXT','Pedido EXT NAO encontrado ' ,'INF'). 
    END.
    ...
    IF peds_web.nr_container <> 0 THEN DO.
       ...
       END.
       ELSE DO:
         RUN incluirLogPed('validarExistContainer','Container Encontrado:' + string(peds_web.nr_container)   ,'INF'). 
       END.
       ...
       END.
       ELSE DO:
         RUN incluirLogPed('validarSitContainer','Container Disponivel'  ,'INF'). 
       END.
    END.
    ...
    IF peds_web.login_preposto <> '' THEN DO.
       ...
       END.
       ELSE DO:
        RUN incluirLogPed('validarPreposto','Preposto encontrado:' + peds_web.login_preposto  ,'INF'). 
       END.
    END.
    ELSE DO:
        RUN incluirLogPed('validarPreposto','Preposto nao informado'  ,'INF'). 
    END.

    IF peds_web.log_novo_cliente THEN DO:
       RUN incluirLogPed('validarNovoCliente','Pedido com cliente novo informado'  ,'INF'). 
       FIND emitente WHERE
    ...
    END.
    ELSE DO:
       RUN incluirLogPed('validarNovoCliente','Pedido SEM cliente novo informado'  ,'INF'). 
       FIND emitente WHERE  
    ...
    ELSE DO:
      RUN incluirLogPed('validarExistCliente',
                        'Encontrado Cliente - ID:' + STRING(peds_web.cliente_id) 
                         + ' CNPJ:' + peds_web.cnpj_novo_cliente 
                        ,'INF').   
    END.

    IF peds_web.log_operac_triang THEN DO.
       RUN incluirLogPed('validarOpTriang','Pedido COM Operacao Triangular' ,'INF').   
       IF peds_web.log_novo_cliente_triang THEN DO:
          RUN incluirLogPed('validarNovoClienteTriang','Pedido COM cliente triangular novo informado'  ,'INF').
          FIND b-emitente WHERE
               b-emitente.cgc = peds_web.cnpj_novo_cliente_triang NO-LOCK NO-ERROR.
       END.
          
       ELSE DO:
          RUN incluirLogPed('validarNovoClienteTriang','Pedido SEM cliente triangular novo informado'  ,'INF').
          FIND b-emitente WHERE
       ...
       ELSE DO:
           RUN incluirLogPed('validarExistCliente',
                             'Encontrado Cliente - ID:' + STRING(peds_web.cliente_triang_id) 
                              + ' CNPJ:' + peds_web.cnpj_novo_cliente_triang ,'INF').
       END.
    END.
    ELSE DO:
        RUN incluirLogPed('validarOpTriang','Pedido SEM Operacao Triangular' ,'INF').   
    END.
    ...
    IF peds_web.transp_id = 0 THEN DO:
       ...
       RUN incluirLogPed('validarTransp','Transportador com c¢digo 0 modificado para 50' ,'INF').   
    ...
    END.
    ELSE DO:
      RUN incluirLogPed('validarTransp','Transportador Encontrado:' + STRING(peds_web.transp_id) ,'INF').   
    END.
    ...
    IF peds_web.transp_redesp_id <> 0 THEN DO:
       ...
       RUN incluirLogPed('validarTranspRedesp',
                         'Transportador de redespacho diferente de zero. Posiconou sem validaá∆o de existencia no transportador:' 
                         + STRING(peds_web.transp_redesp_id) 
                         ,'INF').   
    END.
    ...
    END.
    ELSE DO:
        RUN incluirLogPed('validarMoeda','Moeda encontrada:' + STRING(i-moeda),'INF').
    END.
    ...
    END.
    ELSE DO:
        RUN incluirLogPed('validarFinalidadeCli','Finalidade Encontrada com Sucesso:' + STRING(i-fin-nat) 
                          + ' - CNAE considerado:' 
                          + c-cnae,
                          'INF' ).
    END.
    ...
    IF NOT peds_web.log_operac_triang THEN DO.
       ...
       ELSE DO:
           RUN incluirLogPed('buscarNatOperacao',
                             'SEM Operacao Triangular - Natureza de Operaá∆o encontrada com sucesso:'
                              + c-natur-oper,
                             'INF' ).
       END.
    END.
    ELSE DO.
       ...
       ELSE DO:
         RUN incluirLogPed('buscarNatOperacao',
                           'COM Operacao Triangular - Retornou com sucesso  a natureza de operacao -> pi-calc-natur-oper-> emitente:' 
                             + STRING(emitente.cod-emit) + ' - nome abrev cli triang.:' + b-emitente.nome-abrev + ' natureza encontrada:' + c-natur-oper,
                           'INF').
       END.
    END.
    ...
    RUN incluirLogPed('buscarPrioridade','Prioridade Ped.Web:' + string(peds_web.cod_prioridade) 
                      + " retornou ->" + c-pct-desconto,
                      'INF' ).
    ...
    ELSE DO:
        ...
        RUN incluirLogPed('tratarCondPagto',
                          'Pedido Web com condiá∆o DIFERENTE de  Ö Vista, busca na tabela de condicao de pagamento-> ' 
                          + STRING(peds_web.cond_pagto_id),
                          'INF' ).
    ...
    RUN incluirLogPed('preencherDadosPedidoERP','Preenchimento dos seguintes campos da tabela ped-venda com seus respectivos valores fixos: '
                      + 'cod-cond-pag = 0 , nr-tab-finan = 1, nr-ind-finan = 1','INF' ).
    ...
    ELSE DO:
       RUN incluirLogPed('tratarTipoFrete','Tipo de frete NOT in (1,2) seta ind-tp-frete como 2 e cidade CIF como " "(em branco) -> tipo de frete: '
                          + STRING(peds_web.cod_tipo_frete),'INF').
    ...
    // nao entendi o motivo do find acima, j† que n∆o foi feita nenhuma verificaá∆o. Testar deste jeito e depois excluir e testar se pode ser retirado.
    ...
    IF peds_web.log_a_vista THEN DO.
       ...
       RUN incluirLogPed('tratarPedAVista','Campo peds_web.log_a vista igual a 1, modificado os  seguintes campos da tabela ped-venda-ext:' + 
                         'tp-pedido="Ö Vista" ','INF' ).     
    ...
    FOR EACH itens_ped_web WHERE
        ...
        /*IMPORTANTE: alterar o c¢digo para pegar diretamente da tabela itens_ped_web, pois
        no momento que o pedido foi digitado pode ter existido o liquida ima e no momento
        da integraá∆o pode n∆o existir mais. */
        ...
        IF AVAIL liquida-ima THEN DO:
           ...
           RUN incluirLogPed('itensPedido_rastroOutlet',
                             'Liquida Ima Encontrado e atribuido codigo:' +
                             string(liquida-ima.num-id-liquida-ima) , 'INF').
        END.
        ELSE DO:
           RUN incluirLogPed('itensPedido_rastroOutlet',
                             'Liquida Ima N∆o Encontrado' , 'INF').

        END.
        RUN incluirLogPed('itensPedido','FIM-' + string(itens_ped_web.ITEM_ped_web_id),'INF' ). 
    END.  // Criou os Itens
...
PROCEDURE getAvalEnviarEmail:
    ...
    IF TIME < 28800 OR TIME > 64800 
        OR WEEKDAY(TODAY) = 7 OR WEEKDAY(TODAY) = 1 THEN DO:
