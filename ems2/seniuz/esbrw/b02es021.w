...
*******************************************************************************/
{include/i-prgvrs.i B02ES021 2.04.00.000}
...
&Scoped-define FIELDS-IN-QUERY-br-table mov-est-acbm.num-lote ~
mov-est-acbm.data-mov mov-est-acbm.it-codigo mov-est-acbm.cod-refer ~
...
&Scoped-define QUERY-STRING-br-table FOR EACH mov-est-acbm WHERE ~{&KEY-PHRASE} ~
      AND mov-est-acbm.num-lote >= fi-ini-num-lote ~
 AND mov-est-acbm.num-lote <= fi-fin-num-lote NO-LOCK ~
    BY mov-est-acbm.num-lote ~
       BY mov-est-acbm.data-mov ~
        BY mov-est-acbm.it-codigo ~
         BY mov-est-acbm.cod-refer ~
          BY mov-est-acbm.tipo-tear INDEXED-REPOSITION
...
&Scoped-Define ENABLED-OBJECTS bt-confirma IMAGE-1 IMAGE-2 fi-ini-num-lote ~
fi-fin-num-lote br-table 
&Scoped-Define DISPLAYED-OBJECTS fi-ini-num-lote fi-fin-num-lote 
...
DEFINE VARIABLE fi-fin-num-lote AS INTEGER FORMAT "999999" INITIAL 999999 
     VIEW-AS FILL-IN 
     SIZE 9.57 BY .88 TOOLTIP "N£mero do lote final" NO-UNDO.

DEFINE VARIABLE fi-ini-num-lote AS INTEGER FORMAT ">>>>>9" INITIAL 0 
     LABEL "Numero Lote" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 TOOLTIP "N£mero do lote inicial" NO-UNDO.
...
DEFINE FRAME F-Main
     ...
     fi-ini-num-lote AT ROW 1.13 COL 15.57 COLON-ALIGNED HELP
          "Numero do lote."
     fi-fin-num-lote AT ROW 1.13 COL 42.43 COLON-ALIGNED HELP
          "Numero do lote." NO-LABEL
     br-table AT ROW 2.17 COL 1
     IMAGE-1 AT ROW 1.13 COL 28.14
...
/* BROWSE-TAB br-table fi-fin-num-lote F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br-table
     ...
     _OrdList          = "espec.mov-est-acbm.num-lote|yes,espec.mov-est-acbm.data-mov|yes,espec.mov-est-acbm.it-codigo|yes,espec.mov-est-acbm.cod-refer|yes,espec.mov-est-acbm.tipo-tear|yes"
     _Where[1]         = "espec.mov-est-acbm.num-lote >= fi-ini-num-lote
 AND espec.mov-est-acbm.num-lote <= fi-fin-num-lote"
     _FldNameList[1]   > espec.mov-est-acbm.num-lote
"mov-est-acbm.num-lote" ? ? "integer" ? ? ? ? ? ? no ? no no "7.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[2]   > espec.mov-est-acbm.data-mov
"mov-est-acbm.data-mov" ? ? "date" ? ? ? ? ? ? no ? no no "8.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[3]   > espec.mov-est-acbm.it-codigo
"mov-est-acbm.it-codigo" ? ? "character" ? ? ? ? ? ? no ? no no "14.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > espec.mov-est-acbm.cod-refer
"mov-est-acbm.cod-refer" ? ? "character" ? ? ? ? ? ? no ? no no "9.14" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
"mov-est-acbm.qtd-tot-perf" ? ? "decimal" ? ? ? ? ? ? no ? no no "9.43" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[8]   > espec.mov-est-acbm.qtd-tot-def
"mov-est-acbm.qtd-tot-def" ? ? "decimal" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[9]   > espec.mov-est-acbm.qtd-tot-sob
"mov-est-acbm.qtd-tot-sob" ? ? "decimal" ? ? ? ? ? ? no ? no no "9.72" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
...
DO:
  assign input frame {&frame-name} fi-ini-num-lote fi-fin-num-lote.
