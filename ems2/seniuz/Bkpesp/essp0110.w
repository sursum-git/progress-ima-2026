...
DEFINE VARIABLE h_b02es047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q04es047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v05es047 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadpaifilho-filho /* Manuten‡Æo de Etiquetas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Manuten‡Æo de Etiquetas */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v05es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v05es047 ).
       RUN set-position IN h_v05es047 ( 2.50 , 1.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Etiquetas' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.67 , 89.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es047 ).
       RUN set-position IN h_b02es047 ( 9.33 , 1.86 ) NO-ERROR.
       /* Size in UIB:  ( 8.17 , 87.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q04es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es047.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q04es047 ).
       RUN set-position IN h_q04es047 ( 1.25 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v05es047. */
       RUN add-link IN adm-broker-hdl ( h_q04es047 , 'Record':U , h_v05es047 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_b02es047. */
       RUN add-link IN adm-broker-hdl ( h_q04es047 , 'Record':U , h_b02es047 ).

       /* Links to SmartQuery h_q04es047. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q04es047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q04es047 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v05es047 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v05es047 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02es047 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0110" "2.04.00.000"}
