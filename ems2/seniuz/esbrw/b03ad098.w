...
*******************************************************************************/
{include/i-prgvrs.i B03AD098 2.04.00.000}
...
&Scoped-define FIELDS-IN-QUERY-br-table emitente.cgc emitente.cod-emitente ~
emitente.nome-abrev emitente.nome-emit 
...
&Scoped-define QUERY-STRING-br-table FOR EACH emitente WHERE ~{&KEY-PHRASE} ~
 ...
 AND emitente.cgc >= fi-ini-cgc ~
 AND emitente.cgc <= fi-fin-cgc NO-LOCK ~
    BY emitente.cgc ~
       BY emitente.cod-emitente INDEXED-REPOSITION
...
&Scoped-Define ENABLED-OBJECTS fi-ini-cgc fi-fin-cgc bt-confirma br-table ~
IMAGE-1 IMAGE-2 
&Scoped-Define DISPLAYED-OBJECTS fi-ini-cgc fi-fin-cgc 
...
DEFINE VARIABLE fi-fin-cgc AS CHARACTER FORMAT "x(19)" INITIAL "ZZZZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .88 TOOLTIP "CGC/CPF do cliente final."
     FONT 1 NO-UNDO.

DEFINE VARIABLE fi-ini-cgc AS CHARACTER FORMAT "x(19)" 
     VIEW-AS FILL-IN 
     SIZE 20 BY .88 TOOLTIP "CGC/CPF do cliente inicial."
     FONT 1 NO-UNDO.
...
DEFINE FRAME F-Main
     fi-ini-cgc AT ROW 1.17 COL 2.43 COLON-ALIGNED HELP
          "CGC ou CIC do cliente/fornecedor" NO-LABEL
     fi-fin-cgc AT ROW 1.17 COL 42.43 COLON-ALIGNED HELP
          "CGC ou CIC do cliente/fornecedor" NO-LABEL
     bt-confirma AT ROW 1.17 COL 82.86
     br-table AT ROW 2.17 COL 1
     IMAGE-1 AT ROW 1.17 COL 24.72
...
/* Query rebuild information for BROWSE br-table
     ...
     _OrdList          = "mgcad.emitente.cgc|yes,mgcad.emitente.cod-emitente|yes"
     _Where[1]         = "mgcad.emitente.identific <> 2
 AND mgcad.emitente.cod-rep >= i-cod-rep-ini
 AND mgcad.emitente.cod-rep <= i-cod-rep-fin
 AND mgcad.emitente.cgc >= fi-ini-cgc
 AND mgcad.emitente.cgc <= fi-fin-cgc"
     _FldNameList[1]   = mgcad.emitente.cgc
...
DO:
  assign input frame {&frame-name} fi-ini-cgc fi-fin-cgc.
