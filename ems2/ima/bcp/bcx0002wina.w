...
*******************************************************************************/
{include/i-prgvrs.i BCX0002WINA 1.00.00.000}
...
DEFINE VARIABLE h_q02im002 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02im002 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 4.29 COL 2.57 HELP
          "Salva e sai"
     bt-save AT ROW 4.29 COL 13 HELP
          "Salva e cria novo"
     bt-cancela AT ROW 4.29 COL 23.57 HELP
          "Cancela"
     bt-ajuda AT ROW 4.29 COL 70.72
     rt-button AT ROW 4.04 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Etiqueta Corte"
         HEIGHT             = 4.79
         WIDTH              = 80.72
         MAX-HEIGHT         = 27.96
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsim /* Inclui/Modifica Etiqueta Corte */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Inclui/Modifica Etiqueta Corte */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'clvwr/v02cl005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02im002 ).
       RUN set-position IN h_v02im002 ( 1.00 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.58 , 79.29 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'clqry/q02cl005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q02im002 ).
       RUN set-position IN h_q02im002 ( 3.88 , 46.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02im002. */
       RUN add-link IN adm-broker-hdl ( h_q02im002 , 'Record':U , h_v02im002 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TABLEIO':U , h_v02im002 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02im002 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "BCX0002WINA" "1.00.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  RUN pi-atualiza-parent IN h_v02im002 (INPUT v-row-parent).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q02im002 (input v-row-table).
