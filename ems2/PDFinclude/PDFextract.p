...
******************************************************************************/
{ pdfinclude/pdf_pre.i}
{ {&PDFDIR}/pdfglobal.i "SHARED" }
{ {&PDFDIR}/pdf_func.i h_PDFinc}

def input param pStream    as char no-undo.
def input param pFileName  as char no-undo.

def input param pID        as char no-undo.

def var Text-Line          as char no-undo.
def var Entry4             as char no-undo.

/* variables used for seeking points in the document */
def var obj-ctr            as int  no-undo.
def var root-obj           as int  no-undo.
def var root-gen           as int  no-undo.
def var info-obj           as int  no-undo.
def var info-gen           as int  no-undo.
def var pages-obj          as int  no-undo.
def var pages-gen          as int  no-undo.
def var form-obj           as int  no-undo.
def var form-gen           as int  no-undo.
def var row-ctr            as int  no-undo.
def var seek-ptr           as int  no-undo.
def var xref-ptr           as int  no-undo.

/* variables used to Store Page MediaBox info */
def var decDefMedia1       as dec DECIMALS 5 NO-UNDO.
def var decDefMedia2       as dec DECIMALS 5 NO-UNDO.
def var decDefMedia3       as dec DECIMALS 5 NO-UNDO.
def var decDefMedia4       as dec DECIMALS 5 NO-UNDO.

def var decMedia1          as dec DECIMALS 5 NO-UNDO.
def var decMedia2          as dec DECIMALS 5 NO-UNDO.
def var decMedia3          as dec DECIMALS 5 NO-UNDO.
def var decMedia4          as dec DECIMALS 5 NO-UNDO.

def var decCrop1           as dec DECIMALS 5 NO-UNDO.
def var decCrop2           as dec DECIMALS 5 NO-UNDO.
def var decCrop3           as dec DECIMALS 5 NO-UNDO.
def var decCrop4           as dec DECIMALS 5 NO-UNDO.

/* Variables used for storage of document information */
def var doc-version        as char      NO-UNDO.
def var doc-pages          as int       NO-UNDO.
def var doc-encrypted      as log       NO-UNDO.
def var doc-author         as char      NO-UNDO.
def var doc-createdon      as DATE      NO-UNDO.
def var doc-createdat      as char      NO-UNDO.
def var doc-producer       as char      NO-UNDO.
def var doc-creator        as char      NO-UNDO.
def var doc-subject        as char      NO-UNDO.
def var doc-title          as char      NO-UNDO.
def var doc-keywords       as char      NO-UNDO.
def var doc-moddate        as DATE      NO-UNDO.
def var doc-modtime        as char      NO-UNDO.
def var doc-bookmarks      as int       NO-UNDO.
def var doc-annots         as int       NO-UNDO.

function ReadLine returns char():
  def var L_Byte  as memptr no-undo.
  def var L_Line  as char   no-undo.
  def var L_Seek  as int    no-undo.
...
function UpLine returns logical ():
  def var L_Byte  as memptr no-undo.
  def var L_Byte2 as memptr no-undo.
  def var L_Seek  as int    no-undo.

  def var L_Found as log    no-undo.
...
procedure LoadObjectPointers:
  def input param pPointer as int  no-undo.
  
  def var curr-ptr         as int  no-undo.
  def var prev-ptr         as int  no-undo.

  def var text-ptr         as char no-undo.
  def var text-temp        as char no-undo.

  def buffer B_TT_Object for TT_Object.
  ...
  do while true:
    ...
    if length(text-ptr) > 0 and 
       substr(text-ptr,length(text-ptr),1) = "n" then 
    do:
...
procedure ProcessObjectPointers:
  ...
  for each  TT_Object where  
             TT_Object.obj_stream = pStream and 
             TT_Object.pdf_id     = pID.
      ...
      if  TT_object.obj_id = root-obj and 
          TT_object.gen_id = root-gen then
          TT_Object.obj_type = "~/Root".

      else if  TT_object.obj_id = info-obj  and 
               TT_object.gen_id  = info-gen then
  ...
  for each TT_Object where 
           TT_Object.obj_stream = pStream and 
           TT_Object.pdf_id     = pID     and
           TT_object.obj_type   = "~/Info"
      break by obj_id 
            by gen_id 
            by obj_seq.

      if  first-of( TT_Object.gen_id) then 
      do:
          ...
          ELSE IF  INDEX(text-ptr,"/Creator") > 0 THEN DO:
               ...
               ELSE IF  INDEX(text-ptr,"~/Title") > 0 THEN DO:
                    ...
                    ELSE IF  INDEX(text-ptr,"~/Subject") > 0 THEN DO:
                         ...
                         ELSE IF  INDEX(text-ptr,"~/Keywords") > 0 THEN 
                              DO:
                              ...
                              ELSE IF  INDEX(text-ptr,"~/ModDate") > 0 THEN DO:
                                   ...
                                   ELSE IF  INDEX(text-ptr,"~/CreationDate") > 0 THEN 
                                        DO:
...
procedure CreateInfo:
  def input param pInfo  as char no-undo.
  def input param pValue as char no-undo.
...
PROCEDURE ProcessRootDictionary:
  def var text-ptr   as char no-undo.
  def var temp-text  as char no-undo.
  ...
  do while true:
    ...
    if  index(text-ptr,"~/Pages") > 0 then 
    do:
    ...
    if  index(text-ptr,"~/AcroForm") > 0 then  
    do:
...
PROCEDURE ProcessFieldDictionary:
  def input param pObject  AS int  NO-UNDO.
  def input param pGen     AS int  NO-UNDO.
  def input param obj-name AS char NO-UNDO.
  def input param obj-type AS char NO-UNDO.
  def input param obj-disp AS char NO-UNDO.

  def var par-obj          AS int  NO-UNDO.
  def var par-gen          AS int  NO-UNDO.
  def var par-page         AS int  NO-UNDO.

  def buffer B_TT_Object   FOR TT_object.
  def buffer B_Par_Object  FOR TT_object.
  
  def var text-ptr         AS CHARACTER NO-UNDO.
  def var text-temp        AS CHARACTER NO-UNDO.

  def var obj-rect         AS CHARACTER NO-UNDO.

  def var L_HasKids        AS LOGICAL NO-UNDO.
  ...
  FOR EACH B_TT_Object WHERE B_TT_Object.obj_stream = pStream
    ...
    IF FIRST(B_TT_object.obj_Seq) THEN DO:
      ...
      IF NOT l_HasKids AND obj-type = "TX" THEN 
      DO:
...
PROCEDURE ProcessFieldKids:
  def input param text-ptr as char no-undo.
  def input param obj-name as char no-undo.
  def input param obj-type as char no-undo.
  def input param obj-disp as char no-undo.


  def var L_Loop           as int  no-undo.
  def var L_Obj            as int  no-undo.
  def var L_Gen            as int  no-undo.
...
PROCEDURE ProcessPagesDictionary:
  ...
  def var L_InKids         as log  no-undo.
  def var L_Kids           as char no-undo.

  def var L_Loop           as int  no-undo.
  def var L_Obj            as int  no-undo.
  def var L_Gen            as int  no-undo.

  def var L_Curr           as int  no-undo.
  ...
  for each B_TT_Object where 
           B_TT_Object.obj_stream = pStream and 
           B_TT_Object.pdf_id     = pId     and 
           B_TT_object.obj_id     = pObject and 
           B_TT_object.gen_id     = pGen
           break by B_TT_object.obj_seq:

    if  first(B_TT_object.obj_Seq) then
    do:
