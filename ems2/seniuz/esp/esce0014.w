...
*******************************************************************************/
{include/i-prgvrs.i ESCE0014 2.04.00.000}
...
DEFINE VARIABLE h_b01in403 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q02in172 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02in172 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Estoque para Venda"
         HEIGHT             = 17
         WIDTH              = 90.14
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90.57
...
ON END-ERROR OF w-concom /* Consulta Estoque para Venda */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Consulta Estoque para Venda */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 74.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Referˆncias' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.13 , 1.86 ) NO-ERROR.
       RUN set-size IN h_folder ( 12.63 , 88.57 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02in172.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02in172 ).
       RUN set-position IN h_v02in172 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.25 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01in403.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01in403 ).
       RUN set-position IN h_b01in403 ( 6.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.21 , 87.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q02in172.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01in172.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q02in172 ).
       RUN set-position IN h_q02in172 ( 1.50 , 62.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02in172. */
       RUN add-link IN adm-broker-hdl ( h_q02in172 , 'Record':U , h_v02in172 ).

       /* Links to SmartBrowser h_b01in403. */
       RUN add-link IN adm-broker-hdl ( h_q02in172 , 'Record':U , h_b01in403 ).

       /* Links to SmartQuery h_q02in172. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q02in172 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q02in172 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q02in172 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESCE0014" "2.04.00.000"}
