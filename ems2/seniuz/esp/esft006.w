...
&Scoped-Define ENABLED-OBJECTS rect-rtf RECT-7 RECT-9 IMAGE-96 IMAGE-97 ~
rs-destino bt-config-impr bt-arquivo c-arquivo l-habilitaRtf bt-modelo-rtf ~
rs-execucao text-rtf text-modelo-rtf 
&Scoped-Define DISPLAYED-OBJECTS rs-destino c-arquivo l-habilitaRtf ~
c-modelo-rtf fi-no-ab-reppri-ini fi-no-ab-reppri-fin rs-execucao text-rtf ~
text-modelo-rtf 
...
DEFINE VARIABLE cb-estado-fin AS CHARACTER FORMAT "X(256)":U 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO" 
     DROP-DOWN-LIST
     SIZE 6 BY 1 NO-UNDO.

DEFINE VARIABLE cb-estado-ini AS CHARACTER FORMAT "X(256)":U 
     LABEL "Estado" 
     VIEW-AS COMBO-BOX INNER-LINES 5
     LIST-ITEMS "AC","AL","AP","AM","BA","CE","DF","ES","GO","MA","MT","MS","MG","PA","PB","PR","PE","PI","RJ","RN","RS","RO","RR","SC","SP","SE","TO" 
     DROP-DOWN-LIST
...
DEFINE VARIABLE fi-cliente-fin AS CHARACTER FORMAT "x(16)" INITIAL "ZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 TOOLTIP "Representante final." NO-UNDO.
...
DEFINE VARIABLE fi-data-ini AS DATE FORMAT "99/99/9999":U INITIAL 01/01/00 
     LABEL "Per°odo" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-imp
     ...
     bt-modelo-rtf AT ROW 6.63 COL 43 HELP
          ...
          "Codigo do ÷tem no EMS" WIDGET-ID 4
     fi-no-ab-reppri-fin AT ROW 7.75 COL 42 COLON-ALIGNED HELP
          "Codigo do ÷tem no EMS" NO-LABEL WIDGET-ID 2
     ...
     IMAGE-96 AT ROW 7.75 COL 31.57 WIDGET-ID 6
     IMAGE-97 AT ROW 7.75 COL 40.57 WIDGET-ID 8
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
DEFINE FRAME f-pg-sel
     fi-data-ini AT ROW 2.25 COL 15 COLON-ALIGNED
     fi-data-fin AT ROW 2.25 COL 48.14 COLON-ALIGNED NO-LABEL
     fi-cliente-ini AT ROW 3.25 COL 15 COLON-ALIGNED HELP
          "Codigo do ÷tem no EMS" WIDGET-ID 16
     fi-cliente-fin AT ROW 3.25 COL 48.14 COLON-ALIGNED HELP
          "Codigo do ÷tem no EMS" NO-LABEL WIDGET-ID 14
     cb-estado-ini AT ROW 4.25 COL 15 COLON-ALIGNED WIDGET-ID 22
     cb-estado-fin AT ROW 4.25 COL 48.14 COLON-ALIGNED NO-LABEL WIDGET-ID 24
     tg-excel AT ROW 6.04 COL 17 WIDGET-ID 10
     IMAGE-1 AT ROW 2.25 COL 32
     IMAGE-2 AT ROW 2.25 COL 47.14
     IMAGE-100 AT ROW 3.25 COL 32 WIDGET-ID 18
     IMAGE-101 AT ROW 3.25 COL 47.14 WIDGET-ID 20
     IMAGE-102 AT ROW 4.25 COL 32 WIDGET-ID 26
     IMAGE-103 AT ROW 4.25 COL 47.14 WIDGET-ID 28
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Relat¢rio de Vendas por Cliente"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 28.92
         MAX-WIDTH          = 146.29
...
/* SETTINGS FOR EDITOR c-modelo-rtf IN FRAME f-pg-imp
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-no-ab-reppri-fin IN FRAME f-pg-imp
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-no-ab-reppri-ini IN FRAME f-pg-imp
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FRAME f-pg-sel
                                                                        */
/* SETTINGS FOR TOGGLE-BOX tg-excel IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-relat /* Relat¢rio de Vendas por Cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Relat¢rio de Vendas por Cliente */
DO:
...
&Scoped-define SELF-NAME cb-estado-ini
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL cb-estado-ini w-relat
ON LEAVE OF cb-estado-ini IN FRAME f-pg-sel /* Estado */
DO:
  IF SELF:SCREEN-VALUE <> '' THEN
     ASSIGN cb-estado-fin:SCREEN-VALUE = SELF:SCREEN-VALUE.
END.
...
&Scoped-define SELF-NAME fi-cliente-fin
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cliente-fin w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-cliente-fin IN FRAME f-pg-sel
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad229.w
                     &campo     = fi-cliente-fin
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cliente-ini w-relat
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cliente-ini w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-cliente-ini IN FRAME f-pg-sel /* Cliente */
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z02ad098.w
                     &campo     = fi-cliente-ini
                     &campozoom = nome-abrev}
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-data-ini w-relat
ON LEAVE OF fi-data-ini IN FRAME f-pg-sel /* Per°odo */
DO:
 /* IF fi-data-ini:SCREEN-VALUE <> "01/01/2000" THEN
     ASSIGN fi-data-fin:SCREEN-VALUE = STRING(TODAY). */
END.
...
ON MOUSE-SELECT-DBLCLICK OF fi-no-ab-reppri-fin IN FRAME f-pg-imp
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad229.w
...
ON LEAVE OF fi-no-ab-reppri-ini IN FRAME f-pg-imp /* Representante */
DO:
  IF SELF:SCREEN-VALUE <> '' THEN
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-no-ab-reppri-ini w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-no-ab-reppri-ini IN FRAME f-pg-imp /* Representante */
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad229.w
...
ASSIGN CURRENT-WINDOW                = {&WINDOW-NAME} 
 ...
 {utp/ut9000.i "esft006" "2.06.00.000"} 
...
ASSIGN fi-data-fin = TODAY
       cb-estado-ini = "AC"
       cb-estado-fin = "TO".

fi-cliente-ini:LOAD-MOUSE-POINTER("image/lupa.cur") IN FRAME f-pg-sel.
fi-cliente-fin:LOAD-MOUSE-POINTER("image/lupa.cur") IN FRAME f-pg-sel.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-data-ini fi-data-fin fi-cliente-ini fi-cliente-fin cb-estado-ini 
          cb-estado-fin tg-excel 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-100 IMAGE-101 IMAGE-102 IMAGE-103 fi-data-ini 
         fi-data-fin fi-cliente-ini fi-cliente-fin cb-estado-ini cb-estado-fin 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  {&OPEN-BROWSERS-IN-QUERY-f-pg-sel}
  DISPLAY rs-destino c-arquivo l-habilitaRtf c-modelo-rtf fi-no-ab-reppri-ini 
          fi-no-ab-reppri-fin rs-execucao text-rtf text-modelo-rtf 
      WITH FRAME f-pg-imp IN WINDOW w-relat.
  ENABLE rect-rtf RECT-7 RECT-9 IMAGE-96 IMAGE-97 rs-destino bt-config-impr 
         bt-arquivo c-arquivo l-habilitaRtf bt-modelo-rtf rs-execucao text-rtf 
         text-modelo-rtf 
      WITH FRAME f-pg-imp IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    IF YEAR(INPUT FRAME f-pg-sel fi-data-ini) <>
       YEAR(INPUT FRAME f-pg-sel fi-data-fin) THEN DO.
            
       MESSAGE 'Favor Informar o Per°odo dentro do mesmo ANO'
           VIEW-AS ALERT-BOX INFO BUTTONS OK.

       APPLY "MOUSE-SELECT-CLICK":U TO im-pg-sel IN FRAME f-relat.
       APPLY "ENTRY":U TO fi-data-ini IN FRAME f-pg-sel.
       RETURN ERROR.
    END.
    ...
    /*:T Coloque aqui a/l¢gica de gravaá∆o dos demais campos que devem ser passados
       como parÉmetros para o programa RP.P, atravÇs da temp-table tt-param */
         ...
         ASSIGN tt-param.d-data-ini      = INPUT FRAME f-pg-sel fi-data-ini
                tt-param.d-data-fin      = INPUT FRAME f-pg-sel fi-data-fin
                tt-param.c-cliente-ini   = INPUT FRAME f-pg-sel fi-cliente-ini
                tt-param.c-cliente-fin   = INPUT FRAME f-pg-sel fi-cliente-fin
                tt-param.c-estado-ini    = INPUT FRAME f-pg-sel cb-estado-ini
                tt-param.c-estado-fin    = INPUT FRAME f-pg-sel cb-estado-fin
                tt-param.l-excel         = INPUT FRAME f-pg-sel tg-excel.
    
         IF SUBSTR(fi-data-ini:SCREEN-VALUE,7,4) <> SUBSTR(fi-data-fin:SCREEN-VALUE,7,4) THEN DO:
         MESSAGE "Este Relet¢rio n∆o pode ser executado com per°odo de anos diferentes."
             VIEW-AS ALERT-BOX INFO BUTTONS OK.
    ...
    {include/i-rprun.i esrp\esft006rp.p}
    ...
    IF INPUT FRAME f-pg-sel tg-excel  = NO THEN DO:
       {include/i-rptrm.i}
    END.
