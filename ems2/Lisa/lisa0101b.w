...
DEFINE TEMP-TABLE ttSaldoTerc LIKE saldo-terc
    ...
    FIELD qtAlocada     AS DECIMAL
    FIELD qtDisponivel  AS DECIMAL.
...
DEFINE VARIABLE qtMin  AS DECIMAL     NO-UNDO INIT 0.
...
&Scoped-define FIELDS-IN-QUERY-BROWSE-4 ttSaldoTerc.nro-docto ttSaldoTerc.nat-operacao ttSaldoTerc.sequencia ttSaldoTerc.cod-refer ttSaldoTerc.quantidade ttSaldoTerc.qtalocada ttSaldoTerc.qtdisp   
&Scoped-define ENABLED-FIELDS-IN-QUERY-BROWSE-4   
&Scoped-define SELF-NAME BROWSE-4
&Scoped-define QUERY-STRING-BROWSE-4 FOR EACH ttSaldoTerc WHERE ttSaldoTerc.qtDisp >= qtMin AND ttSaldoTerc.nro-docto >= nfIni AND ttSaldoTerc.nro-docto <= nfFim NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-BROWSE-4 OPEN QUERY {&SELF-NAME} FOR EACH ttSaldoTerc WHERE ttSaldoTerc.qtDisp >= qtMin AND ttSaldoTerc.nro-docto >= nfIni AND ttSaldoTerc.nro-docto <= nfFim NO-LOCK INDEXED-REPOSITION.
...
tgSomenteRemessa fiDtLimite BROWSE-4 bt-ok bt-cancelar bt-ajuda fiqt ~
fiqtLimite tgApenasDisponivel 
&Scoped-Define DISPLAYED-OBJECTS fiProduto fiCodRefer fiNota ~
tgSomenteRemessa fiDtLimite fiqt fiqtLimite tgApenasDisponivel 
...
DEFINE VARIABLE fiDtLimite AS DATE FORMAT "99/99/9999":U 
     LABEL "Dt.Limite" 
     VIEW-AS FILL-IN 
     SIZE 12.14 BY .79 TOOLTIP "Dt.Limite com base na nota fiscal corrente" NO-UNDO.
...
DEFINE VARIABLE tgApenasDisponivel AS LOGICAL INITIAL yes 
     LABEL "Apenas Dispon¡vel" 
     VIEW-AS TOGGLE-BOX
...
ttSaldoTerc.qtalocada COLUMN-LABEL "Qt.Alocada"
ttSaldoTerc.qtdisp    COLUMN-LABEL "Qt.Dispon¡vel"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 97 BY 15.75
         FONT 1
...
DEFINE FRAME F-Main
     ...
     fiDtLimite AT ROW 3.46 COL 7.86 COLON-ALIGNED WIDGET-ID 16
     BROWSE-4 AT ROW 5.25 COL 2 WIDGET-ID 200
     ...
     tgApenasDisponivel AT ROW 21.5 COL 63 WIDGET-ID 18
...
/* SETTINGS FOR FRAME F-Main
   FRAME-NAME                                                           */
/* BROWSE-TAB BROWSE-4 fiDtLimite F-Main */
ASSIGN 
       fiCodRefer:READ-ONLY IN FRAME F-Main        = TRUE.

ASSIGN 
       fiDtLimite:READ-ONLY IN FRAME F-Main        = TRUE.
...
/* Query rebuild information for BROWSE BROWSE-4
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH ttSaldoTerc
WHERE ttSaldoTerc.qtDisp >= qtMin
AND ttSaldoTerc.nro-docto >= nfIni
AND ttSaldoTerc.nro-docto <= nfFim
...
DO:
  IF AVAIL ttSaldoTerc THEN DO:
     ASSIGN fiQT:SCREEN-VALUE = STRING(ttSaldoTerc.qtDisp).
  END.
  ELSE
...
&Scoped-define SELF-NAME tgApenasDisponivel
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tgApenasDisponivel w-window
ON VALUE-CHANGED OF tgApenasDisponivel IN FRAME F-Main /* Apenas Dispon¡vel */
DO:
...
DO:
  RUN setIntervalNFRemessa(logical(self:SCREEN-VALUE)).  
  {&open-query-browse-4}
...
------------------------------------------------------------------------------*/
  DISPLAY fiProduto fiCodRefer fiNota tgSomenteRemessa fiDtLimite fiqt 
          fiqtLimite tgApenasDisponivel 
      WITH FRAME F-Main IN WINDOW w-window.
  ENABLE RECT-1 RECT-108 fiProduto fiCodRefer fiNota tgSomenteRemessa 
         fiDtLimite BROWSE-4 bt-ok bt-cancelar bt-ajuda fiqt fiqtLimite 
         tgApenasDisponivel 
      WITH FRAME F-Main IN WINDOW w-window.
...
------------------------------------------------------------------------------*/
  ...
  IF AVAIL retornos_lisa THEN
     ASSIGN dtFim = retornos_lisa.dt_nf
            fiDtLimite:SCREEN-VALUE IN FRAME {&frame-name} = string(dtFim).
  ...
  {esp/exportarTabelaCsv3.i ttsaldoTerc " " " except r-rowid " "ttsaldoterc"}
END PROCEDURE.
...
DEFINE VARIABLE dAlocado  AS DECIMAL     NO-UNDO.
...
FOR EACH  saldo-terc NO-LOCK
        ...
        FOR EACH ttITemNf
            ...
            //NEXT saldoTerc.
            ASSIGN dAlocado = dAlocado + ttItemNf.qtFaturada.
        END.
        
        ASSIGN dSaldo = saldo-terc.quantidade - dAlocado.
        /*IF dSaldo >  0 THEN        DO:
           CREATE ttsaldoTerc.
           BUFFER-COPY saldo-terc TO ttSaldoTerc.
           ASSIGN ttSaldoTerc.r-rowid = ROWID(saldo-terc)
                  ttSaldoTerc.quantidade = dSaldo.
        ...
        BUFFER-COPY saldo-terc TO ttSaldoTerc.
        ASSIGN ttSaldoTerc.r-rowid      = ROWID(saldo-terc)
               ttSaldoTerc.qtDisp       = dSaldo
               ttSaldoTerc.qtAlocada    = dAlocado.
