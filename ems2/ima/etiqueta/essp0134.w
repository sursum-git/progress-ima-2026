...
&Scoped-Define ENABLED-OBJECTS btn-importa bt-imprime fi-num-etiqueta ~
bt-sair 
...
DEFINE BUTTON btn-importa 
     ...
     LABEL "Button 1" 
     SIZE 5 BY 1.5 TOOLTIP "Importar Dados do Coletor para Localizaá∆o de Etiqueta na Doca.".
...
DEFINE FRAME DEFAULT-FRAME
     btn-importa AT ROW 5.25 COL 75
...
DO:
    ...
    IF ob-etiqueta.nr-lote = '' THEN DO.
       MESSAGE 'Etiqueta est† sem Lote, provavelmente n∆o foi Revisada,' SKIP
               ' Confirme com setor Respons†vel'
                VIEW-AS ALERT-BOX ERROR BUTTONS OK.
       APPLY 'entry' TO fi-num-etiqueta.
       RETURN NO-APPLY.
    END.
    ...
    IF AVAIL ordem-benefic AND
       ob-etiqueta.it-codigo <> ordem-benefic.it-codigo THEN DO.
...
------------------------------------------------------------------------------*/
  ...
  ENABLE btn-importa bt-imprime fi-num-etiqueta bt-sair 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
------------------------------------------------------------------------------*/
    ...
    OUTPUT TO VALUE(c-prog-epl) APPEND.
       PUT UNFORMATTED 
           ...
           IF AVAIL ordem-benefic AND ordem-benefic.cor-etiqueta = 99 THEN "LE210,55,480,50" ELSE "" SKIP
           ...
           "A430,190,0,3,1,2,N," '"' STRING(ob-etiqueta.quantidade,">>9.99") '"' SKIP
