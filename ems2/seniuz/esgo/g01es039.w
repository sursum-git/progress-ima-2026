...
&Scoped-Define ENABLED-FIELDS tp-embala.tipo 
&Scoped-define ENABLED-TABLES tp-embala
&Scoped-define FIRST-ENABLED-TABLE tp-embala
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS tp-embala.tipo 
&Scoped-define DISPLAYED-TABLES tp-embala
&Scoped-define FIRST-DISPLAYED-TABLE tp-embala
...
DEFINE FRAME d-vapara
     tp-embala.tipo AT ROW 1.17 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     fi-descricao AT ROW 1.17 COL 25.29 COLON-ALIGNED NO-LABEL
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Tipos de Embalagem"
...
/* SETTINGS FOR FILL-IN fi-descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para Tipos de Embalagem */
DO:
  find tp-embala no-lock 
      where tp-embala.tipo = input frame {&frame-name} tp-embala.tipo no-error.
  if  not avail tp-embala then do:
      {utp/ut-table.i espec tp-embala 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(tp-embala).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Tipos de Embalagem */
DO:  
...
&Scoped-define SELF-NAME tp-embala.tipo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tp-embala.tipo d-vapara
ON LEAVE OF tp-embala.tipo IN FRAME d-vapara /* Tipo */
DO:
  FIND tp-embala WHERE tp-embala.tipo = INPUT FRAME {&FRAME-NAME} tp-embala.tipo NO-LOCK NO-ERROR.
  IF AVAIL tp-embala THEN
     ASSIGN fi-descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = tp-embala.descricao.
  ELSE DO:
     MESSAGE "Tipo de Embalagem inv lido" VIEW-AS ALERT-BOX.
     APPLY 'entry' TO tp-embala.tipo IN FRAME {&FRAME-NAME}.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-descricao 
      WITH FRAME d-vapara.
  IF AVAILABLE tp-embala THEN 
    DISPLAY tp-embala.tipo 
      WITH FRAME d-vapara.
  ENABLE tp-embala.tipo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
