...
&Scoped-Define ENABLED-OBJECTS rt-button i_cod-rep bt-ok bt-cancela ~
bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_cod-rep 
...
DEFINE VARIABLE i_cod-rep AS INTEGER FORMAT ">>>>9":U INITIAL 0 
     LABEL "Representante" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_cod-rep AT ROW 1.75 COL 30 COLON-ALIGNED WIDGET-ID 2
...
ON GO OF FRAME d-vapara /* V  Para Representante */
DO:
FIND repres NO-LOCK
    WHERE repres.cod-rep EQ INPUT FRAME {&FRAME-NAME} i_cod-rep NO-ERROR.

IF NOT AVAILABLE repres THEN DO:
    {utp/ut-table.i ems2ima repres 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(repres).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Representante */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_cod-rep 
      WITH FRAME d-vapara.
  ENABLE rt-button i_cod-rep bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
