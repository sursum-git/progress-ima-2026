...
DEFINE VAR p-it-codigo LIKE ITEM.it-codigo INIT '500091'.
DEFINE VAR p-tab-preco LIKE preco-item.nr-tabpre.
...
&Scoped-Define ENABLED-OBJECTS bt-exp-preco fi-cod-refer fi-quantidade ~
fi-valor-un br-ref-item bt-ok bt-cancela bt-exp-qtde bt-ajuda RECT-5 ~
rt-buttom 
...
DEFINE BUTTON bt-exp-preco 
     ...
     LABEL "Button 3" 
     SIZE 4 BY 1.21 TOOLTIP "Exporta Preáo para todas Referàncias".

DEFINE BUTTON bt-exp-qtde 
     IMAGE-UP FILE "image/im-aloca.bmp":U NO-FOCUS
     LABEL "Button 2" 
     SIZE 4 BY 1.21 TOOLTIP "Exporta Quantidade para todas Referàncias".
...
DEFINE VARIABLE fi-tot-qt-pedida AS DECIMAL FORMAT "-ZZZ,ZZZ,ZZ9.9999" INITIAL 0 
      VIEW-AS TEXT 
     SIZE 10.29 BY .67
     FGCOLOR 12  NO-UNDO.
...
DEFINE FRAME D-Dialog
     ...
     fi-tot-qt-pedida AT ROW 13.88 COL 37.43 COLON-ALIGNED NO-LABEL
     fi-vlr-total AT ROW 13.88 COL 48.72 COLON-ALIGNED NO-LABEL
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         FONT 1
         TITLE "Digitaá∆o R†pida de Itens"
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Digitaá∆o R†pida de Itens */
DO:  
...
DO:
  ASSIGN tt-ref-item.vl-tot-ref = tt-ref-item.qt-pedida * tt-ref-item.preco-un.
END.
...
DO:
  FOR EACH tt-ref-item.
      ASSIGN tt-ref-item.qt-pedida = INPUT FRAME {&FRAME-NAME} fi-quantidade.
  END.
  {&OPEN-QUERY-br-ref-item}
END.
...
ON LEAVE OF fi-cod-refer IN FRAME D-Dialog /* Referància */
DO:
    FIND referencia WHERE 
         referencia.cod-refer = INPUT FRAME {&FRAME-NAME} fi-cod-refer
         NO-LOCK NO-ERROR.
    ...
    FIND ref-item WHERE
         ref-item.cod-refer = INPUT FRAME {&FRAME-NAME} fi-cod-refer AND
         ref-item.it-codigo = INPUT FRAME {&FRAME-NAME} fi-it-codigo NO-LOCK NO-ERROR.
    ...
    IF NOT AVAIL tt-ref-item THEN DO.
       MESSAGE "Referància sem Saldo em Estoque..."
           VIEW-AS ALERT-BOX ERROR BUTTONS OK.
       APPLY 'entry' TO SELF.
       RETURN NO-APPLY.
    END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-refer D-Dialog
ON RETURN OF fi-cod-refer IN FRAME D-Dialog /* Referància */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-refer D-Dialog
ON VALUE-CHANGED OF fi-cod-refer IN FRAME D-Dialog /* Referància */
DO:
...
------------------------------------------------------------------------------*/
  DISPLAY fi-it-codigo fi-desc-item fi-cod-refer fi-quantidade fi-valor-un 
      ...
      WITH FRAME D-Dialog.
  ENABLE bt-exp-preco fi-cod-refer fi-quantidade fi-valor-un br-ref-item bt-ok 
         bt-cancela bt-exp-qtde bt-ajuda RECT-5 rt-buttom 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH saldo-estoq WHERE
        ...
        FIND tt-ref-item WHERE
             tt-ref-item.cod-refer = saldo-estoq.cod-refer NO-LOCK NO-ERROR.
        IF NOT AVAIL tt-ref-item THEN DO.
           CREATE tt-ref-item.
           ASSIGN tt-ref-item.cod-refer = saldo-estoq.cod-refer.
        END.
        ASSIGN tt-ref-item.qtidade-atu = tt-ref-item.qtidade-atu + saldo-estoq.qtidade-atu.

        FIND preco-item WHERE
             preco-item.nr-tabpre = p-tab-preco AND
             preco-item.it-codigo = saldo-estoq.it-codigo AND
             preco-item.cod-refer = saldo-estoq.cod-refer
             NO-LOCK NO-ERROR.

        IF AVAIL preco-item THEN
           ASSIGN tt-ref-item.preco-un = preco-item.preco-venda.
    END.
