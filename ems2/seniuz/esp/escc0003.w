...
*******************************************************************************/
{include/i-prgvrs.i ESCC0003 2.04.00.000}
...
define temp-table tt-param no-undo
       ...
       FIELD impr-qdo-alt     AS LOG FORMAT "Sim/N∆o"
       FIELD impr-param       AS LOG FORMAT "Sim/N∆o".
...
&Scoped-Define ENABLED-OBJECTS rs-classif rs-cod-desc-item 
&Scoped-Define DISPLAYED-OBJECTS rs-classif rs-cod-desc-item 
...
DEFINE VARIABLE rs-classif AS INTEGER INITIAL 1 
     ...
     RADIO-BUTTONS 
          "Por Item/Data Emiss∆o/Preáo Unit†rio", 1,
"Por Item/Data Entrega/Preáo Unit†rio", 2,
"Por Item/Preáo Unit†rio", 3,
"Por Centro de Custo/Item/Data", 4
     SIZE 30.29 BY 4
     FONT 1 NO-UNDO.

DEFINE VARIABLE rs-cod-desc-item AS INTEGER INITIAL 2 
     VIEW-AS RADIO-SET VERTICAL
     RADIO-BUTTONS 
          "Pelo C¢digo do Item", 1,
"Pela Descriá∆o do Item", 2
     SIZE 20.72 BY 1.75 TOOLTIP "Ao classificar o item, fazà-lo pelo C¢digo ou pela Descriá∆o."
     FONT 1 NO-UNDO.
...
DEFINE VARIABLE to-impr-qdo-alt AS LOGICAL INITIAL yes 
     LABEL "Imprimir Somente Quando Alterar Preáo" 
     VIEW-AS TOGGLE-BOX
     SIZE 29.86 BY .83 TOOLTIP "Imprimir a linha do item somente quando alterar o preáo."
     FONT 1 NO-UNDO.
...
DEFINE VARIABLE fi-fin-nome-abrev AS CHARACTER FORMAT "X(12)" INITIAL "ZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 TOOLTIP "Nome abreviado do fornecedor final." NO-UNDO.
...
DEFINE VARIABLE fi-ini-nome-abrev AS CHARACTER FORMAT "X(12)" 
     ...
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 TOOLTIP "Nome abreviado do forncedor inicial." NO-UNDO.
...
DEFINE FRAME f-pg-par
     ...
     to-impr-qdo-alt AT ROW 9.96 COL 10.14
     to-impr-param AT ROW 9.96 COL 46.29
...
DEFINE FRAME f-pg-cla
     ...
     rs-cod-desc-item AT ROW 2.38 COL 37.29 HELP
          "Classificaá∆o para emiss∆o do relat¢rio" NO-LABEL
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
...
DEFINE FRAME f-pg-sel
     ...
     fi-ini-nome-abrev AT ROW 5.5 COL 16.86 COLON-ALIGNED HELP
          "Nome abreviado do cliente/fornecedor"
     fi-fin-nome-abrev AT ROW 5.5 COL 44.29 COLON-ALIGNED NO-LABEL
     ...
     IMAGE-61 AT ROW 5.5 COL 33.43
     IMAGE-62 AT ROW 5.5 COL 43.14
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Ordens de Compra por Item e Evoluá∆o Preáos"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-relat /* Ordens de Compra por Item e Evoluá∆o Preáos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Ordens de Compra por Item e Evoluá∆o Preáos */
DO:
...
ON LEAVE OF fi-fin-nome-abrev IN FRAME f-pg-sel
DO:
  IF INPUT FRAME {&FRAME-NAME} fi-fin-nome-abrev < 
     INPUT FRAME {&FRAME-NAME} fi-ini-nome-abrev THEN DO:
     MESSAGE "Nome abreviado do fornecedor final deve ser maior ou igual ao inicial."
         VIEW-AS ALERT-BOX INFO BUTTONS OK.
     APPLY 'entry' TO fi-fin-nome-abrev IN FRAME {&FRAME-NAME}.
     RETURN NO-APPLY.
  END.
END.
...
ON MOUSE-SELECT-DBLCLICK OF fi-ini-nome-abrev IN FRAME f-pg-sel /* Nome Abreviado */
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad098.w
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rs-classif w-relat
ON VALUE-CHANGED OF rs-classif IN FRAME f-pg-cla
DO:
  IF INPUT FRAME {&FRAME-NAME} rs-classif = 4 THEN
     ASSIGN to-impr-qdo-alt:SCREEN-VALUE IN FRAME f-pg-par = "no"
            to-impr-qdo-alt:SENSITIVE IN FRAME f-pg-par = NO
            rs-cod-desc-item:SCREEN-VALUE IN FRAME f-pg-cla = "2"
            rs-cod-desc-item:SENSITIVE IN FRAME f-pg-cla = NO.    
  ELSE
     ASSIGN to-impr-qdo-alt:SCREEN-VALUE IN FRAME f-pg-par = "yes"
            to-impr-qdo-alt:SENSITIVE IN FRAME f-pg-par = YES
            rs-cod-desc-item:SCREEN-VALUE IN FRAME f-pg-cla = "2"
            rs-cod-desc-item:SENSITIVE IN FRAME f-pg-cla = YES.    
END.
...
{utp/ut9000.i "ESCC0003" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-ini-ge-codigo fi-fin-ge-codigo fi-ini-it-codigo fi-fin-it-codigo 
          ...
          fi-fin-dt-entrega fi-ini-nome-abrev fi-fin-nome-abrev 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE fi-ini-ge-codigo fi-fin-ge-codigo fi-ini-it-codigo fi-fin-it-codigo 
         fi-ini-dt-emissao fi-fin-dt-emissao fi-ini-dt-entrega 
         fi-fin-dt-entrega fi-ini-nome-abrev fi-fin-nome-abrev IMAGE-29 
         IMAGE-30 IMAGE-31 IMAGE-32 IMAGE-5 IMAGE-6 IMAGE-61 IMAGE-62 IMAGE-67 
         IMAGE-68 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  {&OPEN-BROWSERS-IN-QUERY-f-pg-sel}
  DISPLAY rs-classif rs-cod-desc-item 
      WITH FRAME f-pg-cla IN WINDOW w-relat.
  ENABLE rs-classif rs-cod-desc-item 
      WITH FRAME f-pg-cla IN WINDOW w-relat.
  ...
  DISPLAY rs-data-conv fi-mo-codigo fi-desc-moeda tg-nao-confirm tg-terminada 
          ...
          fi-arq-excel to-impr-qdo-alt to-impr-param 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE rs-data-conv fi-mo-codigo tg-nao-confirm tg-terminada tg-confirm 
         tg-eliminada tg-em-cotacao tg-cotada tg-gerar-excel to-impr-qdo-alt 
         to-impr-param RECT-18 RECT-27 RECT-28 RECT-29 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario          = c-seg-usuario
           ...
           tt-param.cod-desc-item    = INPUT FRAME f-pg-cla rs-cod-desc-item
           ...
           tt-param.nome-abrev-ini   = input frame f-pg-sel fi-ini-nome-abrev
           tt-param.nome-abrev-fin   = input frame f-pg-sel fi-fin-nome-abrev
           ...
           tt-param.impr-qdo-alt     = INPUT FRAME f-pg-par to-impr-qdo-alt.
    ...
    {include/i-rprun.i esrp/escc0003rp.p}
