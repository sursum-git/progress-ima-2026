...
*******************************************************************************/
{include/i-prgvrs.i ESSP0174B 2.04.00.000}
...
&Scoped-define OPEN-QUERY-br-pedidos RUN pi-total. OPEN QUERY {&SELF-NAME} FOR EACH tt-can WHERE                                  tt-can.dia = p-dia NO-LOCK                               BY tt-can.nr-pedcli.
...
&Scoped-Define ENABLED-OBJECTS br-pedidos bt-detalhe bt-imprime bt-ok ~
...
tt-can.nr-pedcli    COLUMN-LABEL "Ped Cli"  FORMAT "x(8)" COLUMN-BGCOLOR 8
...
tt-can.dt-cancela   COLUMN-LABEL "Dt.Cancela"
...
tt-can.valor        FORMAT ">>>>,>>>,>>9.99" COLUMN-LABEL "Valores Cancelados" 
...
DEFINE FRAME D-Dialog
     ...
     fi-tot-qtd-nor AT ROW 13.08 COL 64.86 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-nor AT ROW 13.13 COL 77.57 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-ljs AT ROW 14.08 COL 64.86 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ljs AT ROW 14.08 COL 77.57 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-ind AT ROW 15.08 COL 64.86 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ind AT ROW 15.08 COL 77.57 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-out AT ROW 16.08 COL 64.86 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-out AT ROW 16.08 COL 77.57 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-geral AT ROW 17.08 COL 64.86 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     ...
     bt-ajuda AT ROW 18.38 COL 82.14
     rt-buttom AT ROW 18.13 COL 1.57
     "Normal:" VIEW-AS TEXT
          SIZE 6 BY .88 AT ROW 13 COL 60.72
     "Lojas:" VIEW-AS TEXT
          SIZE 4.57 BY .88 AT ROW 14 COL 61.86
     "Industrializaá∆o:" VIEW-AS TEXT
          SIZE 11 BY .88 AT ROW 15 COL 55.14
     "Total Geral:" VIEW-AS TEXT
          SIZE 8 BY .88 AT ROW 17 COL 58.14
     "Outros:" VIEW-AS TEXT
          SIZE 5 BY .88 AT ROW 16 COL 61.29
     SPACE(28.99) SKIP(2.82)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "Detalha Pedidos - ESSP0174B"
...
/* Query rebuild information for BROWSE br-pedidos
     _START_FREEFORM
RUN pi-total.
OPEN QUERY {&SELF-NAME} FOR EACH tt-can WHERE
                                 tt-can.dia = p-dia NO-LOCK
                              BY tt-can.nr-pedcli.
     _END_FREEFORM
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Detalha Pedidos - ESSP0174B */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-tot-qtd-nor fi-tot-vlr-nor fi-tot-qtd-ljs fi-tot-vlr-ljs 
      ...
      WITH FRAME D-Dialog.
  ENABLE br-pedidos bt-detalhe bt-imprime bt-ok bt-cancela bt-ajuda rt-buttom 
      WITH FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  /* {utp/ut9000.i "ESSP0174B" "2.04.00.000"} */ 
  ...
  IF p-tp-relat = 1 THEN DO:
     ...
     ASSIGN  br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DE PEDIDOS CANCELADOS DO DIA: " + " " + c-data.
  END.
  ELSE
     ASSIGN  br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DE PEDIDOS CANCELADOS DO M“S: " + " " +
...
------------------------------------------------------------------------------*/
    PUT c-empresa  FORMAT "X(40)"                 AT   1
        ...
        SKIP(1).
    IF p-tp-relat = 1 THEN DO:
       PUT "RELATORIO DE DETALHAMENTO DE PEDIDOS CANCELADOS DO DIA " AT 31.
       PUT c-data AT 84 SKIP(1).
    END.
    ELSE DO:
        PUT "RELATORIO DE DETALHAMENTO DE PEDIDOS CANCELADOS DO MES " AT 31.
        PUT SUBSTR(p-periodo,1,2) + "/" + SUBSTR(p-periodo,3,4)       AT 84 SKIP(1).
    END.
 
    PUT "Tipo   Ped Cli Ped. Repres. Cliente      Representante Sit Dt.Cancela Dt.Implant   Qtde Vendida  Valor Vendido Motivo Cancelamento" AT 1.
    PUT "------ ------- ------------ ------------ ------------- --- ---------- ---------- -------------- -------------- ---------------------" AT 1.
...
------------------------------------------------------------------------------*/
  ...
  CASE i-saida:
      ...
      WHEN 3 THEN DO.
          ASSIGN c-saida = SESSION:TEMP-DIRECTORY + "essp0174b.tmp".
          OUTPUT TO VALUE(c-saida).
      END.
  END CASE.
 ...
 DO i-ct = 1 TO i-num-copias.
    ...
    FOR EACH tt-can WHERE
        ...
        PUT tt-can.tipo       FORMAT "x(6)"      AT   1
            ...
            tt-can.qtde  FORMAT ">>>,>>>,>>9.99" AT  82
            tt-can.valor FORMAT ">>>,>>>,>>9.99" AT  97
            tt-can.motivo FORMAT "(X(20)"        AT 112.
   ...
   IF (ACCUM TOTAL tt-can.qtde)  <> 0 OR
      ...
      PUT ACCUM TOTAL tt-can.qtde  FORMAT ">>>,>>>,>>9.99" AT 82.
      PUT ACCUM TOTAL tt-can.valor FORMAT ">>>,>>>,>>9.99" AT 97.
   END.
