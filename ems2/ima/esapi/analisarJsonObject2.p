...
PROCEDURE ProcessObject:
    ...
    DO i = 1 TO iTam:
        ASSIGN lcText = "".
        fix-codepage(lcText) = "utf-8":U NO-ERROR.
...
PROCEDURE criarTtJson:
   ...
   DEFINE BUFFER bf FOR ttJson.
   DEFINE VARIABLE vlFix AS LONGCHAR   NO-UNDO.
   FIX-CODEPAGE ( vlFix ) = "UTF-8".
   
   //retirada de caracter especial conforme erro https://community.progress.com/s/article/Error-1-converting-LONGCHAR-variable-to-cpinternal-for-comparison-11687-returning-XML-from-AppServerhttps://community.progress.com/s/article/Error-1-converting-LONGCHAR-variable-to-cpinternal-for-comparison-11687-returning-XML-from-AppServer         
   ASSIGN pValor = REPLACE(pValor,CHR(146),'').   
   ASSIGN vlFix  = pValor.
  /* 
  ...
  */
  ...
  IF LENGTH(pValor) <= 32000 THEN DO:
      ASSIGN ttJson.valor    = string(vlFix)
  ...
  ELSE DO: // se o conteudo ‚ maior que o comportado em um varchar, ‚ criado um arquivo com o conteudo e na tag ‚ colocado o arquivo gerado e ‚ marcado o campo logarquivo como yes.
      ...
      RUN convLongChar2File(vlFix, INPUT cArqLc) NO-ERROR .
