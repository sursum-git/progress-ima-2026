...
*******************************************************************************/
{include/i-prgvrs.i IMES202 2.04.00.001}
...
DEFINE VARIABLE h_b01imes202 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01imes202 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01imes202 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadpaifilho-filho /* Permiss∆o dos Usu†rios */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Permiss∆o dos Usu†rios */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imvwr/v01imes202.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01imes202 ).
       RUN set-position IN h_v01imes202 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Tipo Pedido' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 4.50 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.50 , 90.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imqry/q01imes202.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom/z01di211.r,
                     ProgVaPara = digo/g01di211.r,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01imes202 ).
       RUN set-position IN h_q01imes202 ( 1.25 , 54.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 12.00 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v01imes202. */
       RUN add-link IN adm-broker-hdl ( h_q01imes202 , 'Record':U , h_v01imes202 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01imes202. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01imes202 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01imes202 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01imes202 ).

    END. /* Page 0 */

    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imbrw/b01imes202.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = imp/imes202a.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b01imes202 ).
       RUN set-position IN h_b01imes202 ( 6.50 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to BrowserCadastro2 h_b01imes202. */
       RUN add-link IN adm-broker-hdl ( h_q01imes202 , 'Record':U , h_b01imes202 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "IMES202" "2.04.00.001"}
