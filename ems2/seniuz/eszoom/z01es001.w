...
DEFINE VARIABLE h_b01es001 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es001 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 13.04 COL 1.57
     bt-ok AT ROW 14.33 COL 3
     bt-cancela AT ROW 14.33 COL 14
     bt-ajuda AT ROW 14.33 COL 76.43
     rt-button AT ROW 14.08 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Amostras por Fam¡lia"
         HEIGHT             = 14.46
         WIDTH              = 87.43
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 91.29
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Amostras por Fam¡lia */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Amostras por Fam¡lia */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Fam¡lia|Ano-Mˆs' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.00 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.83 , 86.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es001.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es001 ).
       RUN set-position IN h_b01es001 ( 2.50 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 84.57 ) */

       /* Links to SmartBrowser h_b01es001. */
       RUN add-link IN adm-broker-hdl ( h_b01es001 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es001.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es001 ).
       RUN set-position IN h_b02es001 ( 2.50 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 84.57 ) */

       /* Links to SmartBrowser h_b02es001. */
       RUN add-link IN adm-broker-hdl ( h_b02es001 , 'State':U , THIS-PROCEDURE ).
