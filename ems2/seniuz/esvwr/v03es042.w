...
fi-largura fi-fator-conv fi-peso-liquido to-indigo 

/* Custom List Definitions                                              */
/* ADM-CREATE-FIELDS,ADM-ASSIGN-FIELDS,ADM-MODIFY-FIELDS,List-4,List-5,List-6 */
&Scoped-define ADM-CREATE-FIELDS fi-peso-liquido 
&Scoped-define ADM-MODIFY-FIELDS fi-peso-liquido 
&Scoped-define List-4 to-indigo 
...
DEFINE VARIABLE to-indigo AS LOGICAL INITIAL no 
     LABEL "Öndigo" 
     VIEW-AS TOGGLE-BOX
     SIZE 11.57 BY .88 TOOLTIP "A fam¡lia ‚ um Öndigo?" NO-UNDO.


/* ************************  Frame Definitions  *********************** */

DEFINE FRAME f-main
     fi-cod-composi AT ROW 1.42 COL 15 COLON-ALIGNED
     fi-desc-composi AT ROW 1.42 COL 20.43 COLON-ALIGNED NO-LABEL
     fi-cod-rlgp AT ROW 2.42 COL 15 COLON-ALIGNED HELP
          "Codigo de Recomendacao de Lavagem por Grupo de Produto"
     fi-largura AT ROW 3.42 COL 15 COLON-ALIGNED HELP
          "Largura do tecido"
     fi-fator-conv AT ROW 4.42 COL 15 COLON-ALIGNED HELP
          "Fator de conversao para Metros"
     fi-peso-liquido AT ROW 5.42 COL 15 COLON-ALIGNED HELP
          "Peso l¡quido do item"
     to-indigo AT ROW 7 COL 17
...
------------------------------------------------------------------------------*/
    ...
    FIND familia-ext WHERE
         familia-ext.fm-codigo = familia.fm-codigo NO-LOCK NO-ERROR.

    IF AVAIL familia-ext THEN DO.
       ASSIGN fi-cod-composi = familia-ext.cod-composi
              fi-cod-rlgp = familia-ext.cod-rlgp
              fi-largura = familia-ext.largura
              fi-fator-conv = familia-ext.fator-conv
              to-indigo = familia-ext.indigo.
    
       FIND composi WHERE
            composi.cod-composi = familia-ext.cod-composi NO-LOCK NO-ERROR.

       IF AVAIL composi THEN
          ASSIGN fi-desc-composi = composi.descricao.
    
       FIND FIRST ITEM WHERE
            ITEM.fm-codigo =  familia.fm-codigo AND
            ITEM.un        <> "kg" NO-LOCK NO-ERROR.
    
       IF AVAIL ITEM THEN 
          ASSIGN fi-peso-liquido = ITEM.peso-liquido
                 fi-fator-conv = 1 / ITEM.peso-liquido.
    END.
    ELSE
       ASSIGN fi-cod-composi = "" 
              fi-cod-rlgp = 0
              fi-largura = 0  
              fi-peso-liquido = 0
              fi-fator-conv = 0
