...
*******************************************************************************/
{include/i-prgvrs.i BCX0002WIN 1.00.00.000}
...
DEFINE VARIABLE h_b03im001 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01im002 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01im002 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-filho ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Etiqueta Corte"
         HEIGHT             = 16.67
         WIDTH              = 90
         MAX-HEIGHT         = 27.17
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadpaifilho-filho /* Manuten‡Æo Etiqueta Corte */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-filho w-cadpaifilho-filho
ON WINDOW-CLOSE OF w-cadpaifilho-filho /* Manuten‡Æo Etiqueta Corte */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'clvwr/v01cl005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01im002 ).
       RUN set-position IN h_v01im002 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Etiq Corte' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 10.75 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 6.92 , 90.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'clbrw/b02cl005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = bcp/bcx0002wina.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b03im001 ).
       RUN set-position IN h_b03im001 ( 12.25 , 14.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.04 , 66.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'clqry/q01cl005.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = clzoom/z01cl005.w,
                     ProgVaPara = clgo/g02cl005.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01im002 ).
       RUN set-position IN h_q01im002 ( 1.00 , 47.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v01im002. */
       RUN add-link IN adm-broker-hdl ( h_q01im002 , 'Record':U , h_v01im002 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_b03im001. */
       RUN add-link IN adm-broker-hdl ( h_q01im002 , 'Record':U , h_b03im001 ).

       /* Links to SmartQuery h_q01im002. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01im002 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01im002 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01im002 ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v01im002 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b03im001 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "BCX0002WIN" "1.00.00.000"}
  ...
  ASSIGN h-viewer = h_v01im002.
  RUN pi-recebe-handle IN h_b03im001 (INPUT h-viewer).
