...
** Setor...: Mecanica
** Objetivo: Imprimir o Relatorio de Incidencia de Manutencao por Funcionario 
** Autor...: Gilvando Souza Araujo - Julho/2000
**
** Conversao para EMS 2.04:
**   Programa: ESMECR04.P  =>  ESSP0082RP.P
**   Autor...: Gilvando Souza Araujo
**   Data....: 08/02/2004
*/

/* include de controle de versÆo */
{include/i-prgvrs.i ESSP0082RP 2.04.00.000}

def temp-table w-work
    field func-exec  like mov-man.func-exec[1]
...
def var i-qtd-os-func  as int.
def var i-min-man-func as int.
def var i-min-par-func as int.
...
form
    w-work.func-exec     label "Func."
    w-work.cod-area      label "Area"
    w-work.cod-maq       label "Maquina"
...
{utp/ut-liter.i Incidˆncia_de_Manuten‡Æo_por_Funcion rio * r}
...
for each mov-man where mov-man.num-os     >= tt-param.ini-num-os
   ...
   do i-cont = 1 to 5:
      ...
      do i-cont1 = 1 to 10:
         if int(mov-man.func-exec[i-cont1]) = 0 then leave.
         assign l-inclui = yes.
         do i-cont2 = 1 to 5:
         ...
         and int(mov-man.ativ-exec[i-cont1]) = i-cont then do:
             ...
             assign i-min-par = min-parado[i-cont1].   
         end.
         if i-minutos <> 0 
         or i-min-par <> 0 then do:
            find first w-work
                 where w-work.func-exec = mov-man.func-exec[i-cont1]
            ...
            if not avail w-work then do:
               create w-work.
               assign w-work.func-exec = mov-man.func-exec[i-cont1]
            ...
            assign w-work.qtd-os  = w-work.qtd-os + 1
                   ...
                   w-work.min-man = w-work.min-man + i-minutos
                   w-work.min-par = w-work.min-par + min-parado[i-cont1]
                   i-min-man-ger  = i-min-man-ger + i-minutos
                   i-min-par-ger  = i-min-par-ger + min-parado[i-cont1].
            assign i-minutos = 0
                   i-min-par = 0.       
         end.
      end.
...
for each w-work break by w-work.func-exec
                      by w-work.cod-area
                      by w-work.cod-maq
                      by w-work.tipo-man
                      by w-work.sist-exec:
   if first-of(w-work.func-exec) then do:
      for each b-w-work where b-w-work.func-exec = w-work.func-exec:
          assign i-qtd-os-func  = i-qtd-os-func +  b-w-work.qtd-os
                 i-min-man-func = i-min-man-func + b-w-work.min-man
                 i-min-par-func = i-min-par-func + b-w-work.min-par.
      end.
      display w-work.func-exec
              with frame f-detalhe.
   end.
   assign i-perc1 = w-work.qtd-os / i-qtd-os-func * 100
          i-perc2 = w-work.min-man / i-min-man-func * 100
          i-perc3 = w-work.min-par / i-min-par-func * 100.
   ...
   if last-of(w-work.func-exec) then do:
      assign i-perc1 = i-qtd-os-func / i-qtd-os-ger * 100
             i-perc2 = i-min-man-func / i-min-man-ger * 100
             i-perc3 = i-min-par-func / i-min-par-ger * 100.
          
      assign c-hor-man = string((i-min-man-func - 
                                (i-min-man-func MODULO 60)) / 
                                 60,"99999") + 
                         string((i-min-man-func MODULO 60),"99")
             c-hor-par = string((i-min-par-func - 
                                (i-min-par-func MODULO 60)) / 
                                 60,"99999") + 
                         string((i-min-par-func MODULO 60),"99").
   
      display "Total"        @ w-work.func-exec
              i-qtd-os-func @ w-work.qtd-os
              i-perc1
              c-hor-man
              i-perc2
              c-hor-par
