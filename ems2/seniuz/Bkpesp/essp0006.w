...
*******************************************************************************/
{include/i-prgvrs.i ESSP0006 2.04.00.000}
...
/* Local Variable Definitions ---                                       */
 /* DEF NEW GLOBAL SHARED VAR h-v06es022 AS HANDLE. */
...
DEFINE VARIABLE h_q01es022 AS HANDLE NO-UNDO.
...
DEFINE VARIABLE h_v06es022 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v07es022 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadcom /* Movimentos da Manuten‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadcom w-cadcom
ON WINDOW-CLOSE OF w-cadcom /* Movimentos da Manuten‡Æo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Dados Gerais|Sistemas|Atividades|Observa‡äes' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 7.63 , 2.00 ) NO-ERROR.
       ...
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es022 ).
       RUN add-link IN adm-broker-hdl ( h_q01es022 , 'Record':U , h_v01es022 ).
    ...
    WHEN 1 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q01es022 , 'Record':U , h_v02es022 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v06es022.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v06es022 ).
       RUN set-position IN h_v06es022 ( 8.75 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 87.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3') NO-ERROR.

       /* Links to SmartViewer h_v06es022. */
       RUN add-link IN adm-broker-hdl ( h_q01es022 , 'Record':U , h_v06es022 ).
       RUN add-link IN adm-broker-hdl ( h_v01es022 , 'GROUP-ASSIGN':U , h_v06es022 ).

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v07es022.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v07es022 ).
       RUN set-position IN h_v07es022 ( 8.75 , 3.14 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 87.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es022.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es022.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01es022 ).
       RUN set-position IN h_q01es022 ( 1.25 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v07es022. */
       RUN add-link IN adm-broker-hdl ( h_q01es022 , 'Record':U , h_v07es022 ).
       RUN add-link IN adm-broker-hdl ( h_v01es022 , 'GROUP-ASSIGN':U , h_v07es022 ).

       /* Links to SmartQuery h_q01es022. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es022 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es022 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es022 ).
    ...
    WHEN 4 THEN DO:
       ...
       RUN add-link IN adm-broker-hdl ( h_q01es022 , 'Record':U , h_v05es022 ).
...
------------------------------------------------------------------------------*/
  ...
  /*
  IF VALID-HANDLE(h_v06es022) THEN
     ASSIGN h-v06es022 = h_v06es022.  */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0006" "2.04.00.000"}
