...
*******************************************************************************/
{include/i-prgvrs.i V02DI154 2.04.00.000}
...
fi-dv to-faturado fi-cod-lote fi-cod-refer fi-qtd-ultent fi-sol-deposito ~
fi-quantidade fi-dat-ultent fi-qtd-volum fi-volume-ini fi-qtd-dispon ~
fi-volume-fin fi-sld-dispon fi-sigla-emb fi-desc-dentro 
...
DEFINE BUTTON bt-sol-depos 
     LABEL "Solicitou Dep¢sito ?" 
     SIZE 16 BY 1.13 TOOLTIP "Exclui a reserva.".
...
DEFINE VARIABLE fi-sol-deposito AS LOGICAL FORMAT "Sim/N∆o":U INITIAL NO 
     VIEW-AS FILL-IN 
     SIZE 4 BY .88 TOOLTIP "Solicitou dep¢sito - Pedidos Ö vista." NO-UNDO.
...
DEFINE FRAME f-main
     ...
     bt-sol-depos AT ROW 8 COL 41.43
     fi-sol-deposito AT ROW 8.13 COL 56 COLON-ALIGNED NO-LABEL
...
/* SETTINGS FOR BUTTON bt-exclui-res IN FRAME f-main
   NO-ENABLE                                                            */
/* SETTINGS FOR BUTTON bt-sol-depos IN FRAME f-main
   NO-ENABLE                                                            */
...
/* SETTINGS FOR FILL-IN fi-sld-dispon IN FRAME f-main
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-sol-deposito IN FRAME f-main
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME bt-sol-depos
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-sol-depos V-table-Win
ON CHOOSE OF bt-sol-depos IN FRAME f-main /* Solicitou Dep¢sito ? */
DO:
  RUN pi-sol-deposito.
  RUN local-display-fields.
END.
...
ON ENTRY OF fi-dv IN FRAME f-main /* DV */
DO:
...
DO:
  ...
  IF AVAIL ref-item-ext THEN DO.
     ASSIGN c-item-ref = ped-item.it-codigo + ped-item.cod-refer
            i-dv-dig-calc = ref-item-ext.dv.
  ...
  */
  ...
  find last movto-estoq where
   ...
   assign de-acm-res = 0.
   FOR each b-ped-item-res WHERE b-ped-item-res.it-codigo =  c-item-ref
...
DO:
  ...
  /*
  IF abs(dec(INPUT FRAME {&frame-name} fi-quantidade) - ped-item.qt-pedida) > 
                                   (ped-item.qt-pedida * fi-perc-var-res * 0.01) THEN DO:
     MESSAGE "A variaá∆o entre Quantidade Pedida e Reservada Ç superior ao ParÉmetro."
             VIEW-AS ALERT-BOX.
  END.
  ELSE
  IF dec(INPUT FRAME {&frame-name} fi-quantidade) - ped-item.qt-pedida <> 0 THEN DO:
     MESSAGE "A variaá∆o entre Quantidade Pedida e Reservada est† dentro do ParÉmetro."
             VIEW-AS ALERT-BOX.
  END.
  */
END.
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN bt-exclui-res:SENSITIVE IN FRAME {&FRAME-NAME}= NO
           bt-sol-depos:SENSITIVE IN FRAME {&FRAME-NAME}= NO.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL ped-item-res THEN DO:
       ASSIGN to-faturado:SCREEN-VALUE IN FRAME {&FRAME-NAME}    = STRING(ped-item-res.faturado)
              ...
              fi-sol-deposito:screen-VALUE IN FRAME {&FRAME-NAME} = string(ped-item-res.sol-deposito).
       IF ped-item-res.faturado = NO THEN
          ASSIGN bt-exclui-res:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
       IF ped-venda.cod-cond-pag > 0 AND ped-venda.cod-cond-pag < 3 THEN DO:
          IF ped-item.cod-sit-item < 3 or ped-item.cod-sit-item = 5 THEN
             ASSIGN bt-sol-depos:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
       END.
    END.
    ELSE
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-sol-deposito V-table-Win 
PROCEDURE pi-sol-deposito :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    MESSAGE "A situaá∆o atual de Solicitaá∆o de Dep¢sito Ç: " 
            string(ped-item-res.sol-deposito,"Sim/N∆o") SKIP
            "Deseja alter†-la ?" 
            VIEW-AS ALERT-BOX WARNING BUTTONS YES-NO UPDATE l-conf-sol.

    IF l-conf-sol = YES THEN DO:
       IF ped-item-res.sol-deposito = YES THEN
          ASSIGN ped-item-res.sol-deposito = NO.
       else
          ASSIGN ped-item-res.sol-deposito = YES.
    END.
...
------------------------------------------------------------------------------*/
    ...
    for each b-ped-item-res where b-ped-item-res.nr-pedcli    =  ped-item.nr-pedcli
                              ...
                              AND b-ped-item-res.nr-sequencia <> ped-item.nr-sequencia
                            NO-LOCK:
        IF (int(INPUT FRAME {&frame-name} fi-volume-ini) >= b-ped-item-res.volume-ini AND
            INT(INPUT FRAME {&FRAME-NAME} fi-volume-ini) <= b-ped-item-res.volume-fim) OR
           (int(INPUT FRAME {&frame-name} fi-volume-ini) <= b-ped-item-res.volume-ini AND
            INT(INPUT FRAME {&FRAME-NAME} fi-volume-fin) >= b-ped-item-res.volume-ini) THEN
            assign l-achou = yes
                   i-volume-ini-aux = b-ped-item-res.volume-ini
                   i-volume-fim-aux = b-ped-item-res.volume-fim.
    end.
    if l-achou = yes then do:
       message "J† existe o intervalo de Volume: " i-volume-ini-aux " a " i-volume-fim-aux VIEW-AS ALERT-BOX.
       ASSIGN fi-volume-ini:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
              fi-volume-fin:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "".
       APPLY 'entry' TO fi-qtd-volum IN FRAME {&FRAME-NAME}.
       return 'ADM-ERROR':U.                                                                    
