...
DEFINE VARIABLE h_b01es027 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es027 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 14.88 COL 1.57
     bt-ok AT ROW 16.21 COL 3
     bt-cancela AT ROW 16.21 COL 14
     bt-ajuda AT ROW 16.21 COL 79
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de SubSistemas da Mecƒnica"
         HEIGHT             = 16.42
         WIDTH              = 90
         MAX-HEIGHT         = 28.92
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-pesquisa /* Pesquisa de SubSistemas da Mecƒnica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de SubSistemas da Mecƒnica */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es027.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es027 ).
       RUN set-position IN h_b01es027 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.00 ) */

       /* Links to SmartBrowser h_b01es027. */
       RUN add-link IN adm-broker-hdl ( h_b01es027 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es027.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es027 ).
       RUN set-position IN h_b02es027 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.00 ) */

       /* Links to SmartBrowser h_b02es027. */
       RUN add-link IN adm-broker-hdl ( h_b02es027 , 'State':U , THIS-PROCEDURE ).
