...
*******************************************************************************/
{include/i-prgvrs.i D02ESSP0190 2.04.00.000}
...
DEFINE OUTPUT PARAMETER p-arq-saida        AS CHAR FORMAT "x(50)".
...
&Scoped-Define ENABLED-OBJECTS rt-buttom fi-arq-saida bt-arq-sai bt-ok ~
bt-cancela bt-ajuda 
...
DEFINE BUTTON bt-arq-sai 
     ...
     LABEL "" 
     SIZE 4 BY .88 TOOLTIP "Arquivo de pagamento de sa¡da (ajuntado).".
...
DEFINE VARIABLE fi-arq-saida AS CHARACTER FORMAT "X(80)":U 
     LABEL "Arquivo Sa¡da" 
     VIEW-AS FILL-IN 
     SIZE 38 BY .88 TOOLTIP "Caminho/nome do arquivo de sa¡da."
     FONT 1 NO-UNDO.
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Parƒmetro da Planilha */
DO:  
     ...
     CASE p-tipo-selecao:
         WHEN 1 THEN
             ASSIGN fi-arq-saida:SCREEN-VALUE IN FRAME {&FRAME-NAME} = SESSION:TEMP-DIRECTORY +
                                    "Carteira a Faturar Em " + 
                                    SUBSTR(p-dt-faturar,1,2) + "-"+ SUBSTR(p-dt-faturar,3,4) + ".xls".
         WHEN 2 THEN
             ASSIGN fi-arq-saida:SCREEN-VALUE IN FRAME {&FRAME-NAME} = SESSION:TEMP-DIRECTORY +
                                    "Faturamento Em " +
                                    STRING(p-dt-faturadas-ini, "99-99-9999") + " A " + 
                                    STRING(p-dt-faturadas-fin, "99-99-9999") + ".xls".
         WHEN 3 THEN
             ASSIGN fi-arq-saida:SCREEN-VALUE IN FRAME {&FRAME-NAME} = SESSION:TEMP-DIRECTORY +
                                    "Vendas Em " +
                                    SUBSTR(p-dt-vendido-ini,1,2) + "-"+ SUBSTR(p-dt-vendido-ini,3,4) + " A " +
                                    SUBSTR(p-dt-vendido-fin,1,2) + "-"+ SUBSTR(p-dt-vendido-fin,3,4) + ".xls".
     END CASE.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-arq-saida 
      WITH FRAME D-Dialog.
  ENABLE rt-buttom fi-arq-saida bt-arq-sai bt-ok bt-cancela bt-ajuda 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "D02ESSP0190" "2.04.00.000"}
