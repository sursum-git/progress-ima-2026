...
&Scoped-Define ENABLED-FIELDS mov-est-acbm.tipo-tear ~
...
mov-est-acbm.it-codigo mov-est-acbm.cod-refer mov-est-acbm.tipo-tear ~
mov-est-acbm.qtd-tot-def mov-est-acbm.qtd-tot-perf mov-est-acbm.qtd-tot-sob 
&Scoped-define DISPLAYED-TABLES mov-est-acbm
&Scoped-define FIRST-DISPLAYED-TABLE mov-est-acbm
&Scoped-Define DISPLAYED-OBJECTS fi-desc-item fi-desc-refer fi-tot-sanfor ~
fi-perc-sobra 
...
DEFINE FRAME f-main
     mov-est-acbm.data-mov AT ROW 1.17 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     mov-est-acbm.num-lote AT ROW 2.17 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 6.86 BY .88
     mov-est-acbm.it-codigo AT ROW 3.17 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 17.14 BY .88
     fi-desc-item AT ROW 3.17 COL 35.29 COLON-ALIGNED NO-LABEL
     mov-est-acbm.cod-refer AT ROW 4.17 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8.86 BY .88
     fi-desc-refer AT ROW 4.17 COL 27 COLON-ALIGNED HELP
          "Descri‡Æo da referˆncia" NO-LABEL
     fi-tot-sanfor AT ROW 5.75 COL 18 COLON-ALIGNED
     mov-est-acbm.tipo-tear AT ROW 5.75 COL 34.86 COLON-ALIGNED
          VIEW-AS COMBO-BOX 
          LIST-ITEMS "Howa","Nissan","Sulzer","Picanol","Tsudakoma","Toyota" 
          DROP-DOWN-LIST
          SIZE 9.86 BY .88
     mov-est-acbm.qtd-tot-def AT ROW 5.75 COL 56.43 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     mov-est-acbm.qtd-tot-perf AT ROW 6.75 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     mov-est-acbm.qtd-tot-sob AT ROW 6.75 COL 56.43 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     fi-perc-sobra AT ROW 6.75 COL 69.86 COLON-ALIGNED NO-LABEL
...
/* SETTINGS FOR FILL-IN mov-est-acbm.cod-refer IN FRAME f-main
   NO-ENABLE                                                            */
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL mov-est-acbm THEN DO.
       ...
       ASSIGN fi-perc-sobra = mov-est-acbm.qtd-tot-sob / fi-tot-sanfor * 100.
       ASSIGN fi-perc-sobra:FGCOLOR IN FRAME {&FRAME-NAME} = 2.

       FIND item WHERE item.it-codigo = mov-est-acbm.it-codigo NO-LOCK NO-ERROR.
       IF AVAIL item THEN
          ASSIGN fi-desc-item = item.desc-item.
       
       FIND referencia WHERE referencia.cod-refer = mov-est-acbm.cod-refer NO-LOCK NO-ERROR.
       IF AVAIL referencia THEN
          ASSIGN fi-desc-refer = referencia.descricao.
    END.
