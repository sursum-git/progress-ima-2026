...
&Scoped-define QUERY-STRING-br-usr FOR EACH tt-usr WHERE                                  tt-usr.cod-usuario = tt-usr-connect.cod-usuario AND                                  tt-usr.maquina = tt-usr-connect.maquina AND                                  tt-usr.pid = tt-usr-connect.pid                                  NO-LOCK INDEXED-REPOSITION
&Scoped-define OPEN-QUERY-br-usr OPEN QUERY {&SELF-NAME} FOR EACH tt-usr WHERE                                  tt-usr.cod-usuario = tt-usr-connect.cod-usuario AND                                  tt-usr.maquina = tt-usr-connect.maquina AND                                  tt-usr.pid = tt-usr-connect.pid                                  NO-LOCK INDEXED-REPOSITION.
...
&Scoped-define FIELDS-IN-QUERY-br-usr-connect tt-usr-connect.usr tt-usr-connect.cod-usuario tt-usr-connect.maquina tt-usr-connect.login   
...
tg-run tg-idle tg-out tg-sup tg-free br-usr-connect br-usr fi-exit bt-super ~
bt-mata-usr bt-atualiza sl-time bt-exit 
&Scoped-Define DISPLAYED-OBJECTS tg-run tg-idle tg-out tg-sup tg-free ~
fi-hora-ini fi-hora-atu fi-exit sl-time fi-usr-all fi-usr-sup fi-usr-idle ~
fi-usr-run fi-usr-out 
...
DEFINE VARIABLE tg-free AS LOGICAL INITIAL yes 
     LABEL "Apenas no DB" 
     VIEW-AS TOGGLE-BOX
     SIZE 15.72 BY .83
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-usr-connect C-Win _FREEFORM
  QUERY br-usr-connect NO-LOCK DISPLAY
      tt-usr-connect.usr            FORMAT ">>>>9"  COLUMN-LABEL "ID FND"   WIDTH 5
...
DEFINE FRAME DEFAULT-FRAME
     ...
     tg-idle AT ROW 1.33 COL 19.72 WIDGET-ID 74
     tg-out AT ROW 1.33 COL 33.14 WIDGET-ID 76
     tg-sup AT ROW 1.33 COL 53.57 WIDGET-ID 78
     tg-free AT ROW 1.38 COL 73 WIDGET-ID 88
     ...
     bt-exit AT ROW 20.25 COL 2 WIDGET-ID 58
...
/* SETTINGS FOR FRAME DEFAULT-FRAME
   FRAME-NAME                                                           */
/* BROWSE-TAB br-usr-connect tg-free DEFAULT-FRAME */
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-usr WHERE
                                 ...
                                 tt-usr.maquina = tt-usr-connect.maquina AND
                                 tt-usr.pid = tt-usr-connect.pid
...
/* CtrlFrame OCXINFO:CREATE-CONTROL from: {F0B88A90-F5DA-11CF-B545-0020AF6ED35A} type: PSTimer */
      CtrlFrame:MOVE-AFTER(fi-usr-idle:HANDLE IN FRAME DEFAULT-FRAME).
...
DO:
  DO i-ct = 1 TO br-usr-connect:NUM-SELECTED-ROWS.
     IF br-usr-connect:FETCH-SELECTED-ROW(i-ct) THEN DO.
        CASE tt-usr-connect.marca.
             WHEN 'i' OR WHEN 'r' OR WHEN 'l' THEN DO.   /* Idle,Run ou Livre */
                 ASSIGN tt-usr-connect.marca = "o"
                        fi-usr-out = fi-usr-out + 1.

                 RUN pi-disconnect-db (INPUT tt-usr-connect.cod-usuario,
                                       INPUT tt-usr-connect.maquina).

                 RUN pi-disconnect-win (INPUT tt-usr-connect.maquina,
                                        INPUT tt-usr-connect.pid).
...
&Scoped-define SELF-NAME tg-free
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tg-free C-Win
ON VALUE-CHANGED OF tg-free IN FRAME DEFAULT-FRAME /* Apenas no DB */
DO:
  RUN pi-popula-browse.
END.
...
------------------------------------------------------------------------------*/
  RUN control_load.
  DISPLAY tg-run tg-idle tg-out tg-sup tg-free fi-hora-ini fi-hora-atu fi-exit 
          sl-time fi-usr-all fi-usr-sup fi-usr-idle fi-usr-run fi-usr-out 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE RECT-14 RECT-15 RECT-16 RECT-17 RECT-18 tg-run tg-idle tg-out tg-sup 
         tg-free br-usr-connect br-usr fi-exit bt-super bt-mata-usr bt-atualiza 
         sl-time bt-exit 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-disconnect-db C-Win 
PROCEDURE pi-disconnect-db :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-usr WHERE
        ...
        RUN szdbu/sum-mark.p tt-usr.banco STRING(tt-usr.usr) tt-usr.cod-usuario.
    END.
END PROCEDURE.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-disconnect-win C-Win 
PROCEDURE pi-disconnect-win :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    ...
    /*ASSIGN c-comando = "TASKKILL /S " + p-maquina + " /F /PID" +  STRING(p-pid) + " /T".*/

    FILE-INFO:FILE-NAME = "szdbu/pskill.exe".
    IF FILE-INFO:FULL-PATHNAME = ? THEN
       FILE-INFO:FILE-NAME = "seniuz/szdbu/pskill.exe".

    FILE-INFO:FILE-NAME = "szdbu/pskill.exe".
    ASSIGN c-comando = FILE-INFO:FULL-PATHNAME + " \\" + p-maquina + " " + STRING(p-pid).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
    ASSIGN INPUT FRAME {&FRAME-NAME} tg-run tg-idle tg-out tg-sup tg-free.
    ...
    ASSIGN c-marcas = c-marcas + IF tg-free = YES THEN "l," ELSE ",".
...
------------------------------------------------------------------------------*/
    ...
    REPEAT i-ct = 1 TO NUM-DBS:
        RUN szdbu/sum-idle.p (INPUT-OUTPUT TABLE tt-usr) LDBNAME(i-ct).
    END. 
  
    EMPTY TEMP-TABLE tt-usr-connect.

    FOR EACH tt-usr WHERE 
             tt-usr.banco MATCHES "*fnd*" BREAK BY tt-usr.pid.

        IF FIRST-OF(tt-usr.pid) THEN
           ASSIGN l-mata = YES
                  l-idle = YES.

        FIND tt-usr-connect WHERE
             tt-usr-connect.cod-usuario = tt-usr.cod-usuario AND
             tt-usr-connect.maquina = tt-usr.maquina AND 
             tt-usr-connect.pid = tt-usr.pid NO-ERROR.
        ...
        IF NOT AVAIL tt-usr-connect THEN DO.
           ...
           ASSIGN tt-usr-connect.usr = tt-usr.usr
                  ...
                  tt-usr-connect.pid = tt-usr.pid
                  tt-usr-connect.marca = tt-usr.marca
                  tt-usr-connect.login = tt-usr.login.
        END.

        IF tt-usr.tempo < INT(sl-time:INPUT-VALUE IN FRAME {&FRAME-NAME}) THEN
           ASSIGN l-mata = NO.

        IF tt-usr.tempo = 0 THEN
           l-idle = NO.

        IF LAST-OF(tt-usr.pid) THEN DO.
           IF tt-usr-connect.marca <> 's' THEN DO.
              ASSIGN tt-usr-connect.marca = 'r'.

              IF l-idle THEN
                 ASSIGN tt-usr-connect.marca = 'i'.
    ...
    FOR EACH tt-usr-connect.
        IF tt-usr-connect.marca = 'o' THEN DO. /* out */
           RUN pi-disconnect-db (INPUT tt-usr-connect.cod-usuario,
                                 INPUT tt-usr-connect.maquina).
        ...
        ELSE DO.
           /*
           RUN szdbu/sum-vrf.p (INPUT tt-usr-connect.maquina,
                                INPUT tt-usr-connect.pid,
                                OUTPUT l-ok).

           IF NOT l-ok THEN
              ASSIGN tt-usr-connect.marca = 'l'.
            */  
        END.
    END.
