...
* 13/05/2021 - tadeu - mudanáa do calculo da comiss∆o para considerar
* os percentuais de comiss∆o especificos de uma campanha de preáo e 
* para calcular comiss∆o de um segundo repres/vend quando a comiss∆o for dividida
...
necess†rios para o correto calculo da comiss∆o
19/01/2022 - tadeu - acrescimo de procedimento setFormaPagto e acrescimo da condiá∆o
de que a venda com cart∆o de crÇdito n∆o tem bonus de comiss∆o.
(conforme e-mail da Ana Fl†via enviado dia 19/01/2022 as 17:54 )
- acrescimo da include {esbo/boComis.i2}  onde Ç colocado um procedimento
    ...
    FOR EACH ttPedItem:
        ...
        END. 
         ...
         IF round(ttPedItem.vlInf,2) < round(ttPedItem.vlOutlet,2) AND lAbaixoOutlet = NO THEN DO:
           ...
           RUN setMsg IN HBoMsgComis(15,'o item:' +  ttPedItem.ItCodigo + " - ref:" + ttPedItem.codRefer + 
                                     ' tem o preáo informado:' + string(round(ttPedItem.vlInf,2)) +
                                     ' menor que o preáo de outlet:' + STRING(round(ttPedItem.vlOutlet,2)),'aviso').
        END. 
...
PROCEDURE calcPercComis:
    ...
    // caso o parametro de bonus esteja ligado, verifica se a tabela calcula bonus
    /*
     * acrescimo de condicao de que o tipo de pagamento 3(cart∆o de crÇdito) n∆o em bonus de comiss∆o
     * conforme e-mail 19/01/2022 enviado pela Ana Fl†via as 17:24 
    */
    IF  iFormaPagto = 4 THEN DO:
        ASSIGN lBonus = FALSE.
        RUN setMsg IN HBoMsgComis(122,'Forma de Pagamento igual a cart∆o de crÇdito - Bonus setado como FALSE' ,'aviso').   
    END.
    ...
    ELSE DO:
        ...
        IF lPrecoAbaixoOutlet THEN DO:
           RUN setMsg IN HBoMsgComis(30,'Encontrado Preáo informado abaixo do Preáo Outlet','aviso').
           ASSIGN lComisEspCamp = NO.
        END.
        ELSE DO:
           RUN setMsg IN HBoMsgComis(130,'NAO Encontrado Preáo informado abaixo da Preáo Outlet','aviso').
        END.
    ...
    IF logDivideComis THEN DO:
       IF iCodRep <> iCodRepCliente AND 
          ...
          RUN setMsg IN HBoMsgComis(210,'Divide Comiss∆o? SIM' ,'aviso').   
          RUN setMsg IN HBoMsgComis(220,'Repres ' + STRING(iCodRepCliente) + ' do Cliente ' + ' Ç Diferente do Vendedor ' + STRING(iCodRep) + ' ? SIM' ,'aviso').   
          ...
          IF cTipoVendCli = 'interno' AND cTipoVend = 'externo' THEN DO:
             RUN setMsg IN HBoMsgComis(251,'repres cliente interno e vendedor externo - metade da comiss∆o padr∆o de cada tipo de repres.','aviso').   
             ASSIGN percFinalCli = percFinalCli / 2
                    percFinal    = percFinal / 2.
          END.
          ELSE DO:
            RUN setMsg IN HBoMsgComis(251,'comiss∆o divida gual a metade da maior comiss∆o para cada tipo de vendedor','aviso').   
            IF percFinal > percFinalCli THEN
                ASSIGN percDiv = percFinal.  
            ELSE 
                ASSIGN percDiv = percFinalCli.
            RUN setMsg IN HBoMsgComis(250,'% Comiss∆o a ser dividido:' + STRING(percDiv) ,'aviso').   
       ...
       END.
       ELSE DO:                                  
          RUN setMsg IN HBoMsgComis(220,'Repres Cliente Diferente Vendedor? N«O' ,'aviso').   
    ...
    ELSE DO:
       IF iCodRepCliente <> 0 THEN DO.
          IF iCodRep <> iCodRepCliente THEN DO:
             ...
             RUN setMsg IN HBoMsgComis(351,'repres cliente diferente do represetnante do pedido - metade da comiss∆o padr∆o de cada tipo de repres.','aviso').   
             ASSIGN percFinalCli = percFinalCli / 2
                    percFinal    = percFinal / 2.
          END.
          ELSE
       ...
       RUN setMsg IN HBoMsgComis(211,'Divide Comiss∆o? N«O' ,'aviso').   
    END.
    RUN setTransacaoLogCalculo IN hBoMsgComis(iTransacao).
    RUN gravarLogCalculo IN hBoMsgComis(3). //calculo comissao
    RUN expTTmsg IN hboMsgComis(SESSION:TEMP-DIRECTORY + "\log_bo_comis_" + STRING(pedWebId) + "_" + STRING(nrPedido)).
END PROCEDURE.
...
PROCEDURE getPercComisTipoVend:
    ...
    IF lComisEspCamp THEN DO:
       RUN setMsg IN HBoMsgComis(300,'Cumprida as condiá‰es para comiss∆o especial da campanha','aviso').    
       RUN setCodRep IN hBoRepres(pRepres).
    ...
    END.
    ELSE DO :
        RUN setMsg IN HBoMsgComis(300,'NAO Cumprida as condiá‰es para comiss∆o especial da campanha','aviso').    
    END.
