...
PROCEDURE incluir:
    ...
    ASSIGN  {&tabela}.dt_hr_criacao          = NOW
     ...
     /*MESSAGE 
          'apos incluir bolisa13' SKIP
          'etq med:' {&tabela}.num_etiqueta SKIP
          'etq lisa:'  {&tabela}.id_etq_lisa SKIP
...
PROCEDURE validacoes:
   ...
   IF NOT AVAIL {&ttReg} THEN DO:
      ...
      RETURN 'nok'.
   END.
   /*MESSAGE 'id_etq_lisa dentro das validaá‰es da bolisa13' SKIP
             {&ttReg}.id_etq_lisa
       VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
   FIND {&tabela} NO-LOCK
        WHERE {&tabela}.etiqueta_lisa_id = {&ttReg}.etiqueta_lisa_id
        //AND {&ttReg}.id_etq_lisa <> '0'
        AND {&ttReg}.etiqueta_lisa_id > 0
        NO-ERROR.
   CASE pAcao:
       WHEN 'incluir' THEN DO:
            IF {&ttReg}.id_etq_lisa = '' THEN  DO:
                RUN setMsg IN {&boMsg}(40,'Numero da Etiqueta Lisa em Branco','erro').
                RETURN 'nok'.
            END.
            IF AVAIL {&tabela} THEN DO:
            ...
            WHERE {&tabela}.id_etq_lisa = {&ttReg}.id_etq_lisa) THEN DO:
               RUN setMsg IN {&boMsg}(11,'Registro j† existe com o numero da etiqueta Lisa:' + {&ttReg}.id_etq_lisa,'erro' ).
               RETURN 'nok'.
            END.

            IF CAN-FIND( {&tabela} 
            WHERE {&tabela}.cod_estabel = {&ttReg}.cod_estabel AND {&tabela}.num_etiqueta = {&ttReg}.num_etiqueta
            AND {&tabela}.num_etiqueta  > 0) THEN DO:
               RUN setMsg IN {&boMsg}(12,'Registro j† existe para o estabelecimento:' + {&ttReg}.cod_estabel + ' e numero da etiqueta IMA:' + STRING({&ttReg}.num_etiqueta),'erro' ).
               RETURN 'nok'.
            END.
       END.
...
PROCEDURE getErros:
    ...
    RUN getErros IN  {&boMsg}( OUTPUT cErros).
