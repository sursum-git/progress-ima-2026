...
FOR EACH {1}._userIO WHERE
    ...
    FIND tt-usr WHERE
         tt-usr.banco = "{1}" AND
         tt-usr.usr = {1}._userio._userIO-usr NO-ERROR.

    IF NOT AVAIL tt-usr THEN DO.
       CREATE tt-usr.
       ASSIGN tt-usr.banco = "{1}"
              tt-usr.cod-usuario = {1}._userio._UserIO-Name
   ...
   ASSIGN tt-usr.marca = ''.
   FIND {1}._user WHERE 
        {1}._user._userid = STRING(tt-usr.usr) AND
        {1}._user._disabled = YES NO-ERROR.
   IF AVAIL {1}._user THEN
      ASSIGN tt-usr.marca = 'o'.
