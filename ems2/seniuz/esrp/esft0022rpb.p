...
**   Programa: ESFT019.P  =>  ESFT0022RPA.P
**   Autor...: Prodb - Toninho
**   Data....: 16/01/2004
**   Observ..: Idˆntico ao ESFT0022RP.P e ‚ executado quando se selciona qualidade Ambas, quando
**             nÆo verifica a tabela ped-item-ext, que nÆo se aplica a tecidos que nÆo tˆm referˆncia,
**             Criado para listas notas de entrada e devolu‡Æo.
*/
...
DEF VAR de-qtd-ger1 AS DEC FORMAT ">>,>>>,>>9.99".
    ...
    */
    ...
    IF it-nota-fisc.un-fatur[1] <> "m" THEN DO:
       ...
       IF not avail item-ext then do:
          PUT "Nao ha fator de conversao para o Item: " it-nota-fisc.it-codigo
              skip.
          NEXT.
       END.
       ASSIGN de-qtd-conv = it-nota-fisc.qt-faturada[1] * item-ext.fator-conv.
    end.
    else
       assign de-qtd-conv = it-nota-fisc.qt-faturada[1].

    ASSIGN de-qtd-ger1 = de-qtd-ger1 + de-qtd-conv.
...
if de-qtd-ger <> 0 or de-vlr-ger <> 0 THEN DO:
   put "Total Geral:"  at   1
       de-qtd-ger      at  92
       de-vlr-ger      at 119.
   PUT "Total Geral - em M:"  AT  1
       de-qtd-ger1            AT 92.
END.
assign de-qtd-ger  = 0
       de-vlr-ger  = 0
