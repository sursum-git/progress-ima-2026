...
DEFINE TEMP-TABLE tt
    ...
    FIELD rowidSaldo    AS CHAR
    FIELD erroLanc      AS CHAR
...
DEFINE VARIABLE cErroComp       AS CHARACTER   NO-UNDO.
...
DEFINE TEMP-TABLE ttComp
    ...
    FIELD acao       AS CHAR
    FIELD rowidSaldo AS CHAR
    FIELD erroLancto AS CHAR
    INDEX ind-pri idSaldo rowidComp.


INPUT FROM c:\temp\lanctos_ajuste_erp-teste.csv.
    REPEAT:
...
FOR EACH tt
    ...
    FIND LAST saldo-terc NO-LOCK
        ...
        NO-ERROR.
    IF AVAIL saldo-terc THEN DO:
       ASSIGN tt.rowidSaldo = STRING(ROWID(saldo-terc)).
       ...
       FOR EACH componente NO-LOCK 
           ...
           ASSIGN ttComp.rowidComp      = string(ROWID(componente))
                  ...
                  ttComp.rowidSaldo     = string(ROWID(saldo-terc)).
...
//gerar alteraá‰es em lanáamentos j† existentes para ajustes necess†rios do saldo
FOR EACH ttComp
    WHERE ttComp.qtOriginal <> ttComp.qtNova 
    AND ttComp.itCodigo <> ''
    .
    /*MESSAGE ttComp.qtOriginal SKIP
            ttcomp.qtNova     SKIP
           ttComp.qtOriginal - ttcomp.qtNova
        VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
    RUN esapi/alterarMovtoTerceiro.p(TO-ROWID(ttComp.rowidComp),ttComp.qtOriginal - ttcomp.qtNova, OUTPUT cErroComp).
    ASSIGN ttComp.erroLancto = IF cErroComp = '' THEN 'OK' ELSE cErroComp .
END.


FOR EACH tt
    WHERE tt.qtRetorno = 0
    //AND  int(tt.nota) < 4053
    :
    ASSIGN iSerie = iSerie + 1.
    /*MESSAGE 'qt. no tt:' tt.quantidade
        VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
    RUN esapi/incluirMovtoTerceiro.p(TO-ROWID(tt.rowidsaldo),tt.quantidade,02.28.2024,STRING(iSerie),'19207i', OUTPUT cErroComp).
    IF cErroComp = '' THEN
       ASSIGN tt.errolanc = "Lancto Criado com sucesso".
    ELSE 
       ASSIGN tt.ErroLanc = cErroComp.

END.
{esp/exportarTabelaCsv3.i  ttComp " " " " "ttComp_depois_lancto"}    
