...
ASSIGN tt-ped-item.tp-preco = ped-venda.tp-preco
...
FIND ped-item OF ped-venda WHERE 
     ped-item.nr-sequencia = tt-ped-item.nr-sequencia SHARE-LOCK NO-ERROR.

IF ped-item.vl-preori <> ped-item.vl-preuni THEN DO.
   IF ped-item.vl-preuni = 0 THEN 
      ASSIGN ped-item.vl-preuni = tt-ped-item.vl-preori.
END.

FIND ped-venda OF ped-item USE-INDEX ch-pedido SHARE-LOCK NO-ERROR.
