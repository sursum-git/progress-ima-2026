...
*******************************************************************************/
{include/i-prgvrs.i ESPD0003 2.04.00.000}
...
DEF VAR fi-nome-abrev LIKE emitente.nome-abrev.
DEF NEW GLOBAL SHARED VAR v-row-ped-venda AS ROWID.
...
&Scoped-Define ENABLED-OBJECTS BUTTON-3 rt-button 
...
DEFINE VARIABLE h_b01di088 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b01di135 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b01di154 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02di154 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01di159 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01di159 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02di159 AS HANDLE NO-UNDO.
...
DEFINE BUTTON BUTTON-3 
     ...
     LABEL "Button 3" 
     SIZE 4 BY 1.17 TOOLTIP "Permite selecionar intervalo para Consulta de Movimentos".
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Pedidos de Vendas por Cliente"
         HEIGHT             = 16.67
         WIDTH              = 95.29
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 98.29
...
ON END-ERROR OF w-concom /* Pedidos de Vendas por Cliente */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Pedidos de Vendas por Cliente */
DO:
...
DO:
  RUN esdlg/d01di154.w.
  RUN adm-open-query-cases IN h_q01di159.
  RUN pi-ultimo IN h_p-navega.
  RUN adm-open-query-cases IN h_b01di154.
END.
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 2,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.33 , 79.43 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01di159 ).
       RUN set-position IN h_v01di159 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.46 , 94.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Dados|Itens|Notas|Faturado|NÆo Faturado|Reservado' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 8.33 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 9.33 , 94.43 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom/z01di159.w,
                     ProgVaPara = digo/g01di159.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01di159 ).
       RUN set-position IN h_q01di159 ( 1.50 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01di159. */
       RUN add-link IN adm-broker-hdl ( h_q01di159 , 'Record':U , h_v01di159 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01di159. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01di159 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01di159 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01di159 ).

    END. /* Page 0 */

    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02di159 ).
       RUN set-position IN h_v02di159 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.75 , 93.00 ) */

       /* Links to SmartViewer h_v02di159. */
       RUN add-link IN adm-broker-hdl ( h_q01di159 , 'Record':U , h_v02di159 ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01di154 ).
       RUN set-position IN h_b01di154 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.00 , 93.57 ) */

       /* Links to SmartBrowser h_b01di154. */
       RUN add-link IN adm-broker-hdl ( h_q01di159 , 'Record':U , h_b01di154 ).

    END. /* Page 2 */

    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01di135.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01di135 ).
       RUN set-position IN h_b01di135 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.96 , 94.00 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('2') NO-ERROR.

       /* Links to SmartBrowser h_b01di135. */
       RUN add-link IN adm-broker-hdl ( h_b01di154 , 'Record':U , h_b01di135 ).

    END. /* Page 3 */

    WHEN 4 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01di088.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01di088 ).
       RUN set-position IN h_b01di088 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.13 , 87.57 ) */

       /* Initialize other pages that this page requires. */
       RUN init-pages IN THIS-PROCEDURE ('3') NO-ERROR.

       /* Links to SmartBrowser h_b01di088. */
       RUN add-link IN adm-broker-hdl ( h_b01di135 , 'Record':U , h_b01di088 ).

    END. /* Page 4 */

    WHEN 5 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02di154 ).
       RUN set-position IN h_b02di154 ( 9.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 66.00 ) */

       /* Links to BrowseDigitacao h_b02di154. */
       RUN add-link IN adm-broker-hdl ( h_q01di159 , 'Record':U , h_b02di154 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESPD0003" "2.04.00.000"}
