...
*******************************************************************************/
{include/i-prgvrs.i G01ES057 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS mp-entr-cam.placa mp-entr-cam.dt-entrada 
&Scoped-define ENABLED-TABLES mp-entr-cam
&Scoped-define FIRST-ENABLED-TABLE mp-entr-cam
&Scoped-Define ENABLED-OBJECTS fi-hr-entrada bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-FIELDS mp-entr-cam.placa mp-entr-cam.dt-entrada 
&Scoped-define DISPLAYED-TABLES mp-entr-cam
&Scoped-define FIRST-DISPLAYED-TABLE mp-entr-cam
&Scoped-Define DISPLAYED-OBJECTS fi-hr-entrada 
...
DEFINE FRAME d-vapara
     mp-entr-cam.placa AT ROW 1.5 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8.86 BY .88
     mp-entr-cam.dt-entrada AT ROW 2.5 COL 22 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 8.86 BY .88
     fi-hr-entrada AT ROW 3.5 COL 22 COLON-ALIGNED
     bt-ok AT ROW 4.92 COL 2.14
     bt-cancela AT ROW 4.92 COL 13
     bt-ajuda AT ROW 4.92 COL 58
     rt-button AT ROW 4.67 COL 1
     SPACE(0.13) SKIP(0.07)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V† Para Entrada de Caminh∆o"
...
ON GO OF FRAME d-vapara /* V† Para Entrada de Caminh∆o */
DO:
  RUN esapi/cv-hora.p (INPUT STRING(INPUT FRAME {&FRAME-NAME} fi-hr-entrada,"xx:xx:xx"), OUTPUT i-hr-entrada).
  find mp-entr-cam no-lock 
      where mp-entr-cam.placa      = input frame {&frame-name} mp-entr-cam.placa 
        AND mp-entr-cam.dt-entrada = INPUT FRAME {&frame-name} mp-entr-cam.dt-entrada 
        AND mp-entr-cam.hr-entrada = i-hr-entrada no-error.
  if  not avail mp-entr-cam then do:
      {utp/ut-table.i espec mp-entr-cam 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mp-entr-cam).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Entrada de Caminh∆o */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-hr-entrada 
      WITH FRAME d-vapara.
  IF AVAILABLE mp-entr-cam THEN 
    DISPLAY mp-entr-cam.placa mp-entr-cam.dt-entrada 
      WITH FRAME d-vapara.
  ENABLE mp-entr-cam.placa mp-entr-cam.dt-entrada fi-hr-entrada bt-ok 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ES057" "2.04.00.000"}
