...
*******************************************************************************/
{include/i-prgvrs.i ESFT0025 2.04.00.000}
...
DEFINE VARIABLE fi-composicoes AS CHARACTER FORMAT "X(256)":U 
     LABEL "Composiá‰es" 
     VIEW-AS FILL-IN 
     SIZE 45 BY .79 TOOLTIP "C¢digos de Composiá∆o a serem considerados." NO-UNDO.
...
DEFINE VARIABLE fi-fin-serie AS CHARACTER FORMAT "x(5)" INITIAL "ZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 6.57 BY .79 TOOLTIP "SÇrie da nota fiscal final." NO-UNDO.
...
DEFINE VARIABLE fi-ini-serie AS CHARACTER FORMAT "x(5)" INITIAL "U" 
     LABEL "SÇrie":R7 
     VIEW-AS FILL-IN 
     SIZE 6.57 BY .88 TOOLTIP "SÇrie da nota fiscal inicial." NO-UNDO.
...
DEFINE FRAME f-pg-sel
     ...
     fi-fin-cod-estabel AT ROW 1.5 COL 42.29 COLON-ALIGNED NO-LABEL
     fi-ini-serie AT ROW 2.5 COL 19 COLON-ALIGNED HELP
          "SÇrie da nota fiscal"
     fi-fin-serie AT ROW 2.5 COL 42.29 COLON-ALIGNED HELP
          "SÇrie da nota fiscal" NO-LABEL
     fi-ini-dt-emissao AT ROW 3.5 COL 19 COLON-ALIGNED
     fi-fin-dt-emissao AT ROW 3.5 COL 42.29 COLON-ALIGNED NO-LABEL
     IMAGE-21 AT ROW 1.5 COL 31.14
     IMAGE-22 AT ROW 1.5 COL 40.86
     IMAGE-3 AT ROW 3.5 COL 31.14
     IMAGE-33 AT ROW 2.5 COL 31.14
     IMAGE-34 AT ROW 2.5 COL 40.86
     IMAGE-4 AT ROW 3.5 COL 40.86
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
DEFINE FRAME f-pg-par
     fi-arq-saida AT ROW 3 COL 18.29 COLON-ALIGNED
     bt-arq-sai AT ROW 3 COL 65.57 HELP
          "Arquivo de pagamento de sa°da (ajuntado)."
     fi-composicoes AT ROW 4 COL 18.29 COLON-ALIGNED
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Exportaá∆o de NF com 50% de Algod∆o"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 28.63
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-relat /* Exportaá∆o de NF com 50% de Algod∆o */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Exportaá∆o de NF com 50% de Algod∆o */
DO:
...
DO:
    ...
    IF OKpressed = TRUE THEN DO:
       IF fi-arq-saida <> "" AND INDEX(fi-arq-saida,".") = 0 THEN
          ASSIGN fi-arq-saida:SCREEN-VALUE = fi-arq-saida + ".csv".
       ELSE
...
DO:
   IF INDEX(fi-arq-saida,".csv") = 0 THEN DO:
      MESSAGE "Arquivo de sa°da deve terminar em .csv."
...
DO:
  ASSIGN fi-arq-saida:SCREEN-VALUE IN FRAME f-pg-par = SESSION:TEMP-DIRECTORY + "notas50%alg.csv".
END.
...
DO:
   IF INDEX(INPUT FRAME {&FRAME-NAME} fi-arq-saida,".csv") = 0 THEN DO:
      MESSAGE "Arquivo de sa°da deve terminar em .csv."
          VIEW-AS ALERT-BOX INFO BUTTONS OK.
      APPLY 'entry' TO fi-arq-saida IN FRAME {&FRAME-NAME}.
      RETURN NO-APPLY.
...
{utp/ut9000.i "ESFT0025" "2.04.00.000"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN fi-ini-cod-estabel:SCREEN-VALUE IN FRAME f-pg-sel = "2"
           ...
           fi-arq-saida:SCREEN-VALUE IN FRAME f-pg-par = SESSION:TEMP-DIRECTORY + "notas50%alg.csv"
           fi-composicoes:SCREEN-VALUE IN FRAME f-pg-par = "05,12,18,21,30,40,43,48,50,53,57,59,60,61,62,63,64,66,70,75,83".
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-ini-cod-estabel fi-fin-cod-estabel fi-ini-serie fi-fin-serie 
          fi-ini-dt-emissao fi-fin-dt-emissao 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE fi-ini-cod-estabel fi-fin-cod-estabel fi-ini-serie fi-fin-serie 
         fi-ini-dt-emissao fi-fin-dt-emissao IMAGE-21 IMAGE-22 IMAGE-3 IMAGE-33 
         IMAGE-34 IMAGE-4 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ...
  DISPLAY fi-arq-saida fi-composicoes to-imp-param 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE fi-arq-saida bt-arq-sai fi-composicoes to-imp-param RECT-21 RECT-32 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.composicoes     = INPUT FRAME f-pg-par fi-composicoes
    ...
    MESSAGE "Atená∆o! - O arquivo ser† gerado no formato excel, separado por ';'"
            VIEW-AS ALERT-BOX.

    {include/i-rprun.i esrp/esft0025rp.p}
