...
*******************************************************************************/
{include/i-prgvrs.i G01ES066 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS fi-dt-mistura bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-OBJECTS fi-dt-mistura 
...
DEFINE FRAME d-vapara
     fi-dt-mistura AT ROW 2 COL 14 COLON-ALIGNED
...
ON GO OF FRAME d-vapara /* V  Para Mistura */
DO:
  find first mp-mistura no-lock 
      where mp-mistura.dt-mistura = input frame {&frame-name} fi-dt-mistura no-error.
  if  not avail mp-mistura then do:
      {utp/ut-table.i espec mp-mistura 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mp-mistura).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Mistura */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-dt-mistura 
      WITH FRAME d-vapara.
  ENABLE fi-dt-mistura bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
