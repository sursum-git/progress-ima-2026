...
&Scoped-Define ENABLED-OBJECTS br-ped-item cb-tp-pedido fi-cod-estabel ~
...
cb-prioridade fi-nome-transp fi-nome-tr-red bt-log fi-cod-rota cb-tp-frete ~
fi-data-base ed-obs tg-bloqueio fi-observ-nf bt-inc bt-sair ~
bt-cons-cliente-tri RECT-1 RECT-2 RECT-3 RECT-4 
&Scoped-Define DISPLAYED-OBJECTS tg-nao-aprovar fi-nome-estabel ~
fi-dt-implant cb-tp-pedido fi-cod-estabel cb-ext-tp-pedido fi-reserva ~
fi-cliente fi-pedido-cli fi-cliente-tri fi-sit-cred fi-fat-max fi-cod-rep ~
...
cb-tipo-pagto cb-prioridade fi-nome-transp fi-nome-tr-red fi-cod-rota ~
...
bt-copia-it-ped fi-observ-nf bt-inc bt-can bt-bloq 
...
DEFINE FRAME F-Main
     ...
     cb-prioridade AT ROW 8.38 COL 86 COLON-ALIGNED WIDGET-ID 12
...
------------------------------------------------------------------------------*/
  DISPLAY tg-nao-aprovar fi-nome-estabel fi-dt-implant cb-tp-pedido 
          fi-cod-estabel cb-ext-tp-pedido fi-reserva fi-cliente fi-pedido-cli 
          fi-cliente-tri fi-sit-cred fi-fat-max fi-cod-rep fi-nome-rep 
          fi-ped-repres cb-preposto fi-natur-oper fi-denominacao fi-perc-comis 
          fi-moeda fi-desc-moeda fi-cod-cond-pag fi-nr-pedido fi-desc-cond-pag 
          cb-tp-preco cb-tab-preco cb-tp-entrega fi-dt-entrega cb-tipo-pagto 
          cb-prioridade fi-nome-transp fi-nome-tr-red fi-cod-rota fi-desc-rota 
  ...
  ENABLE br-ped-item cb-tp-pedido fi-cod-estabel fi-cliente fi-pedido-cli 
         fi-fat-max fi-cod-rep fi-ped-repres fi-natur-oper fi-perc-comis 
         fi-cod-cond-pag fi-nr-pedido cb-tab-preco cb-tp-entrega cb-prioridade 
...
------------------------------------------------------------------------------*/
   IF AVAIL ped-venda THEN DO.
      ...
      IF AVAIL ped-repres THEN
         ASSIGN fi-perc-comis:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(ped-repres.perc-comis).
...
------------------------------------------------------------------------------*/
    ...
    IF INPUT FRAME {&FRAME-NAME} cb-ext-tp-pedido = 'PI' THEN DO.
       FIND tt-itens-ped WHERE 
            tt-itens-ped.it-codigo = INPUT BROWSE {&browse-name} tt-itens-ped.it-codigo AND
            tt-itens-ped.cod-refer = INPUT BROWSE {&browse-name} tt-itens-ped.cod-refer
            NO-ERROR.
       IF NOT AVAIL tt-itens-ped THEN DO:
          IF de-qtidade-atu < INPUT BROWSE {&browse-name} tt-itens-ped.qt-pedida  THEN DO.
...
------------------------------------------------------------------------------*/
    ...
    ELSE DO.
       ...
       FOR EACH ped-venda WHERE
                ...
                ped-venda.tp-pedido = 'PI' NO-LOCK,
           ...
           IF ped-venda.dt-entrega > TODAY THEN NEXT.

           ASSIGN de-qtidade-atu = de-qtidade-atu - ped-item.qt-pedida.
       END.

       /* Subtrai Pedidos PE Incompletos evitando que a quantidade fique dispon°vel para
          venda, em alguma eventual manutená∆o */
       FOR EACH ped-venda WHERE
                ped-venda.cod-sit-ped = 1 AND
                ped-venda.tp-pedido = 'PE' AND 
                ped-venda.completo = NO NO-LOCK,
           EACH ped-item OF ped-venda WHERE
                ped-item.cod-sit-item = 1 AND
                ped-item.it-codigo = ITEM.it-codigo AND
                ped-item.cod-refer = INPUT BROWSE {&BROWSE-NAME} tt-itens-ped.cod-refer
...
------------------------------------------------------------------------------*/
   IF INPUT FRAME {&FRAME-NAME} cb-ext-tp-pedido = 'PI' THEN DO.
      FOR EACH tt-itens-ped WHERE
               tt-itens-ped.it-codigo = tt-itens-ped.it-codigo:SCREEN-VALUE IN BROWSE {&browse-name}  NO-LOCK.
          CREATE tt-ref-item.
         ...
         FOR EACH tt-ref-item WHERE
             ...
             ASSIGN tt-itens-ped.tp-acao = 'Incluir'
                    tt-itens-ped.nr-sequencia = IF AVAIL b-itens-ped
                                                ...
                                                ELSE 10
                    tt-itens-ped.it-codigo = tt-itens-ped.it-codigo:SCREEN-VALUE IN BROWSE {&browse-name}
             ...
             FOR EACH pp-it-container WHERE
                      pp-it-container.it-comprado = tt-itens-ped.it-codigo AND
                      pp-it-container.ref-comprada = tt-itens-ped.cod-refer NO-LOCK.
