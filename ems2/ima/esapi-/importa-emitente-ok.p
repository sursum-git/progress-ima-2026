...
DEFINE VAR l-alteracao         AS LOGICAL INITIAL NO.
DEFINE VAR c-results           AS CHAR.
...
FOR EACH tt-aux.
    ...
    IF AVAIL emitente THEN DO.    
       ...
       BUFFER-COMPARE emitente TO tt-emitente SAVE RESULT IN c-results.
       IF c-results <> '' THEN DO.
          ASSIGN l-alteracao = YES.
          BUFFER-COPY tt-emitente EXCEPT cod-emitente nome-abrev TO emitente.
          OUTPUT TO VALUE(c-arq-log) APPEND.
              PUT UNFORMATTED "Emitente Copiado " AT 7
                  NOW
                  SKIP.
          OUTPUT CLOSE.
       END.
    END.
        ...
        ASSIGN tt_emitente_integr_new.Cod_versao_integracao	    =	01
               ...
               tt_emitente_integr_new.Ep_codigo	                =	param-global.empresa-prin
               ...
               tt_emitente_integr_new.Flag_pag                  =   IF tt-emitente.flag-pag = 1
                                                                    THEN NO ELSE YES
        ...
        IF AVAIL tt_retorno_clien_fornec THEN DO.
           OUTPUT TO VALUE(c-arq-log) APPEND.
              FOR EACH tt_retorno_clien_fornec:
                  PUT UNFORMATTED "ALERTA na Integra‡Æo EMS 5.00" AT 7
                      NOW
                      SKIP.
    
                  PUT tt_retorno_clien_fornec.ttv_des_mensagem AT 10
                      SKIP.
              END.
           OUTPUT CLOSE.
        END.
