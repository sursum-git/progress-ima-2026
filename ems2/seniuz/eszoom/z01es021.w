...
*******************************************************************************/
{include/i-prgvrs.i Z01ES021 2.04.00.000}
...
DEFINE VARIABLE h_b01es021 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es021 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 13.29 COL 1.57
     bt-ok AT ROW 15 COL 3
     bt-cancela AT ROW 15 COL 14
     bt-ajuda AT ROW 15 COL 79
     rt-button AT ROW 14.75 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Movimento Produtos Acabados"
         HEIGHT             = 15.33
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Movimento Produtos Acabados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Movimento Produtos Acabados */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Por Data|Por Lote' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 1.17 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.88 , 88.57 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es021.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es021 ).
       RUN set-position IN h_b01es021 ( 2.58 , 2.29 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 87.00 ) */

       /* Links to SmartBrowser h_b01es021. */
       RUN add-link IN adm-broker-hdl ( h_b01es021 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es021 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es021.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es021 ).
       RUN set-position IN h_b02es021 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 87.00 ) */

       /* Links to SmartBrowser h_b02es021. */
       RUN add-link IN adm-broker-hdl ( h_b02es021 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02es021 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01ES021" "2.04.00.000"}
