...
&Scoped-Define ENABLED-OBJECTS fi-it-codigo fi-cod-refer fi-lote bt-ajuda ~
bt-ok bt-cancelar RECT-1 
...
DEFINE VARIABLE fi-cod-refer AS CHARACTER FORMAT "X(7)":U 
     LABEL "Referància" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fi-lote AS CHARACTER FORMAT "X(2)":U INITIAL "RP" 
     LABEL "Lote" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME F-Main
     fi-it-codigo AT ROW 3 COL 21 COLON-ALIGNED
     fi-desc-item AT ROW 3 COL 34 COLON-ALIGNED NO-LABEL
     fi-cod-refer AT ROW 4 COL 21 COLON-ALIGNED
     fi-lote AT ROW 5 COL 21 COLON-ALIGNED
     bt-ajuda AT ROW 7.42 COL 70.14
     bt-ok AT ROW 7.46 COL 2.14
     bt-cancelar AT ROW 7.46 COL 13.14
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _XFTR "SmartWindowCues" w-digita _INLINE
...
DO:
    ASSIGN c-arquivo = "m:\inv-" + INPUT FRAME {&FRAME-NAME} fi-it-codigo + ".csv".
    OUTPUT TO VALUE(c-arquivo).
    FOR EACH inv-acab WHERE
             inv-acab.it-codigo = INPUT FRAME {&FRAME-NAME} fi-it-codigo AND
             inv-acab.cod-refer = INPUT FRAME {&FRAME-NAME} fi-cod-refer and
             inv-acab.lote BEGINS INPUT FRAME {&FRAME-NAME} fi-lote NO-LOCK.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-it-codigo w-digita
ON LEAVE OF fi-it-codigo IN FRAME F-Main /* Item */
DO:
  FIND ITEM WHERE ITEM.it-codigo = SELF:INPUT-VALUE NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-it-codigo fi-desc-item fi-cod-refer fi-lote 
      WITH FRAME F-Main IN WINDOW w-digita.
  ENABLE fi-it-codigo fi-cod-refer fi-lote bt-ajuda bt-ok bt-cancelar RECT-1 
      WITH FRAME F-Main IN WINDOW w-digita.
