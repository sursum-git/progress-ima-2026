...
{include/i-prgvrs.i ESSP0057 2.04.00.000}
...
DEFINE VARIABLE h_b01es020 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadsim AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01es021 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es021 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02es021 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadcom ASSIGN
         HIDDEN             = YES
         TITLE              = "Movimento Estoque Produtos Acabados"
         HEIGHT             = 17.5
         WIDTH              = 90
         MAX-HEIGHT         = 28.13
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadcom /* Movimento Estoque Produtos Acabados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadcom w-cadcom
ON WINDOW-CLOSE OF w-cadcom /* Movimento Estoque Produtos Acabados */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es021.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es021 ).
       RUN set-position IN h_v01es021 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Defeitos|Nuances' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 10.00 , 2.14 ) NO-ERROR.
       RUN set-size IN h_folder ( 8.50 , 87.86 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es021.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es021.w,
                     ProgVaPara = esgo/g01es021.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es021 ).
       RUN set-position IN h_q01es021 ( 1.25 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es021. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es021 ).
       RUN add-link IN adm-broker-hdl ( h_q01es021 , 'Record':U , h_v01es021 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01es021. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es021 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es021 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es021 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es021 ,
             h_p-exihel , 'AFTER':U ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es020.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es020 ).
       RUN set-position IN h_b01es020 ( 11.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.88 , 86.00 ) */

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es020 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02es021.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02es021 ).
       RUN set-position IN h_v02es021 ( 11.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 6.75 , 86.00 ) */

       /* Links to SmartViewer h_v02es021. */
       RUN add-link IN adm-broker-hdl ( h_q01es021 , 'Record':U , h_v02es021 ).
       RUN add-link IN adm-broker-hdl ( h_v01es021 , 'group-assign':U , h_v02es021 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02es021 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE local-create-objects w-cadcom 
...
------------------------------------------------------------------------------*/
  ...
  RUN pi-obtem-handle IN h_v01es021 (INPUT THIS-PROCEDURE:HANDLE,
                                     INPUT h_b01es020).

  RUN pi-obtem-handle IN h_b01es020 (INPUT THIS-PROCEDURE:HANDLE,
                                     INPUT h_v01es021).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0057" "2.04.00.000"}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-select-page w-cadcom 
PROCEDURE pi-select-page :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
  DEF INPUT PARAMETER i-pagina AS INT.
  RUN select-page IN THIS-PROCEDURE ( i-pagina ).
END PROCEDURE.

/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME

&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-trata-folder w-cadcom 
PROCEDURE pi-trata-folder :
/*------------------------------------------------------------------------------
  Purpose:     
  Parameters:  <none>
  Notes:       
------------------------------------------------------------------------------*/
    DEF INPUT PARAMETER funcao   AS CHAR.
    DEF INPUT PARAMETER i-pagina AS INT.

    IF VALID-HANDLE(h_folder) THEN
       IF funcao = "D" THEN
          RUN disable-folder-page IN h_folder (INPUT i-pagina).
       ELSE
          RUN enable-folder-page IN h_folder (INPUT i-pagina).
END PROCEDURE.
