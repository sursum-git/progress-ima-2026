...
*******************************************************************************/
{include/i-prgvrs.i B02AD098 2.04.00.000}
...
&Scoped-define FIELDS-IN-QUERY-br-table emitente.nome-abrev ~
emitente.cod-emitente emitente.cgc emitente.nome-emit 
...
&Scoped-define QUERY-STRING-br-table FOR EACH emitente WHERE ~{&KEY-PHRASE} ~
 ...
 AND emitente.nome-abrev >= fi-ini-nome-abrev ~
 AND emitente.nome-abrev <= fi-fin-nome-abrev NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS bt-confirma fi-ini-nome-abrev ~
fi-fin-nome-abrev br-table IMAGE-1 IMAGE-2 
&Scoped-Define DISPLAYED-OBJECTS fi-ini-nome-abrev fi-fin-nome-abrev 
...
DEFINE VARIABLE fi-fin-nome-abrev AS CHARACTER FORMAT "X(12)" INITIAL "ZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 13 BY .88 TOOLTIP "Nome abreviado do cliente final."
     FONT 1 NO-UNDO.

DEFINE VARIABLE fi-ini-nome-abrev AS CHARACTER FORMAT "X(12)" 
     VIEW-AS FILL-IN 
     SIZE 13 BY .88 TOOLTIP "Nome abreviado do cliente inicial."
     FONT 1 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-table B-table-Win _STRUCTURED
  QUERY br-table NO-LOCK DISPLAY
      emitente.nome-abrev FORMAT "X(12)":U WIDTH 17.43
      emitente.cod-emitente FORMAT ">>>>>>>>9":U
      emitente.cgc FORMAT "x(19)":U
      emitente.nome-emit FORMAT "X(40)":U WIDTH 39.43
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
DEFINE FRAME F-Main
     bt-confirma AT ROW 1.13 COL 82.72
     fi-ini-nome-abrev AT ROW 1.17 COL 2.43 COLON-ALIGNED HELP
          "Nome abreviado do cliente/fornecedor" NO-LABEL
     fi-fin-nome-abrev AT ROW 1.17 COL 42.43 COLON-ALIGNED HELP
          "Nome abreviado do cliente/fornecedor" NO-LABEL
     br-table AT ROW 2.17 COL 1
     IMAGE-1 AT ROW 1.17 COL 17.72
...
/* BROWSE-TAB br-table fi-fin-nome-abrev F-Main */
ASSIGN 
...
/* Query rebuild information for BROWSE br-table
 ...
 AND mgcad.emitente.nome-abrev >= fi-ini-nome-abrev
 AND mgcad.emitente.nome-abrev <= fi-fin-nome-abrev"
     _FldNameList[1]   > mgcad.emitente.nome-abrev
"emitente.nome-abrev" ? ? "character" ? ? ? ? ? ? no ? no no "17.43" yes no no "U" "" ""
     _FldNameList[2]   = mgcad.emitente.cod-emitente
     _FldNameList[3]   = mgcad.emitente.cgc
     _FldNameList[4]   > mgcad.emitente.nome-emit
"emitente.nome-emit" ? ? "character" ? ? ? ? ? ? no ? no no "39.43" yes no no "U" "" ""
...
DO:
  assign input frame {&frame-name} fi-ini-nome-abrev fi-fin-nome-abrev.
