...
REPEAT iDB = 1 TO NUM-DBS:
    ...
    hQueryMD:QUERY-PREPARE('for each ' + cBanco + '._file no-lock where _file-name ="' + fiTabela:SCREEN-VALUE IN FRAME {&FRAME-NAME} + '" , each _field of _file no-lock ').
    ...
    REPEAT:
      ...
      IF(bhField:BUFFER-FIELD('_extent'):BUFFER-VALUE() > 0) THEN DO:
        DO i = 1 TO bhfield:EXTENT:
           CREATE ttMD.
           ASSIGN ttMD.labelCampo = bhField:BUFFER-FIELD('_label'):BUFFER-VALUE()
                  ttMD.nomeCampo  = bhField:BUFFER-FIELD('_field-name'):BUFFER-VALUE() + "-" + bhField:BUFFER-FIELD('_extent'):BUFFER-VALUE()
                  ttMD.tipo       = bhField:BUFFER-FIELD('_data-type'):BUFFER-VALUE() 
                  ttMD.formato    = bhField:BUFFER-FIELD('_format'):BUFFER-VALUE()
                  ttMD.iExtent    = bhField:BUFFER-FIELD('_extent'):BUFFER-VALUE() .
        END.
      END.
      ELSE DO :
         CREATE ttMD.
...
REPEAT:
    ...
    FOR EACH ttMD:
        IF ttMD.iExtent > 0 THEN
           ASSIGN ttMD.valor = bhTabelas[1]:BUFFER-FIELD(ttMD.nomeCampo):BUFFER-VALUE(ttMD.iExtent).
        ELSE
           ASSIGN ttMD.valor = bhTabelas[1]:BUFFER-FIELD(ttMD.nomeCampo):BUFFER-VALUE(). 
    END.
END.
hQuery:QUERY-CLOSE().
...
OUTPUT TO c:\temp\ttmd.txt NO-CONVERT APPEND .
FOR EACH ttMD:
