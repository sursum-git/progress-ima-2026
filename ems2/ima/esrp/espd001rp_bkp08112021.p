...
DEF VAR lEnviarEmail       AS LOGICAL     NO-UNDO.
...
ASSIGN cPastaLog = IF c-base = 'base-pro' 
                   THEN 't:\especificos\logs'
...
PROCEDURE pi-integrarPedidos:
    ...
    FOR EACH peds_web WHERE
        ...
        IF NOT AVAIL repres THEN DO: 
           RUN incluirLogPed('validarRepresPed','Representante NAO encontrado:' + STRING(i-cod-rep) ,'ERRO').
           RUN finalizarLogPed.
           NEXT.
        END.
        ELSE DO:
           RUN incluirLogPed('validarRepresPed','Representante econtrado:' + string(i-cod-rep)  ,'INF').
        END.
        ...
        IF i-cod-vend <> 0 THEN DO.
           ...
           IF NOT AVAIL b-repres THEN DO: 
              RUN incluirLogPed('validarRepresPed','Vendedor NAO encontrado:' + string(i-cod-vend) ,'ERRO').
              RUN finalizarLogPed.
              NEXT.
           END.
           ELSE DO:
              RUN incluirLogPed('validarRepresPed','Vendedor econtrado:' + string(i-cod-vend)  ,'INF').
           END.
        END.
        ...
        */
        ...
        FOR EACH itens_ped_web WHERE
            ...
            /*
            ASSIGN ped-item-ext.cod_controle_preco = itens_ped_web.cod_controle_preco.

            IF INTEGER(itens_ped_web.num_id_liquida_ima) <> 0 THEN
               ASSIGN ped-item-ext.liquida-ima = YES
                      ped-item-ext.num-id-liquida-ima = itens_ped_web.num_id_liquida_ima.
            */
...
PROCEDURE pi-cria-repres.
    ...
    IF AVAIL b-repres THEN DO.
       ...
       IF NOT AVAIL ped-repre THEN DO.
          RUN incluirLogPed ( 'pi-cri-repres','Nío encontrou registro do Vendedor na ped-repre, vai tentar criar a comissío - % portal:' + 
                               STRING(de-perc-comis-ven) + '-pedido web:' + STRING(peds_web.ped_web_id) ).
