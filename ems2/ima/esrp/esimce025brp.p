DEF SHARED TEMP-TABLE tt-saldo-estoq
...
DEF VAR c-estab-aux LIKE movadm.ped-venda.cod-estabel.
...
{utp/ut-liter.i Calculando_Saldo_Disponivel *}
RUN pi-inicializar IN h-acomp (INPUT RETURN-VALUE).

FOR EACH movadm.saldo-estoq WHERE
         movadm.saldo-estoq.cod-estabel = movadm.para-ped.estab-padrao AND
         movadm.saldo-estoq.cod-refer   >= var-glb-refer-ini AND 
         movadm.saldo-estoq.cod-refer   <= var-glb-refer-fim AND 
         movadm.saldo-estoq.cod-depos   >= var-glb-cod-depos-ini AND 
         movadm.saldo-estoq.cod-depos   <= var-glb-cod-depos-fim AND 
         movadm.saldo-estoq.qtidade-atu <> 0 NO-LOCK,
    FIRST movadm.ITEM WHERE
          movadm.ITEM.it-codigo = movadm.saldo-estoq.it-codigo AND
          movadm.ITEM.ge-codigo >= var-glb-ge-ini AND
          movadm.ITEM.ge-codigo <= var-glb-ge-fim NO-LOCK
          BY movadm.saldo-estoq.it-codigo.
   
    IF movadm.saldo-estoq.cod-refer <> movadm.saldo-estoq.lote THEN NEXT.
   
    RUN pi-acompanhar IN h-acomp(INPUT 'Item: ' + movadm.saldo-estoq.it-codigo + "   " +
                                       'Ref: ' + movadm.saldo-estoq.cod-refer).
...
{utp/ut-liter.i Verificando_Pedidos_Abertos *}
RUN pi-inicializar IN h-acomp (INPUT RETURN-VALUE).

/* Subtrai Pedidos em Aberto da IMA */
FOR EACH movadm.ped-venda WHERE
         movadm.ped-venda.cod-sit-ped = 1 NO-LOCK,
    EACH movadm.ped-item OF movadm.ped-venda WHERE
         movadm.ped-item.cod-sit-item = 1 NO-LOCK.
    /*
    IF movadm.ped-venda.tp-pedido = 'PI' AND
       movadm.ped-venda.dt-entrega > TODAY THEN NEXT.
    */
    IF movadm.ped-venda.tp-pedido = 'PI' THEN NEXT.

    RUN pi-acompanhar IN h-acomp(INPUT 'Pedido: ' + movadm.ped-item.nr-pedcli + "   " +
                                       'Item: ' + movadm.ped-item.it-codigo   + "   " +
                                       'Ref: ' + movadm.ped-item.cod-refer).
    ...
    FIND tt-saldo-estoq WHERE 
         ...
         tt-saldo-estoq.cod-estabel = movadm.ped-venda.cod-estabel AND 
    ...
    IF movadm.ped-venda.tp-pedido = 'PI' THEN
       ASSIGN tt-saldo-estoq.qt-aloc-pi = tt-saldo-estoq.qt-aloc-pi + movadm.ped-item.qt-pedida.
    ELSE
...
{utp/ut-liter.i Verificando_Notas_Pendentes *}
RUN pi-inicializar IN h-acomp (INPUT RETURN-VALUE).

/* Subtrai Notas em Aberto da IMA */ 
FOR EACH movadm.nota-fiscal WHERE
    ...
    EACH movadm.it-nota-fisc OF nota-fiscal  NO-LOCK.
    ...
    FIND tt-saldo-estoq WHERE 
         ...
         tt-saldo-estoq.cod-estabel = movadm.nota-fiscal.cod-estabel AND 
         tt-saldo-estoq.cod-depos   = 'ARM' NO-ERROR.
    IF NOT avail tt-saldo-estoq THEN DO.
       FIND mgadm.empresa WHERE 
            mgadm.empresa.ep-codigo = fnd_usuar_univ.cod_empresa NO-LOCK NO-ERROR.
...
IF var-glb-estab = 9 THEN DO:
   ...
   FOR EACH dbaux.saldo-estoq WHERE 
            ...
            dbaux.saldo-estoq.cod-depos   <= var-glb-cod-depos-fim AND
            dbaux.saldo-estoq.qtidade-atu <> 0 NO-LOCK,
       FIRST dbaux.ITEM WHERE
             dbaux.ITEM.it-codigo = dbaux.saldo-estoq.it-codigo AND
             dbaux.ITEM.ge-codigo >= var-glb-ge-ini AND
             dbaux.ITEM.ge-codigo <= var-glb-ge-fim NO-LOCK.

       IF dbaux.saldo-estoq.cod-refer <> dbaux.saldo-estoq.lote THEN NEXT.

       RUN pi-acompanhar IN h-acomp(INPUT 'Item: ' + dbaux.saldo-estoq.it-codigo + "   " +
                                          'Ref: ' + dbaux.saldo-estoq.cod-refer).
   ...
   FOR EACH dbaux.ped-venda WHERE
            dbaux.ped-venda.cod-sit-ped = 1 NO-LOCK,
       EACH dbaux.ped-item OF dbaux.ped-venda WHERE
            dbaux.ped-item.cod-sit-item = 1 NO-LOCK.
       /*
       IF dbaux.ped-venda.tp-pedido = 'PI' AND
          dbaux.ped-venda.dt-entrega > TODAY THEN NEXT.
       */
       IF dbaux.ped-venda.tp-pedido = 'PI' THEN NEXT.

       RUN pi-acompanhar IN h-acomp(INPUT 'Pedido: ' + dbaux.ped-item.nr-pedcli + "   " +
                                          'Item: ' + dbaux.ped-item.it-codigo   + "   " +
                                          'Ref: ' + dbaux.ped-item.cod-refer).
       ...
       IF dbaux.ped-venda.tp-pedido = 'PI' THEN
          ASSIGN tt-saldo-estoq.qt-aloc-pi = tt-saldo-estoq.qt-aloc-pi + dbaux.ped-item.qt-pedida.
       ELSE
   ...
   FOR EACH dbaux.nota-fiscal WHERE
       ...
       EACH dbaux.it-nota-fisc OF nota-fiscal  NO-LOCK.
       ...
       IF NOT avail tt-saldo-estoq THEN DO.
          ...
          CREATE tt-saldo-estoq.
          ASSIGN tt-saldo-estoq.cod-estabel = dbaux.nota-fiscal.cod-estabel
                 tt-saldo-estoq.empresa     = mgadm.empresa.nome
