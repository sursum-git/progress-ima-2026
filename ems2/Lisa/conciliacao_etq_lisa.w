...
*******************************************************************************/
{include/i-prgvrs.i conciliacao_etq_lisa 9.99.99.999}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i conciliacao_etq_lisa cep}
&ENDIF
...
DEFINE VARIABLE tgApenasEstoque AS LOGICAL INITIAL yes 
     LABEL "Apenas Em Estoque?" 
     VIEW-AS TOGGLE-BOX
     SIZE 18 BY .83 NO-UNDO.

DEFINE VARIABLE tgAtu AS LOGICAL INITIAL no 
     LABEL "Atualizar informaá‰es Etiquetas" 
     VIEW-AS TOGGLE-BOX
     SIZE 29 BY .83 NO-UNDO.

DEFINE VARIABLE tgConciliacao AS LOGICAL INITIAL yes 
     LABEL "Conciliar?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-par
     ed-ajuda AT ROW 1.5 COL 40 NO-LABEL WIDGET-ID 12
     fiItCodigo AT ROW 3 COL 21 COLON-ALIGNED WIDGET-ID 4
     fiCodRefer AT ROW 4.25 COL 21 COLON-ALIGNED WIDGET-ID 6
     tgAtu AT ROW 5.5 COL 9 WIDGET-ID 14
     tgConciliacao AT ROW 6.42 COL 9 WIDGET-ID 16
     tgApenasEstoque AT ROW 7.25 COL 9 WIDGET-ID 20
     tgGerarLog AT ROW 10 COL 40 WIDGET-ID 2
     RECT-12 AT ROW 1.58 COL 3 WIDGET-ID 18
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Conciliaá∆o Etiquetas LISA"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 41.33
         MAX-WIDTH          = 274.29
...
ON END-ERROR OF w-relat /* Conciliaá∆o Etiquetas LISA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Conciliaá∆o Etiquetas LISA */
DO:
...
&Scoped-define SELF-NAME fiCodRefer
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiCodRefer w-relat
ON LEAVE OF fiCodRefer IN FRAME f-pg-par /* Referància */
DO:
   RUN validarFiltro.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fiItCodigo w-relat
ON LEAVE OF fiItCodigo IN FRAME f-pg-par /* C¢digo Produto */
DO:
  RUN validarFiltro.
END.
...
{utp/ut9000.i "conciliacao_etq_lisa" "9.99.99.999"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN ed-ajuda:SCREEN-VALUE IN FRAME f-pg-par = 
        '- Este Relat¢rio ir† buscar as Etiquetas da LISA via API e comparar com as Etiquetas existentes no ERP gerando uma planilha de conciliaá∆o como resultado.' 
        + CHR(13) + CHR(13) +
        '- A T E N Ä « O' + chr(13)  + 'Para Conciliar todo o estoque deixe os campos CODIGO DO PRODUTO e REFERENCIA em branco'.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY ed-ajuda fiItCodigo fiCodRefer tgAtu tgConciliacao tgApenasEstoque 
          tgGerarLog 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-12 fiItCodigo fiCodRefer tgAtu tgConciliacao tgApenasEstoque 
         tgGerarLog 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.logApenasEstoq  = logical(tgApenasEstoque:SCREEN-VALUE IN FRAME f-pg-par)
    ...
    {include/i-rprun.i lisa/conciliacao_etq_lisa_rp.p}
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE validarFiltro w-relat 
PROCEDURE validarFiltro :
IF fiItCodigo:SCREEN-VALUE IN FRAME f-pg-par = '' 
   AND fiCodRefer:SCREEN-VALUE IN FRAME f-pg-par = '' THEN DO:
      ASSIGN tgApenasEstoque:SCREEN-VALUE IN FRAME f-pg-par = 'yes'.   
   END.
