...
*******************************************************************************/
{include/i-prgvrs.i G01ES061 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS fi-nr-fardo bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-OBJECTS fi-nr-fardo 
...
DEFINE VARIABLE fi-nr-fardo AS INTEGER FORMAT ">>>>>>>>9" INITIAL 0 
     LABEL "Fardo" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88 TOOLTIP "N£mero do fardo." NO-UNDO.
...
DEFINE FRAME d-vapara
     fi-nr-fardo AT ROW 1.21 COL 21.86 COLON-ALIGNED
...
ON GO OF FRAME d-vapara /* V† Para Fardo de Algod∆o */
DO:
  find mp-fardo no-lock 
      where mp-fardo.nr-fardo = input frame {&frame-name} fi-nr-fardo no-error.
  if  not avail mp-fardo then do:
      {utp/ut-table.i espec mp-fardo 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mp-fardo).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Fardo de Algod∆o */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-nr-fardo 
      WITH FRAME d-vapara.
  ENABLE fi-nr-fardo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
/*  {utp/ut9000.i "G01ES061" "2.04.00.000"} */
