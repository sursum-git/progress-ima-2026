...
PROCEDURE getTipoVendedor:

    DEFINE OUTPUT PARAMETER cTipoVendedor AS CHARACTER   NO-UNDO.
    FIND cm-ext-repres
        WHERE cm-ext-repres.cod-rep = iRepres NO-LOCK NO-ERROR.
    IF AVAIL cm-ext-repres THEN DO:
       IF  cm-ext-repres.classe = 4   THEN
           ASSIGN cTipoVendedor = 'interno'.
       ELSE 
          ASSIGN cTipoVendedor = 'externo'.
    END.
