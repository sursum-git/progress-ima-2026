...
DEFINE INPUT  PARAMETER p-cod-depos    LIKE saldo-estoq.cod-depos.
...
&Scoped-define FIELDS-IN-QUERY-br-ref-item tt-ref-item.cod-refer tt-ref-item.qtidade-atu tt-ref-item.qt-pedida tt-ref-item.preco-un tt-ref-item.vl-tot-ref tt-ref-item.cod-depos   
...
&Scoped-Define ENABLED-OBJECTS RECT-5 rt-buttom bt-exp-preco fi-cod-refer ~
fi-quantidade fi-valor-un br-ref-item bt-ok bt-cancela bt-ajuda bt-exp-qtde 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-ref-item D-Dialog _FREEFORM
  QUERY br-ref-item NO-LOCK DISPLAY
      ...
      tt-ref-item.cod-depos  COLUMN-LABEL "Depos"
ENABLE
...
DO:
    ...
    IF NOT AVAIL tt-ref-item THEN DO.
       MESSAGE "Referˆncia sem Saldo em Estoque..."
           VIEW-AS ALERT-BOX ERROR BUTTONS OK.
       APPLY 'entry' TO SELF.
       RETURN NO-APPLY.
    END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-it-codigo fi-desc-item fi-cod-refer fi-quantidade fi-valor-un 
      ...
      WITH FRAME D-Dialog.
  ENABLE RECT-5 rt-buttom bt-exp-preco fi-cod-refer fi-quantidade fi-valor-un 
         br-ref-item bt-ok bt-cancela bt-ajuda bt-exp-qtde 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
    ...
    FOR EACH tt-saldo-estoq WHERE 
             tt-saldo-estoq.cod-estabel = ped-venda.cod-estabel NO-LOCK.

        FIND tt-ref-item WHERE
             tt-ref-item.cod-refer = tt-saldo-estoq.cod-refer AND
             tt-ref-item.cod-depos = tt-saldo-estoq.cod-depos
             NO-LOCK NO-ERROR.
        IF NOT AVAIL tt-ref-item THEN DO.
           CREATE tt-ref-item.
           ASSIGN tt-ref-item.cod-refer = tt-saldo-estoq.cod-refer
                  tt-ref-item.cod-depos = tt-saldo-estoq.cod-depos.
        END.
        ASSIGN tt-ref-item.qtidade-atu = tt-ref-item.qtidade-atu + tt-saldo-estoq.qt-disponivel.

        FIND preco-item WHERE
             preco-item.nr-tabpre = p-tab-preco AND
             preco-item.it-codigo = tt-saldo-estoq.it-codigo AND
             preco-item.cod-refer = tt-saldo-estoq.cod-refer
             NO-LOCK NO-ERROR.
    ...
    IF p-tab-preco <> "" THEN
       ASSIGN fi-valor-un:SENSITIVE = NO
              bt-exp-preco:SENSITIVE = NO.
