...
*******************************************************************************/
{include/i-prgvrs.i ESSP0024 2.04.00.000}
...
DEFINE VARIABLE h_q01es044 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es044 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Cotas de Vendas por Ötem"
         HEIGHT             = 22.5
         WIDTH              = 90
         MAX-HEIGHT         = 23.46
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Cotas de Vendas por Ötem */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Cotas de Vendas por Ötem */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es044.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es044 ).
       RUN set-position IN h_v01es044 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 20.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es044.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q01es044 ).
       RUN set-position IN h_q01es044 ( 1.50 , 65.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es044. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es044 ).
       RUN add-link IN adm-broker-hdl ( h_q01es044 , 'Record':U , h_v01es044 ).

       /* Links to SmartQuery h_q01es044. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es044 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es044 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es044 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es044 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0024" "2.04.00.000"}
