...
*******************************************************************************/
{include/i-prgvrs.i atu_localiz_etq_lisa 9.99.99.999}
...
&IF "{&EMSFND_VERSION}" >= "1.00" &THEN
    {include/i-license-manager.i atu_localiz_etq_lisa cep}
&ENDIF
...
DEFINE VARIABLE lGrpSup AS LOGICAL     NO-UNDO.
...
DEFINE VARIABLE ed-ajuda AS CHARACTER 
     VIEW-AS EDITOR
     SIZE 36 BY 7.88
     FONT 2 NO-UNDO.

DEFINE VARIABLE fiCodRefer AS CHARACTER FORMAT "X(20)":U 
     LABEL "Referància" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fiItCodigo AS CHARACTER FORMAT "X(20)":U 
     LABEL "C¢digo Produto" 
     VIEW-AS FILL-IN 
     SIZE 11.86 BY .79 NO-UNDO.
...
DEFINE VARIABLE tgDtInf AS LOGICAL INITIAL no 
     LABEL "INFORMAR Data de Atualiz.Localiz." 
     VIEW-AS TOGGLE-BOX
     SIZE 28 BY .83 NO-UNDO.

DEFINE VARIABLE tgGerarLog AS LOGICAL INITIAL no 
     LABEL "Gerar Log API" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-par
     ed-ajuda AT ROW 1.13 COL 40 NO-LABEL WIDGET-ID 12
     fiItCodigo AT ROW 3 COL 21 COLON-ALIGNED WIDGET-ID 4
     fiCodRefer AT ROW 4.25 COL 21 COLON-ALIGNED WIDGET-ID 6
     tgGerarLog AT ROW 5.46 COL 23.29 WIDGET-ID 2
     tgDtInf AT ROW 6.5 COL 4 WIDGET-ID 20
     fiDtIni AT ROW 7.75 COL 13.43 COLON-ALIGNED WIDGET-ID 24
     fiDtFim AT ROW 8.75 COL 13.29 COLON-ALIGNED WIDGET-ID 26
     RECT-10 AT ROW 6.75 COL 2 WIDGET-ID 18
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 2.57 ROW 3.17
         SIZE 76.72 BY 10
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Atualizaá∆o Localiz Etiquetas LISA"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 41.33
         MAX-WIDTH          = 274.29
...
/* SETTINGS FOR FRAME f-pg-par
                                                                        */
/* SETTINGS FOR EDITOR ed-ajuda IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fiDtFim IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fiDtIni IN FRAME f-pg-par
   NO-ENABLE                                                            */
/* SETTINGS FOR TOGGLE-BOX tgDtInf IN FRAME f-pg-par
   1                                                                    */
...
ON END-ERROR OF w-relat /* Atualizaá∆o Localiz Etiquetas LISA */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Atualizaá∆o Localiz Etiquetas LISA */
DO:
...
DO:
  // {include/ajuda.i}
  OS-COMMAND SILENT VALUE('start https://imaonline.imatextil.com.br/iol_tst/bl_manual_dinamico/bl_manual_dinamico.php?codigo=conciliacao_etq_lisa ') NO-WAIT.
END.
...
&Scoped-define SELF-NAME tgDtInf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tgDtInf w-relat
ON VALUE-CHANGED OF tgDtInf IN FRAME f-pg-par /* INFORMAR Data de Atualiz.Localiz. */
DO:
  ASSIGN fiDtIni:SENSITIVE IN FRAME f-pg-par = tgDtInf:SCREEN-VALUE = 'yes'
          fiDtFim:SENSITIVE IN FRAME f-pg-par = tgDtInf:SCREEN-VALUE = 'yes'
          .
  IF  tgDtInf:SCREEN-VALUE = 'yes' THEN
  DO:
     ASSIGN fiDtIni:SCREEN-VALUE IN FRAME f-pg-par = STRING(TODAY) 
            fiDtFim:SCREEN-VALUE IN FRAME f-pg-par = STRING(TODAY)
            .
  END.
END.
...
{utp/ut9000.i "atu_localiz_etq_lisa" "9.99.99.999"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN ed-ajuda:SCREEN-VALUE IN FRAME f-pg-par = 
        '- Este Programa ir† buscar as Etiquetas da LISA via API conforme o filtro de data de atualizaá∆o da localizaá∆o e atualizar a localizaá∆o do item no datasul. Por padr∆o s¢ busca atualizaá‰es do dia CORRENTE ' 
        + CHR(13) + CHR(13) +
        '- A T E N Ä « O' + chr(13)  + 'Para Atualizar todos itens e referencias do periodo de atualizaá∆o da localizaá∆o deixe os campos CODIGO DO PRODUTO e REFERENCIA em branco'.

    RUN esapi/verificUsuarioGrupo.p('sup',c-seg-usuario,OUTPUT lGrpSup).

    ASSIGN tgGerarLog:VISIBLE IN FRAME f-pg-par = lGrpSup.
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY ed-ajuda fiItCodigo fiCodRefer tgGerarLog tgDtInf fiDtIni fiDtFim 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-10 fiItCodigo fiCodRefer tgGerarLog tgDtInf 
      WITH FRAME f-pg-par IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario         = c-seg-usuario
           ...
           tt-param.lGerarLogAPI    = logical(tgGerarLog:SCREEN-VALUE IN FRAME f-pg-par)
           //tt-param.lAtuEtq         = logical(tgAtu:SCREEN-VALUE IN FRAME f-pg-par)
           tt-param.itCodigo        = fiItCodigo:SCREEN-VALUE IN FRAME f-pg-par
           tt-param.codRefer        = upper(ficodRefer:SCREEN-VALUE IN FRAME f-pg-par)
           //tt-param.lConciliar      = INPUT FRAME  f-pg-par tgConciliacao 
           tt-param.lDtEndInf       = INPUT FRAME  f-pg-par tgDtInf
           tt-param.dtIniEnd        = INPUT FRAME  f-pg-par fiDtIni
           tt-param.dtFimEnd        = INPUT FRAME  f-pg-par fiDtFim
    ...
    {include/i-rprun.i lisa/atu_localiz_etq_lisa_rp.p}
