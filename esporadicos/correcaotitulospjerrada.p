OUTPUT TO c:\temp\titproblema.txt.
FOR EACH tit_acr:
    FIND FIRST ems5.cliente
        WHERE tit_acr.cdn_cliente = cliente.cdn_cliente
        AND   tit_acr.num_pessoa  <> cliente.num_pessoa NO-LOCK NO-ERROR.
    IF AVAIL ems5.cliente THEN DO:
       EXPORT DELIMITER "|" STRING(ROWID(tit_acr)) tit_acr.cod_empresa tit_acr.cod_estab tit_acr.cod_espec_docto cod_ser_docto cod_parcela   tit_acr.cod_tit_acr  cdn_clien_matriz tit_acr.cdn_cliente tit_acr.num_pessoa cliente.num_pessoa .
       ASSIGN tit_acr.cdn_clien_matriz = tit_acr.cdn_cliente
              tit_acr.num_pessoa       = cliente.num_pessoa
