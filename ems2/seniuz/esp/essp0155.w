...
DEFINE VAR c-cod-estabel-fin   AS CHAR INIT "ZZZZZ".
...
DEFINE VAR de-perc-comis-ini   LIKE ped-repre.perc-comis. 
DEFINE VAR de-perc-comis-fin   LIKE ped-repre.perc-comis.
...
DEFINE VAR i-nr-tabpre         AS INTEGER INIT 3.
...
DEFINE VAR l-tab-x             AS LOG INIT NO.
...
bt-inclui bt-modifica bt-troca-moeda bt-cancela bt-suspende bt-status ~
bt-vapara bt-consulta bt-log bt-imprime bt-excel bt-imprime-txt bt-exit ~
fi-restricao fi-isf fi-faturado fi-aprovado fi-separado 
&Scoped-Define DISPLAYED-OBJECTS fi-tot-pedida fi-tot-fatur fi-tot-cancel ~
fi-tot-aberto fi-restricao fi-isf fi-faturado fi-aprovado fi-separado 
...
DEFINE BUTTON bt-troca-moeda AUTO-GO 
     IMAGE-UP FILE "image/im-troca-moeda.gif":U
     LABEL "" 
     SIZE 5.86 BY 1.29 TOOLTIP "Altera Moeda do Pedido para Real"
     BGCOLOR 8 FONT 10.
...
DEFINE VARIABLE fi-aprovado AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     VIEW-AS FILL-IN 
     SIZE 2 BY .58
     BGCOLOR 16 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE fi-faturado AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     VIEW-AS FILL-IN 
     SIZE 2 BY .58
     BGCOLOR 0 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE fi-isf AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     VIEW-AS FILL-IN 
     SIZE 2 BY .58
     BGCOLOR 2 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE fi-restricao AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     VIEW-AS FILL-IN 
     SIZE 2 BY .58
     BGCOLOR 12 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE fi-separado AS CHARACTER FORMAT "X(256)":U INITIAL "*" 
     VIEW-AS FILL-IN 
     SIZE 2 BY .58
     BGCOLOR 9 FGCOLOR 15  NO-UNDO.

DEFINE VARIABLE fi-tot-aberto AS DECIMAL FORMAT "ZZZ,ZZZ,ZZ9.99":U INITIAL 0 
     LABEL "Aberto" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88
     FONT 6 NO-UNDO.
...
DEFINE VARIABLE fi-tot-fatur AS DECIMAL FORMAT "ZZZ,ZZZ,ZZ9.99":U INITIAL 0 
     LABEL "Faturado" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88
     FONT 6 NO-UNDO.

DEFINE VARIABLE fi-tot-pedida AS DECIMAL FORMAT "ZZZ,ZZZ,ZZ9.99":U INITIAL 0 
     LABEL "Carteira" 
     VIEW-AS FILL-IN 
     SIZE 14 BY .88
     FONT 6 NO-UNDO.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-pedidos C-Win _FREEFORM
  QUERY br-pedidos NO-LOCK DISPLAY
      ...
      tt-ped-venda.nr-tabpre    COLUMN-LABEL "TAB Ref"                              WIDTH 10                  
      ...
      tt-ped-venda.desc-restr   COLUMN-LABEL "Restriá∆o"      FORMAT "x(30)" 
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ROW-MARKERS SEPARATORS SIZE 118 BY 18.25
         FONT 6
...
DEFINE FRAME DEFAULT-FRAME
     ...
     bt-param AT ROW 1.5 COL 122.29
     bt-inclui AT ROW 3 COL 122.14
     bt-modifica AT ROW 4.17 COL 122.14
     bt-troca-moeda AT ROW 5.42 COL 122.14 WIDGET-ID 10
     bt-cancela AT ROW 7.13 COL 122.14
     bt-suspende AT ROW 8.42 COL 122.14
     bt-status AT ROW 9.67 COL 122.14
     bt-vapara AT ROW 12.21 COL 122.14
     bt-consulta AT ROW 13.46 COL 122.14
     bt-log AT ROW 14.71 COL 122.14
     bt-imprime AT ROW 16.29 COL 122.14
     bt-excel AT ROW 17.58 COL 122.14
     bt-imprime-txt AT ROW 19.04 COL 122
     fi-tot-pedida AT ROW 19.75 COL 10 COLON-ALIGNED
     fi-tot-fatur AT ROW 19.75 COL 42 COLON-ALIGNED
     fi-tot-cancel AT ROW 19.75 COL 76 COLON-ALIGNED
     fi-tot-aberto AT ROW 19.75 COL 103 COLON-ALIGNED
     bt-exit AT ROW 21 COL 121
     fi-restricao AT ROW 21.25 COL 1 COLON-ALIGNED NO-LABEL
     fi-isf AT ROW 21.33 COL 38 COLON-ALIGNED NO-LABEL
     fi-faturado AT ROW 21.33 COL 85 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     fi-aprovado AT ROW 22 COL 3 NO-LABEL
     fi-separado AT ROW 22.08 COL 38 COLON-ALIGNED NO-LABEL WIDGET-ID 2
     "Faturado" VIEW-AS TEXT
          SIZE 9.29 BY .67 AT ROW 21.25 COL 89.72 WIDGET-ID 8
          BGCOLOR 8 FONT 6
     "Aprovado e ISF Impressa (em Separaá∆o)" VIEW-AS TEXT
          SIZE 35.29 BY .67 AT ROW 21.25 COL 42.72
          BGCOLOR 8 FGCOLOR 2 FONT 6
     "Aprovado (Aguardando Separaá∆o)" VIEW-AS TEXT
          SIZE 30.57 BY .67 AT ROW 22 COL 5.43
          BGCOLOR 8 FGCOLOR 16 FONT 6
     "Restriá∆o de Aprovaá∆o" VIEW-AS TEXT
          SIZE 21.43 BY .67 AT ROW 21.25 COL 5.43
          BGCOLOR 8 FGCOLOR 12 FONT 6
     "Separado (Aguardando NF)" VIEW-AS TEXT
          SIZE 25.29 BY .67 AT ROW 22 COL 42.72 WIDGET-ID 4
          BGCOLOR 8 FGCOLOR 9 FONT 6
     RECT-4 AT ROW 21 COL 2.14
     rt-buttom AT ROW 1.25 COL 121
...
/* SETTINGS FOR BUTTON bt-excel IN FRAME DEFAULT-FRAME
   6                                                                    */
/* SETTINGS FOR FILL-IN fi-aprovado IN FRAME DEFAULT-FRAME
   ALIGN-L                                                              */
ASSIGN 
       fi-aprovado:READ-ONLY IN FRAME DEFAULT-FRAME        = TRUE.

ASSIGN 
       fi-faturado:READ-ONLY IN FRAME DEFAULT-FRAME        = TRUE.

ASSIGN 
       fi-isf:READ-ONLY IN FRAME DEFAULT-FRAME        = TRUE.

ASSIGN 
       fi-restricao:READ-ONLY IN FRAME DEFAULT-FRAME        = TRUE.

ASSIGN 
       fi-separado:READ-ONLY IN FRAME DEFAULT-FRAME        = TRUE.
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-ped-venda /*WHERE
                                 (tt-ped-venda.l-restricao = (fi-restricao = "*") OR fi-restricao = "") AND
                                 ((tt-ped-venda.l-restricao = NO AND fi-aprovado = "*") OR fi-aprovado = "") AND
                                 (tt-ped-venda.l-impresso = (fi-isf = "*") OR fi-isf = "") */
                                 NO-LOCK.
     _END_FREEFORM
...
DO:
    ...
    ELSE IF tt-ped-venda.l-impresso THEN
         ASSIGN i-cor = 2. /* Verde */

    IF tt-ped-venda.reservado THEN
       ASSIGN i-cor = 9.  /* Azul */

    IF tt-ped-venda.cod-sit-ped = 3 THEN
       ASSIGN i-cor = 0.

    RUN pi-cor (INPUT i-cor).
END.
...
DO:
   ...
   IF l-ok THEN DO.
      ...
      IF c-aprov-canc <> '' THEN
         ASSIGN c-motivo = c-motivo + " // " + 
                          "Cancelamento Aprovado por: " + c-aprov-canc.
      ...
      IF c-aprov-canc <> '' THEN DO.
         FIND CURRENT ped-venda-ext SHARE-LOCK NO-ERROR.
         ASSIGN ped-venda-ext.aprov-canc = c-aprov-canc.
...
DO:
   ...
   RUN esdlg/d02essp0155.w (OUTPUT arq-saida,
                            INPUT IF l-sit-can THEN da-dt-cancela-ini ELSE da-dt-implant-ini,
                            INPUT IF l-sit-can THEN da-dt-cancela-fin ELSE da-dt-implant-fin).
...
DO:
   ...
   IF ped-venda.cod-sit-ped >= 3 THEN DO.
      MESSAGE 'Pedido foi Atendido Total ou Cancelado, Alteraá∆o N«O PERMITIDA'
          VIEW-AS ALERT-BOX ERROR BUTTONS OK.
      RETURN NO-APPLY.
   END.
   ...
   IF tt-ped-venda.l-impresso THEN DO.   /* Pedido em Separaá∆o */
   ...
   */
   ...
   // Verifica se Pedido possui Itens, se n∆o tiver, cancela...
   ...
   IF NOT AVAIL ped-item AND
      ...
      ASSIGN c-motivo = 'Cancelamento Autom†tico do Pedido, pois n∆o possui Itens'.

      RUN esapi/cancela-pedvenda.p (INPUT ped-venda.nr-pedcli,
                                    INPUT c-motivo).

      IF RETURN-VALUE <> 'ADM-ERROR' THEN DO.
         ASSIGN ped-venda.cd-cancela = 2.  // Sistema

         IF c-aprov-canc <> '' THEN DO.
            FIND CURRENT ped-venda-ext SHARE-LOCK NO-ERROR.
            ASSIGN ped-venda-ext.aprov-canc = 'Sistema'.
            FIND CURRENT ped-venda-ext NO-LOCK NO-ERROR.
         END.
    
         RUN pi-cancela-reserva.
    
         ASSIGN tt-ped-venda.situacao = 'CAN'.
...
DO:
   ...
   ASSIGN c-win:SENSITIVE = NO.
   RUN esp/essp0155a.w (INPUT-OUTPUT c-cod-estabel-ini,
                        ...
                        INPUT-OUTPUT de-perc-comis-ini, 
                        INPUT-OUTPUT de-perc-comis-fin,
                        INPUT-OUTPUT i-nr-tabpre,
                        INPUT-OUTPUT i-situacao,
                        INPUT-OUTPUT i-preco,
                        INPUT-OUTPUT i-aprovar,
                        INPUT-OUTPUT l-tab-x,
...
&Scoped-define SELF-NAME bt-troca-moeda
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-troca-moeda C-Win
ON CHOOSE OF bt-troca-moeda IN FRAME DEFAULT-FRAME
DO:
   RUN pi-ver-permissao.
   IF NOT l-ok THEN DO.
      MESSAGE "Usu†rio: " + c-seg-usuario + ", n∆o tem permiss∆o para essa rotina."
   ...
   FIND FIRST b-tt-ped-venda WHERE 
              b-tt-ped-venda.tp-pedido = 'PI' NO-LOCK NO-ERROR.
   IF AVAIL b-tt-ped-venda THEN DO.
      MESSAGE 'Existem Pedidos PI na Seleá∆o, Verifique...' 
          VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
      RETURN NO-APPLY.
   END.

   MESSAGE 'Deseja Realmente alterar Moeda dos Pedidos para Real ?' 
       VIEW-AS ALERT-BOX QUESTION BUTTONS YES-NO UPDATE l-conf-moeda AS LOGICAL.
   IF NOT l-conf-moeda THEN RETURN NO-APPLY.
   ...
   FOR EACH b-tt-ped-venda WHERE 
            b-tt-ped-venda.tp-pedido = 'PE' AND
            b-tt-ped-venda.nr-container <> 0 AND 
            b-tt-ped-venda.mo-codigo <> 0 NO-LOCK
       BREAK BY b-tt-ped-venda.nr-container.

       RUN pi-acompanhar IN h-acomp (INPUT "Pedido: " + b-tt-ped-venda.nr-pedcli).

       IF FIRST-OF(b-tt-ped-venda.nr-container) THEN DO.
          ASSIGN l-ok = NO.

          FIND pp-container WHERE
               pp-container.nr-container = b-tt-ped-venda.nr-container
               NO-LOCK NO-ERROR.

           IF pp-container.situacao <> 3 THEN 
              MESSAGE 'Container ' pp-container.nr-container ' n∆o est† Fechado,' SKIP
                      'Preáos n∆o poder∆o ser Convertidos para Real...'
                  VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
    
           IF pp-container.ptax = 0 THEN 
              MESSAGE 'PTAX do Container ' pp-container.nr-container ' n∆o foi Informado (ESPP001),' SKIP
                      'Preáos n∆o poder∆o ser Convertidos para Real...'
                  VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.
...
&Scoped-define SELF-NAME fi-aprovado
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-aprovado C-Win
ON LEAVE OF fi-aprovado IN FRAME DEFAULT-FRAME
DO:
    ASSIGN INPUT FRAME {&FRAME-NAME} fi-aprovado.
    {&OPEN-QUERY-br-pedidos}
    APPLY 'value-changed' TO br-pedidos.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-aprovado C-Win
ON MOUSE-SELECT-CLICK OF fi-aprovado IN FRAME DEFAULT-FRAME
DO:
...
&Scoped-define SELF-NAME fi-faturado
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-faturado C-Win
ON MOUSE-SELECT-CLICK OF fi-faturado IN FRAME DEFAULT-FRAME
DO:
    IF SELF:SCREEN-VALUE = '' THEN
     ASSIGN SELF:SCREEN-VALUE = "*".
  ELSE
...
&Scoped-define SELF-NAME fi-isf
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-isf C-Win
ON MOUSE-SELECT-CLICK OF fi-isf IN FRAME DEFAULT-FRAME
DO:
    IF SELF:SCREEN-VALUE = '' THEN
       ASSIGN SELF:SCREEN-VALUE = "*".
    ELSE
    ...
    ASSIGN INPUT FRAME {&FRAME-NAME} fi-isf.
    {&OPEN-QUERY-br-pedidos}
    APPLY 'value-changed' TO br-pedidos.
    APPLY 'entry' TO br-pedidos.
...
&Scoped-define SELF-NAME fi-restricao
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-restricao C-Win
ON MOUSE-SELECT-CLICK OF fi-restricao IN FRAME DEFAULT-FRAME
DO:
  IF SELF:SCREEN-VALUE = '' THEN
     ASSIGN SELF:SCREEN-VALUE = "*".
  ELSE
  ...
  ASSIGN INPUT FRAME {&FRAME-NAME} fi-restricao.
  {&OPEN-QUERY-br-pedidos}
  APPLY 'value-changed' TO br-pedidos.
  APPLY 'entry' TO br-pedidos.
...
&Scoped-define SELF-NAME fi-separado
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-separado C-Win
ON MOUSE-SELECT-CLICK OF fi-separado IN FRAME DEFAULT-FRAME
DO:
  IF SELF:SCREEN-VALUE = '' THEN
     ASSIGN SELF:SCREEN-VALUE = "*".
  ELSE
...
------------------------------------------------------------------------------*/
  DISPLAY fi-tot-pedida fi-tot-fatur fi-tot-cancel fi-tot-aberto fi-restricao 
          fi-isf fi-faturado fi-aprovado fi-separado 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
  ENABLE RECT-4 rt-buttom br-pedidos bt-param bt-inclui bt-modifica 
         bt-troca-moeda bt-cancela bt-suspende bt-status bt-vapara bt-consulta 
         bt-log bt-imprime bt-excel bt-imprime-txt bt-exit fi-restricao fi-isf 
         fi-faturado fi-aprovado fi-separado 
      WITH FRAME DEFAULT-FRAME IN WINDOW C-Win.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-altera-pedido C-Win 
...
------------------------------------------------------------------------------*/
    ...
    FIND ped-venda WHERE
         ...
         ped-venda.nome-abrev = b-tt-ped-venda.nome-abrev 
         SHARE-LOCK NO-ERROR.
    ...
    BUFFER-COPY ped-venda TO tt-pedidos
        ASSIGN tt-pedidos.mo-codigo = 0.

    RUN esapi/altera-pedvenda.p (INPUT TABLE tt-pedidos).
    IF RETURN-VALUE = 'ADM-ERROR' THEN
       RETURN 'ADM-ERROR'.
    
    RUN esapi/cria-log-pedvenda.p (INPUT ped-venda.nr-pedcli,
                                   INPUT ped-venda.nome-abrev,
                                   INPUT "Alterado Moeda do Pedido para Real",
                                   INPUT NO).

    FOR EACH ped-item OF ped-venda WHERE
             ped-item.cod-sit-item <> 6 SHARE-LOCK.
        ...
        ASSIGN tt-itens-ped.vl-preori = tt-itens-ped.vl-preori * pp-container.ptax
               tt-itens-ped.vl-preuni = tt-itens-ped.vl-preori * pp-container.ptax.

        // altera o Preáo do Item
        RUN esapi/altera-peditem-v2.p (INPUT TABLE tt-itens-ped).
    END.
...
------------------------------------------------------------------------------*/
    ...
    tt-ped-venda.cod-estabel:FGCOLOR IN BROWSE {&browse-name} = p-cor.
    ...
    tt-ped-venda.nr-tabpre:FGCOLOR IN BROWSE {&browse-name} = p-cor.    
    ...
    tt-ped-venda.desc-restr:FGCOLOR IN BROWSE {&browse-name} = p-cor. 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE pi-envia-email-pedido C-Win 
...
------------------------------------------------------------------------------*/
    ...
    FIND peds_web WHERE
         peds_web.nr_pedido_erp = ped-venda.nr-pedido NO-LOCK NO-ERROR.
    ...
    //ASSIGN c-destinatario = c-destinatario + "," + 'eduardomagnoa@gmail.com'.
    ...
    MESSAGE 'Essa Mensagem ser† Enviada aos seguintes Destinat†rios:' SKIP
            c-destinatario
        VIEW-AS ALERT-BOX INFO BUTTONS OK.

    IF p-tipo = 'HTML' THEN DO.
...
------------------------------------------------------------------------------*/
   ...
   /* Incluir Uma Nova Planilha */
    ...
    ASSIGN chWorksheet:range("A" + STRING(i-lin)):VALUE = "EMISS«O"
           ...
           chWorksheet:range("E" + STRING(i-lin)):VALUE = "TRANSPORTADORA"
           chWorksheet:range("F" + STRING(i-lin)):VALUE = "PAGTO"
           chWorksheet:range("G" + STRING(i-lin)):VALUE = "SIT"
           chWorksheet:range("H" + STRING(i-lin)):VALUE = "QTD PEDIDA"
           chWorksheet:range("I" + STRING(i-lin)):VALUE = "RESERVA"
           chWorksheet:range("J" + STRING(i-lin)):VALUE = "ABERTO"
           chWorksheet:range("K" + STRING(i-lin)):VALUE = "DESCONTO".
           chWorksheet:range("L" + STRING(i-lin)):VALUE = "VALOR TOTAL".
           chWorksheet:range("M" + STRING(i-lin)):VALUE = "DT. APROV.".
           chWorksheet:range("N" + STRING(i-lin)):VALUE = "DT. ENTREGA".
           chWorksheet:range("O" + STRING(i-lin)):VALUE = "DT. IMPLANT".     
           chWorksheet:range("P" + STRING(i-lin)):VALUE = "DT. CANCEL".    
           chWorksheet:range("Q" + STRING(i-lin)):VALUE = "USR CANCEL".    
           chWorksheet:range("R" + STRING(i-lin)):VALUE = "MOTIVO CANCEL".    
           chWorksheet:range("S" + STRING(i-lin)):VALUE = "MOEDA".    
           chWorksheet:range("T" + STRING(i-lin)):VALUE = "CONTAINER".   
           chWorksheet:range("U" + STRING(i-lin)):VALUE = "PEDIDO WEB".   
           chWorksheet:range("V" + STRING(i-lin)):VALUE = "OBSERVAÄÂES".
    ...
    FOR EACH tt-ped-venda NO-LOCK
        ...
        chWorksheet:range("E" + STRING(i-lin)):VALUE = tt-ped-venda.nome-transp.
        chWorksheet:range("F" + STRING(i-lin)):VALUE = c-desc-cond-pag.
        chWorksheet:range("G" + STRING(i-lin)):VALUE = tt-ped-venda.situacao.
        chWorksheet:range("H" + STRING(i-lin)):VALUE = tt-ped-venda.qt-pedida.
        chWorksheet:range("I" + STRING(i-lin)):VALUE = tt-ped-venda.qt-reservada.
        chWorksheet:range("J" + STRING(i-lin)):VALUE = tt-ped-venda.qt-aberto.
        chWorksheet:range("K" + STRING(i-lin)):VALUE = tt-ped-venda.vl-desconto.
        chWorksheet:range("L" + STRING(i-lin)):VALUE = tt-ped-venda.vl-tot-ped.
        chWorksheet:range("M" + STRING(i-lin)):VALUE = tt-ped-venda.dt-apr-cred.
        chWorksheet:range("N" + STRING(i-lin)):VALUE = tt-ped-venda.dt-entrega.   
        chWorksheet:range("O" + STRING(i-lin)):VALUE = tt-ped-venda.dt-implant.   
        chWorksheet:range("P" + STRING(i-lin)):VALUE = tt-ped-venda.dt-cancel.   
        chWorksheet:range("Q" + STRING(i-lin)):VALUE = tt-ped-venda.user-canc.
        chWorksheet:range("R" + STRING(i-lin)):VALUE = tt-ped-venda.desc-cancela.
        chWorksheet:range("S" + STRING(i-lin)):VALUE = moeda.descricao.   
        chWorksheet:range("T" + STRING(i-lin)):VALUE = tt-ped-venda.nr-container.   
        chWorksheet:range("U" + STRING(i-lin)):VALUE = tt-ped-venda.ped_web_id.
        chWorksheet:range("V" + STRING(i-lin)):VALUE = tt-ped-venda.observacoes.
   
        chWorkSheet:range("H" + STRING(i-lin) + ":L" + STRING(i-lin)):NumberFormat = "#.##0,00".
    ...
    ASSIGN chWorksheet:range("F" + STRING(i-lin)):VALUE = "TOTAL"
           chWorkSheet:range("I" + STRING(i-lin) + ":L" + STRING(i-lin)):NumberFormat = "#.##0,00"
           chWorkSheet:range("G" + STRING(i-lin) + ":L" + STRING(i-lin)):FONT:ColorIndex     =  3
           chWorkSheet:range("A" + STRING(i-lin) + ":L" + STRING(i-lin)):FONT:Bold           = TRUE
           chWorkSheet:range("A" + STRING(i-lin) + ":L" + STRING(i-lin)):FONT:NAME = "Courrier New"
           chWorkSheet:range("A" + STRING(i-lin) + ":L" + STRING(i-lin)):FONT:SIZE = 12.

    ASSIGN chWorksheet:range("H" + STRING(i-lin)):VALUE = (ACCUM TOTAL tt-ped-venda.qt-pedida)
           chWorksheet:range("I" + STRING(i-lin)):VALUE = (ACCUM TOTAL tt-ped-venda.qt-reservada)  
           chWorksheet:range("J" + STRING(i-lin)):VALUE = (ACCUM TOTAL tt-ped-venda.qt-aberto)
           chWorksheet:range("K" + STRING(i-lin)):VALUE = (ACCUM TOTAL tt-ped-venda.vl-desconto)
           chWorksheet:range("L" + STRING(i-lin)):VALUE = (ACCUM TOTAL tt-ped-venda.vl-tot-ped).
...
------------------------------------------------------------------------------*/
    ...
    DEF VAR l-reservado AS LOG.
    DEF VAR l-avista AS LOGICAL.
    ...
    FOR EACH ped-venda WHERE
        ...
        IF AVAIL cond-pagto AND
           (cond-pagto.cod-vencto = 2 OR cond-pagto.cod-vencto = 3) THEN /* A Vista e Antecipado */
           ASSIGN l-avista = YES.


        IF ENTRY(1,c-tp-pedido) <> 'Todos' THEN DO.
           IF ENTRY(1,c-tp-pedido) = '∑ Vista' THEN DO.
              IF l-avista = NO THEN NEXT.
           END.
           ELSE DO.
              IF l-avista = YES THEN NEXT.
              IF ped-venda-ext.tp-pedido <> ENTRY(1,c-tp-pedido) THEN NEXT.
           END.
        END.
        ...
        IF AVAIL ped-repre THEN DO.
           IF ped-repre.perc-comis < de-perc-comis-ini OR
              ped-repre.perc-comis > de-perc-comis-fin THEN NEXT.
        END.

        IF i-nr-tabpre <> 3 AND ped-venda-ext.tb_preco_id <> i-nr-tabpre THEN NEXT.
        IF l-tab-x = YES AND ped-venda-ext.l_tab_x = NO THEN NEXT.

        ASSIGN de-qt-pedida = 0         de-qt-faturada = 0
               de-qt-reservada = 0      de-qt-cancelada = 0      de-tot-desc = 0.

        ASSIGN l-reservado = YES.
        FOR EACH ped-item OF ped-venda WHERE
            ...
            IF l-corte AND AVAIL ped-item-ext AND 
               ped-item-ext.retirar-corte = NO THEN NEXT.
        ...
        IF AVAIL ped-repres AND ped-repres.cod-classif <> '' THEN 
           ASSIGN l-restricao = YES
        ...
        ASSIGN tt-ped-venda.nr-container = ped-venda-ext.nr-container
               tt-ped-venda.nr-tabpre = IF AVAIL tbs_preco 
                                        THEN  UPPER(tbs_preco.descricao)
                                        ELSE 'PADRAO'
               ...
               tt-ped-venda.desc-restr = c-restricao
               tt-ped-venda.reservado = l-reservado.
...
------------------------------------------------------------------------------*/
    ...
    CASE SELF:NAME.
        ...
        WHEN "bt-modifica" OR WHEN "bt-divide" OR WHEN 'bt-troca-moeda' THEN
