...
*******************************************************************************/
{include/i-prgvrs.i ESSP0031 2.04.00.000}
...
DEFINE VAR cod-maq-ini LIKE maq-mec.codigo.
DEFINE VAR cod-maq-fin LIKE maq-mec.codigo.
...
DEFINE VARIABLE h_q01es019 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es019 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-consim /* M quinas da Mecƒnica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* M quinas da Mecƒnica */
DO:
...
DO:
  RUN esdlg/d01es019.w (OUTPUT cod-maq-ini, OUTPUT cod-maq-fin, OUTPUT arq-saida).

  IF arq-saida <> "" THEN DO:
     RUN pi-gera-excel (INPUT cod-maq-ini, INPUT cod-maq-fin, INPUT arq-saida).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es019.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es019 ).
       RUN set-position IN h_v01es019 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 3.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es019.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es019.w,
                     ProgVaPara = esgo/g01es019.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es019 ).
       RUN set-position IN h_q01es019 ( 1.50 , 59.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es019. */
       RUN add-link IN adm-broker-hdl ( h_q01es019 , 'Record':U , h_v01es019 ).

       /* Links to SmartQuery h_q01es019. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es019 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es019 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es019 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es019 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0031" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER p-cod-maq-ini LIKE maq-mec.codigo.
    DEF INPUT PARAMETER p-cod-maq-fin LIKE maq-mec.codigo.
    ...
    ASSIGN c-Lin = c-empresa + " - " + " TABELA DE MµQUINAS DA MEC¶NICA - DATA: " + STRING(TODAY) +
                                       " - " + STRING(TIME,"HH:MM").
    ...
    FOR EACH maq-mec WHERE maq-mec.codigo >= p-cod-maq-ini
                       AND maq-mec.codigo <= p-cod-maq-fin
                     NO-LOCK:
    
        DDE SEND i-canal SOURCE maq-mec.codigo    ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
        DDE SEND i-canal SOURCE maq-mec.descricao ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
