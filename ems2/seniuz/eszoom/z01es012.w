...
*******************************************************************************/
{include/i-prgvrs.i Z01ES0012 2.04.00.000}
...
DEFINE VARIABLE h_b01es012 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es012 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b03es012 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Invent rio de Ötens Acabados"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Invent rio de Ötens Acabados */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Invent rio de Ötens Acabados */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Documento|Ötem|Data' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es012.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es012 ).
       RUN set-position IN h_b01es012 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.86 ) */

       /* Links to SmartBrowser h_b01es012. */
       RUN add-link IN adm-broker-hdl ( h_b01es012 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es012 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es012.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es012 ).
       RUN set-position IN h_b02es012 ( 2.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.86 ) */

       /* Links to SmartBrowser h_b02es012. */
       RUN add-link IN adm-broker-hdl ( h_b02es012 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02es012 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03es012.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03es012 ).
       RUN set-position IN h_b03es012 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.86 ) */

       /* Links to SmartBrowser h_b03es012. */
       RUN add-link IN adm-broker-hdl ( h_b03es012 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b03es012 ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01ES012" "2.04.00.000"}
