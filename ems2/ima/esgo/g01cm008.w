...
&Scoped-Define ENABLED-OBJECTS rt-button i_codigo bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS i_codigo 
...
DEFINE VARIABLE i_codigo AS INTEGER FORMAT "999":U INITIAL 0 
     LABEL "Codigo" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME d-vapara
     i_codigo AT ROW 2 COL 23 COLON-ALIGNED WIDGET-ID 2
...
DO:
FIND cm-descontos NO-LOCK
    WHERE cm-descontos.codigo EQ INPUT FRAME {&FRAME-NAME} i_codigo NO-ERROR.

IF NOT AVAILABLE cm-descontos THEN DO:
    {utp/ut-table.i espec cm-descontos 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(cm-descontos).
END.
...
------------------------------------------------------------------------------*/
  DISPLAY i_codigo 
      WITH FRAME d-vapara.
  ENABLE rt-button i_codigo bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
