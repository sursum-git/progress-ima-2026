...
DEFINE VARIABLE cDocumento AS CHARACTER   NO-UNDO.
UPDATE icliente /* cdocumento*/ .
FIND FIRST ems5.cliente 
    WHERE cliente.cdn_cliente = iCliente NO-LOCK NO-ERROR.
/*IF AVAIL cliente THEN
    DISP cliente.num_pessoa .*/

FOR EACH tit_acr
    where   cdn_cliente     = iCliente 
    AND   cod_espec_docto = 'DM'
    AND   cdn_clien_matriz <> cdn_cliente .
    ASSIGN cdn_clien_matriz  =  cdn_cliente
           num_pessoa = cliente.num_pessoa
           nom_abrev  = cliente.nom_abrev .
