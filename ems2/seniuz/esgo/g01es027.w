...
&Scoped-Define ENABLED-FIELDS ssist-mec.codigo 
&Scoped-define ENABLED-TABLES ssist-mec
&Scoped-define FIRST-ENABLED-TABLE ssist-mec
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS ssist-mec.codigo ssist-mec.descricao 
&Scoped-define DISPLAYED-TABLES ssist-mec
&Scoped-define FIRST-DISPLAYED-TABLE ssist-mec
...
DEFINE FRAME d-vapara
     ssist-mec.codigo AT ROW 1.17 COL 12 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9.14 BY .88
     ssist-mec.descricao AT ROW 1.17 COL 21.14 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 31.14 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Sub-sistemas da Mecƒnica"
...
/* SETTINGS FOR FILL-IN ssist-mec.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para Sub-sistemas da Mecƒnica */
DO:
  find ssist-mec no-lock 
      where ssist-mec.codigo = input frame {&frame-name} ssist-mec.codigo no-error.
  if  not avail ssist-mec then do:
      {utp/ut-table.i espec ssist-mec 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(ssist-mec).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Sub-sistemas da Mecƒnica */
DO:  
...
&Scoped-define SELF-NAME ssist-mec.codigo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL ssist-mec.codigo d-vapara
ON LEAVE OF ssist-mec.codigo IN FRAME d-vapara /* Codigo */
DO:
  FIND ssist-mec WHERE ssist-mec.codigo = INPUT FRAME {&FRAME-NAME} ssist-mec.codigo NO-LOCK NO-ERROR.
  IF AVAIL ssist-mec THEN 
     ASSIGN ssist-mec.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ssist-mec.descricao.
  ELSE DO:
     MESSAGE "Sub-sistema inv lido." VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE ssist-mec THEN 
    DISPLAY ssist-mec.codigo ssist-mec.descricao 
      WITH FRAME d-vapara.
  ENABLE ssist-mec.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
