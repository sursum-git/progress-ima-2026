...
&Scoped-Define ENABLED-FIELDS area-mec.codigo 
...
DEFINE FRAME d-vapara
     area-mec.codigo AT ROW 1.17 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 5.14 BY .88
     area-mec.descricao AT ROW 1.17 COL 14.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 21.14 BY .88
...
/* SETTINGS FOR FILL-IN area-mec.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para µrea da Mecƒnica */
DO:
  find area-mec no-lock 
      where area-mec.codigo = input frame {&frame-name} area-mec.codigo no-error.
  if  not avail area-mec then do:
      {utp/ut-table.i espec area-mec 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(area-mec).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para µrea da Mecƒnica */
DO:  
...
&Scoped-define SELF-NAME area-mec.codigo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL area-mec.codigo d-vapara
ON LEAVE OF area-mec.codigo IN FRAME d-vapara /* Codigo */
DO:
  FIND area-mec WHERE area-mec.codigo = INPUT FRAME {&FRAME-NAME} area-mec.codigo NO-LOCK NO-ERROR.
  IF AVAIL area-mec THEN
     ASSIGN area-mec.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = area-mec.descricao.
  ELSE DO:
     MESSAGE "µrea inv lida." VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE area-mec THEN 
    DISPLAY area-mec.codigo area-mec.descricao 
      WITH FRAME d-vapara.
  ENABLE area-mec.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
