...
*******************************************************************************/
{include/i-prgvrs.i G01AD098 2.04.00.000}
...
DEF NEW GLOBAL SHARED VAR v-nome-abrev LIKE emitente.nome-abrev.
...
&Scoped-Define ENABLED-OBJECTS fi-cliente bt-ok bt-cancela bt-ajuda RECT-1 ~
rt-button 
&Scoped-Define DISPLAYED-OBJECTS fi-cliente 
...
DEFINE FRAME d-vapara
     ...
     bt-ok AT ROW 3.58 COL 2.57
     bt-cancela AT ROW 3.58 COL 13.43
     bt-ajuda AT ROW 3.58 COL 47.86
     RECT-1 AT ROW 1.21 COL 1.72
     rt-button AT ROW 3.33 COL 1.43
     SPACE(0.85) SKIP(0.00)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V† Para Cliente"
...
ON GO OF FRAME d-vapara /* V† Para Cliente */
DO:
  FIND emitente WHERE emitente.nome-abrev = INPUT FRAME {&FRAME-NAME} fi-cliente
                  AND emitente.identific <> 2
                NO-LOCK NO-ERROR.
  IF AVAIL emitente THEN
     ASSIGN p-row-tabela = ROWID(emitente).
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Cliente */
DO:  
...
DO:
  FIND emitente WHERE emitente.nome-abrev =  SELF:SCREEN-VALUE
                  AND emitente.identific  <> 2
                NO-LOCK NO-ERROR.
  IF NOT AVAIL emitente THEN
     FIND emitente WHERE emitente.cod-emitente =  INT(SELF:SCREEN-VALUE) 
                     AND emitente.identific    <> 2 
                   NO-LOCK NO-ERROR.
  IF AVAIL emitente THEN DO:
     IF emitente.cod-rep < i-cod-rep-ini OR
        emitente.cod-rep > i-cod-rep-fin THEN DO:
        MESSAGE "Este cliente n∆o pertence Ö Carteira do Representante " + 
                STRING(i-cod-rep-ini,"9999") VIEW-AS ALERT-BOX.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-cliente 
      WITH FRAME d-vapara.
  ENABLE fi-cliente bt-ok bt-cancela bt-ajuda RECT-1 rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01AD098" "2.04.00.000"}
