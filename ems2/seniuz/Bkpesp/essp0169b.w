...
DEFINE INPUT-OUTPUT PARAMETER i-etiqueta-ini  LIKE ob-etiqueta.num-etiqueta.
DEFINE INPUT-OUTPUT PARAMETER i-etiqueta-fin  LIKE ob-etiqueta.num-etiqueta.
DEFINE INPUT-OUTPUT PARAMETER l-sintetico     AS LOG.
...
&Scoped-Define ENABLED-OBJECTS IMAGE-88 IMAGE-89 RECT-1 RECT-50 ~
fi-num-etiqueta-ini fi-num-etiqueta-fin tg-sintetico bt-ajuda bt-ok ~
bt-cancelar 
...
DEFINE VARIABLE fi-num-etiqueta-fin AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 999999999 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 TOOLTIP "N£mero da Etiqueta Final" NO-UNDO.

DEFINE VARIABLE fi-num-etiqueta-ini AS INTEGER FORMAT ">>>,>>>,>>9" INITIAL 0 
     LABEL "Numero da  Etiqueta" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 TOOLTIP "N£mero da Etiqueta Inicial" NO-UNDO.
...
DEFINE VARIABLE tg-sintetico AS LOGICAL INITIAL yes 
     LABEL "Imprime o Relatorio Sint‚tico" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME F-Main
     fi-num-etiqueta-ini AT ROW 1.75 COL 21.72 COLON-ALIGNED
     fi-num-etiqueta-fin AT ROW 1.75 COL 49 COLON-ALIGNED NO-LABEL
     tg-sintetico AT ROW 3.5 COL 25
     bt-ajuda AT ROW 5.83 COL 59.86
     bt-ok AT ROW 5.88 COL 3.29
     bt-cancelar AT ROW 5.88 COL 14.29
     " Sele‡Æo" VIEW-AS TEXT
          SIZE 7 BY .54 AT ROW 1 COL 4.29
     IMAGE-88 AT ROW 1.75 COL 33.86
     IMAGE-89 AT ROW 1.75 COL 48
     RECT-1 AT ROW 5.67 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-digita ASSIGN
         HIDDEN             = YES
         TITLE              = "Sele‡Æo das Etiquetas do Relat¢rio"
         COLUMN             = 26.43
         ROW                = 9.38
         HEIGHT             = 5.92
         WIDTH              = 70.72
...
ON END-ERROR OF w-digita /* Sele‡Æo das Etiquetas do Relat¢rio */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-digita w-digita
ON WINDOW-CLOSE OF w-digita /* Sele‡Æo das Etiquetas do Relat¢rio */
DO:
...
DO:
    ...
    ASSIGN i-etiqueta-ini  = INPUT FRAME {&FRAME-NAME} fi-num-etiqueta-ini   
         i-etiqueta-fin  = INPUT FRAME {&FRAME-NAME} fi-num-etiqueta-fin   
         l-sintetico     = INPUT FRAME {&FRAME-NAME} tg-sintetico
         l-ok            = YES.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-num-etiqueta-ini fi-num-etiqueta-fin tg-sintetico 
      WITH FRAME F-Main IN WINDOW w-digita.
  ENABLE IMAGE-88 IMAGE-89 RECT-1 RECT-50 fi-num-etiqueta-ini 
         fi-num-etiqueta-fin tg-sintetico bt-ajuda bt-ok bt-cancelar 
      WITH FRAME F-Main IN WINDOW w-digita.
...
------------------------------------------------------------------------------*/
  ...
  /* Code placed here will execute PRIOR to standard behavior. */
  ASSIGN fi-num-etiqueta-ini = i-etiqueta-ini   
         fi-num-etiqueta-fin = i-etiqueta-fin
         tg-sintetico        = l-sintetico.
