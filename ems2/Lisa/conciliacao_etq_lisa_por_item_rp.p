//02/12/2025 - tadeu - retirada do efeito do filtro de referencia
...
RUN esapi/getDadosEtqMed.p('505', // estab. LISA
                           tt-param.itCodigo,
                           '', 
                           INPUT-OUTPUT TABLE ttEtq BY-REFERENCE).  
RUN pi-acompanhar IN h-acomp(INPUT " Buscando Etiquetas API LISA  ").
PUT STREAM str-rp "Inicio busca Etiquetas Lisa:" NOW SKIP.                           
                          
FOR EACH ttEtq BREAK BY ttEtq.itCodigo :
    IF  FIRST-OF(ttEtq.itCodigo) THEN  DO:
       RUN pi-acompanhar IN h-acomp(INPUT "Etg LISA - Item:" + ttEtq.itCodigo  ). 
       RUN lisa/getDadosEtqLisa2.p(ttEtq.itCodigo,             
                                 '', 
                                 NO,
                                 01.01.2001,
                                 01.01.2999,
