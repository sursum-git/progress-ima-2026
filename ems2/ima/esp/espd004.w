...
*******************************************************************************/
{include/i-prgvrs.i espd004 9.99.99.999}
...
DEFINE VARIABLE mesAtual AS CHARACTER   NO-UNDO.
DEFINE VARIABLE anoAtual AS CHARACTER   NO-UNDO.
...
DEFINE VARIABLE c-fim-ano AS INTEGER FORMAT "9999":U INITIAL 9999 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c-fim-mes AS INTEGER FORMAT "99":U INITIAL 12 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c-fim-repres AS INTEGER FORMAT "99999":U INITIAL 99999 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c-ini-ano AS INTEGER FORMAT "9999":U INITIAL 2000 
     LABEL "Ano" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c-ini-mes AS INTEGER FORMAT "99":U INITIAL 1 
     LABEL "Màs" 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.

DEFINE VARIABLE c-ini-repres AS INTEGER FORMAT "99999":U INITIAL 0 
     LABEL "Repres." 
     VIEW-AS FILL-IN 
     SIZE 15 BY .88 NO-UNDO.
...
DEFINE VARIABLE tgMesCorrente AS LOGICAL INITIAL yes 
     LABEL "Màs Corrente" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME f-pg-sel
     c-ini-mes AT ROW 4.83 COL 15 COLON-ALIGNED
     c-fim-mes AT ROW 4.83 COL 48.14 COLON-ALIGNED NO-LABEL
     c-ini-ano AT ROW 6.33 COL 15 COLON-ALIGNED WIDGET-ID 8
     c-fim-ano AT ROW 6.33 COL 48.14 COLON-ALIGNED NO-LABEL WIDGET-ID 6
     c-ini-repres AT ROW 7.71 COL 15 COLON-ALIGNED WIDGET-ID 16
     c-fim-repres AT ROW 7.71 COL 48.14 COLON-ALIGNED NO-LABEL WIDGET-ID 14
     tgMesCorrente AT ROW 9.79 COL 17.14 WIDGET-ID 22
     IMAGE-1 AT ROW 4.83 COL 31.86
     IMAGE-2 AT ROW 4.83 COL 47.29
     IMAGE-3 AT ROW 6.38 COL 31.86 WIDGET-ID 10
     IMAGE-4 AT ROW 6.38 COL 47.29 WIDGET-ID 12
     IMAGE-5 AT ROW 7.75 COL 31.86 WIDGET-ID 18
     IMAGE-6 AT ROW 7.75 COL 47.29 WIDGET-ID 20
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
/* SETTINGS FOR FRAME f-pg-sel
                                                                        */
/* SETTINGS FOR FILL-IN c-fim-ano IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN c-fim-mes IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN c-ini-ano IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN c-ini-mes IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
&Scoped-define SELF-NAME tgMesCorrente
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL tgMesCorrente w-relat
ON VALUE-CHANGED OF tgMesCorrente IN FRAME f-pg-sel /* Màs Corrente */
DO:
  IF SELF:SCREEN-VALUE = 'no' THEN  DO:
  
     ASSIGN  c-ini-mes:SENSITIVE IN FRAME f-pg-sel = YES
             c-fim-mes:SENSITIVE IN FRAME f-pg-sel = YES
             c-ini-ano:SENSITIVE IN FRAME f-pg-sel = YES
             c-fim-ano:SENSITIVE IN FRAME f-pg-sel = YES
             .           
  END.
  ELSE DO:
     ASSIGN  c-ini-mes:SENSITIVE IN FRAME f-pg-sel = NO
             c-fim-mes:SENSITIVE IN FRAME f-pg-sel = NO
             c-ini-ano:SENSITIVE IN FRAME f-pg-sel = NO
             c-fim-ano:SENSITIVE IN FRAME f-pg-sel = NO
             .             
  END.
END.
...
{utp/ut9000.i "espd004" "9.99.99.999"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN mesAtual = string(month(TODAY))
           anoAtual = STRING(YEAR(TODAY))
           c-ini-mes:SCREEN-VALUE IN FRAME f-pg-sel = mesAtual
           c-fim-mes:SCREEN-VALUE IN FRAME f-pg-sel = mesAtual
           c-ini-ano:SCREEN-VALUE IN FRAME f-pg-sel = anoAtual
           c-fim-ano:SCREEN-VALUE IN FRAME f-pg-sel = anoAtual .
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY c-ini-mes c-fim-mes c-ini-ano c-fim-ano c-ini-repres c-fim-repres 
          tgMesCorrente 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-5 IMAGE-6 c-ini-repres 
         c-fim-repres tgMesCorrente 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    create tt-param.
    assign tt-param.mesIni          = INPUT FRAME f-pg-sel c-ini-mes
           tt-param.mesFim          = INPUT FRAME f-pg-sel c-fim-mes
           tt-param.AnoINI          = INPUT FRAME f-pg-sel c-ini-ano
           tt-param.AnoFim          = INPUT FRAME f-pg-sel c-fim-ano
           tt-param.codRepIni       = INPUT FRAME f-pg-sel c-ini-repres
           tt-param.codRepFim       = INPUT FRAME f-pg-sel c-fim-repres
    ...
    IF tgMesCorrente:SCREEN-VALUE IN FRAME f-pg-sel = 'yes' THEN  DO:
       ASSIGN tt-param.mesIni   = MONTH(TODAY)
              tt-param.mesFim   = MONTH(TODAY)
              tt-param.anoIni   = YEAR(TODAY)
              tt-param.anoFim   = YEAR(TODAY)  .    
    ...
    {include/i-rprun.i esrp/espd004rp.p}
