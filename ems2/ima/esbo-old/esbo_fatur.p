...
PROCEDURE _getDescTbPreco:
    ...
    FIND FIRST tbs_preco
         WHERE tbs_preco.tb_preco_id = pTbPrecoId NO-LOCK NO-ERROR.
...
PROCEDURE _getPercComisReppri:
    ...
    FIND FIRST ped-repre OF ped-venda 
         WHERE ped-repre.nome-ab-rep = ped-venda.no-ab-reppri NO-LOCK NO-ERROR .
...
RUN pi-inicializar IN h-acomp(INPUT "Buscando NFs Venda").
 ...
 FOR EACH nota-fiscal 
         ...
         NO-LOCK ,FIRST natur-oper OF nota-fiscal NO-LOCK
...
PROCEDURE buscarPrecosItem.
     ...
     FIND LAST ITEM_custos
          WHERE ITEM_custos.it_codigo = pItCodigo
          AND   date(ITEM_custos.dt_hr_custo) <= pDtRefer NO-LOCK NO-ERROR.

     IF AVAIL ITEM_custos THEN
     ...
     IF dVlDolar > 0 AND dVlReal = 0 THEN DO:
       FIND LAST hist_cotacoes_bc NO-LOCK
          WHERE hist_cotacoes_bc.data <= pDtRefer NO-ERROR.
