...
*******************************************************************************/
{include/i-prgvrs.i ESPD0023 2.04.00.000}
...
DEFINE VARIABLE h_q02di159 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v03di159 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Pedidos de Venda - Tipo de Pedido"
         HEIGHT             = 6.04
         WIDTH              = 89.29
         MAX-HEIGHT         = 27.79
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsi2 /* Manuten‡Æo Pedidos de Venda - Tipo de Pedido */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Manuten‡Æo Pedidos de Venda - Tipo de Pedido */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v03di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v03di159 ).
       RUN set-position IN h_v03di159 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q02di159.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom/z01di159.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q02di159 ).
       RUN set-position IN h_q02di159 ( 1.50 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v03di159. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v03di159 ).
       RUN add-link IN adm-broker-hdl ( h_q02di159 , 'Record':U , h_v03di159 ).

       /* Links to SmartQuery h_q02di159. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_q02di159 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q02di159 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q02di159 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q02di159 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v03di159 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESPD0023" "2.04.00.000"}
