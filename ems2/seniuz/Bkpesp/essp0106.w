...
*******************************************************************************/
{include/i-prgvrs.i ESSP0106 2.04.00.000}
...
DEFINE VARIABLE h_b01es048 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b01es049 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q03es047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v03es047 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v04es047 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Revisäes de OB's"
         HEIGHT             = 16.96
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-concom /* Consulta Revisäes de OB's */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Consulta Revisäes de OB's */
DO:
...
DO:
  RUN esdlg/d02es047.w.
  RUN adm-open-query-cases IN h_q03es047.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v03es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v03es047 ).
       RUN set-position IN h_v03es047 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 5.50 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Dados|Etiquetas|Acondicion.' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q03es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es047.w,
                     ProgVaPara = esgo/g01es047.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q03es047 ).
       RUN set-position IN h_q03es047 ( 1.50 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v03es047. */
       RUN add-link IN adm-broker-hdl ( h_q03es047 , 'Record':U , h_v03es047 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q03es047. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q03es047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q03es047 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q03es047 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v03es047 ,
             BUTTON-3:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v03es047 , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v04es047.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v04es047 ).
       RUN set-position IN h_v04es047 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.92 , 87.00 ) */

       /* Links to SmartViewer h_v04es047. */
       RUN add-link IN adm-broker-hdl ( h_q03es047 , 'Record':U , h_v04es047 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v04es047 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es049 ).
       RUN set-position IN h_b01es049 ( 9.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.63 , 87.00 ) */

       /* Links to SmartBrowser h_b01es049. */
       RUN add-link IN adm-broker-hdl ( h_q03es047 , 'Record':U , h_b01es049 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es049 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es048.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es048 ).
       RUN set-position IN h_b01es048 ( 9.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.50 , 88.00 ) */

       /* Links to SmartBrowser h_b01es048. */
       RUN add-link IN adm-broker-hdl ( h_q03es047 , 'Record':U , h_b01es048 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es048 ,
             h_folder , 'AFTER':U ).
    END. /* Page 3 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0106" "2.04.00.000"}
