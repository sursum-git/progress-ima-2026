...
&Scoped-Define ENABLED-FIELDS teste-prod.num-of teste-prod.data-teste ~
teste-prod.num-seq 
...
DEFINE FRAME d-vapara
     teste-prod.num-of AT ROW 1.17 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 16.14 BY .88
     teste-prod.data-teste AT ROW 2.17 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     teste-prod.num-seq AT ROW 3 COL 17 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 2.29 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Testes de Produtos"
...
ON GO OF FRAME d-vapara /* V  Para Testes de Produtos */
DO:
  find teste-prod no-lock 
      where teste-prod.num-of     = input frame {&frame-name} teste-prod.num-of
        ...
        AND teste-prod.num-seq    = INPUT FRAME {&FRAME-NAME} teste-prod.num-seq
       no-error.
  if  not avail teste-prod then do:
      {utp/ut-table.i espec teste-prod 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(teste-prod).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Testes de Produtos */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE teste-prod THEN 
    DISPLAY teste-prod.num-of teste-prod.data-teste teste-prod.num-seq 
      WITH FRAME d-vapara.
  ENABLE teste-prod.num-of teste-prod.data-teste teste-prod.num-seq bt-ok 
