...
*******************************************************************************/
{include/i-prgvrs.i G01ES065 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS fi-codigo bt-ok bt-cancela bt-ajuda ~
rt-button 
...
DEFINE VARIABLE fi-codigo AS CHARACTER FORMAT "!" 
     LABEL "C¢digo" 
     VIEW-AS FILL-IN 
     SIZE 3 BY .79 TOOLTIP "C¢digo do Corte Comercial - Letra: A a Z." NO-UNDO.
...
DEFINE FRAME d-vapara
     fi-codigo AT ROW 1.54 COL 18.72 COLON-ALIGNED
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         FONT 1
         TITLE "V  Para Cortes Comerciais"
...
ON GO OF FRAME d-vapara /* V  Para Cortes Comerciais */
DO:
  find corte-comerc no-lock 
      where corte-comerc.codigo = input frame {&frame-name} fi-codigo no-error.
  if  not avail corte-comerc then do:
      {utp/ut-table.i espec corte-comerc 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(corte-comerc).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Cortes Comerciais */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-codigo 
      WITH FRAME d-vapara.
  ENABLE fi-codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ES065" "2.04.00.000"}
