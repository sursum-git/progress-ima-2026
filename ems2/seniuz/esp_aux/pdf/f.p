...
*/
 FOR EACH pedido-compr WHERE  
          pedido-compr.num-pedido >= 36500 AND
          pedido-compr.num-pedido <= 36550 NO-LOCK.
...
/* RUN pdf_close("Spdf"). /* Fechar Arquivo pdf */ */
 ...
 PROCEDURE pi-imp-linha.
     ...
     RUN pdf_text_xy  IN h_PDFinc ("Spdf", prazo-compra.quantidade,380,i-lin)  .
     ...
     RUN pdf_text_xy  IN h_PDFinc ("Spdf", (STRING(((prazo-compra.quantidade * ordem-compra.preco-fornec) +                                           
                                           ((prazo-compra.quantidade * ordem-compra.preco-fornec) * ordem-compra.aliquota-ipi * 0.01) +              
                                           ((prazo-compra.quantidade * ordem-compra.preco-fornec) * ordem-compra.valor-taxa * 0.01) -                
                                           ((prazo-compra.quantidade * ordem-compra.preco-fornec) * ordem-compra.perc-descto * 0.01)),">>>>,>>9.99")),585,i-lin).
     RUN pdf_text_xy  IN h_PDFinc ("Spdf", STRING(prazo-compra.data-entrega, "99/99/9999"),665,i-lin)  .
 ...
 PROCEDURE pi-cabec-form.
   ...
   RUN pdf_text_xy IN h_PDFinc ("Spdf", "INSCRIÄ«O ESTADUAL: " + estabelec.ins-estadual ,165,530).
   ...
   RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica",10).
   RUN pdf_text_xy IN h_PDFinc ("Spdf", "Endereáo: ",28,468).
   RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica", 9).
   ...
   RUN pdf_text_xy IN h_PDFinc ("Spdf", "PEDIDO Nß: ",450,480).
 ...
 PROCEDURE pi-rodape.
   ...
   RUN pdf_text_xy IN h_PDFinc  ("Spdf", STRING(de-total,">>>,>>>,>>9.99"),520,i-lin-item - 15).       
   RUN pdf_text_Color("Spdf",0,0,0).
   RUN pdf_text_xy IN h_PDFinc ("Spdf", "PAGAMENTO",460,i-lin-item - 33).
   RUN pdf_text_xy IN h_PDFinc  ("Spdf", IF AVAIL cond-pagto      
                                         THEN cond-pagto.descricao ELSE "",533,i-lin-item - 33).
   ...
   RUN pdf_text_xy IN h_PDFinc  ("Spdf", IF AVAIL transporte          
                                         THEN TRIM(transporte.nome) + " / " + transporte.telefone ELSE "" ,600,i-lin-item - 93).
   RUN pi-desenha-linha (595,i-lin-item - 60,595,i-lin-item - 100,1). /* Traáo Vertical */ 
   RUN pi-desenha-linha ( 15, 13,250,13,1). /* Traáo Horizontal */
   RUN pdf_set_font IN h_PDFinc ("Spdf", "Helvetica",10).
   RUN pdf_text_xy IN h_PDFinc ("Spdf", "Gerància de Compras",75,3).
