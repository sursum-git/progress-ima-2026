...
*******************************************************************************/
{include/i-prgvrs.i impd026A 2.02.00.001}
...
DEFINE VARIABLE h_q02im026 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02im026 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     bt-ok AT ROW 8.92 COL 2.14 HELP
          "Salva e sai"
     bt-save AT ROW 8.92 COL 13.14 HELP
          "Salva e cria novo registro"
     bt-cancela AT ROW 8.92 COL 24.14 HELP
          "Cancela"
     bt-ajuda AT ROW 8.92 COL 79.14
     rt-button AT ROW 8.71 COL 1
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-incmdp ASSIGN
         HIDDEN             = YES
         TITLE              = "Inclui/Modifica Tabela de Pre‡o"
         HEIGHT             = 9.29
         WIDTH              = 90
         MAX-HEIGHT         = 22
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-incmdp /* Inclui/Modifica Tabela de Pre‡o */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-incmdp w-incmdp
ON WINDOW-CLOSE OF w-incmdp /* Inclui/Modifica Tabela de Pre‡o */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imvwr/v02im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02im026 ).
       RUN set-position IN h_v02im026 ( 1.25 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 7.25 , 89.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'imqry/q02im026.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom\z01di163.w,
                     ProgVaPara = ,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q02im026 ).
       RUN set-position IN h_q02im026 ( 8.63 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02im026. */
       RUN add-link IN adm-broker-hdl ( h_q02im026 , 'Record':U , h_v02im026 ).
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'TableIO':U , h_v02im026 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02im026 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "impf026A" "2.04.00.001"}
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
  RUN pi-reposiciona-query IN h_q02im026 (INPUT v-row-table). 
