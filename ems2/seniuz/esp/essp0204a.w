...
DEF INPUT-OUTPUT PARAMETER c-nr-pedcli-ini    LIKE ped-item.nr-pedcli.
DEF INPUT-OUTPUT PARAMETER c-nr-pedcli-fin    LIKE ped-item.nr-pedcli.
...
IMAGE-82 RECT-1 RECT-49 IMAGE-3 IMAGE-4 fi-cod-estabel fi-nr-pedcli-ini ~
fi-nr-pedcli-fin fi-it-codigo-ini fi-it-codigo-fin fi-cod-refer-ini ~
fi-cod-refer-fin fi-cod-qualid-ini fi-cod-qualid-fin fi-cod-depos ~
tg-lote-todos bt-ok bt-cancelar bt-ajuda 
...
DEFINE FRAME F-Main
     ...
     fi-nr-pedcli-ini AT ROW 3 COL 15.43 COLON-ALIGNED HELP
          "N£mero do pedido do cliente" WIDGET-ID 16
     fi-nr-pedcli-fin AT ROW 3 COL 45.43 COLON-ALIGNED HELP
          "N£mero do pedido do cliente" NO-LABEL WIDGET-ID 14
     ...
     fi-cod-depos AT ROW 7 COL 15.43 COLON-ALIGNED HELP
          "C¢digo do Dep¢sito"
     tg-lote-todos AT ROW 8 COL 17.57
     tg-lote-pp AT ROW 8 COL 29.43
     tg-lote-pd AT ROW 8 COL 35.43
     tg-lote-rp AT ROW 8 COL 41.43
     tg-lote-rd AT ROW 8 COL 47.43
     tg-lote-sc AT ROW 8 COL 53.43
     tg-lote-ca AT ROW 8 COL 59.43
     bt-ok AT ROW 9.33 COL 3
     bt-cancelar AT ROW 9.33 COL 14
     bt-ajuda AT ROW 9.33 COL 58.14
     " Sele‡Æo" VIEW-AS TEXT
          SIZE 8.72 BY .54 AT ROW 1.17 COL 4.29
          FGCOLOR 9 FONT 6
     "Lote:" VIEW-AS TEXT
          SIZE 3.57 BY .54 AT ROW 8.13 COL 13.57
     ...
     RECT-1 AT ROW 9.13 COL 2
     RECT-49 AT ROW 1.46 COL 2.14
     IMAGE-3 AT ROW 3 COL 34.43 WIDGET-ID 18
     IMAGE-4 AT ROW 3 COL 43.43 WIDGET-ID 20
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-window ASSIGN
         HIDDEN             = YES
         TITLE              = "Parƒmetros parra Sele‡Æo de Pedidos - ESSP0204a"
         HEIGHT             = 9.67
         WIDTH              = 68.72
         MAX-HEIGHT         = 29.13
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-window /* Parƒmetros parra Sele‡Æo de Pedidos - ESSP0204a */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-window w-window
ON WINDOW-CLOSE OF w-window /* Parƒmetros parra Sele‡Æo de Pedidos - ESSP0204a */
DO:
...
DO:
    ...
    ASSIGN fi-cod-depos:SCREEN-VALUE = 'ARM'.

    IF AVAIL saldo-estoq THEN
       ASSIGN fi-cod-depos:SCREEN-VALUE = saldo-estoq.cod-depos.
END.
...
DO:
  IF INPUT FRAME {&FRAME-NAME} tg-lote-todos = NO THEN DO.
     ...
     ASSIGN tg-lote-pp:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(l-lote-pp)
            ...
            tg-lote-sc:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(l-lote-sc)
            tg-lote-ca:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(l-lote-ca).
  END.
  ELSE DO.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-cod-estabel fi-nome-estabel fi-nr-pedcli-ini fi-nr-pedcli-fin 
          fi-it-codigo-ini fi-it-codigo-fin fi-cod-refer-ini fi-cod-refer-fin 
          fi-cod-qualid-ini fi-cod-qualid-fin fi-cod-depos tg-lote-todos 
          tg-lote-pp tg-lote-pd tg-lote-rp tg-lote-rd tg-lote-sc tg-lote-ca 
      WITH FRAME F-Main IN WINDOW w-window.
  ENABLE IMAGE-2 IMAGE-51 IMAGE-52 IMAGE-73 IMAGE-75 IMAGE-82 RECT-1 RECT-49 
         IMAGE-3 IMAGE-4 fi-cod-estabel fi-nr-pedcli-ini fi-nr-pedcli-fin 
         fi-it-codigo-ini fi-it-codigo-fin fi-cod-refer-ini fi-cod-refer-fin 
         fi-cod-qualid-ini fi-cod-qualid-fin fi-cod-depos tg-lote-todos bt-ok 
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN fi-cod-estabel      = c-cod-estabel
         fi-nr-pedcli-ini    = c-it-codigo-ini 
         fi-nr-pedcli-fin    = c-it-codigo-fin 
