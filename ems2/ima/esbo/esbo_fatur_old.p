...
RUN pi-inicializar IN h-acomp("Buscando Notas").
 ...
 FOR EACH nota-fiscal USE-INDEX ch-distancia WHERE      
     ...
     FIND natur-oper OF nota-fiscal NO-LOCK
          WHERE natur-oper.tp-rec-desp = 1 
           AND  natur-oper.tipo-compra <> 3 NO-ERROR.
     ...
     FOR EACH it-nota-fisc OF nota-fiscal NO-LOCK.
         ...
         IF nota-fiscal.nr-pedcli <> "" THEN DO:            
            ...
            FIND pr-it-per WHERE                                    
                 ...
                 pr-it-per.periodo     = da-dt-emis-nota-fin  NO-LOCK NO-ERROR.
        ...
        ASSIGN 
               ...
               tt-fatur.cod_estabel     = it-nota-fisc.cod-estabel
               tt-fatur.dt_emis         = it-nota-fisc.dt-emis-nota
               tt-fatur.cod_cli         = nota-fiscal.cod-emitente    
               tt-fatur.nome_abrev      = it-nota-fisc.nome-ab-cli
               ...
               tt-fatur.vl_tot_it_nf    = IF (1 - percDescPed) > 0 THEN  it-nota-fisc.vl-tot-item / (1 - percDescPed) ELSE 0
        ...
        RUN buscarPrecosItem (it-nota-fisc.it-codigo,  IF AVAIL ped-venda THEN ped-venda.dt-implant ELSE tt-fatur.dt_emis,  OUTPUT tt-fatur.preco_tab,
             OUTPUT tt-fatur.preco_out_atual , OUTPUT tt-fatur.preco_out_epoca).
    ...
    IF AVAIL emitente THEN DO:
       ...
       IF AVAIL ext-emitente THEN DO:
           ...
           FIND ramo-ativ WHERE
            ramo-ativ.cod-ramo-ativ = iCodRamoAtiv NO-LOCK NO-ERROR.
       ...
       FIND emitente_cnae WHERE
            emitente_cnae.cod_emitente = iCliente AND 
            emitente_cnae.cod_tipo_cnae = 1 NO-LOCK NO-ERROR.
       ...
       FIND cnaes WHERE
            cnaes.cod_cnae = codCNAE NO-LOCK NO-ERROR.
...
PROCEDURE buscarDadosRepres.
    ...
    FIND FIRST repres WHERE
            repres.cod-rep = iRepres NO-LOCK NO-ERROR.
    IF AVAIL repres THEN DO:
       ...
       FIND FIRST repres_financ WHERE
                  repres_financ.cdn_repres = iRepres AND
                  repres_financ.cod_empresa = "500".
...
PROCEDURE buscarDadosItem.
    ...
    DEFINE OUTPUT  PARAMETER itDescFamComer  LIKE fam-comerc.descricao  NO-UNDO.
        ...
        FIND ITEM WHERE
             ITEM.it-codigo = cItem NO-LOCK NO-ERROR.
        IF AVAIL ITEM THEN
        ...
        FIND grup-estoque WHERE
             grup-estoque.ge-codigo = grupoItem NO-LOCK NO-ERROR.
        ...
        FIND familia WHERE
             familia.fm-codigo = itFamilia NO-LOCK NO-ERROR.
        ...
        FIND fam-comerc WHERE
            fam-comerc.fm-cod-com = itfamComer NO-LOCK NO-ERROR.
...
PROCEDURE buscarPrecosItem.
     ...
     DEFINE INPUT PARAMETER cPrecoItem     LIKE ITEM.it-codigo NO-UNDO.
     DEFINE INPUT  PARAMETER dtReferencia  AS DATE        NO-UNDO.
    ...
    IF dtReferencia = ?  THEN
       ASSIGN dtReferencia = TODAY.

     FIND ITEM WHERE
             ITEM.it-codigo = cPrecoItem NO-LOCK NO-ERROR.

     FIND im-param WHERE
          im-param.cod-param = "tabela_med" NO-LOCK NO-ERROR.
     ASSIGN tabPreco = im-param.val-param.

     FIND FIRST  preco-item WHERE
          preco-item.it-codigo = cPrecoItem
          AND preco-item.nr-tabpre = tabPreco NO-LOCK NO-ERROR.

     ASSIGN precoTabItem = IF AVAIL preco-item THEN preco-item.preco-venda ELSE 0.

     FIND LAST liquida-ima WHERE
               liquida-ima.it-codigo = cPrecoItem
               AND liquida-ima.dt-inicio <= TODAY
               AND (liquida-ima.dt-final  >= TODAY OR liquida-ima.dt-final = ?)
               NO-LOCK NO-ERROR. 

     ASSIGN PrecoOutItem = IF AVAIL liquida-ima THEN liquida-ima.preco-tab ELSE 0.

     FIND LAST liquida-ima WHERE
               liquida-ima.it-codigo = cPrecoItem
               AND liquida-ima.dt-inicio <= dtReferencia
               AND (liquida-ima.dt-final  >= dtReferencia OR liquida-ima.dt-final = ?)
               NO-LOCK NO-ERROR. 
     ASSIGN precoOutEpoca = IF AVAIL liquida-ima THEN liquida-ima.preco-tab ELSE 0.
...
PROCEDURE exportarTtFatur.
    ...
    PUT " Nome Rep Completo | Ativo | Tipo Rep | Comissao | Cod ITEM | DESC ITEM | Grupo ITEM | DESC Grupo ITEM |ITEM Familia | Descr Familia | Preco TAB ITEM | Preco Pi Atual |".
    ...
    PUT  "|% Desc| Descri‡Æo Faixa| Descri‡Æo Cond.Pagto | Cod.Fam. Comercial | Descri‡Æo  Fam. Comercial| Vl.12|%12|Descri‡Æo RegiÆo Gerencial REPRES|Coligada  " skip.
...
PROCEDURE buscarDevolucao.
 ...
 //DEFINE BUFFER nfEmitNota FOR emitente .
    ...
    FOR EACH docum-est WHERE             
             ...
             docum-est.dt-trans <= da-dt-emis-nota-fin  NO-LOCK.
        ...
        FOR EACH item-doc-est OF docum-est NO-LOCK.    
            ...
            ASSIGN 
                ...
                tt-fatur.qt_nf           = ITEM-doc-est.quantidade
           ...
           RUN buscarPrecosItem (item-doc-est.it-codigo, IF AVAIL ped-venda THEN ped-venda.dt-implant ELSE dEmisNF,   OUTPUT tt-fatur.preco_tab, OUTPUT tt-fatur.preco_out_atual,
