...
DEFINE VARIABLE logMaxFreteParcial AS LOGICAL     NO-UNDO.



FUNCTION getMaxFreteCifParcial RETURNS LOGICAL(codEstabel AS CHAR, estado AS CHAR):


    DEFINE VARIABLE cListaUfs501 AS CHARACTER   NO-UNDO.
    DEFINE VARIABLE cListaUfs505 AS CHARACTER   NO-UNDO.

    ASSIGN cListaUfs501 = 'TO,MT,MS,AC,AP,AM,PA,RO,RR'
           cListaUfs505 = 'AC,AL,AP,AM,BA,CE,DF,MA,MT,MS,GO,PA,PB,PE,PI,RN,RO,RR,SE,TO'
          .

    CASE codEstabel:
        WHEN '501' THEN DO:
            IF LOOKUP(estado,cListaUfs501) > 0 THEN
               RETURN YES.
        END.

        WHEN '505' THEN DO:
            IF LOOKUP(estado,cListaUfs505) > 0 THEN
               RETURN YES.
        END.
...
OUTPUT TO c:\temp\ped-venda-rubi-v3.txt NO-CONVERT.
PUT "Estab.|Dt.Implant|Nr.Pedido|Repres.|Nome Abrev.Cliente|Vl.Tot.Pedido|UF|Tp.Frete|% Comis.|Situa‡Æo|M‚dia Prazo|Moeda|Pre‡o Abaixo Tb.|% Tb.|Tot.Informado|Tot.Tabela|Errado?|Cond.Pagto|Id Pre‡o Diferente?|Valor Frete|Max.Frete Parcial" SKIP.
...
FOR EACH nota-fiscal NO-LOCK,
    ...
    AND   nota-fiscal.dt-emis-nota >= 09.01.2023
    AND   nota-fiscal.dt-emis-nota <= 09.30.2023 
    ...
    FOR EACH ped-item OF ped-venda
      ...
      /*MESSAGE dVlReal SKIP
              ped-item.vl-preuni SKIP
        ...
        IF ped-venda.mo-codigo = 0 THEN DO:
           ASSIGN dVlUnit = dvlReal.  
        END.
        ELSE DO:
           ASSIGN dVlUnit = dvlDolar.
        END.
...
/*         IF ped-venda.nr-pedido = 315530 THEN             */
    ...
    ASSIGN logMaxFreteParcial = getMaxFreteCifParcial(ped-venda.cod-estabel,ped-venda.estado).

    IF ped-repre.perc-comis = 3 AND logmaxFreteParcial AND 
       (ped-venda-ext.tp-frete = 'CIF' OR
        ped-venda-ext.tp-frete = 'CIF TOTAL' OR 
        ped-venda-ext.tp-frete = 'CIF DESTAQUE NF' ) AND
        (ped-venda.val-frete = 0 OR percTb < 1.03) THEN
        ASSIGN lErrado = YES.
    ...
    EXPORT DELIMITER "|" 
        ...
        idPreco <>  ped-item-ext.cod_controle_preco
        ped-venda.val-frete
        logMaxFreteParcial
