...
*******************************************************************************/
{include/i-prgvrs.i B01IN172 2.04.00.000}
...
&Scoped-define QUERY-STRING-br-table FOR EACH item WHERE ~{&KEY-PHRASE} ~
      AND item.it-codigo >= fi-ini-it-codigo ~
 AND item.it-codigo <= fi-fin-it-codigo NO-LOCK ~
...
&Scoped-Define ENABLED-OBJECTS fi-ini-it-codigo fi-fin-it-codigo ~
...
RUN set-attribute-list (
    'Keys-Accepted = "class-fiscal,cod-comprado,cod-estabel,fm-cod-com,ge-codigo,nr-linha,un,nat-despesa,cod-servico",
     Keys-Supplied = "class-fiscal,cod-comprado,cod-estabel,fm-cod-com,ge-codigo,nr-linha,un,nat-despesa,cod-servico,fm-codigo,it-codigo,ct-codigo,cd-planejado,cod-refer,sc-codigo,cd-tag,cod-tax"':U).
...
DEFINE VARIABLE fi-fin-it-codigo AS CHARACTER FORMAT "x(16)" INITIAL "5ZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 18.57 BY .88 TOOLTIP "C¢digo do item final."
     FONT 1 NO-UNDO.

DEFINE VARIABLE fi-ini-it-codigo AS CHARACTER FORMAT "x(16)" INITIAL "5" 
     VIEW-AS FILL-IN 
     SIZE 10.14 BY .88 TOOLTIP "C¢digo do item inicial."
     FONT 1 NO-UNDO.
...
DEFINE FRAME F-Main
     fi-ini-it-codigo AT ROW 1 COL 2.43 COLON-ALIGNED HELP
          "C¢digo do Item" NO-LABEL
     fi-fin-it-codigo AT ROW 1 COL 42.43 COLON-ALIGNED HELP
          "C¢digo do Item" NO-LABEL
...
/* Query rebuild information for BROWSE br-table
     ...
     _OrdList          = "mgcad.item.it-codigo|yes"
     _Where[1]         = "mgcad.item.it-codigo >= fi-ini-it-codigo
 AND mgcad.item.it-codigo <= fi-fin-it-codigo"
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-fin-it-codigo B-table-Win
ON LEAVE OF fi-fin-it-codigo IN FRAME F-Main
DO:
  IF INPUT FRAME {&FRAME-NAME} fi-fin-it-codigo < "5" THEN
     ASSIGN fi-fin-it-codigo:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "5".

  IF INPUT FRAME {&FRAME-NAME} fi-fin-it-codigo > "5ZZZZZZZZZZZZZZZ" THEN
...
ON LEAVE OF fi-ini-it-codigo IN FRAME F-Main
DO:
  IF INPUT FRAME {&FRAME-NAME} fi-ini-it-codigo < "5" THEN
     ASSIGN fi-ini-it-codigo:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "5".

  IF INPUT FRAME {&FRAME-NAME} fi-ini-it-codigo > "5ZZZZZZZZZZZZZZZ" THEN
     ASSIGN fi-ini-it-codigo:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "5ZZZZZZZZZZZZZZZ".
END.
