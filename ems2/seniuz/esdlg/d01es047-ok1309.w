...
fi-qtd-def6 fi-descricao tg-emb-neutra tg-finaliza c-hora 
...
DEFINE FRAME Dialog-Frame
     ...
     "Qtde Cortes:" VIEW-AS TEXT
          ...
          FONT 10
     "Carro:" VIEW-AS TEXT
          SIZE 12 BY 1.25 AT ROW 1.75 COL 95.86
          FONT 10
...
DEFINE FRAME Dialog-Frame
     ...
     "Revisadeira:" VIEW-AS TEXT
          SIZE 23 BY 1.25 AT ROW 3.38 COL 84.57
          FONT 10
     ...
     " Defeitos" VIEW-AS TEXT
          SIZE 17 BY 1.25 AT ROW 6.25 COL 91.43
          BGCOLOR 8 FONT 10
     "CLASSE" VIEW-AS TEXT
...
DO:
   DO TRANSACTION:
      ...
      FIND ob-acondic WHERE ob-acondic.nr-ob    = ordem-benefic.nr-ob
                        AND ob-acondic.dt-ob    = ordem-benefic.dt-ob
                        AND ob-acondic.nr-carro = ordem-benefic.nr-carro
                        AND ob-acondic.acondic  = ob-etiqueta.acondic 
                      NO-LOCK NO-ERROR.
      ...
      IF ob-etiqueta.tipo-ordem = 1 THEN DO.
         ...
         IF NOT AVAIL mov-est-acbm THEN DO.
            CREATE mov-est-acbm.
            ASSIGN mov-est-acbm.data-mov = ob-etiqueta.dt-emissao
...
OCXFile = SEARCH( "d01es047-ok1309.wrx":U ).
IF OCXFile = ? THEN
...
END.
ELSE MESSAGE "d01es047-ok1309.wrx":U SKIP(1)
