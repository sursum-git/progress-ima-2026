...
//RUN incrValor(INPUT-OUTPUT cListaTabelas,'repres',',').
RUN incrValor(INPUT-OUTPUT cListaTabelas,'natur-oper',',').
...
RUN setTipoRetCpExtent IN hBo('separado'). 
RUN setTabelasDireto        IN Hbo(INPUT TABLE ttTbsDireto).
RUN getDadosTbsDireto       IN hBo. 
RUN setCaracterSubst        IN hbo("-","_").
RUN gerarDDlPorBancoDados   IN hBo('postgresql',
                                   "c:\temp\importante\03def.sql",
                                   FALSE).
