...
DEF VAR de-qt-ajuste AS DEC FORMAT "->>>,>>9.99".
...
&Scoped-Define ENABLED-OBJECTS RECT-10 rt-button rs-reservas bt-param ~
br-pedidos br-itens ed-obs br-etq-estoque br-etq-reservadas bt-leitor ~
bt-coletor bt-add bt-del 
&Scoped-Define DISPLAYED-OBJECTS rs-reservas ed-obs fi-tot-estoque ~
fi-tot-reservado fi-qt-pedida 

/* Custom List Definitions                                              */
/* List-1,List-2,List-3,List-4,List-5,List-6                            */
&Scoped-define List-4 bt-log bt-fardo bt-vapara bt-modifica bt-consulta ~
bt-imprime 
...
DEFINE BUTTON bt-fardo AUTO-GO 
     ...
     LABEL "" 
     SIZE 5 BY 1.25 TOOLTIP "Alteraá‰es Fardos"
     BGCOLOR 8 FONT 10.
...
DEFINE BUTTON bt-log AUTO-GO 
     ...
     LABEL "" 
     SIZE 5 BY 1.25 TOOLTIP "Log de Alteraá‰es do Pedido"
     BGCOLOR 8 FONT 10.
...
DEFINE FRAME f-cad
     ...
     bt-vapara AT ROW 18.33 COL 3.14
     bt-modifica AT ROW 18.33 COL 8.29
     bt-consulta AT ROW 18.33 COL 13.43
     bt-imprime AT ROW 18.33 COL 18.72
...
/* SETTINGS FOR FRAME f-cad
   FRAME-NAME L-To-R                                                    */
/* BROWSE-TAB br-pedidos bt-fardo f-cad */
...
DO:
   ...
   IF AVAIL tt-etq-estoque THEN DO.
      ...
      IF AVAIL ped-venda-ext AND
         ped-venda-ext.num-reserva <> 0 THEN
         ASSIGN bt-add:SENSITIVE IN FRAME {&FRAME-NAME} = NO
                bt-del:SENSITIVE IN FRAME {&FRAME-NAME} = NO
                bt-leitor:SENSITIVE IN FRAME {&FRAME-NAME} = NO
                bt-coletor:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
...
DO:
  MESSAGE "Deseja Receber Dados do Coletor?" 
   ...
   IF l-receber THEN DO.
      INPUT FROM "M:\ems206\especificos\Seniuz\Coletor\Receber\sep.txt".
      REPEAT.
    ...
    FOR EACH tt-itens-ped NO-LOCK BY tt-itens-ped.nr-sequencia.
        ...
        DO TRANSACTION.
               ...
               CASE tt-etq-reservadas.tp-acao.
                    WHEN 'Inc' THEN DO.
                        ...
                        FIND ped-item-res WHERE
                             ped-item-res.cod-estabel  = tt-ped-venda.cod-estabel  AND  /*  daf  */
                             ped-item-res.nome-abrev   = tt-itens-ped.nome-abrev   AND
                             ped-item-res.nr-pedcli    = tt-itens-ped.nr-pedcli    AND
                             ped-item-res.nr-sequencia = tt-itens-ped.nr-sequencia AND
                             ped-item-res.it-codigo    = tt-itens-ped.it-codigo    AND
                        ...
                        END.
                        ASSIGN ped-item-res.qt-pedida = ped-item-res.qt-pedida + tt-etq-reservadas.quantidade. 
                        ...
                        ASSIGN ped-item-rom.cod-estabel = tt-ped-venda.cod-estabel
                               ...
                               ped-item-rom.quantidade = tt-etq-reservadas.quantidade.

                        FIND ped-item-ext WHERE
                             ped-item-ext.cod-estabel = tt-ped-venda.cod-estabel AND  /* daf */
                    ...
                    WHEN 'Del' THEN DO.
                        ...
                        FIND ped-item-rom WHERE
                             ...
                             ped-item-rom.num-etiqueta = tt-etq-reservadas.num-etiqueta NO-ERROR.
           ...
           IF AVAIL ped-item-res AND
              ...
              IF de-qt-ajuste > 0 THEN DO.  /* Retorna Quantidade para Estoque */
                 FIND CURRENT saldo-estoq SHARE-LOCK NO-ERROR.
                 ASSIGN saldo-estoq.qtidade-atu = saldo-estoq.qtidade-atu - de-qt-ajuste.
                 FIND CURRENT saldo-estoq NO-LOCK NO-ERROR.
              END.

              ASSIGN tt-itens-ped.qt-pedida = ped-item-res.qt-pedida.

              ASSIGN c-desc-reserva = c-desc-reserva + " RESERVADA e Ajustada a Quantidade, Para: " + TRIM(STRING(ped-item-res.qt-pedida,">>>,>>9.99")).
           END.
           ELSE
...
------------------------------------------------------------------------------*/
  ...
  ENABLE RECT-10 rt-button rs-reservas bt-param br-pedidos br-itens ed-obs 
         br-etq-estoque br-etq-reservadas bt-leitor bt-coletor bt-add bt-del 
      WITH FRAME f-cad IN WINDOW w-livre.
