...
def temp-table tt_erro_relatorio_razao no-undo
    field tta_num_lote_ctbl                as integer format ">>>,>>>,>>9" initial 1 label "Lote Contabil" column-label "Lote ContÙbil"
    field tta_num_lancto_ctbl              as integer format ">>,>>>,>>9" initial 10 label "Lan»amento ContÙbil" column-label "Lan?amento ContÙbil"
...
def temp-table tt_erro_relatorio_razao_acum no-undo
    ...
    field tta_num_lancto_ctbl              as integer format ">>,>>>,>>9" initial 10 label "Lan»amento Contabil" column-label "Lan»amento Contabil"
    field tta_num_seq_lancto_ctbl          as integer format ">>>>9" initial 0 label "Sequencia Lancto" column-label "Sequencia Lancto"
    field ttv_des_ajuda                    as character format "x(50)" label "Ajuda" column-label "Ajuda"
...
DEFINE RECTANGLE RECT-15
     ...
     SIZE 72 BY 4.79 TOOLTIP "Par³metros".

DEFINE VARIABLE tg-sdo-apuracao-resultado AS LOGICAL INITIAL yes 
     LABEL "Considera Saldo Apura»’o de Resultados ?" 
     VIEW-AS TOGGLE-BOX
...
DEFINE RECTANGLE rt-folder
     ...
     SIZE 79 BY 11.38 TOOLTIP "Sele»’o"
     FGCOLOR 0 .
...
DEFINE FRAME f-relat
     bt-executar AT ROW 12.75 COL 2 HELP
          "Dispara a execu»’o do relat½rio"
     bt-cancelar AT ROW 12.75 COL 13 HELP
          "Fechar"
     bt-ajuda AT ROW 12.75 COL 69 HELP
          "Ajuda"
...
DEFINE FRAME f-pg-sel
     ...
     contacontabil-ini AT ROW 8.67 COL 17 COLON-ALIGNED HELP
          "C½digo Conta Contÿbil" WIDGET-ID 20
     contacontabil-fim AT ROW 8.67 COL 49.14 COLON-ALIGNED HELP
          "C½digo Conta Contÿbil" NO-LABEL WIDGET-ID 24
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Raz’o por Centro de Custo Excel - espfgl999"
         HEIGHT             = 12.75
         WIDTH              = 79
         MAX-HEIGHT         = 33.29
         MAX-WIDTH          = 273.14
...
ON END-ERROR OF w-relat /* Raz’o por Centro de Custo Excel - espfgl999 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Raz’o por Centro de Custo Excel - espfgl999 */
DO:
...
do on error undo, return error on stop  undo, return error:
   ...
   /* ASSIGN lContinua = YES.*/
    ...
    IF AVAIL ttConta THEN
       MESSAGE 'Existem contas sem Matriz de Tradu»’o Cadastrada para contas externas.' SKIP
     ...
     IF AVAIL tt_erro_relatorio_razao_acum THEN
        MESSAGE 'Existem inconscist¼ncias de saldos.' SKIP
    ...
    END. 
    ELSE
       ASSIGN cArquivoExcel = SESSION:TEMP-DIRECTORY + IF cb-excel:SCREEN-VALUE IN FRAME f-pg-sel = '1' THEN 'razao.xlsx' ELSE 'razaofilial.xlsx'.

    /*tratamento para quando o arquivo de configura»ao estiver em branco */
    ...
    DOS SILENT START excel /n value(cArquivoExcel).
...
/*busca a matriz de tradu»’o da empresa*/
   ...
   IF AVAIL matriz_trad_cta_ctbl_ext THEN DO:
      IF lContinua THEN
         MESSAGE 'posicionei na matriz da conta contabil externa' 
             ...
             VIEW-AS ALERT-BOX INFO BUTTONS OK.
      /*busca a matriz de tradu»’o para o estabelecimento , conta , centro de custo(opcional) do lan»amento*/
...
FOR EACH ITEM_lancto_ctbl NO-LOCK
   ...
   /*  verificar se as contas transitorias farÆo parte da apura‡Æo, pois, estÆo marcadas como apura‡Æo.
   FIND FIRST cta_ctbl
...
/*                                            */
    ...
    IF AVAIL lote_ctbl THEN DO: 
       ...
       IF NOT AVAIL ttLancamentos THEN DO:
          ...
          END.
       END.
       ELSE DO: /*caso exista um lancamento para o mesmo lote, conta contabil, centro de custo e codigo do lancamento, soma o valor do lancamento e preenche campo hist¢rico e sequencias*/
...
FOR EACH EMS5.empresa NO-LOCK:
    ASSIGN cItem = EMS5.empresa.cod_empresa + '-' + EMS5.empresa.nom_abrev + "-" + EMS5.empresa.nom_razao_social + "," + EMS5.empresa.cod_empresa  .
...
FOR EACH ttLancamentos BREAK BY ttLancamentos.centroCusto BY ttLancamentos.contaContabil  BY ttLancamentos.data:
    /*FIND FIRST ttSaldoConta
        ...
        AND   ttSaldoConta.codEstabel    = ttLancamentos.codEst-77777777788888887abel
        NO-LOCK NO-ERROR.
    ...
    END.*/
    IF /*FIRST-OF(ttLancamentos.contaContabil) AND*/ FIRST-OF(ttLancamentos.contaContabil) THEN DO:
       ...
       IF NOT AVAIL ttSaldoContaCC THEN DO:  
           /*MESSAGE 'nÆo achei o registro da ttsaldocontacc'
               VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
...
/* esse bloco foi comentado, pois, ² necessario apenas se for utilizado ordena»’o por conta,por²m, estÿ fora do escopo atual
...
FOR EACH ttLancamentos
    ...
    /*Busca Descri»’o do Centro de Custo*/
    FIND FIRST EMS5.cCusto
        ...
        NO-LOCK NO-ERROR.                                                     
    ASSIGN cDescCentroCusto = IF AVAIL cCusto THEN EMS5.cCusto.des_tit_ctbl ELSE ''.
   ...
   ASSIGN 
          ...
          dSaldoCCEstoque   = IF NOT AVAIL ttEstoque THEN dSaldoCCEstoque + dValorAbsoluto ELSE dSaldoCCEstoque. /*verifica se existe pelo menos um  registro de estoque. caso 
                                exista, n’o  soma o valor total do lan»amento na variavel dContaccEstoq, deixando a soma para o for da tabela ttEstoque */
   /*IF cNaturezaConta = 'CR' THEN 
...
*/
OUTPUT TO value(session:TEMP-DIRECTORY + "profile.txt") APPEND.
EXPORT "|" "Fim Exporta»’o" STRING(TIME,"hh:mm:ss").
OUTPUT CLOSE.
...
DEFINE INPUT  PARAMETER pReferencia     AS CHARACTER   NO-UNDO.
...
FOR EACH movto-estoq NO-LOCK
    ...
    AND   movto-estoq.ct-codigo     = pContaContabil 
    AND   movto-estoq.sc-codigo     = pCentroCusto
    ...
    IF iGrupoItem = INT(pGrupoMov) THEN DO:
        ...
        ASSIGN ttEstoque.rRowidLanc     = pID
               ...
               ttEstoque.valor          = IF movto-estoq.valor-nota = 0 THEN movto-estoq.valor-mat-m[1] + movto-estoq.valor-ggf-m[1] + movto-estoq.valor-mob-m[1] ELSE movto-estoq.valor-nota
