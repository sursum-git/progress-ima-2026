/* Programa: ESSP0044RP.P (Chamado pelo programa ESSP0044.W)
** Objetivo: Imprimir o relat¢rio de Testes de Goma 
**           (tabela: teste-goma)
** Sistema.: Espec¡ficos
** M¢dulo..: Espec¡ficos
...
{include/i-prgvrs.i ESSP0044RP 2.04.00.000}
...
FORM   
   teste-goma.fm-codigo    label "Familia"
   familia.descricao       label "Descricao"
   teste-goma.data-teste   label "Data-Teste"
   teste-goma.cod-maq      label "Maq."
   maq-benef.descricao     label "Descricao"
   teste-goma.num-seq      label "S"
   teste-goma.nome-proc    label "Processo"
   teste-goma.num-ob       label "OB"
   teste-goma.res-esqrd    label "ResEsq"
   teste-goma.res-centro   label "ResCen"
   teste-goma.res-direita  label "ResDir"
...
FORM   
    teste-goma.data-teste  label "Data"
    teste-goma.res-esqrd   label "ResEsq"
    teste-goma.res-centro  label "ResCen"
    teste-goma.res-direita label "ResDir"
    i-cont-dat             label "Tsts"
...
{utp/ut-liter.i Testes_de_Goma * r}
...
if tt-param.classific = 1 then do: /* Por Fam¡lia */
   FOR each teste-goma no-lock
      where teste-goma.fm-codigo   >= tt-param.fi-ini-fm-codigo
        ...
        AND teste-goma.nome-proc   >= tt-param.fi-ini-nome-proc
        AND teste-goma.nome-proc   <= tt-param.fi-fin-nome-proc
        AND teste-goma.cod-maq     >= tt-param.fi-ini-cod-maq   
        AND teste-goma.cod-maq     <= tt-param.fi-fin-cod-maq  
        AND teste-goma.num-ob      >= tt-param.fi-ini-num-ob    
        AND teste-goma.num-ob      <= tt-param.fi-fin-num-ob
        AND teste-goma.res-esqrd   >= tt-param.fi-ini-res-esqrd
        AND teste-goma.res-esqrd   <= tt-param.fi-fin-res-esqrd
        AND teste-goma.res-centro  >= tt-param.fi-ini-res-centro
        AND teste-goma.res-centro  <= tt-param.fi-fin-res-centro
        AND teste-goma.res-direita >= tt-param.fi-ini-res-direita
        AND teste-goma.res-direita <= tt-param.fi-fin-res-direita
      BREAK BY teste-goma.fm-codigo
      on stop undo,leave:
      run pi-acompanhar in h-acomp (input string(teste-goma.fm-codigo)).
      
      assign i-cont-fam     = i-cont-fam + 1
             de-tot-res-esq = de-tot-res-esq + teste-goma.res-esqrd
             de-tot-res-cen = de-tot-res-cen + teste-goma.res-centro
             de-tot-res-dir = de-tot-res-dir + teste-goma.res-direita.

      /* Acumula para Estatisticas */
      assign i-cont-ger = i-cont-ger + 1.
      if  teste-goma.res-esqrd >= tt-param.fi-ini-res-aprov
      and teste-goma.res-esqrd <= tt-param.fi-fin-res-aprov then
          assign i-cont-esq-apr  =  i-cont-esq-apr + 1.
      else
          assign i-cont-esq-rep  =  i-cont-esq-rep + 1.

      if  teste-goma.res-centro >= tt-param.fi-ini-res-aprov
      and teste-goma.res-centro <= tt-param.fi-fin-res-aprov then
          assign i-cont-cen-apr   =  i-cont-cen-apr + 1.
      else
          assign i-cont-cen-rep   =  i-cont-cen-rep + 1.
       
      if  teste-goma.res-direita >= tt-param.fi-ini-res-aprov
      and teste-goma.res-direita <= tt-param.fi-fin-res-aprov then
      ...
      IF tt-param.tipo-relatorio = 1 THEN DO: /* Sint‚tico */
         IF LAST-OF(teste-goma.fm-codigo) THEN DO:
            FIND familia WHERE familia.fm-codigo = teste-goma.fm-codigo
                         NO-LOCK NO-ERROR.
            DISPLAY STREAM str-rp
                    teste-goma.fm-codigo
                    familia.descricao
                    de-tot-res-esq / i-cont-fam @ teste-goma.res-esqrd
                    de-tot-res-cen / i-cont-fam @ teste-goma.res-centro
                    de-tot-res-dir / i-cont-fam @ teste-goma.res-direita
                    i-cont-fam
      ...
      ELSE DO: /*Anal¡tico */
         FIND familia WHERE familia.fm-codigo = teste-goma.fm-codigo
                      NO-LOCK NO-ERROR.
         FIND maq-benef WHERE maq-benef.codigo = teste-goma.cod-maq
                        NO-LOCK NO-ERROR.
         DISPLAY STREAM str-rp
                 teste-goma.fm-codigo WHEN FIRST-OF(teste-goma.fm-codigo)
                 familia.descricao WHEN FIRST-OF(teste-goma.fm-codigo) AND AVAIL familia
   ...
   FOR each teste-goma no-lock
      where teste-goma.fm-codigo   >= tt-param.fi-ini-fm-codigo
        ...
        AND teste-goma.res-direita >= tt-param.fi-ini-res-direita
        AND teste-goma.res-direita <= tt-param.fi-fin-res-direita
   BREAK BY teste-goma.data-teste
   ...
   on stop undo,leave:
      ...
      IF tt-param.tipo-relatorio = 1 THEN do: /* Sint‚tico */
         IF LAST-OF(teste-goma.data-teste) THEN DO:
          DISPLAY STREAM str-rp
                  teste-goma.data-teste
                  de-tot-res-esq / i-cont-dat @ teste-goma.res-esqrd
                  de-tot-res-cen / i-cont-dat @ teste-goma.res-centro
                  de-tot-res-dir / i-cont-dat @ teste-goma.res-direita
                  i-cont-dat
      ...
      ELSE DO: /*Anal¡tico */
          ...
          find maq-benef where maq-benef.codigo = teste-goma.cod-maq
                         no-lock no-error.
          display stream str-rp
                  teste-goma.data-teste WHEN FIRST-OF(teste-goma.data-teste)
                  teste-goma.fm-codigo   
                  familia.descricao       
                  teste-goma.cod-maq      
                  maq-benef.descricao
