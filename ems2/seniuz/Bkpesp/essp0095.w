...
*******************************************************************************/
{include/i-prgvrs.i ESSP0095 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS btn-importa btn-parametro rt-button 
...
DEFINE VARIABLE h_q03di154 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v03di154 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON btn-importa 
     IMAGE-UP FILE "image/emstec.ico":U NO-FOCUS FLAT-BUTTON
     LABEL "Button 1" 
     SIZE 5 BY 1.5 TOOLTIP "Importar Dados do Coletor".

DEFINE BUTTON btn-parametro 
     IMAGE-UP FILE "image/im-param2.bmp":U
     LABEL "Button 1" 
     SIZE 4 BY 1.17 TOOLTIP "Sele‡Æo do intervalo de Ötens de Pedidos.".
...
DEFINE FRAME f-cad
     btn-importa AT ROW 1.21 COL 58
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Pedidos - Reservas por Separa‡Æo"
         HEIGHT             = 14.5
         WIDTH              = 90
         MAX-HEIGHT         = 28
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsi2 /* Pedidos - Reservas por Separa‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Pedidos - Reservas por Separa‡Æo */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-importa w-cadsi2
ON CHOOSE OF btn-importa IN FRAME f-cad /* Button 1 */
DO:
  ASSIGN g-tp-dados = "ITEM".
...
DO:
  RUN esdlg/d04di154.w.
  RUN adm-open-query-cases IN h_q03di154.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v03di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v03di154 ).
       RUN set-position IN h_v03di154 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q03di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom/z03di154.w,
                     ProgVaPara = esgo/g01di154.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q03di154 ).
       RUN set-position IN h_q03di154 ( 1.50 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v03di154. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v03di154 ).
       RUN add-link IN adm-broker-hdl ( h_q03di154 , 'Record':U , h_v03di154 ).
       RUN add-link IN adm-broker-hdl ( h_v03di154 , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q03di154. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q03di154 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q03di154 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q03di154 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v03di154 ,
             btn-parametro:HANDLE IN FRAME f-cad , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ENABLE btn-importa btn-parametro rt-button 
...
------------------------------------------------------------------------------*/
  ...
  IF VALID-HANDLE(h_p-cadsi2) THEN 
     ASSIGN h-p-cadsi2 = h_p-cadsi2.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0095" "2.04.00.000"}

  RUN esdlg/d04di154.w. /* For‡a a ida na sele‡Æo, antes de abrir a query */
...
-------------------------------------------------------------*/
  ...
  /*
  if p-state = "nao-habilitar":U then do:
     run enable-modificar in h_p-navega (INPUT NO).
  end.

  if p-state = "habilitar":U then do:
     run enable-modificar in h_p-navega (INPUT YES).
  end.
  */
