...
*******************************************************************************/
{include/i-prgvrs.i ESSP0146 2.04.00.000}
...
DEFINE VARIABLE h_v03es049 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-consim /* Consulta Etiqueta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Etiqueta */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v03es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v03es049 ).
       RUN set-position IN h_v03es049 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.50 , 88.00 ) */
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'ProgPesquisa = eszoom\z01es049.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01es049 ).
       RUN set-position IN h_q01es049 ( 1.25 , 51.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v03es049. */
       RUN add-link IN adm-broker-hdl ( h_q01es049 , 'Record':U , h_v03es049 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v03es049 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0146" "2.04.00.000"}

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF gr-ob-etiqueta <> ? THEN
     RUN pi-reposiciona-query IN h_q01es049 (INPUT gr-ob-etiqueta).
  ELSE
     RUN dispatch IN h_q01es049 ( INPUT "get-first":U).
