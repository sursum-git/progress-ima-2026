...
*******************************************************************************/
{include/i-prgvrs.i B01DT0912Z3 2.06.00.001}
...
&Scoped-Define ENABLED-OBJECTS IMAGE-25 IMAGE-26 IMAGE-27 IMAGE-28 IMAGE-29 ~
IMAGE-30 c-inicial c-final c-inicial-3 c-final-3 bt-confirma br-table 
&Scoped-Define DISPLAYED-OBJECTS c-inicial c-final c-inicial-3 c-final-3 ~
c-inicial-2 c-final-2 
...
<FOREIGN-KEYS>
cod-emitente|y|y|movind.saldo-terc.cod-emitente
num-pedido||y|movind.saldo-terc.num-pedido
sequencia||y|movind.saldo-terc.sequencia
it-codigo||y|movind.saldo-terc.it-codigo
nr-ord-produ||y|movind.saldo-terc.nr-ord-produ
numero-ordem||y|movind.saldo-terc.numero-ordem
cod-refer||y|movind.saldo-terc.cod-refer
</FOREIGN-KEYS> 
<EXECUTING-CODE>
**************************
...
DEFINE VARIABLE c-final-3 AS CHARACTER FORMAT "x(16)" INITIAL "ZZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 16.72 BY .88 NO-UNDO.
...
DEFINE VARIABLE c-inicial-3 AS CHARACTER FORMAT "x(16)" 
     LABEL "Documento" 
     VIEW-AS FILL-IN 
     SIZE 15.29 BY .88 NO-UNDO.
...
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
    WITH NO-ASSIGN SEPARATORS SIZE 92 BY 9.42
         FONT 1.
...
DEFINE FRAME F-Main
     c-inicial AT ROW 1.13 COL 16 COLON-ALIGNED WIDGET-ID 20
     c-final AT ROW 1.13 COL 48.29 COLON-ALIGNED NO-LABEL WIDGET-ID 18
     c-inicial-3 AT ROW 2.08 COL 10 COLON-ALIGNED WIDGET-ID 36
     c-final-3 AT ROW 2.08 COL 48.29 COLON-ALIGNED NO-LABEL WIDGET-ID 34
     bt-confirma AT ROW 3 COL 86
     c-inicial-2 AT ROW 3.08 COL 10 COLON-ALIGNED WIDGET-ID 28
     c-final-2 AT ROW 3.08 COL 48.29 COLON-ALIGNED NO-LABEL WIDGET-ID 26
     br-table AT ROW 4.25 COL 1
     IMAGE-25 AT ROW 1.13 COL 27.72 WIDGET-ID 22
     IMAGE-26 AT ROW 1.13 COL 46.86 WIDGET-ID 24
     IMAGE-27 AT ROW 2.96 COL 27.72 WIDGET-ID 30
     IMAGE-28 AT ROW 2.96 COL 46.86 WIDGET-ID 32
     IMAGE-29 AT ROW 2.08 COL 27.72 WIDGET-ID 38
     IMAGE-30 AT ROW 2.08 COL 46.86 WIDGET-ID 40
...
OPEN QUERY {&SELF-NAME} FOR EACH tt-saldo-terc  NO-LOCK .
     ...
     _Where[1]         = "movind.saldo-terc.it-codigo >= c-inicial
 AND movind.saldo-terc.it-codigo <= c-final
 AND movind.saldo-terc.cod-emitente >= c-inicial-2
 AND movind.saldo-terc.cod-emitente <= c-final-2"
...
FOR EACH saldo-terc 
    WHERE saldo-terc.serie-docto >= c-inicial
      AND saldo-terc.serie-docto <= c-final
      AND saldo-terc.nro-docto   >= c-inicial-3
      AND saldo-terc.nro-docto   <= c-final-3
      AND saldo-terc.cod-emitente >= c-inicial-2
      AND saldo-terc.cod-emitente <= c-final-2 NO-LOCK.
...
ASSIGN c-inicial  :screen-value in frame {&frame-name}  = ""
       ...
       c-inicial-3:screen-value in frame {&frame-name}  = ""
       c-final    :screen-value in frame {&frame-name}  = "ZZZZZ"
       c-final-2  :screen-value in frame {&frame-name}  = STRING(iEmitente)  
       c-final-3  :screen-value in frame {&frame-name}  = "ZZZZZZZZZZZZ"  
       c-inicial  :sensitive    in frame {&frame-name}  = YES
       c-inicial-2:sensitive    in frame {&frame-name}  = no
       c-inicial-3:sensitive    in frame {&frame-name}  = YES
       c-final    :sensitive    in frame {&frame-name}  = YES
       c-final-2  :sensitive    in frame {&frame-name}  = no   
       c-final-3  :sensitive    in frame {&frame-name}  = YES   
       cNatAux     = cNatureza
       deQuantItem = pQtItem.
