...
DEFINE VARIABLE h_b01es087 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 13.75 COL 2
     bt-ok AT ROW 15.25 COL 3
     bt-cancela AT ROW 15.25 COL 14
     bt-ajuda AT ROW 15.25 COL 69
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 12.33 , 80.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es087.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es087 ).
       RUN set-position IN h_b01es087 ( 3.25 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 80.14 ) */

       /* Links to SmartBrowser h_b01es087. */
       RUN add-link IN adm-broker-hdl ( THIS-PROCEDURE , 'State':U , h_b01es087 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es087 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
