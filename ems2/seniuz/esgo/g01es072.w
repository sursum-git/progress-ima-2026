...
*******************************************************************************/
{include/i-prgvrs.i G01ES072 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS ob-trf.num-trf 
&Scoped-define ENABLED-TABLES ob-trf
&Scoped-define FIRST-ENABLED-TABLE ob-trf
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS ob-trf.num-trf 
&Scoped-define DISPLAYED-TABLES ob-trf
&Scoped-define FIRST-DISPLAYED-TABLE ob-trf
...
DEFINE FRAME d-vapara
     ob-trf.num-trf AT ROW 2 COL 20 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10 BY .88
...
ON GO OF FRAME d-vapara /* V  Para Transforma‡Æo */
DO:
  find ob-trf no-lock 
      where ob-trf.num-trf = input frame {&frame-name} ob-trf.num-trf no-error.
  if  not avail ob-trf then do:
      {utp/ut-table.i espec ob-trf 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(ob-trf).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Transforma‡Æo */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE ob-trf THEN 
    DISPLAY ob-trf.num-trf 
      WITH FRAME d-vapara.
  ENABLE ob-trf.num-trf bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01ES072" "2.04.00.000"}
