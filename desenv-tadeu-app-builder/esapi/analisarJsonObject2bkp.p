...
DEFINE VARIABLE txtjsonConv     AS LONGCHAR          NO-UNDO.
       ...
       /* WHEN 1 THEN DO:
        ...
        OTHERWISE DO:
            ...
            /*MESSAGE 'criei registro na raiz:'
                 ...
                 'agrup:' iNivelFinal 
                VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
            RUN getTextoJson(oJsonObject,cTag).
    ...
    DO i = 1 TO iTam:
        ...
        CASE poJsonObject:getType(INPUT cTag):
            ...
            OTHERWISE DO:
                ...
                RUN getTextoJson(poJsonObject,cTag).
                RUN criarTtJson(INPUT pParente,
                                INPUT cTag,
                                INPUT txtJson).
                /*CREATE ttJson.
...
PROCEDURE criarTtJson:
  ...
  */
  ...
  ELSE DO: // se o conteudo ‚ maior que o comportado em um varchar, ‚ criado um arquivo com o conteudo e na tag ‚ colocado o arquivo gerado e ‚ marcado o campo logarquivo como yes.
      ASSIGN iRand = RANDOM(1,10000000).
      ASSIGN cArqLc = cArqLc + "\" + STRING(TIME) + '_' +  STRING(iRand) + "_" +  ".lc".
...
PROCEDURE getTextoJson:

    DEFINE INPUT  PARAMETER pJson    AS jsonObject  NO-UNDO.
    DEFINE INPUT  PARAMETER pTag     AS CHARACTER   NO-UNDO.
    ASSIGN txtJsonConv        = ''.
    fix-codepage(txtJsonConv) = 'utf-8'.
    ASSIGN txtJsonConv = pJson:getLongchar( INPUT pTag,"iso8859-1").
    IF ptag = 'observacoes' THEN DO:
      COPY-LOB txtJsonConv TO FILE 'c:\temp\observacoes.txt' .
    END.
       
    COPY-LOB txtJsonConv TO mem.
    COPY-LOB mem TO txtJson CONVERT SOURCE CODEPAGE  "utf-8" TARGET CODEPAGE SESSION:CHARSET .
    SET-SIZE(mem) = 0.
