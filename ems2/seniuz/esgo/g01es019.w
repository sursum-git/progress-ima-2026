...
&Scoped-Define ENABLED-FIELDS maq-mec.codigo 
&Scoped-define ENABLED-TABLES maq-mec
&Scoped-define FIRST-ENABLED-TABLE maq-mec
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS maq-mec.codigo maq-mec.descricao 
&Scoped-define DISPLAYED-TABLES maq-mec
&Scoped-define FIRST-DISPLAYED-TABLE maq-mec
...
DEFINE FRAME d-vapara
     maq-mec.codigo AT ROW 1.17 COL 9 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 10.14 BY .88
     maq-mec.descricao AT ROW 1.17 COL 19.14 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 31.14 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para M quinas da Mecƒnica"
...
/* SETTINGS FOR FILL-IN maq-mec.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para M quinas da Mecƒnica */
DO:
  find maq-mec no-lock 
      where maq-mec.codigo = input frame {&frame-name} maq-mec.codigo no-error.
  if  not avail maq-mec then do:
      {utp/ut-table.i espec maq-mec 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(maq-mec).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para M quinas da Mecƒnica */
DO:  
...
&Scoped-define SELF-NAME maq-mec.codigo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL maq-mec.codigo d-vapara
ON LEAVE OF maq-mec.codigo IN FRAME d-vapara /* Codigo */
DO:
  FIND maq-mec WHERE maq-mec.codigo = INPUT FRAME {&FRAME-NAME} maq-mec.codigo NO-LOCK NO-ERROR.
  IF AVAIL maq-mec THEN
     ASSIGN maq-mec.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = maq-mec.descricao.
  ELSE DO:
...
------------------------------------------------------------------------------*/
  IF AVAILABLE maq-mec THEN 
    DISPLAY maq-mec.codigo maq-mec.descricao 
      WITH FRAME d-vapara.
  ENABLE maq-mec.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
