...
*******************************************************************************/
{include/i-prgvrs.i ESSP0124 2.04.00.000}
...
DEFINE VARIABLE h_q02es061 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v02es061 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consim ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Fardo de Algod∆o"
         HEIGHT             = 14.75
         WIDTH              = 90
         MAX-HEIGHT         = 27.88
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-consim /* Consulta Fardo de Algod∆o */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* Consulta Fardo de Algod∆o */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02es061.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02es061 ).
       RUN set-position IN h_v02es061 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 13.00 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q02es061.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom\z01es061.w,
                     ProgVaPara = esgo\g01es061.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q02es061 ).
       RUN set-position IN h_q02es061 ( 1.25 , 58.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02es061. */
       RUN add-link IN adm-broker-hdl ( h_q02es061 , 'Record':U , h_v02es061 ).

       /* Links to SmartQuery h_q02es061. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q02es061 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q02es061 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q02es061 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v02es061 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0124" "2.04.00.000"}

  RUN set-attribute-list IN h_q02es061 (INPUT "Reposition-Pending=YES":U). 

  /* Dispatch standard ADM method.                             */
  RUN dispatch IN THIS-PROCEDURE ( INPUT 'initialize':U ) .

  IF gr-mp-fardo <> ? THEN
     RUN pi-reposiciona-query IN h_q02es061 (INPUT gr-mp-fardo).
  ELSE
     RUN dispatch IN h_q02es061 ( INPUT "get-first":U).
