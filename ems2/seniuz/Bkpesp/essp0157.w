...
*******************************************************************************/
{include/i-prgvrs.i ESSP0157 2.04.00.000}
...
DEFINE VARIABLE h_q01es076 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es076 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-cad
     BUTTON-1 AT ROW 1.42 COL 61.14
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsim ASSIGN
         HIDDEN             = YES
         TITLE              = "Manuten‡Æo Parƒmetros das Politicas Estoque X Faturamento"
         HEIGHT             = 12.75
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-cadsim /* Manuten‡Æo Parƒmetros das Politicas Estoque X Faturamento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Manuten‡Æo Parƒmetros das Politicas Estoque X Faturamento */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Edge-Pixels = 0,
                     ...
                     Right-to-Left = First-On-Left':U ,
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.38 , 74.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.25 , 16.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es076.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es076 ).
       RUN set-position IN h_v01es076 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.75 , 88.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es076.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q01es076 ).
       RUN set-position IN h_q01es076 ( 1.50 , 66.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es076. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es076 ).
       RUN add-link IN adm-broker-hdl ( h_q01es076 , 'Record':U , h_v01es076 ).

       /* Links to SmartQuery h_q01es076. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es076 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es076 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es076 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es076 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0157" "2.04.00.000"}
