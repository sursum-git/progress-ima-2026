...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL ref-item THEN DO.
       FIND ITEM WHERE
            ITEM.it-codigo = ref-item.it-codigo NO-LOCK NO-ERROR.
       IF AVAIL ITEM THEN
          ASSIGN fi-desc-item:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ITEM.desc-item.
        
       FIND referencia WHERE
            referencia.cod-ref = ref-item.cod-refer NO-LOCK NO-ERROR.
       IF AVAIL referencia THEN
          ASSIGN fi-desc-ref:SCREEN-VALUE IN FRAME {&FRAME-NAME} = referencia.descricao.


       RUN pi-mostra-etq IN WIDGET-HANDLE(c-container) (INPUT ref-item.it-codigo,
