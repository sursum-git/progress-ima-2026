...
*******************************************************************************/
{include/i-prgvrs.i G0UN128 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS fi-cod_prog_dtsul bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-OBJECTS fi-cod_prog_dtsul 
...
DEFINE VARIABLE fi-cod_prog_dtsul AS CHARACTER FORMAT "x(50)" 
     LABEL "Programa":R10 
     VIEW-AS FILL-IN 
     SIZE 14 BY 1 TOOLTIP "C¢digo do programa."
     FONT 1 NO-UNDO.
...
DEFINE FRAME d-vapara
     fi-cod_prog_dtsul AT ROW 1.75 COL 17 COLON-ALIGNED HELP
          "C¢digo do Programa"
...
ON GO OF FRAME d-vapara /* V  Para Programa */
DO:
  find prog_dtsul no-lock 
      where prog_dtsul.cod_prog_dtsul = input frame {&frame-name} fi-cod_prog_dtsul no-error.
  if  not avail prog_dtsul then do:
      {utp/ut-table.i mgcad prog_dtsul 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(prog_dtsul).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Programa */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY fi-cod_prog_dtsul 
      WITH FRAME d-vapara.
  ENABLE fi-cod_prog_dtsul bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01UN128" "2.04.00.000"}
