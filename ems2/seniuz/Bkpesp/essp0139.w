...
DEF VAR c-cidade      AS CHAR FORMAT "x(30)". 
DEF VAR c-nome-transp AS CHAR FORMAT "x(30)".
...
DEFINE VARIABLE fi-num-etiqueta AS CHARACTER FORMAT "x(13)":U 
     VIEW-AS FILL-IN 
     SIZE 26 BY 1.75
     FONT 20 NO-UNDO.
...
DEFINE FRAME DEFAULT-FRAME
     bt-fardo AT ROW 6.88 COL 45.43 WIDGET-ID 10
...
DO:
    ...
    IF ped-venda-ext.qt-fardos = i-tot-etq THEN DO.  /* Peáas Avulsas */
       FOR EACH ped-item-res WHERE
                ped-item-res.cod-estabel = nota-fiscal.cod-estabel AND
                ped-item-res.serie       = nota-fiscal.serie AND
                ped-item-res.nr-nota-fis = INT(nota-fiscal.nr-nota-fis) AND
       ...
       FOR EACH it-nota-fisc OF nota-fiscal SHARE-LOCK.
           IF it-nota-fisc.qt-faturada[1] <> it-nota-fisc.int-2 THEN DO.
              MESSAGE "Item " it-nota-fisc.it-codigo " est† com Divergància," SKIP
                      "Nota Fiscal N«O poder† ser Expedida" SKIP(1)
                      "Qtde Faturada: " it-nota-fisc.qt-faturada[1] SKIP
                      "Qtde Lida: " it-nota-fisc.int-2
                   VIEW-AS ALERT-BOX INFO BUTTONS OK.
              ASSIGN l-erro = YES.
           END.
           ASSIGN it-nota-fisc.int-2 = 0.
       END.
       */
    END.
    ELSE DO.  /* Gerou Fardos/Volumes */
       FOR EACH ob-etiqueta WHERE
                ob-etiqueta.cod-estabel = nota-fiscal.cod-estabel AND
                ob-etiqueta.situacao = 5 AND 
                ob-etiqueta.ob-origem = nota-fiscal.nr-pedcli NO-LOCK.
        
           IF ob-etiqueta.dt-fatur = ? THEN DO.
              MESSAGE "A Etiqueta " ob-etiqueta.num-etiqueta " n∆o foi Impressa para essa Nota..." SKIP(1)
                  VIEW-AS ALERT-BOX ERROR BUTTONS OK.
              ASSIGN l-erro = YES.
           END.
        END.
...
DO:
    ...
    */ 
    ...
    IF LENGTH(fi-num-etiqueta:SCREEN-VALUE) = 13 THEN DO. /* Ç ean13 para COLCHAS*/

       FIND item-mat WHERE
            item-mat.cod-ean = fi-num-etiqueta:SCREEN-VALUE
            NO-LOCK NO-ERROR.

       FIND it-nota-fisc OF nota-fiscal WHERE
            it-nota-fisc.it-codigo = item-mat.it-codigo
            SHARE-LOCK NO-ERROR.
    ...
    ELSE DO.  /* Etiqueta Logistica */
       ...
       IF ob-etiqueta.ob-origem <> "" THEN DO.  /* ê de Fardo */
          FIND ped-venda WHERE
               ped-venda.nr-pedido = INTEGER(ob-etiqueta.ob-origem) NO-LOCK NO-ERROR.
     
          FIND FIRST b2-nota-fiscal WHERE 
                     b2-nota-fiscal.nome-ab-cli = ped-venda.nome-abrev AND
                     b2-nota-fiscal.nr-pedcli = ped-venda.nr-pedcli NO-LOCK NO-ERROR.
    
          IF NOT AVAIL b2-nota-fiscal OR 
             (AVAIL b2-nota-fiscal AND 
              b2-nota-fiscal.nr-nota-fis <> nota-fiscal.nr-nota-fis) THEN DO.
             MESSAGE 'Etiqueta n∆o Pertence a essa NOTA FISCAL'
                  VIEW-AS ALERT-BOX ERROR BUTTONS OK.
       ...
       ELSE DO.
          ...
          IF NOT AVAIL ped-item-rom THEN DO.
             FIND ped-item-rom WHERE
                  ...
                  NO-LOCK NO-ERROR.
             IF NOT AVAIL ped-item-rom THEN DO.
                MESSAGE 'N∆o Encontrado Reserva dessa Etiqueta para essa Nota Fiscal'
                    VIEW-AS ALERT-BOX INFO BUTTONS OK.
          ...
          /* 
          IF ped-item-rom.marca <> '' AND l-valida = NO AND c-seg-usuario <> 'super' THEN DO. /* Etiqueta j† foi Impressa */
             RUN pi-valida.
...
DO:
   ...
   FIND ped-venda-ext WHERE
        ...
        ped-venda-ext.nr-pedido = INTEGER(nota-fiscal.nr-pedcli) NO-LOCK NO-ERROR.
...
DO ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
  ...
  IF c-cod-estabel = '1' THEN /* Ima Textil */
     ASSIGN c-form-epl = SEARCH("etiqueta\form-nf-ima.epl").
  ELSE
     ASSIGN c-form-epl = SEARCH("etiqueta\form-nf-med.epl").
...
------------------------------------------------------------------------------*/
    ...
    IF ob-etiqueta.it-codigo <> '' THEN DO.
       FIND item WHERE
    ...
    END.
    ELSE 
       ASSIGN c-desc-item = "Diversos (Itens Mesclados)".
    ...
    ASSIGN c-nome-emit = SUBSTR(emitente.nome-emit,1,39)
           c-cidade = SUBSTR(emitente.cidade,1,26) + "/" + emitente.estado.
    ...
    IF AVAIL ped-item-rom AND ped-item-rom.marca <> '' THEN
    ...
    IF ob-etiqueta.ob-origem = "" THEN DO.  /* N∆o Ç de Fardo */
    ...
    END.
    IF ob-etiqueta.dt-fatur = ? THEN
       ASSIGN ob-etiqueta.dt-fatur =  TODAY.
    ...
    OUTPUT TO VALUE(c-prog-epl) APPEND. 
       PUT UNFORMATTED 
           ...
           "A125,188,0,1,1,2,N," '"' TRIM(c-nome-emit) '"' SKIP
           "A540,188,0,1,1,2,N," '"' TRIM(c-cidade) '"' SKIP
           "A125,220,0,1,1,2,N," '"' TRIM(c-nome-transp) '"' SKIP 
...
------------------------------------------------------------------------------*/
  ...
  FIND ped-venda-ext WHERE
       ped-venda-ext.nr-pedido = p-nr-pedido NO-LOCK NO-ERROR.

  IF AVAIL ped-venda-ext AND
     ped-venda-ext.qt-fardos <> 0 THEN DO.
     ASSIGN i-tot-etq = ped-venda-ext.qt-fardos.
  END.
  ELSE DO.
     FOR EACH ped-item-res WHERE
