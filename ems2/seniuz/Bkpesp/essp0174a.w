...
*******************************************************************************/
{include/i-prgvrs.i ESSP0174A 2.04.00.000}
...
&Scoped-define OPEN-QUERY-br-pedidos RUN pi-total. OPEN QUERY {&SELF-NAME} FOR EACH tt-vda WHERE                                  tt-vda.dia = p-dia NO-LOCK                               BY tt-vda.nr-pedcli.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-pedidos D-Dialog _FREEFORM
  QUERY br-pedidos NO-LOCK DISPLAY
      ...
      tt-vda.nr-pedcli    COLUMN-LABEL "Ped Cli"  COLUMN-BGCOLOR 8
...
DEFINE FRAME D-Dialog
     ...
     fi-tot-qtd-nor AT ROW 13.08 COL 66 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-nor AT ROW 13.13 COL 78.43 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-ljs AT ROW 14.08 COL 66 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ljs AT ROW 14.08 COL 78.43 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-ind AT ROW 15.08 COL 66 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-ind AT ROW 15.08 COL 78.43 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-out AT ROW 16.08 COL 66 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-out AT ROW 16.08 COL 78.43 COLON-ALIGNED NO-LABEL
     fi-tot-qtd-geral AT ROW 17.08 COL 66 COLON-ALIGNED NO-LABEL NO-TAB-STOP 
     fi-tot-vlr-geral AT ROW 17.08 COL 78.43 COLON-ALIGNED NO-LABEL
     bt-ok AT ROW 18.42 COL 2.57
     bt-cancela AT ROW 18.42 COL 13.57
     bt-ajuda AT ROW 18.42 COL 82.14
     rt-buttom AT ROW 18.17 COL 1.57
     "Normal:" VIEW-AS TEXT
          SIZE 6 BY .88 AT ROW 13 COL 61.86
     "Lojas:" VIEW-AS TEXT
          SIZE 4.57 BY .88 AT ROW 14 COL 63
     "Industrializaá∆o:" VIEW-AS TEXT
          SIZE 11 BY .88 AT ROW 15 COL 56.29
     "Total Geral:" VIEW-AS TEXT
          SIZE 8 BY .88 AT ROW 17 COL 59.29
     "Outros:" VIEW-AS TEXT
          SIZE 5.57 BY .88 AT ROW 16 COL 62.29
     SPACE(27.42) SKIP(2.90)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "Detalha Pedidos - ESSP0174A"
...
/* Query rebuild information for BROWSE br-pedidos
     _START_FREEFORM
RUN pi-total.
OPEN QUERY {&SELF-NAME} FOR EACH tt-vda WHERE
                                 tt-vda.dia = p-dia NO-LOCK
                              BY tt-vda.nr-pedcli.
     _END_FREEFORM
...
ON WINDOW-CLOSE OF FRAME D-Dialog /* Detalha Pedidos - ESSP0174A */
DO:  
...
------------------------------------------------------------------------------*/
  ...
  IF p-tp-relat = 1 THEN DO:
     ...
     ASSIGN br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DE PEDIDOS DE VENDAS DO DIA: " + " " + c-data.
  END.
  ELSE 
     ASSIGN br-pedidos:TITLE IN FRAME {&FRAME-NAME} = "DETALHAMENTO DE PEDIDOS DE VENDAS DO M“S: " + " " +
...
------------------------------------------------------------------------------*/
    PUT c-empresa  FORMAT "X(40)"                 AT   1
        ...
        SKIP(1).
    IF p-tp-relat = 1 THEN DO:
       PUT "RELATORIO DE DETALHAMENTO DE PEDIDOS DE VENDA DO DIA " AT 31.
       PUT c-data AT 84 SKIP(1).
    END.
    ELSE DO:
        PUT "RELATORIO DE DETALHAMENTO DE PEDIDOS DE VENDA DO MES " AT 31.
...
------------------------------------------------------------------------------*/
 ...
 CASE i-saida:
     ...
     WHEN 3 THEN DO.
         ASSIGN c-saida = SESSION:TEMP-DIRECTORY + "essp0174a.tmp".
         OUTPUT TO VALUE(c-saida).
     END.
 END CASE.
