...
DEF INPUT PARAMETER v-row-item AS ROWID NO-UNDO.
...
DEFINE VARIABLE h_q03in172 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Faturamento por Produto"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-concom /* Consulta Faturamento por Produto */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Consulta Faturamento por Produto */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_v03in172 ).
       RUN set-position IN h_v03in172 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.25 , 87.86 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Itens de NFs' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.25 , 1.57 ) NO-ERROR.
       RUN set-size IN h_folder ( 12.42 , 88.57 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Layout = ':U ,
             OUTPUT h_b01di088 ).
       RUN set-position IN h_b01di088 ( 6.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.08 , 87.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q03in172.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  '':U ,
             OUTPUT h_q03in172 ).
       RUN set-position IN h_q03in172 ( 1.50 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v03in172. */
       RUN add-link IN adm-broker-hdl ( h_q03in172 , 'Record':U , h_v03in172 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartBrowser h_b01di088. */
       RUN add-link IN adm-broker-hdl ( h_q03in172 , 'Record':U , h_b01di088 ).

       /* Links to SmartQuery h_q03in172. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q03in172 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_v03in172 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01di088 ,
             h_folder , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  IF v-row-item <> ? THEN
     RUN pi-reposiciona-query IN h_q03in172 (INPUT v-row-item).
  ELSE
     RUN dispatch IN h_q03in172 (INPUT "get-first":U).
