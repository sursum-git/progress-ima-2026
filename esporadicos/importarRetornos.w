...
&Scoped-Define ENABLED-OBJECTS rt-button btgetDir btImportar fiDiretorio 
&Scoped-Define DISPLAYED-OBJECTS fiDiretorio 
...
DEFINE VARIABLE fiDiretorio AS CHARACTER FORMAT "X(256)":U 
     LABEL "Diret¢rio" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-cad
     btgetDir AT ROW 3.54 COL 64.29 WIDGET-ID 6
     btImportar AT ROW 3.54 COL 67.86 WIDGET-ID 4
     fiDiretorio AT ROW 3.58 COL 18 COLON-ALIGNED WIDGET-ID 2
...
/* SETTINGS FOR FRAME f-cad
   FRAME-NAME L-To-R                                                    */
ASSIGN 
       fiDiretorio:READ-ONLY IN FRAME f-cad        = TRUE.
...
ON END-ERROR OF w-livre /* Importar Retornos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-livre w-livre
ON WINDOW-CLOSE OF w-livre /* Importar Retornos */
DO:
...
&Scoped-define SELF-NAME btgetDir
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btgetDir w-livre
ON CHOOSE OF btgetDir IN FRAME f-cad /* ... */
DO:
  RUN selDir('\\pv1\pv2\integracao_lisa',
             "Sele‡Æo Diret¢rio para Processar as NFs de Retorno ",
             OUTPUT cDiretorio
             ).
  IF cDiretorio <> '' THEN
     ASSIGN fiDiretorio:SCREEN-VALUE =  cDiretorio . 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btImportar w-livre
...
DO:
  ...
  RUN pi-acompanhar IN h-acomp("Buscando Arquivos no diret¢rio").
  EMPTY TEMP-TABLE ttArquivo.
  RUN esapi/getArqsDir.p(fiDiretorio:SCREEN-VALUE, NO, INPUT-OUTPUT TABLE ttArquivo ).
  ...
  FOR EACH ttArquivo:
      ...
      IF SEARCH(cArquivo) <> ? AND getExtensaoArq(cArquivo) = 'json' THEN DO:
         ...
         RUN esapi/extrairDadosNFRetorno.p(INPUT TABLE ttJson, OUTPUT logJaProcessado,
                                           OUTPUT TABLE ttRomaneioErro) .
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             OUTPUT h_p-exihel ).
       RUN set-position IN h_p-exihel ( 1.13 , 82.72 ) NO-ERROR.
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             btgetDir:HANDLE IN FRAME f-cad , 'BEFORE':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  ENABLE rt-button btgetDir btImportar fiDiretorio 
      WITH FRAME f-cad IN WINDOW w-livre.
...
------------------------------------------------------------------------------*/
  ...
  ASSIGN fiDiretorio:SCREEN-VALUE  = '\\pv1\pv2\integracao_lisa\retorno'.
END PROCEDURE.
