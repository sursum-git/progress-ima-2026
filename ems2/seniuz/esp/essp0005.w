...
*******************************************************************************/
{include/i-prgvrs.i ESSP0005 2.04.00.000}
...
DEFINE VARIABLE h_b01es009 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadpai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01es037 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es037 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadpaifilho-ambos /* Defeitos de Fabrica‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Defeitos de Fabrica‡Æo */
DO:
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?':U ,
             OUTPUT h_v01es037 ).
       RUN set-position IN h_v01es037 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.75 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'adm/objects/folder.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Defeitos' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.54 , 1.43 ) NO-ERROR.
       RUN set-size IN h_folder ( 10.50 , 89.29 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es037.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom\z01es037.w,
                     ProgVaPara = esgo\g01es037.w,
                     ProgIncMod = esp\essp0005a.w,
                     Implantar = no':U ,
             OUTPUT h_q01es037 ).
       RUN set-position IN h_q01es037 ( 1.25 , 67.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , THIS-PROCEDURE ).

       /* Links to SmartViewer h_v01es037. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_v01es037 ).
       RUN add-link IN adm-broker-hdl ( h_q01es037 , 'Record':U , h_v01es037 ).

       /* Links to SmartFolder h_folder. */
       RUN add-link IN adm-broker-hdl ( h_folder , 'Page':U , THIS-PROCEDURE ).

       /* Links to SmartQuery h_q01es037. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_q01es037 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es037 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es037 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es037 ).

    END. /* Page 0 */

    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es009.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     Hide-on-Init = no,
                     Disable-on-Init = no,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp\essp0005b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b01es009 ).
       RUN set-position IN h_b01es009 ( 6.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.92 , 88.00 ) */

       /* Links to BrowserCadastro2 h_b01es009. */
       RUN add-link IN adm-broker-hdl ( h_q01es037 , 'Record':U , h_b01es009 ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0005" "2.04.00.000"}
