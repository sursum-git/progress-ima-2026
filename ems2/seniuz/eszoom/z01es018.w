...
DEFINE VARIABLE h_b01es018 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es018 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     ...
     bt-ok AT ROW 16.21 COL 3
     bt-cancela AT ROW 16.21 COL 14
     bt-ajuda AT ROW 16.21 COL 76.43
     rt-button AT ROW 15.96 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de M quinas do Beneficiamento"
         HEIGHT             = 16.38
         WIDTH              = 86.14
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90.57
...
ON END-ERROR OF w-pesquisa /* Pesquisa de M quinas do Beneficiamento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de M quinas do Beneficiamento */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN set-size IN h_folder ( 13.67 , 85.43 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es018.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es018 ).
       RUN set-position IN h_b01es018 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 85.00 ) */

       /* Links to SmartBrowser h_b01es018. */
       RUN add-link IN adm-broker-hdl ( h_b01es018 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es018.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es018 ).
       RUN set-position IN h_b02es018 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.00 , 85.14 ) */

       /* Links to SmartBrowser h_b02es018. */
       RUN add-link IN adm-broker-hdl ( h_b02es018 , 'State':U , THIS-PROCEDURE ).
