...
&Scoped-Define ENABLED-FIELDS mov-man.num-os 
...
&Scoped-Define DISPLAYED-FIELDS mov-man.num-os 
...
DEFINE FRAME d-vapara
     mov-man.num-os AT ROW 1.17 COL 13 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 11.14 BY .88
     bt-ok AT ROW 2.58 COL 2.14
     bt-cancela AT ROW 2.58 COL 13
     bt-ajuda AT ROW 2.58 COL 58
     rt-button AT ROW 2.33 COL 1
     SPACE(0.13) SKIP(0.16)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V  Para Movimento da Manuten‡Æo"
...
ON GO OF FRAME d-vapara /* V  Para Movimento da Manuten‡Æo */
DO:
  find mov-man no-lock 
      where mov-man.num-os = input frame {&frame-name} mov-man.num-os no-error.
  if  not avail mov-man then do:
      {utp/ut-table.i espec mov-man 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mov-man).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Movimento da Manuten‡Æo */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE mov-man THEN 
    DISPLAY mov-man.num-os 
      WITH FRAME d-vapara.
  ENABLE mov-man.num-os bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
