...
*******************************************************************************/
{include/i-prgvrs.i G01ES056 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS mp-classificacao.codigo 
...
DEFINE FRAME d-vapara
     mp-classificacao.codigo AT ROW 1.5 COL 28.29 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 4 BY .88 TOOLTIP "C¢digo da classificaá∆o."
     bt-ok AT ROW 3.13 COL 2.14
     bt-cancela AT ROW 3.13 COL 13
     bt-ajuda AT ROW 3.13 COL 58.14
     rt-button AT ROW 2.88 COL 1
     SPACE(0.00) SKIP(0.11)
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         SIDE-LABELS NO-UNDERLINE THREE-D  SCROLLABLE 
         FONT 1
         TITLE "V† Para Classificaá∆o de Algod∆o"
...
ON GO OF FRAME d-vapara /* V† Para Classificaá∆o de Algod∆o */
DO:
  find mp-classificacao no-lock 
      where mp-classificacao.codigo = input frame {&frame-name} mp-classificacao.codigo no-error.
  if  not avail mp-classificacao then do:
      {utp/ut-table.i espec mp-classificacao 1}
      run utp/ut-msgs.p (input "show":U,
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(mp-classificacao).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† Para Classificaá∆o de Algod∆o */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE mp-classificacao THEN 
    DISPLAY mp-classificacao.codigo 
      WITH FRAME d-vapara.
  ENABLE mp-classificacao.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
