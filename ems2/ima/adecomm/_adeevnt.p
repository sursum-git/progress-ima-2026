...
DEFINE VARIABLE cbkpArquivo  AS CHARACTER   NO-UNDO FORMAT 'x(200)'.

DEFINE VARIABLE cComando AS CHARACTER   NO-UNDO FORMAT 'x(200)'.
...
IF  p_event = "save"
AND (p_product = "editor":U OR  p_product = "UIB":U OR p_product = "procedure window":U) THEN DO:
    /*MESSAGE "passei aqui:copy "    p_other  "p:\"  p_other STRING(NOW,"99-99-9999_hh_mm_ss") 
        VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
   
    
    ASSIGN cBkpArquivo = ENTRY(NUM-ENTRIES(p_other,"\"),p_other,"\")
           cComando    = 'copy ' +   p_other + ' t:\especificos\bkp_fontes\' + cBkpArquivo + STRING(TIME) .
    /*MESSAGE cComando
        VIEW-AS ALERT-BOX INFO BUTTONS OK.*/
     OS-COMMAND  SILENT VALUE(cComando  ). 
     /*MESSAGE 'copy ' +   p_other + ' C:\backup\' + cBkpArquivo + STRING(TIME)
        VIEW-AS ALERT-BOX INFO BUTTONS OK.*/

   /* OS-COMMAND SILENT  VALUE('copy ' +   p_other + ' C:\backup\' + cBkpArquivo + STRING(TIME)   ).*/
END.
