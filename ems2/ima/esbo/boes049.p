...
Programa: boes049.p
Autor: Tadeu silva
Objetivo: Centralizar todas as manuten‡äes da tabela ob-etiqueta
Data: 12/2023
Modificacoes:
...
DEFINE VARIABLE  numEtqLisa      AS CHARACTER   NO-UNDO.
DEFINE VARIABLE  nrPedido        AS INTEGER     NO-UNDO.
DEFINE VARIABLE  prePedido       AS INTEGER     NO-UNDO.
DEFINE VARIABLE  numOrigem       AS INTEGER     NO-UNDO.
DEFINE VARIABLE  logCorteIma     AS LOGICAL     NO-UNDO.
...
//propriedades especificas para inclusÆo das etiquetas Lisa
PROCEDURE setEtqLisa:
    DEFINE INPUT  PARAMETER pEtqLisa AS CHARACTER   NO-UNDO.
    ASSIGN numEtqLisa = pEtqLisa.
END PROCEDURE.
...
// final propriedade especificas da inclus‡Æo das etiquetas Lisa

PROCEDURE incluir:

   
   DEFINE VARIABLE lIntegraLISA  AS LOGICAL     NO-UNDO.
   DEFINE VARIABLE cErrosIncluir AS CHARACTER   NO-UNDO.
   ASSIGN numEtqNova = 0.
   ...
   IF lErro THEN DO: 
      RUN getErros IN {&boMsg}(OUTPUT cErrosIncluir).        
      /*MESSAGE cErrosIncluir
   ...
   ELSE DO:       
   END.
     //no caso de estabelecimentos que tenham integra‡Æo com a Lisa 
   ...
   ASSIGN bf.cod-estabel     = {&ttReg}.cod-estabel
           ...
           .
   ASSIGN numEtqNova    =  bf.num-etiqueta.          
   RELEASE bf NO-ERROR.
...
PROCEDURE incluirEtqLisa:
    ...
    IF cErros <> '' THEN DO:
       RUN SETMsg IN {&boMsg}(91,'Erros na inclusÆo de Etiqueta LISA:' + CHR(13) +
                              cErros,'erro').
      /* MESSAGE 'erros etq lisa' SKIP
                cErros         
           VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
    END.
...
PROCEDURE validacoes:
   ...
   IF NOT AVAIL {&ttReg} THEN DO:
      RUN setMsg IN {&bomsg}(1,'Dados nÆo inseridos na tabela Tempor ria','erro').
      RETURN 'nok'.
   END.
   ...
   CASE pAcao:
       WHEN 'incluir' THEN DO:
           IF numEtq > 0 THEN
              RUN setMsg IN {&bomsg}(2,'Etiqueta j  existe com o Id:' + STRING(numEtq),'erro').

       END.
       WHEN 'alterar' THEN DO:
           IF numEtq = 0 THEN
              RUN setMsg IN {&bomsg}(3,"NÆo foi encontrada Etiqueta para Altera‡Æo",'erro').

       END.
       WHEN 'excluir' THEN DO:
           IF numEtq = 0 THEN
              RUN setMsg IN {&bomsg}(4,"NÆo foi encontrada Etiqueta para ExclusÆo",'erro').
       END.
       WHEN 'comum' THEN DO:
...
PROCEDURE getErros:
    ...
    DEFINE OUTPUT PARAMETER pErros AS CHARACTER   NO-UNDO.
...
PROCEDURE verifExist:
   ...
   FIND FIRST {&ttReg} NO-ERROR.
   //verifica‡Æo de duplica‡Æo por chave primaria  
   ...
   IF lIntegra THEN DO:
       /*MESSAGE  'verificando etiqueta j  existente' SKIP
           
           {&ttReg}.it-codigo SKIP
           {&ttReg}.cod-refer SKIP
           {&ttReg}.nr-container SKIP
           {&ttReg}.num-rolo-imp  SKIP                     
      ...
      IF AVAIL bf THEN DO:
      /*MESSAGE 'etiqueta encontrada:' SKIP
                bf.num-etiqueta
          VIEW-AS ALERT-BOX INFORMATION BUTTONS OK.*/
         ASSIGN  numEtq = bf.num-etiqueta.
         RETURN 'ok'.
...
PROCEDURE getUltEtqIncluida:

    DEFINE OUTPUT PARAMETER pEtq AS INTEGER     NO-UNDO.
    ASSIGN pEtq =  numEtqNova.

END PROCEDURE.

PROCEDURE getHandleMsg:

    DEFINE OUTPUT PARAMETER hMsg AS HANDLE      NO-UNDO.
    ASSIGN hMsg = {&boMsg}.
