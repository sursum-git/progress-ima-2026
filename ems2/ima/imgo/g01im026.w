...
*******************************************************************************/
{include/i-prgvrs.i g01im026 2.04.00.001}
...
&Scoped-Define ENABLED-FIELDS tb-preco.nr-tabpre 
&Scoped-define ENABLED-TABLES tb-preco
&Scoped-define FIRST-ENABLED-TABLE tb-preco
&Scoped-Define ENABLED-OBJECTS rt-button bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-FIELDS tb-preco.nr-tabpre 
&Scoped-define DISPLAYED-TABLES tb-preco
&Scoped-define FIRST-DISPLAYED-TABLE tb-preco
...
DEFINE FRAME d-vapara
     tb-preco.nr-tabpre AT ROW 2 COL 33 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.14 BY 1
...
ON GO OF FRAME d-vapara /* V  Para Tabela de Pre‡os */
DO:
  find tb-preco no-lock 
      where tb-preco.nr-tabpre = input frame {&frame-name} tb-preco.nr-tabpre no-error.
  if  not avail tb-preco then do:
      {utp/ut-table.i mgdis tb-preco 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(tb-preco).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Tabela de Pre‡os */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE tb-preco THEN 
    DISPLAY tb-preco.nr-tabpre 
      WITH FRAME d-vapara.
  ENABLE rt-button tb-preco.nr-tabpre bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01im026" "2.04.00.001"}
