...
FOR EACH emitente 
    ...
    END.
    RUN retornarFinalidadeCliente IN hBo(emitente.cod-emitente, OUTPUT iFinalidade,OUTPUT codCnaeCli).

    FIND FIRST finalidades 
        WHERE finalidades.cod_finali = iFinalidade
        NO-LOCK NO-ERROR.
    ...
    EXPORT DELIMITER ";" cAval cCorrigido emitente.cod-emitente emitente.nome-emit emitente.nome-abrev iTipoAtividade cDescTipoAtiv codCNAE IF AVAIL ext-emitente THEN ext-emitente.dt-ult-fat-med ELSE ?  
        IF AVAIL finalidades THEN finalidades.desc_finalidade_venda ELSE ''.
