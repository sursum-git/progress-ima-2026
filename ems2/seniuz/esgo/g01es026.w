...
&Scoped-Define ENABLED-FIELDS set-mec.codigo 
&Scoped-define ENABLED-TABLES set-mec
&Scoped-define FIRST-ENABLED-TABLE set-mec
&Scoped-Define ENABLED-OBJECTS bt-ok bt-cancela bt-ajuda rt-button 
&Scoped-Define DISPLAYED-FIELDS set-mec.codigo set-mec.descricao 
&Scoped-define DISPLAYED-TABLES set-mec
&Scoped-define FIRST-DISPLAYED-TABLE set-mec
...
DEFINE FRAME d-vapara
     set-mec.codigo AT ROW 1.17 COL 12 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 7.14 BY .88
     set-mec.descricao AT ROW 1.17 COL 19.29 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 28.72 BY .88
    ...
    WITH VIEW-AS DIALOG-BOX KEEP-TAB-ORDER 
         ...
         TITLE "V  Para Setores da Mecƒnica"
...
/* SETTINGS FOR FILL-IN set-mec.descricao IN FRAME d-vapara
   NO-ENABLE                                                            */
...
ON GO OF FRAME d-vapara /* V  Para Setores da Mecƒnica */
DO:
  find set-mec no-lock 
      where set-mec.codigo = input frame {&frame-name} set-mec.codigo no-error.
  if  not avail set-mec then do:
      {utp/ut-table.i espec set-mec 1}
      run utp/ut-msgs.p (input "show",
                         input 2,
                         input RETURN-VALUE).
      return no-apply.
  end.
  assign p-row-tabela = rowid(set-mec).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Setores da Mecƒnica */
DO:  
...
&Scoped-define SELF-NAME set-mec.codigo
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL set-mec.codigo d-vapara
ON LEAVE OF set-mec.codigo IN FRAME d-vapara /* Codigo */
DO:
  FIND set-mec WHERE set-mec.codigo = INPUT FRAME {&FRAME-NAME} set-mec.codigo NO-LOCK NO-ERROR.
  IF AVAIL set-mec THEN
     ASSIGN set-mec.descricao:SCREEN-VALUE IN FRAME {&FRAME-NAME} = set-mec.descricao.
  ELSE DO:
     MESSAGE "Setor inv lido." VIEW-AS ALERT-BOX.
     RETURN NO-APPLY.
  END.
END.
...
------------------------------------------------------------------------------*/
  IF AVAILABLE set-mec THEN 
    DISPLAY set-mec.codigo set-mec.descricao 
      WITH FRAME d-vapara.
  ENABLE set-mec.codigo bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
