...
OUTPUT TO c:\temp\tst\retorno-cortes-01.txt APPEND.
    FOR EACH ttEtiquetas.
        PUT UNFORMATTED 
            ttEtiquetas.acao "-"   
...
// ajusta as Etiquetas
OUTPUT TO VALUE('c:\temp\tst\ajuste_etq.' + STRING(TIME) + '.txt').
FOR EACH lisa-integra WHERE
    ...
    IF lisa-integra.chave <> '0' THEN DO.
       ...
       IF lisa-integra.acao = 'ALTERAR' THEN DO.
          ...
          PUT 'etiqueta alterada:' ob-etiqueta.num-etiqueta "-" ob-etiqueta.cod-estabel   SKIP.
       END.
...
PROCEDURE pi-cria-etiqueta.
    ...
    ASSIGN ob-etiqueta.cod-estabel     = '505'
     ...
     ASSIGN ob-etiqueta.num-rolo-imp    = INTEGER(lisa-integra.val-livre-1)
      ...
      PUT 'etiqueta criada:' ob-etiqueta.num-etiqueta "-" ob-etiqueta.cod-estabel SKIP.
