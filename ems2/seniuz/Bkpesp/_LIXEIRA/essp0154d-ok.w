...
&Scoped-Define ENABLED-OBJECTS rt-buttom RECT-1 fi-qt-fardos tg-imp-etq ~
bt-ok bt-cancela bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS fi-tot-etq fi-qt-fardos tg-imp-etq 
...
DEFINE VARIABLE fi-qt-fardos AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     LABEL "Qtde de Fardos/Volumes" 
     VIEW-AS FILL-IN 
     SIZE 6.29 BY .88
     FONT 6 NO-UNDO.

DEFINE VARIABLE fi-tot-etq AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     LABEL "Total de Etiquetas Separadas" 
     VIEW-AS FILL-IN 
     SIZE 6.29 BY .88
     FONT 6 NO-UNDO.
...
DEFINE VARIABLE tg-imp-etq AS LOGICAL INITIAL no 
     LABEL "Imprimir Etiquetas de Fardo" 
     VIEW-AS TOGGLE-BOX
...
DEFINE FRAME D-Dialog
     fi-tot-etq AT ROW 2 COL 23 COLON-ALIGNED WIDGET-ID 14
     fi-qt-fardos AT ROW 3 COL 23 COLON-ALIGNED WIDGET-ID 2
     tg-imp-etq AT ROW 4.5 COL 25 WIDGET-ID 10
     bt-ok AT ROW 6.5 COL 3.29
     bt-cancela AT ROW 6.5 COL 14.14
     bt-ajuda AT ROW 6.5 COL 42.29 WIDGET-ID 8
     rt-buttom AT ROW 6.25 COL 2
     RECT-1 AT ROW 1.08 COL 1.72 WIDGET-ID 4
     SPACE(0.70) SKIP(1.87)
...
DO:
   IF INPUT FRAME {&FRAME-NAME} fi-qt-fardos = 0 THEN DO.
      MESSAGE 'Favor Informar a Quantidade de Fardos'
   ...
   IF NOT AVAIL ped-venda-ext THEN DO.
      ...
      RETURN NO-APPLY.
   END.
   ASSIGN ped-venda-ext.qt-fardos = INPUT FRAME {&FRAME-NAME} fi-qt-fardos.

   FIND CURRENT ped-venda-ext NO-LOCK NO-ERROR.

   IF INPUT FRAME {&FRAME-NAME} tg-imp-etq THEN DO.
      DO i-ct = 1 TO INPUT FRAME {&FRAME-NAME} fi-qt-fardos.
         ...
         ASSIGN ob-etiqueta.num-etiqueta = NEXT-VALUE(seq-etq-estoq-ima)
                ...
                ob-etiqueta.ob-origem = ped-venda-ext.nr-pedcli.
...
ON LEAVE OF fi-qt-fardos IN FRAME D-Dialog /* Qtde de Fardos/Volumes */
DO:
  IF INPUT FRAME {&FRAME-NAME} fi-qt-fardos >= INPUT FRAME {&FRAME-NAME} fi-tot-etq  THEN DO.
     MESSAGE 'Total de Fardos deve ser menor que a Quantidade de Etiquetas...'
         VIEW-AS ALERT-BOX INFO BUTTONS OK.
     APPLY 'ENTRY' TO SELF.
     RETURN NO-APPLY.
  END.
...
&Scoped-define SELF-NAME tg-imp-etq
&UNDEFINE SELF-NAME
...
------------------------------------------------------------------------------*/
  DISPLAY fi-tot-etq fi-qt-fardos tg-imp-etq 
      WITH FRAME D-Dialog.
  ENABLE rt-buttom RECT-1 fi-qt-fardos tg-imp-etq bt-ok bt-cancela bt-ajuda 
      WITH FRAME D-Dialog.
  VIEW FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  IF fi-qt-fardos = 0 THEN
     ASSIGN fi-qt-fardos = fi-tot-etq.
...
------------------------------------------------------------------------------*/
    ...
    IF ped-venda.cod-estabel = '1' THEN
       ASSIGN c-form-epl = "M:\ems206\especificos\etiqueta\form-fardo-ima.epl".
    ELSE
       ASSIGN c-form-epl = "M:\ems206\especificos\etiqueta\form-fardo-med.epl".
    ...
    OUTPUT TO VALUE(c-prog-epl) APPEND. 
       PUT UNFORMATTED 
           ...
           "A130,219,0,1,1,2,N," '"' TRIM(emitente.cidade) '"' SKIP 
           "A530,219,0,1,1,2,N," '"' TRIM(emitente.estado) '"' SKIP 
           "A580,219,0,1,1,2,N," '"' STRING(fi-tot-etq) '"' SKIP 
           "A120,260,0,1,2,4,N," '"' TRIM(ped-venda.nr-pedcli) '"' SKIP 
           "A310,275,0,1,1,2,N," '"' TRIM(transporte.nome) '"' SKIP 
   /*  */  "A60,345,0,3,3,6,N," '"' "VOLUME" '"' SKIP
           "A400,395,0,3,3,4,N," '"' STRING(ob-etiqueta.num-etiqueta,"999999999") '"' SKIP
           "B445,320,0,1,3,7,70,N," '"' STRING(i-num-bar,"9999999999") '"' SKIP.
