...
DEFINE VARIABLE hLisa0120 AS HANDLE      NO-UNDO.
...
fi-lbl-qt-sel brReprocSepracao btEtqsSeparadas btEtqsEst RECT-2 RECT-4 ~
RECT-96 IMAGE-108 IMAGE-109 RECT-97 IMAGE-3 IMAGE-4 RECT-101 br-itens 
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION-FORWARD getNfTriangular B-table-Win 
FUNCTION getNfTriangular RETURNS LOGICAL
  ( ped AS CHAR )  FORWARD.
...
DEFINE BUTTON btEtqsEst 
     LABEL "Etqs. Em Estoque" 
     SIZE 15 BY 1.13.
...
DEFINE FRAME F-Main
     ...
     btExecAcao AT ROW 22.38 COL 25.57 WIDGET-ID 536
     btEnvLisa AT ROW 18.5 COL 93 WIDGET-ID 530
     btPedLisa AT ROW 22.21 COL 50.57 WIDGET-ID 514
     ...
     brReprocSepracao AT ROW 22.21 COL 56.29 WIDGET-ID 520
     btEtqsSeparadas AT ROW 18.5 COL 108.14 WIDGET-ID 532
     btLogCorte AT ROW 22.38 COL 41.29 WIDGET-ID 540
     btEtqsEst AT ROW 18.5 COL 123.43 WIDGET-ID 542
...
/* SETTINGS FOR BUTTON bt-refresh IN FRAME F-Main
   1                                                                    */
/* SETTINGS FOR BUTTON btLogCorte IN FRAME F-Main
   NO-ENABLE                                                            */
...
DO:
   ...
   IF getNfTriangular(tt-pedidos.nr-pedcli) THEN DO:
       ASSIGN tt-pedidos.nr-pedcli:FGCOLOR IN BROWSE br-pedidos = 15
              tt-pedidos.nr-pedcli:BGCOLOR IN BROWSE br-pedidos = 17 .                
    END.          
...
DO:
   ...
   IF AVAIL tt-pedidos THEN DO.
      FOR EACH ped-item OF tt-pedidos WHERE 
          ...
          ASSIGN btLogCorte:SENSITIVE IN FRAME {&frame-name} = CAN-FIND(FIRST hist_corte_separacao 
          WHERE hist_corte_separacao.nr_pedido = INT(tt-Pedidos.nr-pedcli)) OR
          CAN-FIND(FIRST transacoes WHERE transacoes.calculo_Id = 200 AND transacoes.chave = tt-pedidos.nr-pedcli).
...
&Scoped-define SELF-NAME btEtqsEst
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btEtqsEst B-table-Win
ON CHOOSE OF btEtqsEst IN FRAME F-Main /* Etqs. Em Estoque */
DO:
   IF AVAIL tt-pedidos AND AVAIL tt-itens THEN DO:
      RUN lisa/lisa0130.w(tt-itens.it-codigo, tt-itens.cod-refer ).
...
&Scoped-define SELF-NAME btLogCorte
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btLogCorte B-table-Win
ON CHOOSE OF btLogCorte IN FRAME F-Main /* Log.Corte */
DO:

 IF AVAIL tt-Pedidos THEN DO:
    RUN lisa/lisa0120.w (INT(tt-pedidos.nr-pedcli)) .  
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _FUNCTION getNfTriangular B-table-Win 
FUNCTION getNfTriangular RETURNS LOGICAL
  ( ped AS CHAR ) :
/*------------------------------------------------------------------------------
  Purpose:  
    Notes:  
------------------------------------------------------------------------------*/
FOR FIRST ped-venda fields(nat-operacao cod-estabel nr-pedido)NO-LOCK
    WHERE ped-venda.nr-pedido = int(ped),
    FIRST natur-oper field(log-oper-triang) OF ped-venda
    WHERE natur-oper.log-oper-triang :
    RETURN TRUE.
