...
*******************************************************************************/
{include/i-prgvrs.i V01IN172 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS item.it-codigo 
&Scoped-define ENABLED-TABLES item
&Scoped-define FIRST-ENABLED-TABLE item
&Scoped-Define ENABLED-OBJECTS rt-key rt-mold fi-fator-conv 
&Scoped-Define DISPLAYED-FIELDS item.it-codigo item.desc-item item.un 
...
to-indigo fi-largura fi-margem-pcp fi-fator-conv fi-peso-liquido 

/* Custom List Definitions                                              */
/* ADM-CREATE-FIELDS,ADM-ASSIGN-FIELDS,ADM-MODIFY-FIELDS,List-4,List-5,List-6 */
&Scoped-define List-4 fi-cod-composi fi-cod-rlgp to-indigo fi-largura ~
fi-margem-pcp fi-fator-conv fi-peso-liquido 
...
DEFINE VARIABLE fi-fator-conv AS DECIMAL FORMAT ">>9.9999" INITIAL 0 
     LABEL "Fator M" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 TOOLTIP "Fator de convers∆o de Kg para M" NO-UNDO.
...
DEFINE VARIABLE fi-margem-pcp AS DECIMAL FORMAT ">>9.99" INITIAL 0 
     LABEL "Margem PCP":R10 
     VIEW-AS FILL-IN 
     SIZE 11 BY .88 TOOLTIP "Margem de Perda PCP" NO-UNDO.
...
DEFINE FRAME f-main
     item.it-codigo AT ROW 1.17 COL 3.29 COLON-ALIGNED FORMAT "x(16)"
          VIEW-AS FILL-IN 
          SIZE 14.72 BY .88
     item.desc-item AT ROW 1.17 COL 18.43 COLON-ALIGNED NO-LABEL
          VIEW-AS FILL-IN 
          SIZE 55.57 BY .88
     item.un AT ROW 1.17 COL 83 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 3 BY .88
     ...
     to-indigo AT ROW 3.71 COL 46
          ...
          "Largura do tecido"
     fi-margem-pcp AT ROW 4.75 COL 44 COLON-ALIGNED HELP
          "Margem de Perda PCP"
...
/* Settings for THIS-PROCEDURE
   Type: SmartViewer
   External Tables: ems206cad.item
...
/* SETTINGS FOR TOGGLE-BOX to-indigo IN FRAME f-main
   NO-ENABLE 4                                                          */
/* SETTINGS FOR FILL-IN item.un IN FRAME f-main
   NO-ENABLE                                                            */
...
------------------------------------------------------------------------------*/
  ...
  *    return 'ADM-ERROR':U. 
  ...
  FIND item-ext WHERE item-ext.it-codigo = item.it-codigo NO-ERROR.
  /*
  IF NOT AVAIL item-ext THEN DO:
     CREATE item-ext.
     ASSIGN item-ext.it-codigo = item.it-codigo.
  END.
  ASSIGN item-ext.cod-composi = INPUT FRAME {&FRAME-NAME} fi-cod-composi 
         item-ext.cod-rlgp    = DEC(INPUT FRAME {&frame-name} fi-cod-rlgp)
         item-ext.largura     = DEC(INPUT FRAME {&FRAME-NAME} fi-largura)
         item-ext.margem-pcp  = DEC(INPUT FRAME {&FRAME-NAME} fi-margem-pcp)
         item-ext.fator-conv  = DEC(INPUT FRAME {&FRAME-NAME} fi-fator-conv)
         item-ext.indigo      = INPUT FRAME {&FRAME-NAME} to-indigo.
  */  
  ASSIGN i-cont = 0.
  FOR EACH ITEM WHERE ITEM.it-codigo = item-ext.it-codigo:
      IF ITEM.un = "kg" THEN
  ...
  END.
  MESSAGE "Peso L°quido/Bruto Atualizado para " + TRIM(STRING(i-cont)) + " ÷tem(ns)." VIEW-AS ALERT-BOX.
...
------------------------------------------------------------------------------*/
    ...
    IF ITEM.un = "M" OR ITEM.un = "kg" THEN
       ASSIGN fi-fator-conv:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
...
------------------------------------------------------------------------------*/
    ...
    IF AVAIL item-ext THEN DO:
       ...
       IF AVAIL ITEM THEN DO:
          IF (ITEM.ge-codigo >= 50 AND ITEM.ge-codigo <= 59) OR
             ...
             END.
             ELSE
             IF (ITEM.ge-codigo >= 50 AND ITEM.ge-codigo <= 59) AND SUBSTR(ITEM.it-codigo,6,1) <> "0" THEN DO:
             ...
             ELSE DO:
                IF item.peso-liquido <> 0 THEN
                   ASSIGN fi-fator-conv:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(DEC(1 / item.peso-liquido)).
             END.
          END.
       END.
       ELSE DO:
    ...
    ELSE DO:
        ASSIGN fi-cod-composi:SCREEN-VALUE IN FRAME {&FRAME-NAME}  = "" 
               ...
               fi-margem-pcp:SCREEN-VALUE IN FRAME {&FRAME-NAME}   = ""
               fi-peso-liquido:SCREEN-VALUE IN FRAME {&FRAME-NAME} = ""
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN item.it-codigo:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
