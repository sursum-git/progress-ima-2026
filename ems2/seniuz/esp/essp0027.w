...
*******************************************************************************/
{include/i-prgvrs.i ESSP0027 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS btn-parametro rt-button 
...
DEFINE VARIABLE h_q01di154 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01di154 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadsi2 ASSIGN
         HIDDEN             = YES
         TITLE              = "Pedidos - Dados Complementares"
         HEIGHT             = 12.83
         WIDTH              = 90
         MAX-HEIGHT         = 27.79
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadsi2 /* Pedidos - Dados Complementares */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsi2 w-cadsi2
ON WINDOW-CLOSE OF w-cadsi2 /* Pedidos - Dados Complementares */
DO:
...
DO:
  ...
  RUN adm-open-query-cases IN h_q01di154.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01di154 ).
       RUN set-position IN h_v01di154 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01di154.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = dizoom/z03di154.w,
                     ...
                     Implantar = no':U ,
             OUTPUT h_q01di154 ).
       RUN set-position IN h_q01di154 ( 1.50 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01di154. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'TableIO':U , h_v01di154 ).
       RUN add-link IN adm-broker-hdl ( h_q01di154 , 'Record':U , h_v01di154 ).

       /* Links to SmartQuery h_q01di154. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsi2 , 'State':U , h_q01di154 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01di154 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01di154 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01di154 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsi2 , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01di154 ,
...
------------------------------------------------------------------------------*/
  ENABLE btn-parametro rt-button 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0027" "2.04.00.000"}
