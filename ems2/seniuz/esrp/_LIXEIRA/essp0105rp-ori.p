...
IF tt-param.tg-localiz THEN DO:
   ...
   PUT STREAM str-rp "Etiqueta     Item      Referància    Quantidade  Doca" SKIP.
   PUT STREAM str-rp "-----------  --------  ----------  ------------  -------" SKIP.

   FOR EACH tt-ob-etiqueta NO-LOCK
       BREAK BY tt-ob-etiqueta.it-codigo
             BY tt-ob-etiqueta.cod-refer
             BY tt-ob-etiqueta.num-etiqueta.
    
       ACCUMULATE tt-ob-etiqueta.num-etiqueta (COUNT BY tt-ob-etiqueta.it-codigo).
       ACCUMULATE tt-ob-etiqueta.quantidade (TOTAL BY tt-ob-etiqueta.it-codigo).
    
       ACCUMULATE tt-ob-etiqueta.num-etiqueta (COUNT BY tt-ob-etiqueta.cod-refer).
       ACCUMULATE tt-ob-etiqueta.quantidade (TOTAL BY tt-ob-etiqueta.cod-refer).
    
       PUT STREAM str-rp
           tt-ob-etiqueta.num-etiqueta  AT 1
           tt-ob-etiqueta.it-codigo     AT 14  FORMAT "x(8)" 
           tt-ob-etiqueta.cod-refer     AT 24
           tt-ob-etiqueta.quantidade    AT 38
           tt-ob-etiqueta.localiz       AT 50
           SKIP.
    
       ASSIGN i-qt-pecas = i-qt-pecas + 1.
    
       IF LAST-OF(tt-ob-etiqueta.cod-refer) THEN DO.
          IF tt-ob-etiqueta.cod-refer <> "" THEN DO.
             PUT STREAM str-rp
                 "------------" AT 36
                 SKIP.
             ...
             PUT STREAM str-rp
                 "Total da Ref....." AT 14
                 (ACCUM TOTAL BY tt-ob-etiqueta.cod-refer tt-ob-etiqueta.quantidade) AT 38
                 "  Peáas Lidas: "     AT 48
                 STRING((ACCUM COUNT BY tt-ob-etiqueta.cod-refer tt-ob-etiqueta.num-etiqueta))
                 SKIP(1).
          END.
          PUT STREAM str-rp SKIP.
       END.
        
        IF LAST-OF(tt-ob-etiqueta.it-codigo) AND
          tt-ob-etiqueta.it-codigo <> '' THEN DO.

          PUT STREAM str-rp
              "Total do Item...." AT 14
              (ACCUM TOTAL BY tt-ob-etiqueta.it-codigo tt-ob-etiqueta.quantidade) AT 38
              "  Peáas Lidas: "  AT 48
              STRING((ACCUM COUNT BY tt-ob-etiqueta.it-codigo tt-ob-etiqueta.num-etiqueta))
              SKIP(2).
       END.
   END.
   ...
   PUT STREAM str-rp
       "Total Geral......" AT 14
       (ACCUM TOTAL tt-ob-etiqueta.quantidade) AT 38
       "  Peáas Lidas: "  AT 48
       STRING((ACCUM COUNT tt-ob-etiqueta.num-etiqueta))
       SKIP.
...
IF tt-param.tg-localiz THEN DO:
   ASSIGN i-col = 1
          c-lin = "".
   FOR EACH tt-ob-etiqueta NO-LOCK.
       ASSIGN c-nota = FILL(" ",9).
       ASSIGN c-lin = c-lin  + FILL(" ",7) + "Doca: " + SUBSTR(tt-ob-etiqueta.localizacao,1,3)  + "/"  +
                               SUBSTR(tt-ob-etiqueta.localizacao,4,3 ) +
                               "  Etiqueta: " + STRING(tt-ob-etiqueta.num-etiqueta, "999999999") + 
                               "  NF: " +   c-nota + FILL(" ",7).
...
PROCEDURE pi-grava-avulsa.
    ...
    FOR EACH tt-imp-ped,
        ...
        RUN pi-acompanhar IN h-acomp (INPUT tt-imp-item.it-codigo + " " + 
                                            tt-imp-item.cod-refer).
    ...
    FOR EACH tt-imp-ped NO-LOCK,
        ...
        RUN pi-acompanhar IN h-acomp (INPUT tt-imp-ped.nome-abrev + " " + 
                                            tt-imp-ped.nr-pedcli + " " +
                                            STRING(ped-item-res.nr-sequencia)).
...
PROCEDURE pi-transf-etq.
    ASSIGN i-num-etiqueta = INT(SUBSTR(c-linha,7,9)).

    IF i-num-etiqueta = 0 THEN NEXT.
    ...
    FIND ob-etiqueta where
         ...
         ob-etiqueta.num-etiqueta = i-num-etiqueta NO-ERROR.
    IF NOT AVAIL ob-etiqueta THEN NEXT.

    CREATE b-etiqueta.
    BUFFER-COPY ob-etiqueta TO b-etiqueta
        ASSIGN b-etiqueta.cod-estab = '1'
               b-etiqueta.num-etiqueta = NEXT-VALUE(seq-etq-estoq-ima)
               b-etiqueta.situacao = 3
               SUBSTR(b-etiqueta.char-1,1330,1) = 'T'. /* Etiqueta Transferida */
