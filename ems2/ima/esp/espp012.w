...
DEFINE VARIABLE fi-cod-refer-fim AS CHARACTER FORMAT "X(15)":U INITIAL "ZZZZZZ" 
     VIEW-AS FILL-IN 
     SIZE 7.14 BY .88 NO-UNDO.

DEFINE VARIABLE fi-cod-refer-ini AS CHARACTER FORMAT "X(15)":U 
     LABEL "Referencia" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fi-dt-chegada-fim AS DATE FORMAT "99/99/9999" INITIAL 12/31/9999 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.

DEFINE VARIABLE fi-dt-chegada-ini AS DATE FORMAT "99/99/9999" INITIAL 01/01/001 
     LABEL "Chegada" 
     VIEW-AS FILL-IN 
     SIZE 12 BY .88 NO-UNDO.
...
DEFINE VARIABLE fi-no-ab-reppri-fim AS CHARACTER FORMAT "x(16)" INITIAL "ZZZZZZZZZZZZZZZ" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fi-nr-container-fim AS INTEGER FORMAT ">>>9,99":U INITIAL 999999 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi-nr-container-ini AS INTEGER FORMAT ">>>9,99":U INITIAL 0 
     LABEL "Container" 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 NO-UNDO.

DEFINE VARIABLE fi-qt-dias AS INTEGER FORMAT ">>9":U INITIAL 90 
     LABEL "Dias Faturamento" 
     VIEW-AS FILL-IN 
...
DEFINE FRAME f-pg-sel
     fi-cod-estabel AT ROW 1.5 COL 18 COLON-ALIGNED WIDGET-ID 2
     fi-desc-estab AT ROW 1.5 COL 22.43 COLON-ALIGNED NO-LABEL WIDGET-ID 4 NO-TAB-STOP 
     fi-nr-container-ini AT ROW 2.5 COL 18 COLON-ALIGNED WIDGET-ID 72
     fi-nr-container-fim AT ROW 2.5 COL 47.86 COLON-ALIGNED NO-LABEL WIDGET-ID 16
     fi-dt-chegada-ini AT ROW 3.5 COL 18 COLON-ALIGNED WIDGET-ID 64
     fi-dt-chegada-fim AT ROW 3.5 COL 48 COLON-ALIGNED NO-LABEL WIDGET-ID 70
     fi-it-codigo-ini AT ROW 4.5 COL 18 COLON-ALIGNED
     fi-it-codigo-fim AT ROW 4.5 COL 47.86 COLON-ALIGNED NO-LABEL
     fi-cod-refer-ini AT ROW 5.5 COL 18 COLON-ALIGNED WIDGET-ID 30
     fi-cod-refer-fim AT ROW 5.5 COL 47.86 COLON-ALIGNED NO-LABEL WIDGET-ID 28
     fi-no-ab-reppri-ini AT ROW 6.5 COL 18 COLON-ALIGNED HELP
          "Codigo do ÷tem no EMS" WIDGET-ID 46
     fi-no-ab-reppri-fim AT ROW 6.5 COL 48 COLON-ALIGNED HELP
          "Codigo do ÷tem no EMS" NO-LABEL WIDGET-ID 44
     fi-qt-dias AT ROW 8 COL 18 COLON-ALIGNED WIDGET-ID 74
     tg-detalhado AT ROW 9.67 COL 20 WIDGET-ID 60
     "(Dias de Fatuaramento a analisar apos a Data de Chegada do Container)" VIEW-AS TEXT
          SIZE 51.72 BY .54 AT ROW 8.17 COL 25.29 WIDGET-ID 76
     IMAGE-1 AT ROW 4.5 COL 35.43
     IMAGE-2 AT ROW 4.5 COL 46.43
     IMAGE-5 AT ROW 5.5 COL 35.43 WIDGET-ID 32
     IMAGE-6 AT ROW 5.5 COL 46.43 WIDGET-ID 34
     IMAGE-96 AT ROW 6.5 COL 35.57 WIDGET-ID 48
     IMAGE-97 AT ROW 6.5 COL 46.57 WIDGET-ID 50
     IMAGE-98 AT ROW 3.5 COL 35.57 WIDGET-ID 66
     IMAGE-99 AT ROW 3.5 COL 46.57 WIDGET-ID 68
     IMAGE-3 AT ROW 2.5 COL 35.43 WIDGET-ID 8
     IMAGE-4 AT ROW 2.5 COL 46.43 WIDGET-ID 10
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.85
         SIZE 76.86 BY 10.62
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Faturamento por Container - ESSP012.W"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 22.33
         MAX-WIDTH          = 114.29
...
/* SETTINGS FOR FRAME f-pg-sel
                                                                        */
ASSIGN 
       fi-desc-estab:READ-ONLY IN FRAME f-pg-sel        = TRUE.
...
ON END-ERROR OF w-relat /* Faturamento por Container - ESSP012.W */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Faturamento por Container - ESSP012.W */
DO:
...
&Scoped-define SELF-NAME fi-no-ab-reppri-fim
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-no-ab-reppri-fim w-relat
ON LEAVE OF fi-no-ab-reppri-fim IN FRAME f-pg-sel
DO:
  IF SELF:SCREEN-VALUE <> '' THEN DO.
     FIND repres WHERE 
          repres.nome-abrev = SELF:SCREEN-VALUE
          NO-LOCK NO-ERROR.
     IF NOT AVAIL repres THEN
        FIND repres WHERE 
             STRING(repres.cod-rep) = SELF:SCREEN-VALUE
             NO-LOCK NO-ERROR.

     IF AVAIL repres THEN
        ASSIGN fi-no-ab-reppri-fim:SCREEN-VALUE IN FRAME {&FRAME-NAME} = repres.nome-abrev.
  END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-no-ab-reppri-fim w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-no-ab-reppri-fim IN FRAME f-pg-sel
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad229.w
                     &campo     = fi-no-ab-reppri-fim
...
DO:
   IF SELF:SCREEN-VALUE <> '' THEN DO.
      FIND repres WHERE 
       ...
       ASSIGN fi-no-ab-reppri-fim:SCREEN-VALUE = SELF:SCREEN-VALUE.
   END.
END.
...
&Scoped-define SELF-NAME fi-nr-container-fim
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-nr-container-fim w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-nr-container-fim IN FRAME f-pg-sel
DO:
  {include/zoomvar.i &prog-zoom=eszoom\z02pp001.w
                     &campo=fi-nr-container-fim
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-nr-container-ini w-relat
ON LEAVE OF fi-nr-container-ini IN FRAME f-pg-sel /* Container */
DO:
  IF INT(SELF:SCREEN-VALUE) <> 0 THEN
     ASSIGN fi-nr-container-fim:SCREEN-VALUE = SELF:SCREEN-VALUE.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-nr-container-ini w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-nr-container-ini IN FRAME f-pg-sel /* Container */
DO:
  {include/zoomvar.i &prog-zoom=eszoom\z02pp001.w
...
fi-nr-container-ini:LOAD-MOUSE-POINTER("image/lupa.cur") IN FRAME f-pg-sel.
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    ASSIGN fi-cod-estabel:SCREEN-VALUE IN FRAME f-pg-sel = para-ped.estab-pad.

    ASSIGN fi-dt-chegada-ini:SCREEN-VALUE IN FRAME f-pg-sel = STRING(TODAY - DAY(TODAY) + 1)
           fi-dt-chegada-fim:SCREEN-VALUE IN FRAME f-pg-sel = STRING(TODAY).
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-cod-estabel fi-desc-estab fi-nr-container-ini fi-nr-container-fim 
          fi-dt-chegada-ini fi-dt-chegada-fim fi-it-codigo-ini fi-it-codigo-fim 
          fi-cod-refer-ini fi-cod-refer-fim fi-no-ab-reppri-ini 
          fi-no-ab-reppri-fim fi-qt-dias tg-detalhado 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE IMAGE-1 IMAGE-2 IMAGE-5 IMAGE-6 IMAGE-96 IMAGE-97 IMAGE-98 IMAGE-99 
         IMAGE-3 IMAGE-4 fi-cod-estabel fi-desc-estab fi-nr-container-ini 
         fi-nr-container-fim fi-dt-chegada-ini fi-dt-chegada-fim 
         fi-it-codigo-ini fi-it-codigo-fim fi-cod-refer-ini fi-cod-refer-fim 
         fi-no-ab-reppri-ini fi-no-ab-reppri-fim fi-qt-dias tg-detalhado 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    ASSIGN tt-param.cod-estabel = INPUT FRAME f-pg-sel fi-cod-estabel
           tt-param.nr-container-ini = INPUT FRAME f-pg-sel fi-nr-container-ini
           tt-param.nr-container-fim = INPUT FRAME f-pg-sel fi-nr-container-fim
           tt-param.dt-chegada-ini = INPUT FRAME f-pg-sel fi-dt-chegada-ini
           tt-param.dt-chegada-fim = INPUT FRAME f-pg-sel fi-dt-chegada-fim
           ...
           tt-param.no-ab-reppri-fim = INPUT FRAME f-pg-sel fi-no-ab-reppri-fim
           tt-param.qt-dias = INPUT FRAME f-pg-sel fi-qt-dias
           tt-param.resumido = NOT INPUT FRAME f-pg-sel tg-detalhado.
    ...
    {include/i-rprun.i esrp/espp012rp.p}
