...
&Scoped-Define ENABLED-OBJECTS rt-button bt-consulta 
...
DEFINE VARIABLE h_b01es071 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01es049 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v05es049 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON bt-consulta 
     IMAGE-UP FILE "image/im-exc.bmp":U
     LABEL "Button 2" 
     SIZE 4 BY 1.21 TOOLTIP "Consulta Etiquetas Excluidas".
...
DEFINE FRAME f-cad
     bt-consulta AT ROW 1.17 COL 51.86 WIDGET-ID 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-consultarelacionamento ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Movimentos de uma Etiqueta"
         HEIGHT             = 21.38
         WIDTH              = 90
         MAX-HEIGHT         = 21.38
         MAX-WIDTH          = 114.29
...
ON END-ERROR OF w-consultarelacionamento /* Consulta Movimentos de uma Etiqueta */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consultarelacionamento w-consultarelacionamento
ON WINDOW-CLOSE OF w-consultarelacionamento /* Consulta Movimentos de uma Etiqueta */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-consulta w-consultarelacionamento
ON CHOOSE OF bt-consulta IN FRAME f-cad /* Button 2 */
DO:
  ASSIGN w-consultarelacionamento:SENSITIVE = NO.
  RUN esp/essp0148b.w.
  ASSIGN w-consultarelacionamento:SENSITIVE = YES.
END.
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Movimentos' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 10.79 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.50 , 89.00 ) NO-ERROR.

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es049.w,
                     ProgVaPara = esgo/g01es049.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es049 ).
       RUN set-position IN h_q01es049 ( 1.00 , 58.00 ) NO-ERROR.
       ...
       /* Links to SmartQuery h_q01es049. */
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es049 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es049 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-navega ,
             bt-consulta:HANDLE IN FRAME f-cad , 'BEFORE':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             bt-consulta:HANDLE IN FRAME f-cad , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_folder ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v05es049.r':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v05es049 ).
       RUN set-position IN h_v05es049 ( 2.58 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 8.17 , 89.57 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es071.r':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es071 ).
       RUN set-position IN h_b01es071 ( 12.17 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.75 , 86.00 ) */

       /* Links to SmartViewer h_v05es049. */
       RUN add-link IN adm-broker-hdl ( h_q01es049 , 'Record':U , h_v05es049 ).

       /* Links to SmartBrowser h_b01es071. */
       RUN add-link IN adm-broker-hdl ( h_q01es049 , 'Record':U , h_b01es071 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v05es049 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es071 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0148" "2.04.00.001"}
