...
DEFINE VARIABLE h_b03es009 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b04es009 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 13.08 COL 1.57
     bt-ok AT ROW 14.46 COL 3
     bt-cancela AT ROW 14.46 COL 14
     bt-ajuda AT ROW 14.46 COL 79
     rt-button AT ROW 14.21 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Defeitos de Fabrica‡Æo"
         HEIGHT             = 14.63
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Defeitos de Fabrica‡Æo */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Defeitos de Fabrica‡Æo */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03es009.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03es009 ).
       RUN set-position IN h_b03es009 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.00 , 87.00 ) */

       /* Links to SmartBrowser h_b03es009. */
       RUN add-link IN adm-broker-hdl ( h_b03es009 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b04es009.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b04es009 ).
       RUN set-position IN h_b04es009 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 10.17 , 87.00 ) */

       /* Links to SmartBrowser h_b04es009. */
       RUN add-link IN adm-broker-hdl ( h_b04es009 , 'State':U , THIS-PROCEDURE ).
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
DEFINE INPUT PARAMETER p-tipo-def AS CHAR NO-UNDO.
ASSIGN p-cod-tipo-def = p-tipo-def.
END PROCEDURE.
