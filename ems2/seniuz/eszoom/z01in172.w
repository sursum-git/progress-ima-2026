...
*******************************************************************************/
{include/i-prgvrs.i Z01IN172 2.04.00.000}
...
DEFINE VARIABLE h_b01in172 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02in172 AS HANDLE NO-UNDO.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Itens para Venda"
         HEIGHT             = 15.67
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Itens para Venda */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Itens para Venda */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01in172.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01in172 ).
       RUN set-position IN h_b01in172 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.00 ) */

       /* Links to SmartBrowser h_b01in172. */
       RUN add-link IN adm-broker-hdl ( h_b01in172 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01in172 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02in172.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02in172 ).
       RUN set-position IN h_b02in172 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.00 ) */

       /* Links to SmartBrowser h_b02in172. */
       RUN add-link IN adm-broker-hdl ( h_b02in172 , 'State':U , THIS-PROCEDURE ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_b02in172 ,
             h_folder , 'AFTER':U ).
    END. /* Page 2 */
    WHEN 3 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b03in172.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b03in172 ).
       RUN set-position IN h_b03in172 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 87.00 ) */
