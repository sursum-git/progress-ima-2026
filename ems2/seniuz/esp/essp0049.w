...
*******************************************************************************/
{include/i-prgvrs.i ESSP0049 2.04.00.000}
...
DEFINE VARIABLE h_q01es033 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es033 AS HANDLE NO-UNDO.
...
ON END-ERROR OF w-cadsim /* Testes de NIP */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadsim w-cadsim
ON WINDOW-CLOSE OF w-cadsim /* Testes de NIP */
DO:
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es033.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es033 ).
       RUN set-position IN h_v01es033 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 11.83 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es033.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es033.w,
                     ProgVaPara = esgo/g01es033.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es033 ).
       RUN set-position IN h_q01es033 ( 1.50 , 64.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartPanel h_p-exihel. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'State':U , h_p-exihel ).

       /* Links to SmartViewer h_v01es033. */
       RUN add-link IN adm-broker-hdl ( h_p-cadsim , 'TableIO':U , h_v01es033 ).
       RUN add-link IN adm-broker-hdl ( h_q01es033 , 'Record':U , h_v01es033 ).

       /* Links to SmartQuery h_q01es033. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es033 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es033 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es033 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-cadsim , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es033 ,
             h_p-exihel , 'AFTER':U ).
    END. /* Page 0 */
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0049" "2.04.00.000"}
