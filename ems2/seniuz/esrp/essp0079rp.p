...
def var i-qtd-os-maq  as int.
def var i-min-man-maq as int.
def var i-min-par-maq as int.
...
form
    ...
    w-work.tipo-man      label "Tip"
    w-work.sist-const    label "S"
...
{utp/ut-liter.i Incidˆncia_de_Manuten‡Æo_por_M quina * r}
...
for each mov-man where mov-man.num-os     >= tt-param.ini-num-os
   ...
   do i-cont = 1 to 5:
      ...
      or i-min-par <> 0 then do:
         find first w-work
              where w-work.cod-maq    = mov-man.cod-maq
                and w-work.tipo-man   = mov-man.tipo-man
                and w-work.sist-const = mov-man.sist-const
                and w-work.ssist-exec = mov-man.ssist-exec[i-cont]
              no-error.
         if not avail w-work then do:
            create w-work.
            assign w-work.cod-maq    = mov-man.cod-maq
                   w-work.tipo-man   = mov-man.tipo-man
                   w-work.sist-const = mov-man.sist-const
...
for each w-work break by w-work.cod-maq
                      by w-work.tipo-man
                      by w-work.sist-const
                      by w-work.ssist-exec:
   if first-of(w-work.cod-maq) then do:
      for each b-w-work where b-w-work.cod-maq = w-work.cod-maq:
          assign i-qtd-os-maq  = i-qtd-os-maq +  b-w-work.qtd-os
                 i-min-man-maq = i-min-man-maq + b-w-work.min-man
                 i-min-par-maq = i-min-par-maq + b-w-work.min-par.
      end.
      find maq-mec where maq-mec.codigo = w-work.cod-maq
                   no-lock no-error.
      display w-work.cod-maq
              maq-mec.descricao
              with frame f-detalhe.
   end.
   assign i-perc1 = w-work.qtd-os / i-qtd-os-maq * 100
          i-perc2 = w-work.min-man / i-min-man-maq * 100
          i-perc3 = w-work.min-par / i-min-par-maq * 100.
   ...
   if last-of(w-work.cod-maq) then do:
      assign i-perc1 = i-qtd-os-maq / i-qtd-os-ger * 100
             i-perc2 = i-min-man-maq / i-min-man-ger * 100
             i-perc3 = i-min-par-maq / i-min-par-ger * 100.
          
      assign c-hor-man = string((i-min-man-maq - 
                                (i-min-man-maq MODULO 60)) / 
                                 60,"99999") + 
                         string((i-min-man-maq MODULO 60),"99")
             c-hor-par = string((i-min-par-maq - 
                                (i-min-par-maq MODULO 60)) / 
                                 60,"99999") + 
                         string((i-min-par-maq MODULO 60),"99").
   
      display "Total"      @ w-work.cod-maq
              i-qtd-os-maq @ w-work.qtd-os
              i-perc1
              c-hor-man
              i-perc2
              c-hor-par
