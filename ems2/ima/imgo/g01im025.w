...
*******************************************************************************/
{include/i-prgvrs.i G01IM025 2.04.00.001}
...
&Scoped-Define DISPLAYED-FIELDS item.it-codigo 
...
DEFINE FRAME d-vapara
     item.it-codigo AT ROW 2 COL 16 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 17.14 BY .88
     bt-ok AT ROW 4.5 COL 2.14
     bt-cancela AT ROW 4.5 COL 13
     bt-ajuda AT ROW 4.5 COL 40
...
ON GO OF FRAME d-vapara /* V† para item */
DO:
  find item no-lock 
      where item.it-codigo = input frame {&frame-name} item.it-codigo no-error.
  if  not avail item then do:
      run esp/message.p (input "Item n∆o foi encontrado.",
                         input "O c¢digo do item informado n∆o foi encontrado.").
      return no-apply.
  end.
  assign p-row-tabela = rowid(item).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V† para item */
DO:  
...
------------------------------------------------------------------------------*/
  IF AVAILABLE item THEN 
    DISPLAY item.it-codigo 
      WITH FRAME d-vapara.
  ENABLE rt-button item.it-codigo bt-ok bt-cancela bt-ajuda 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "G01IM025" "2.04.00.001"}
