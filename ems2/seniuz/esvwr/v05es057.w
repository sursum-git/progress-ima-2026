...
*******************************************************************************/
{include/i-prgvrs.i V05ES057 2.04.00.000}
...
&Scoped-Define ENABLED-FIELDS mp-entr-cam.dt-saida mp-entr-cam.peso-nf ~
...
mp-entr-cam.peso-bruto mp-entr-cam.peso-tara 
&Scoped-define DISPLAYED-TABLES mp-entr-cam
&Scoped-define FIRST-DISPLAYED-TABLE mp-entr-cam
&Scoped-Define DISPLAYED-OBJECTS fi-hr-saida fi-peso-liq-balanca ~
fi-diferenca 

/* Custom List Definitions                                              */
/* ADM-CREATE-FIELDS,ADM-ASSIGN-FIELDS,ADM-MODIFY-FIELDS,List-4,List-5,List-6 */
&Scoped-define ADM-ASSIGN-FIELDS mp-entr-cam.responsavel mp-entr-cam.nr-cdr ~
mp-entr-cam.dt-saida fi-hr-saida mp-entr-cam.peso-nf ~
mp-entr-cam.peso-liquido mp-entr-cam.peso-bruto mp-entr-cam.peso-tara 
...
DEFINE VARIABLE fi-peso-liq-balanca AS DECIMAL FORMAT "->>>,>>9.99":U INITIAL 0 
...
DEFINE FRAME f-main
     mp-entr-cam.responsavel AT ROW 1.5 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 13 BY .88
     mp-entr-cam.nr-cdr AT ROW 1.5 COL 45 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 9.14 BY .88
     mp-entr-cam.dt-saida AT ROW 2.5 COL 18 COLON-ALIGNED
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     fi-hr-saida AT ROW 3.5 COL 18 COLON-ALIGNED
     mp-entr-cam.peso-nf AT ROW 4.5 COL 18 COLON-ALIGNED
          LABEL "Peso Bruto da NF"
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     mp-entr-cam.peso-liquido AT ROW 5.5 COL 18 COLON-ALIGNED
          LABEL "Peso Liquido da NF"
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     mp-entr-cam.peso-bruto AT ROW 6.5 COL 18 COLON-ALIGNED
          LABEL "Peso Bruto do Ve°culo"
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     bt-peso-bruto AT ROW 6.5 COL 32.72
     mp-entr-cam.peso-tara AT ROW 7.5 COL 18 COLON-ALIGNED
          LABEL "Peso Tara do Ve°culo"
          VIEW-AS FILL-IN 
          SIZE 12.57 BY .88
     bt-peso-tara AT ROW 7.5 COL 32.72
     fi-peso-liq-balanca AT ROW 8.5 COL 18 COLON-ALIGNED
     fi-diferenca AT ROW 9.5 COL 18 COLON-ALIGNED
...
/* SETTINGS FOR FILL-IN fi-hr-saida IN FRAME f-main
   NO-ENABLE 2                                                          */
/* SETTINGS FOR FILL-IN fi-peso-liq-balanca IN FRAME f-main
   NO-ENABLE                                                            */
...
*/
     ASSIGN fi-peso-liq-balanca:SCREEN-VALUE IN FRAME {&FRAME-NAME}  = 
...
DO:
   ASSIGN fi-peso-liq-balanca:SCREEN-VALUE IN FRAME {&FRAME-NAME} = STRING(DEC(INPUT FRAME {&FRAME-NAME} mp-entr-cam.peso-bruto) -
                                                                           DEC(INPUT FRAME {&FRAME-NAME} mp-entr-cam.peso-tara))
          fi-diferenca:SCREEN-VALUE IN FRAME {&FRAME-NAME}  =  STRING(DEC(INPUT FRAME {&FRAME-NAME} mp-entr-cam.peso-nf) -  
                                                                      DEC(INPUT FRAME {&FRAME-NAME} fi-peso-liq-balanca)).
...
------------------------------------------------------------------------------*/
    ...
    ASSIGN fi-hr-saida:SENSITIVE IN FRAME {&FRAME-NAME} = NO.
...
------------------------------------------------------------------------------*/
    ...
    IF g-tipo-mov = 2 THEN DO:
       ...
       IF AVAIL mp-entr-cam THEN DO.
          ASSIGN fi-hr-saida:SCREEN-VALUE IN FRAME {&FRAME-NAME} = 
                 STRING(mp-entr-cam.hr-saida,"HH:MM").
...
------------------------------------------------------------------------------*/
  ...
  DEF VAR fi-mod-op-balanca AS CHAR.

  RUN Get-Field-Screen-Value IN adm-broker-hdl (INPUT THIS-PROCEDURE,
                                                INPUT "fi-mod-op-balanca").

  ASSIGN fi-mod-op-balanca = RETURN-VALUE.

  IF NOT AVAIL mp-param THEN
     FIND FIRST mp-param NO-LOCK NO-ERROR.

  IF p-novo-registro = YES THEN DO:
     ASSIGN mp-entr-cam.peso-nf:SENSITIVE IN FRAME {&FRAME-NAME}      = YES
            mp-entr-cam.peso-liquido:SENSITIVE IN FRAME {&FRAME-NAME} = YES.
     IF fi-mod-op-balanca = "Manual"  THEN  /* Balanáa Manual */
  ...
  ELSE DO:
     ...
     IF fi-mod-op-balanca = "Manual" THEN DO: /* Balanáa Manual */
...
------------------------------------------------------------------------------*/
  IF NOT AVAIL mp-param THEN
    ...
    IF AVAIL emitente THEN
       PUT "FORNECEDOR...........:"              AT  1
           emitente.nome-emit  FORMAT "x(35)"    AT 25.
    PUT "CONTRATO.............:"                 AT  1.
    IF AVAIL mp-entr-mat THEN
       PUT mp-entr-mat.nro-contrato              AT 25.
    PUT "NOTA FISCAL..........:"                 AT  1.
    IF AVAIL mp-entr-mat THEN
       PUT STRING(mp-entr-mat.nro-docto)            AT 25.
    PUT "PLACA................:"                 AT  1
        mp-entr-cam.placa                        AT 25
        "DATA DA ENTRADA......:"                 AT  1
        mp-entr-cam.dt-entrada                   AT 25
        "HORA DA ENTRADA......:"                 AT  1
        STRING(mp-entr-cam.hr-entrada,"HH:MM")   AT 25 
        "DATA DA SAIDA........:"                 AT  1
        mp-entr-cam.dt-saida                     AT 25
        "HORA DA SAIDA........:"                 AT  1
        STRING(mp-entr-cam.hr-saida,"HH:MM")     AT 25.
    PUT "PESO BRUTO DA NF.....:"                 AT  1
        mp-entr-cam.peso-nf                      AT 25
        "PESO LIQUIDO DA NF...:"                 AT  1
        mp-entr-cam.peso-liquido                 AT 25.
    PUT "QTD FARDOS:"                            AT 40.
    IF AVAIL mp-entr-mat  THEN
       PUT  STRING(mp-entr-mat.qtd-fardo[1] +
                   mp-entr-mat.qtd-fardo[2] +
                   mp-entr-mat.qtd-fardo[3] +
                   mp-entr-mat.qtd-fardo[4] +
                   mp-entr-mat.qtd-fardo[5])         AT 52.
    PUT "PESO BRUTO DO VEICULO:"                 AT  1
        mp-entr-cam.peso-bruto                   AT 25
        "PESO TARA DO VEICULO.:"                 AT  1
        mp-entr-cam.peso-tara                    AT 25
        "PESO BRUTO DA BALANCA:"                 AT  1
        STRING(mp-entr-cam.peso-bruto - mp-entr-cam.peso-tara, "->>>,>>9.99") 
                                                 AT 24 FORMAT "X(11)"
        "DIFERENCA(BALANCA/NF):"                 AT  1 
        STRING(((mp-entr-cam.peso-bruto - mp-entr-cam.peso-tara) - mp-entr-cam.peso-nf)
                 , "->>>,>>9.99") 
                                                 AT 24 FORMAT "X(11)".
    PUT FILL("-",80) FORMAT "x(80)"    AT  1 SKIP.
...
------------------------------------------------------------------------------*/
 ...
 IF g-tipo-mov = 2 THEN DO: /* Descarga de Algod∆o */ 
    if adm-new-record = yes THEN DO: /* NOVO REGISTRO */
       IF INPUT FRAME {&frame-name} mp-entr-cam.peso-nf = 0 THEN DO:
            MESSAGE "O peso BRUTO n∆o pode ser zero." SKIP 
                    "Favor informar o Peso Bruto da Nota" SKIP 
                    "Fiscal de ENTRADA DE ALGOD«O." VIEW-AS ALERT-BOX. 
            APPLY 'entry' TO mp-entr-cam.peso-nf.
            return 'ADM-ERROR':U.
       END. 
       IF INPUT FRAME {&frame-name} mp-entr-cam.peso-liquido = 0 THEN DO:
            ...
            return 'ADM-ERROR':U.
       END. 
       IF INPUT FRAME {&frame-name} mp-entr-cam.peso-liquido >
          INPUT FRAME {&frame-name} mp-entr-cam.peso-nf  THEN DO:
            MESSAGE "O peso LIQUIDO da NF n∆o pode ser Maior" SKIP 
                    "que o peso Bruto"  VIEW-AS ALERT-BOX. 
            APPLY 'entry' TO mp-entr-cam.peso-liquido.
            return 'ADM-ERROR':U.
       END. 
       IF ABS(INPUT FRAME {&frame-name} mp-entr-cam.peso-nf - 
              INPUT FRAME {&frame-name} mp-entr-cam.peso-liquido) >  mp-param.peso-toler-nf THEN DO:
          MESSAGE "A Diferenáa entre o Peso Bruto da NF e o Liquido," SKIP 
       ...
       IF INPUT FRAME {&frame-name} mp-entr-cam.peso-bruto = 0 THEN DO:
             MESSAGE "O peso bruto do Veiculo n∆o pode ser zero." VIEW-AS ALERT-BOX. 
             APPLY 'entry' TO mp-entr-cam.peso-bruto.
            return 'ADM-ERROR':U.
       END. 
       IF INPUT FRAME {&frame-name} mp-entr-cam.peso-liquido >
          INPUT FRAME {&frame-name} mp-entr-cam.peso-nf  THEN DO:
            MESSAGE "O peso LIQUIDO da Nota Fiscal n∆o pode ser" SKIP 
                    "Maior do que o peso BRUTO."  VIEW-AS ALERT-BOX. 
            APPLY 'entry' TO mp-entr-cam.peso-liquido.
            return 'ADM-ERROR':U.
       END.
       /*
       IF ABS(INPUT FRAME {&frame-name} mp-entr-cam.peso-bruto - 
              INPUT FRAME {&frame-name} mp-entr-cam.peso-liquido) >  mp-param.peso-toler-nf THEN DO:
          MESSAGE "A Diferenáa entre o peso Bruto da NF e o do Veiculo," SKIP
