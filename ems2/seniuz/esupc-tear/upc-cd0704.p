      ...
      IF h-objeto:TYPE <> "field-group" THEN DO:
         CASE h-objeto:NAME.
             WHEN "endereco" THEN DO:
                ...
                ON 'leave':U OF h-objeto PERSISTENT RUN esupc/upc-cd0704l2.p.
             END.
...
IF p-ind-event = "BEFORE-INITIALIZE" AND
  ...
  /* {esinc/i-dsallrb.i emitente-ext.tipo-cliente c-tipos}*/
   FOR EACH tipo-cliente NO-LOCK.
       ASSIGN c-tipos = c-tipos + "," + TRIM(tipo-cliente.descricao) + "," + STRING(tipo-cliente.cod-tp-cli).
   END.
...
IF p-ind-event = "ADD" AND
   p-ind-object = "VIEWER" AND
   c-objeto = "'advwr\v24ad098.w'" THEN DO:
   FIND LAST tipo-cliente NO-LOCK NO-ERROR.
   ASSIGN wh-tp-cliente:SCREEN-VALUE = tipo-cliente.descricao /* "Normal" */
...
IF p-ind-event = "DISPLAY" AND
   ...
   IF AVAIL emitente-ext THEN
      ASSIGN wh-tp-cliente:SCREEN-VALUE = STRING(emitente-ext.cod-tp-cli).
...
IF p-ind-event = "VALIDATE" AND
   ...
   IF LOOKUP(wh-tp-cliente:SCREEN-VALUE,c-tipos)= 0 THEN DO. 
      FIND tipo-cliente WHERE
           tipo-cliente.descricao = wh-tp-cliente:SCREEN-VALUE NO-ERROR.
      IF NOT AVAIL emitente-ext THEN DO:
         MESSAGE "O Tipo do Cliente N∆o Cadastrado ..." VIEW-AS ALERT-BOX.
...
IF p-ind-event = "ASSIGN" AND
   ...
   END.
   ASSIGN emitente-ext.cod-tp-cli = INT(wh-tp-cliente:SCREEN-VALUE).
END.
