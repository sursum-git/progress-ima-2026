...
*******************************************************************************/
{include/i-prgvrs.i g01pp001 2.04.00.001}
...
&Scoped-Define ENABLED-OBJECTS i_nr-container bt-ok bt-cancela bt-ajuda ~
rt-button 
&Scoped-Define DISPLAYED-OBJECTS i_nr-container 
...
DEFINE VARIABLE i_nr-container AS INTEGER FORMAT ">>>,>>9":U INITIAL 0 
     LABEL "Container" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88
     FONT 1 NO-UNDO.
...
ON GO OF FRAME d-vapara /* V  Para Container */
DO:
FIND pp-container NO-LOCK
    WHERE pp-container.nr-container EQ INPUT FRAME {&FRAME-NAME} i_nr-container NO-ERROR.

IF NOT AVAILABLE pp-container THEN DO:
    {utp/ut-table.i ems2ima pp-container 1}
    RUN utp/ut-msgs.p(INPUT "show",
                      INPUT 2,
                      INPUT RETURN-VALUE).
    RETURN NO-APPLY.
END.
ASSIGN p-row-tabela = ROWID(pp-container).
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL d-vapara d-vapara
ON WINDOW-CLOSE OF FRAME d-vapara /* V  Para Container */
DO:  
...
------------------------------------------------------------------------------*/
  DISPLAY i_nr-container 
      WITH FRAME d-vapara.
  ENABLE i_nr-container bt-ok bt-cancela bt-ajuda rt-button 
      WITH FRAME d-vapara.
  VIEW FRAME d-vapara.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "g01pp001" "2.00.00.001"}
