...
*******************************************************************************/
{include/i-prgvrs.i ESSP0018 2.04.00.000}
...
DEFINE VAR cod-area-ini LIKE area-mec.codigo.
DEFINE VAR cod-area-fin LIKE area-mec.codigo.
...
&Scoped-Define ENABLED-OBJECTS BUTTON-1 rt-button 
...
DEFINE VARIABLE h_q01es002 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es002 AS HANDLE NO-UNDO.

/* Definitions of the field level widgets                               */
DEFINE BUTTON BUTTON-1 
     IMAGE-UP FILE "image/excel.bmp":U
     LABEL "Button 1" 
     SIZE 5 BY 1.13 TOOLTIP "Gerar uma planilha Excel, com os dados das µreas da Mecƒnica.".
...
ON END-ERROR OF w-consim /* µreas da Mecƒnica */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-consim w-consim
ON WINDOW-CLOSE OF w-consim /* µreas da Mecƒnica */
DO:
...
DO:
  RUN esdlg/d01es002.w (OUTPUT cod-area-ini, OUTPUT cod-area-fin, OUTPUT arq-saida).

  IF arq-saida <> "" THEN DO:
     RUN pi-gera-excel (INPUT cod-area-ini, INPUT cod-area-fin, INPUT arq-saida).
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es002.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es002 ).
       RUN set-position IN h_v01es002 ( 2.75 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 4.00 , 88.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es002.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom/z01es002.w,
                     ProgVaPara = esgo/g01es002.w,
                     ProgIncMod = ,
                     Implantar = no':U ,
             OUTPUT h_q01es002 ).
       RUN set-position IN h_q01es002 ( 1.50 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v01es002. */
       RUN add-link IN adm-broker-hdl ( h_q01es002 , 'Record':U , h_v01es002 ).

       /* Links to SmartQuery h_q01es002. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es002 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es002 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es002 ).
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             h_p-navega , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es002 ,
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0018" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
    ...
    DEF INPUT PARAMETER p-cod-area-ini LIKE area-mec.codigo.
    DEF INPUT PARAMETER p-cod-area-fin LIKE area-mec.codigo.
    ...
    ASSIGN c-Lin = c-empresa + " - " + " TABELA DE µREAS DA MEC¶NICA - DATA: " + STRING(TODAY) +
                                       " - " + STRING(TIME,"HH:MM").
    ...
    DDE SEND i-canal SOURCE "Pfx-OS"    ITEM "L3C3".
    
    DDE EXECUTE i-canal COMMAND "[select(~"C1~")]". 
    DDE EXECUTE sys     COMMAND "[column.width(7.29)]".
    
    DDE EXECUTE i-canal COMMAND "[select(~"C2~")]". 
    DDE EXECUTE sys     COMMAND "[column.width(32.43)]". 

    DDE EXECUTE i-canal COMMAND "[select(~"C3~")]". 
    DDE EXECUTE sys     COMMAND "[column.width(7.29)]".

    DDE EXECUTE i-canal COMMAND '[select(~"C1:C3~")]'.
    ...
    FOR EACH area-mec WHERE area-mec.codigo >= p-cod-area-ini
                        AND area-mec.codigo <= p-cod-area-fin
                     NO-LOCK:
    
        DDE SEND i-canal SOURCE area-mec.codigo     ITEM "L" + TRIM(STRING(i-Lin)) + "C1".
        DDE SEND i-canal SOURCE area-mec.descricao  ITEM "L" + TRIM(STRING(i-Lin)) + "C2".
        DDE SEND i-canal SOURCE area-mec.prefixo-os ITEM "L" + TRIM(STRING(i-Lin)) + "C3".
