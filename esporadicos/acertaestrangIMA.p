...
OUTPUT TO c:\temp\apropIMA.txt.
FOR EACH aprop_ctbl_ap EXCLUSIVE-LOCK
    WHERE (cod_empresa = '100'
               AND cod_cta_ctbl = '21202001')
    /*OR (ind_natur_lancto_ctbl = 'cr'
    AND (cod_cta_ctbl = '21201001' OR cod_cta_ctbl = '21202001'))*/ ,
    EACH movto_tit_ap OF aprop_ctbl_ap NO-LOCK
    WHERE movto_tit_ap.dat_transacao >= 01.01.2014
    /*WHERE movto_tit_ap.ind_trans_ap_abrev = 'bxa' AND log_bxa_contra_antecip */ ,
    EACH tit_ap OF movto_tit_ap NO-LOCK
    WHERE dat_emis_docto < 04/06/2015:
    DISP aprop_ctbl_ap.cod_empresa tit_ap.cod_tit_ap tit_ap.cod_espec_docto  tit_ap.cdn_fornecedor movto_tit_ap.dat_transacao val_movto_ap aprop_ctbl_ap.num_id_movto_tit_ap WITH   WIDTH 550.

    ASSIGN aprop_ctbl_ap.cod_cta_ctbl = '21201001'.              
    ...
    ASSIGN tt.numMovAP     = aprop_ctbl_ap.num_id_movto_tit_ap
           tt.contaCredito = aprop_ctbl_ap.cod_cta_ctbl
