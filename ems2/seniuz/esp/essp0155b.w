...
DEF VAR h-bo-logs-calculos AS HANDLE.
DEF VAR cChave             AS CHAR.
...
&Scoped-define FRAME-NAME D-Dialog
&Scoped-define BROWSE-NAME br-calc-comis

/* Internal Tables (found by Frame, Query & Browse Queries)             */
&Scoped-define INTERNAL-TABLES ttLog his-ped-venda-ext

/* Definitions for BROWSE br-calc-comis                                 */
&Scoped-define FIELDS-IN-QUERY-br-calc-comis ttlog.dt_hr_registro ttLog.descricao   
&Scoped-define ENABLED-FIELDS-IN-QUERY-br-calc-comis   
&Scoped-define SELF-NAME br-calc-comis
&Scoped-define QUERY-STRING-br-calc-comis FOR EACH ttLog
&Scoped-define OPEN-QUERY-br-calc-comis OPEN QUERY {&SELF-NAME} FOR EACH ttLog .
&Scoped-define TABLES-IN-QUERY-br-calc-comis ttLog
&Scoped-define FIRST-TABLE-IN-QUERY-br-calc-comis ttLog
...
&Scoped-define OPEN-BROWSERS-IN-QUERY-D-Dialog ~
    ~{&OPEN-QUERY-br-calc-comis}~
    ~{&OPEN-QUERY-br-historico}

/* Standard List Definitions                                            */
&Scoped-Define ENABLED-OBJECTS RECT-1 rt-buttom rs-tp-log br-calc-comis ~
br-historico bt-ok bt-cancela bt-imprime bt-ajuda 
&Scoped-Define DISPLAYED-OBJECTS fi-nr-pedcli rs-tp-log fi-cliente ~
fi-nome-cli 
...
DEFINE VARIABLE rs-tp-log AS INTEGER 
     VIEW-AS RADIO-SET HORIZONTAL
     RADIO-BUTTONS 
          "Hist¢rico de Manutená‰es", 1,
"Mem¢ria de C†lculo da Comiss∆o", 2
     SIZE 56 BY .75 NO-UNDO.
...
/* Query definitions                                                    */
&ANALYZE-SUSPEND
DEFINE QUERY br-calc-comis FOR 
      ttLog SCROLLING.
...
/* Browse definitions                                                   */
DEFINE BROWSE br-calc-comis
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _DISPLAY-FIELDS br-calc-comis D-Dialog _FREEFORM
  QUERY br-calc-comis DISPLAY
      ttlog.dt_hr_registro COLUMN-LABEL "Data             Hora" WIDTH 19
ttLog.descricao      COLUMN-LABEL "Descriá∆o"
/* _UIB-CODE-BLOCK-END */
&ANALYZE-RESUME
...
/* BROWSE-TAB br-calc-comis fi-nome-cli D-Dialog */
/* BROWSE-TAB br-historico br-calc-comis D-Dialog */
ASSIGN 
...
&ANALYZE-SUSPEND _QUERY-BLOCK BROWSE br-calc-comis
/* Query rebuild information for BROWSE br-calc-comis
     _START_FREEFORM
OPEN QUERY {&SELF-NAME} FOR EACH ttLog .
     _END_FREEFORM
     _Query            is OPENED
*/  /* BROWSE br-calc-comis */
&ANALYZE-RESUME
...
"his-ped-venda-ext.usuario" ? ? "character" ? ? ? ? ? ? no ? no no "10" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _FldNameList[4]   > espec.his-ped-venda-ext.ocorrencia
"his-ped-venda-ext.ocorrencia" ? ? "character" ? ? ? ? ? ? no ? no no "200" yes no no "U" "" "" "" "" "" "" 0 no 0 no no
     _Query            is OPENED
*/  /* BROWSE br-historico */
&ANALYZE-RESUME
...
&Scoped-define SELF-NAME rs-tp-log
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL rs-tp-log D-Dialog
ON VALUE-CHANGED OF rs-tp-log IN FRAME D-Dialog
DO:
   IF SELF:INPUT-VALUE = 1 THEN DO.
      ASSIGN br-historico:VISIBLE = YES.
      ASSIGN br-calc-comis:VISIBLE = NO.
   END.
   ELSE DO.
      ASSIGN br-historico:VISIBLE = NO.
      ASSIGN br-calc-comis:VISIBLE = YES.
   END.
END.
...
------------------------------------------------------------------------------*/
  DISPLAY fi-nr-pedcli rs-tp-log fi-cliente fi-nome-cli 
      WITH FRAME D-Dialog.
  ENABLE RECT-1 rt-buttom rs-tp-log br-calc-comis br-historico bt-ok bt-cancela 
         bt-imprime bt-ajuda 
      WITH FRAME D-Dialog.
...
------------------------------------------------------------------------------*/
  ...
  APPLY 'VALUE-CHANGED' TO rs-tp-log.

  RUN esbo/boLogsCalculos.p PERSISTENT SET h-bo-logs-calculos.
  RUN iniciarBos IN h-bo-logs-calculos.
  RUN limparTT IN h-bo-logs-calculos.
  RUN setCalculo IN h-bo-logs-calculos ( INPUT 3 ).

  ASSIGN cChave = 'PV-' + ped-venda.cod-estabel + '-' + STRING(ped-venda.nr-pedido). 

  RUN getLogsUltTransPorChave IN h-bo-logs-calculos (INPUT cChave,
                                                     OUTPUT TABLE ttLog).
  {&OPEN-QUERY-br-calc-comis}
