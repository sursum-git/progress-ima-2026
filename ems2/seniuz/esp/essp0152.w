...
*******************************************************************************/
{include/i-prgvrs.i ESSP0152 2.04.000}
...
DEF NEW GLOBAL SHARED VAR i-situacao AS INT.
...
&Scoped-Define ENABLED-OBJECTS rt-button bt-sel bt-imprime 
...
DEFINE VARIABLE h_b01es073 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_folder AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-cadpai AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-exihel AS HANDLE NO-UNDO.
DEFINE VARIABLE h_p-navega AS HANDLE NO-UNDO.
DEFINE VARIABLE h_q01es072 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_v01es072 AS HANDLE NO-UNDO.
...
DEFINE BUTTON bt-sel 
     ...
     LABEL "Button 3" 
     SIZE 4.14 BY 1.25 TOOLTIP "Seleciona Transforma‡äes".
...
DEFINE FRAME f-cad
     bt-sel AT ROW 1.13 COL 48.57 WIDGET-ID 2
     bt-imprime AT ROW 1.13 COL 54
     rt-button AT ROW 1 COL 1
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 1 ROW 1
         SIZE 106.86 BY 19.42.
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-cadpaifilho-ambos ASSIGN
         HIDDEN             = YES
         TITLE              = "Manut. Transform. Cortes de Tecidos"
         HEIGHT             = 15.54
         WIDTH              = 90.86
         MAX-HEIGHT         = 28.21
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-cadpaifilho-ambos /* Manut. Transform. Cortes de Tecidos */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-cadpaifilho-ambos w-cadpaifilho-ambos
ON WINDOW-CLOSE OF w-cadpaifilho-ambos /* Manut. Transform. Cortes de Tecidos */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-imprime w-cadpaifilho-ambos
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL bt-sel w-cadpaifilho-ambos
ON CHOOSE OF bt-sel IN FRAME f-cad /* Button 3 */
DO:
   ...
   RUN adm-open-query IN h_q01es072.
...
------------------------------------------------------------------------------*/
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'Etiquetas' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 5.17 , 1.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.25 , 90.00 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v01es072.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v01es072 ).
       RUN set-position IN h_v01es072 ( 2.50 , 1.00 ) NO-ERROR.
       /* Size in UIB:  ( 2.50 , 90.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es073.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Initial-Lock = NO-LOCK,
                     ...
                     Create-On-Add = ?,
                     ProgAtributo = ,
                     ProgIncMod = esp\essp0152b.w,
                     MessageNum = 0,
                     MessageParam = ':U ,
             OUTPUT h_b01es073 ).
       RUN set-position IN h_b01es073 ( 6.58 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.46 , 87.00 ) */

       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esqry/q01es072.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'ProgPesquisa = eszoom\z01es072.w,
                     ProgVaPara = esgo\g01es072.w,
                     ProgIncMod = esp/essp0152a.w,
                     Implantar = no':U ,
             OUTPUT h_q01es072 ).
       RUN set-position IN h_q01es072 ( 1.25 , 63.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.00 , 7.72 ) */

       /* Links to SmartViewer h_v01es072. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'TableIO':U , h_v01es072 ).
       RUN add-link IN adm-broker-hdl ( h_q01es072 , 'Record':U , h_v01es072 ).
       RUN add-link IN adm-broker-hdl ( h_v01es072 , 'State':U , THIS-PROCEDURE ).

       /* Links to BrowserCadastro2 h_b01es073. */
       RUN add-link IN adm-broker-hdl ( h_q01es072 , 'Record':U , h_b01es073 ).

       /* Links to SmartQuery h_q01es072. */
       RUN add-link IN adm-broker-hdl ( h_p-cadpai , 'State':U , h_q01es072 ).
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es072 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'Navigation':U , h_q01es072 ).
       RUN add-link IN adm-broker-hdl ( h_p-navega , 'State':U , h_q01es072 ).

       /* Adjust the tab order of the smart objects. */
       RUN adjust-tab-order IN adm-broker-hdl ( h_v01es072 ,
             h_p-exihel , 'AFTER':U ).
       RUN adjust-tab-order IN adm-broker-hdl ( h_b01es073 ,
             h_folder , 'AFTER':U ).
    END. /* Page 1 */
...
------------------------------------------------------------------------------*/
  ENABLE rt-button bt-sel bt-imprime 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0152" "2.04.00.000"}
  
  ASSIGN i-situacao = 1
         i-num-trf-ini = 0
         i-num-trf-fim = 9999999  
         da-dt-solic-ini = TODAY - 90
         da-dt-solic-fim = TODAY
         c-num-trf = "".
