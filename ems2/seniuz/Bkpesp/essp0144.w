...
*******************************************************************************/
{include/i-prgvrs.i ESSP0144 2.00.00.000}
...
DEF VAR p-row-etiqueta AS ROWID.
DEF VAR i-tp-consulta AS INT INIT 1.
...
&Scoped-Define ENABLED-OBJECTS btn-parametro RECT-1 rt-button 
...
DEFINE FRAME f-cad
     btn-parametro AT ROW 1.38 COL 42.29 HELP
          "Paramˆtros do Programa"
     RECT-1 AT ROW 2.79 COL 2
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-concom ASSIGN
         HIDDEN             = YES
         TITLE              = "Consulta Etiquetas / Docas"
         HEIGHT             = 13.54
         WIDTH              = 90
         MAX-HEIGHT         = 28.21
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-concom /* Consulta Etiquetas / Docas */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-concom w-concom
ON WINDOW-CLOSE OF w-concom /* Consulta Etiquetas / Docas */
DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL btn-parametro w-concom
ON CHOOSE OF btn-parametro IN FRAME f-cad /* Button 1 */
DO:
  RUN esdlg/d02es049.w (INPUT-OUTPUT i-tp-consulta, INPUT-OUTPUT c-doca, OUTPUT p-row-etiqueta). 

  IF i-tp-consulta = 1 THEN DO:
     RUN pi-reposiciona-query IN h_q01es049 (INPUT p-row-etiqueta).
     RUN enable-folder-page IN h_folder ( INPUT 1).
     RUN disable-folder-page IN h_folder ( INPUT 2).
     RUN pi-mostra-etiqueta IN h_v02es049.
     RUN select-page IN THIS-PROCEDURE ( 1 ).
  END.
  ELSE DO.
     RUN enable-folder-page IN h_folder ( INPUT 2).
     RUN disable-folder-page IN h_folder ( INPUT 1).
     RUN pi-atribui-doca IN h_b04es049 (INPUT c-doca). 
     RUN pi-totaliza-doca IN h_b04es049 (INPUT c-doca). 
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'FOLDER-LABELS = ':U + 'Etiqueta|Doca' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
       RUN set-position IN h_folder ( 2.75 , 2.00 ) NO-ERROR.
       RUN set-size IN h_folder ( 11.50 , 88.14 ) NO-ERROR.
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esvwr/v02es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_v02es049 ).
       RUN set-position IN h_v02es049 ( 3.75 , 2.00 ) NO-ERROR.
       ...
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  '':U ,
             OUTPUT h_q01es049 ).
       RUN set-position IN h_q01es049 ( 1.25 , 55.00 ) NO-ERROR.
       /* Size in UIB:  ( 1.63 , 7.72 ) */

       /* Links to SmartViewer h_v02es049. */
       RUN add-link IN adm-broker-hdl ( h_q01es049 , 'Record':U , h_v02es049 ).

       /* Links to SmartQuery h_q01es049. */
       RUN add-link IN adm-broker-hdl ( h_p-exihel , 'State':U , h_q01es049 ).
    ...
    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b04es049.w':U ,
             INPUT  FRAME f-cad:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b04es049 ).
       RUN set-position IN h_b04es049 ( 4.25 , 3.00 ) NO-ERROR.
       /* Size in UIB:  ( 9.83 , 86.00 ) */
...
------------------------------------------------------------------------------*/
  ENABLE btn-parametro RECT-1 rt-button 
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "ESSP0144" "2.04.00.000"}
