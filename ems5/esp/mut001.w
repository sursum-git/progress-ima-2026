...
&Scoped-Define ENABLED-OBJECTS rt-button edContas fiDtInicial fiDtFinal ~
btBuscar 
&Scoped-Define DISPLAYED-OBJECTS edContas fiDtInicial fiDtFinal 
...
DEFINE VARIABLE edContas AS CHARACTER 
     VIEW-AS EDITOR NO-WORD-WRAP SCROLLBAR-HORIZONTAL SCROLLBAR-VERTICAL
     SIZE 81 BY 5 NO-UNDO.
...
DEFINE FRAME f-cad
     edContas AT ROW 3.75 COL 4 NO-LABEL WIDGET-ID 4
     fiDtInicial AT ROW 9.17 COL 12 COLON-ALIGNED WIDGET-ID 10
     fiDtFinal AT ROW 9.25 COL 69 COLON-ALIGNED WIDGET-ID 12
     btBuscar AT ROW 12.75 COL 3 WIDGET-ID 6
     "Contas Cont beis a Serem Consideradas no Mutuo(separadas por virgula)" VIEW-AS TEXT
          SIZE 70 BY 1.25 AT ROW 2.5 COL 4 WIDGET-ID 8
...
DO:
  ...
  FOR EACH movto_tit_ap
      ...
      ASSIGN deValor = 0.
      FIND FIRST val_movto_ap_correc_val OF movto_tit_ap 
           WHERE ind_forma_conver = 'multimoeda' NO-LOCK NO-ERROR.
     IF AVAIL val_movto_ap_correc_val THEN DO:
        ASSIGN deValor = val_movto_ap_correc_val.val_movto_ap.
     END.
      DISP movto_tit_ap WITH 1 COL WIDTH 550 TITLE "movto ap".
      IF movto_tit_ap.log_ctbz_aprop_ctbl = YES THEN DO:
          FOR EACH aprop_ctbl_ap OF movto_tit_ap 
              WHERE aprop_ctbl_ap.log_ctbz_aprop_ctbl = YES NO-LOCK:
             DISP aprop_ctbl_ap WITH 1 COL WIDTH 550.
              /*WHERE LOOKUP(aprop_ctbl_ap.cod_cta_ctbl,edContas:SCREEN-VALUE IN FRAME {&FRAME-NAME}) > 0 NO-LOCK NO-ERROR.*/
             IF NOT AVAIL val_movto_ap_correc_val  THEN
                ASSIGN deValor = aprop_ctbl_ap.val_aprop_ctbl.
             CREATE tt.
             ASSIGN tt.cod_empresa              = movto_tit_ap.cod_empresa
                    tt.data_transacao           = aprop_ctbl_ap.dat_transacao
                    tt.valor                    = IF aprop_ctbl_ap.ind_natur_lancto_ctbl = "CR" THEN deValor * -1 ELSE deValor
                    tt.cod_modulo               = "APB"
                    ...
                    /*FIND FIRST compl_movto_pagto OF movto_tit_ap NO-LOCK NO-ERROR.
                    IF AVAIL compl_movto_pagto THEN 
                    MESSAGE 
                     tt.num_documento           SKIP
                     tt.cod_emitente            SKIP
                     tt.cod_serie               SKIP    
                     tt.cod_parcela             SKIP     
                     tt.cod_especie             SKIP    
                   val_cotac_indic_econ    SKIP
                  compl_movto_pagto.val_movto_ap SKIP
  ...
  OS-COMMAND SILENT  VALUE(" start excel /n t:\especificos\excel\mutuo.xls").
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       ...
       RUN adjust-tab-order IN adm-broker-hdl ( h_p-exihel ,
             edContas:HANDLE IN FRAME f-cad , 'BEFORE':U ).
    END. /* Page 0 */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _PROCEDURE buscarParametro w-livre 
...
/*------------------------------------------------------------------------------
  ...
  Notes:       
------------------------------------------------------------------------------*/
FIND FIRST im-param NO-LOCK NO-ERROR.
...
------------------------------------------------------------------------------*/
  DISPLAY edContas fiDtInicial fiDtFinal 
      WITH FRAME f-cad IN WINDOW w-livre.
  ENABLE rt-button edContas fiDtInicial fiDtFinal btBuscar 
      WITH FRAME f-cad IN WINDOW w-livre.
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "mut001" "9.99.99.999"}
