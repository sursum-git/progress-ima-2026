...
*******************************************************************************/
{include/i-prgvrs.i ESFT0013 2.04.00.000}
...
&Scoped-Define ENABLED-OBJECTS RECT-16 RECT-7 RECT-9 rs-destino bt-arquivo ~
...
DEFINE VARIABLE fi-arq-fatura AS CHARACTER FORMAT "X(45)":U 
     LABEL "Faturas" 
     VIEW-AS FILL-IN 
...
DEFINE VARIABLE fi-cod-rep1 AS INTEGER FORMAT ">>>>9" INITIAL 0 
     LABEL "Representante1":R16 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 TOOLTIP "C¢digo do representante (Hermantex)." NO-UNDO.

DEFINE VARIABLE fi-cod-rep2 AS INTEGER FORMAT ">>>>9" INITIAL 0 
     LABEL "Representante2":R16 
     VIEW-AS FILL-IN 
     SIZE 6 BY .88 TOOLTIP "C¢digo do representante (Hermantex)." NO-UNDO.
...
DEFINE VARIABLE fi-fin-serie AS CHARACTER FORMAT "x(5)" 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 TOOLTIP "SÇrie final." NO-UNDO.
...
DEFINE VARIABLE fi-ini-serie AS CHARACTER FORMAT "x(5)" 
     LABEL "SÇrie":R7 
     VIEW-AS FILL-IN 
     SIZE 7 BY .88 TOOLTIP "SÇrie inicial." NO-UNDO.

DEFINE VARIABLE fi-nome-rep1 AS CHARACTER FORMAT "x(40)" 
     VIEW-AS FILL-IN 
     SIZE 47.86 BY .88 NO-UNDO.

DEFINE VARIABLE fi-nome-rep2 AS CHARACTER FORMAT "x(40)" 
     VIEW-AS FILL-IN 
     SIZE 47.86 BY .88 NO-UNDO.
...
DEFINE FRAME f-pg-par
     fi-arq-fatura AT ROW 2.33 COL 9.72 COLON-ALIGNED
     tg-impr-param AT ROW 2.33 COL 53
     to-enviar-e-mail AT ROW 4.21 COL 3.86
     fi-assunto-e-mail AT ROW 6.04 COL 12.29 COLON-ALIGNED
     fi-texto-e-mail AT ROW 7.04 COL 13 NO-LABEL
     "Arquivo de sa°da a ser gerado" VIEW-AS TEXT
          SIZE 21 BY .54 AT ROW 1.46 COL 9.29
     " Dados da Mensagem" VIEW-AS TEXT
          SIZE 16 BY .75 AT ROW 5.21 COL 6
     RECT-13 AT ROW 5.54 COL 3.86
     RECT-17 AT ROW 1.25 COL 1
     RECT-21 AT ROW 1.75 COL 3.86
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 2.71
         SIZE 77.14 BY 10.92
...
DEFINE FRAME f-pg-sel
     ...
     fi-fin-cod-estabel AT ROW 1.92 COL 43 COLON-ALIGNED NO-LABEL
     fi-ini-serie AT ROW 2.92 COL 19 COLON-ALIGNED HELP
          "SÇrie da nota fiscal"
     fi-fin-serie AT ROW 2.92 COL 43 COLON-ALIGNED HELP
          "SÇrie da nota fiscal" NO-LABEL
     cb-ini-esp-docto AT ROW 3.92 COL 19 COLON-ALIGNED
     cb-fin-esp-docto AT ROW 3.92 COL 43 COLON-ALIGNED NO-LABEL
     fi-ini-dt-emissao AT ROW 4.92 COL 19 COLON-ALIGNED
     fi-fin-dt-emissao AT ROW 4.92 COL 43 COLON-ALIGNED NO-LABEL
     fi-cod-rep1 AT ROW 5.92 COL 19 COLON-ALIGNED HELP
          "C¢digo do representante"
     fi-nome-rep1 AT ROW 5.92 COL 25.14 COLON-ALIGNED HELP
          "Nome do Representante" NO-LABEL
     fi-cod-rep2 AT ROW 6.92 COL 19 COLON-ALIGNED HELP
          "C¢digo do representante"
     fi-nome-rep2 AT ROW 6.92 COL 25.14 COLON-ALIGNED HELP
          "Nome do Representante" NO-LABEL
     IMAGE-10 AT ROW 2.92 COL 41.57
     IMAGE-19 AT ROW 4.92 COL 31.57
     IMAGE-20 AT ROW 4.92 COL 41.57
     IMAGE-29 AT ROW 1.92 COL 31.43
     IMAGE-30 AT ROW 1.92 COL 41.57
     IMAGE-7 AT ROW 3.92 COL 31.57
     IMAGE-8 AT ROW 3.92 COL 41.57
     IMAGE-9 AT ROW 2.92 COL 31.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Faturamento para integraá∆o com a Hermantex"
         COLUMN             = 17
         ROW                = 9.71
         HEIGHT             = 15.04
         WIDTH              = 80.72
...
/* SETTINGS FOR FRAME f-pg-sel
   Custom                                                               */
/* SETTINGS FOR FILL-IN fi-nome-rep1 IN FRAME f-pg-sel
   NO-ENABLE                                                            */
/* SETTINGS FOR FILL-IN fi-nome-rep2 IN FRAME f-pg-sel
   NO-ENABLE                                                            */
...
ON END-ERROR OF w-relat /* Faturamento para integraá∆o com a Hermantex */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Faturamento para integraá∆o com a Hermantex */
DO:
...
&Scoped-define SELF-NAME fi-cod-rep1
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep1 w-relat
ON ENTRY OF fi-cod-rep1 IN FRAME f-pg-sel /* Representante1 */
DO:
  IF SELF:SCREEN-VALUE <> "" THEN DO:
     FIND repres WHERE repres.cod-rep = INPUT FRAME {&FRAME-NAME} fi-cod-rep1 NO-LOCK NO-ERROR.
     IF NOT AVAIL repres THEN DO:
        MESSAGE "Representante inv†lido." VIEW-AS ALERT-BOX.
        RETURN NO-APPLY.
     END.
     ASSIGN fi-nome-rep1:SCREEN-VALUE IN FRAME {&FRAME-NAME} = repres.nome.
  END.
END.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep1 w-relat
ON LEAVE OF fi-cod-rep1 IN FRAME f-pg-sel /* Representante1 */
DO:
  FIND repres WHERE repres.cod-rep = INPUT FRAME {&FRAME-NAME} fi-cod-rep1 NO-LOCK NO-ERROR.
  IF NOT AVAIL repres THEN DO:
     MESSAGE "Representante inv†lido." VIEW-AS ALERT-BOX.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep1 w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-cod-rep1 IN FRAME f-pg-sel /* Representante1 */
DO:
...
&Scoped-define SELF-NAME fi-cod-rep2
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep2 w-relat
ON ENTRY OF fi-cod-rep2 IN FRAME f-pg-sel /* Representante2 */
DO:
  IF SELF:SCREEN-VALUE <> "" THEN DO:
     FIND repres WHERE repres.cod-rep = INPUT FRAME {&FRAME-NAME} fi-cod-rep1 NO-LOCK NO-ERROR.
     IF NOT AVAIL repres THEN DO:
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep2 w-relat
ON LEAVE OF fi-cod-rep2 IN FRAME f-pg-sel /* Representante2 */
DO:
  FIND repres WHERE repres.cod-rep = INPUT FRAME {&FRAME-NAME} fi-cod-rep1 NO-LOCK NO-ERROR.
  IF NOT AVAIL repres THEN DO:
     MESSAGE "Representante inv†lido." VIEW-AS ALERT-BOX.
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-cod-rep2 w-relat
ON MOUSE-SELECT-DBLCLICK OF fi-cod-rep2 IN FRAME f-pg-sel /* Representante2 */
DO:
  {include/zoomvar.i &prog-zoom = adzoom/z01ad229.w
                     &campo     = fi-cod-rep1
                     &campozoom = cod-rep
...
&Scoped-define SELF-NAME fi-ini-dt-emissao
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-ini-dt-emissao w-relat
ON LEAVE OF fi-ini-dt-emissao IN FRAME f-pg-sel /* Emiss∆o */
DO:
  assign fi-arq-fatura:SCREEN-VALUE IN FRAME f-pg-par = "spool/fat" + 
          string(year(INPUT FRAME {&FRAME-NAME} fi-ini-dt-emissao),"9999") +
          string(month(INPUT FRAME {&FRAME-NAME} fi-ini-dt-emissao),"99") +
          string(day(INPUT FRAME {&FRAME-NAME} fi-ini-dt-emissao),"99") + 
          ".txt".
END.
...
ON MOUSE-SELECT-DBLCLICK OF fi-ini-serie IN FRAME f-pg-sel /* SÇrie */
DO:
  {include/zoomvar.i &prog-zoom = inzoom/z01in407.w
                     &campo     = fi-ini-serie
                     &campozoom = serie
...
DO:
  IF INPUT FRAME {&frame-name} to-enviar-e-mail = YES THEN DO:
     ...
     ASSIGN fi-assunto-e-mail:SCREEN-VALUE IN FRAME f-pg-par = "Arquivo de Pedidos Faturados".

     ASSIGN fi-texto-e-mail:SCREEN-VALUE IN FRAME f-pg-par = 
            "Segue anexo Arquivo de Pedidos faturados no per°odo de #PER-INI a #PER-FIN." +
...
{utp/ut9000.i "ESFT0013" "2.04.00.000"}
...
DO  ON ERROR   UNDO MAIN-BLOCK, LEAVE MAIN-BLOCK
    ...
    fi-cod-rep1:LOAD-MOUSE-POINTER("image/lupa.cur") IN FRAME f-pg-sel.
    fi-cod-rep2:LOAD-MOUSE-POINTER("image/lupa.cur") IN FRAME f-pg-sel.
    ...
    ASSIGN fi-ini-cod-estabel:SCREEN-VALUE IN FRAME f-pg-sel = "2"
           ...
           fi-ini-serie:SCREEN-VALUE IN FRAME f-pg-sel = "3"
           fi-fin-serie:SCREEN-VALUE IN FRAME f-pg-sel = "3"
           ...
           fi-ini-dt-emissao:SCREEN-VALUE IN FRAME f-pg-sel = STRING(TODAY - 1)
           fi-fin-dt-emissao:SCREEN-VALUE IN FRAME f-pg-sel = STRING(TODAY - 1)
           fi-cod-rep1:SCREEN-VALUE IN FRAME f-pg-sel = "14".
           fi-cod-rep2:SCREEN-VALUE IN FRAME f-pg-sel = "119".

    FIND repres WHERE repres.cod-rep = int(fi-cod-rep1:SCREEN-VALUE IN FRAME f-pg-sel)
                NO-LOCK NO-ERROR.
    IF AVAIL repres THEN
       ASSIGN fi-nome-rep1:SCREEN-VALUE IN FRAME f-pg-sel = repres.nome.
    ELSE
       ASSIGN fi-nome-rep1:SCREEN-VALUE IN FRAME f-pg-sel = "".
    FIND repres WHERE repres.cod-rep = int(fi-cod-rep2:SCREEN-VALUE IN FRAME f-pg-sel)
                NO-LOCK NO-ERROR.
    IF AVAIL repres THEN
       ASSIGN fi-nome-rep2:SCREEN-VALUE IN FRAME f-pg-sel = repres.nome.
    ELSE
       ASSIGN fi-nome-rep2:SCREEN-VALUE IN FRAME f-pg-sel = "".

    assign fi-arq-fatura:SCREEN-VALUE IN FRAME f-pg-par = SESSION:TEMP-DIRECTORY + 
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-arq-fatura tg-impr-param to-enviar-e-mail fi-texto-e-mail 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE RECT-13 RECT-17 RECT-21 fi-arq-fatura tg-impr-param to-enviar-e-mail 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ...
  DISPLAY fi-ini-cod-estabel fi-fin-cod-estabel fi-ini-serie fi-fin-serie 
          ...
          fi-cod-rep1 fi-nome-rep1 fi-cod-rep2 fi-nome-rep2 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE fi-ini-cod-estabel fi-fin-cod-estabel fi-ini-serie fi-fin-serie 
         cb-ini-esp-docto cb-fin-esp-docto fi-ini-dt-emissao fi-fin-dt-emissao 
         fi-cod-rep1 fi-cod-rep2 IMAGE-10 IMAGE-19 IMAGE-20 IMAGE-29 IMAGE-30 
         IMAGE-7 IMAGE-8 IMAGE-9 RECT-24 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
...
do on error undo, return error on stop  undo, return error:
    ...
    /*:T Coloque aqui a l¢gica de gravaá∆o dos demais campos que devem ser passados
     ...
     ASSIGN tt-param.estab-ini      = input frame f-pg-sel fi-ini-cod-estabel
            ...
            tt-param.cod-rep1       = INPUT FRAME f-pg-sel fi-cod-rep1
            tt-param.cod-rep2       = INPUT FRAME f-pg-sel fi-cod-rep2
            tt-param.arq-fatura     = input frame f-pg-par fi-arq-fatura
    ...
    {include/i-rprun.i esrp/esft0013rp.p} 
