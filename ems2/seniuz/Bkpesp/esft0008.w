...
*******************************************************************************/
{include/i-prgvrs.i ESFT0008 2.04.00.000}
...
define temp-table tt-param  no-undo
       ...
       FIELD inc-topazio       AS LOG FORMAT "Sim/N∆o"
       FIELD inc-prata         AS LOG FORMAT "Sim/N∆o"
...
DEFINE VARIABLE fi-qtd-clientes AS INTEGER FORMAT ">>>9":U INITIAL 100 
     LABEL "Quantidade de Clientes a Imprimir (10-1000)" 
     VIEW-AS FILL-IN 
     SIZE 5 BY .79 TOOLTIP "Quantidade de Clientes a imprimir (10 a 100)" NO-UNDO.
...
DEFINE VARIABLE tg-inc-prata AS LOGICAL INITIAL no 
     LABEL "Incluir parcela Prata" 
     VIEW-AS TOGGLE-BOX
     SIZE 17 BY .83 TOOLTIP "Incluir parcela Prata?" NO-UNDO.

DEFINE VARIABLE tg-inc-topazio AS LOGICAL INITIAL no 
     LABEL "Incluir parcela Top†zio" 
     VIEW-AS TOGGLE-BOX
     SIZE 18 BY .88 TOOLTIP "Incluir parcela Top†zio?" NO-UNDO.
...
DEFINE VARIABLE fi-dt-acumulo AS DATE FORMAT "99/99/9999" INITIAL 01/01/001 
     LABEL "Dt Ac£mulo":R12 
     VIEW-AS FILL-IN 
     SIZE 10 BY .88 TOOLTIP "Data inicial de ac£mulo" NO-UNDO.
...
DEFINE VARIABLE fi-fin-ge-codigo AS INTEGER FORMAT ">9" INITIAL 59 
     VIEW-AS FILL-IN 
     SIZE 3 BY .79 TOOLTIP "Grupo de Estoque final" NO-UNDO.
...
DEFINE VARIABLE fi-ini-ge-codigo AS INTEGER FORMAT ">9" INITIAL 50 
...
DEFINE FRAME f-pg-sel
     ...
     fi-ini-ge-codigo AT ROW 5.54 COL 15.29 COLON-ALIGNED HELP
          "Grupo de estoque a que pertence o item"
     fi-fin-ge-codigo AT ROW 5.54 COL 48.29 COLON-ALIGNED HELP
          "Grupo de estoque a que pertence o item" NO-LABEL
     fi-dt-acumulo AT ROW 6.54 COL 15.29 COLON-ALIGNED HELP
          "Data em que o pedido foi emitido pelo representante/cliente"
     IMAGE-1 AT ROW 2.54 COL 27.86
     IMAGE-2 AT ROW 2.54 COL 46.72
     IMAGE-3 AT ROW 3.54 COL 27.86
     IMAGE-4 AT ROW 4.54 COL 27.86
     IMAGE-6 AT ROW 3.54 COL 46.72
     IMAGE-7 AT ROW 4.54 COL 46.72
     IMAGE-8 AT ROW 5.54 COL 27.86
     IMAGE-9 AT ROW 5.54 COL 46.72
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3.72 ROW 3.21
         SIZE 75.86 BY 10.25
         FONT 1.

DEFINE FRAME f-pg-par
     fi-qtd-clientes AT ROW 2.5 COL 41.43 COLON-ALIGNED
     tg-inc-prata AT ROW 5.96 COL 28
     tg-inc-topazio AT ROW 4.96 COL 28
     to-imp-param AT ROW 8.21 COL 29.86
     RECT-27 AT ROW 7.63 COL 6
     RECT-28 AT ROW 4.67 COL 6
     RECT-29 AT ROW 1.71 COL 6
    WITH 1 DOWN NO-BOX KEEP-TAB-ORDER OVERLAY 
         SIDE-LABELS NO-UNDERLINE THREE-D 
         AT COL 3 ROW 3
         SIZE 77.14 BY 10.5
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-relat ASSIGN
         HIDDEN             = YES
         TITLE              = "Os maiores Clientes do Faturamento"
         HEIGHT             = 15
         WIDTH              = 81.14
         MAX-HEIGHT         = 28.63
         MAX-WIDTH          = 146.29
...
ON END-ERROR OF w-relat /* Os maiores Clientes do Faturamento */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-relat w-relat
ON WINDOW-CLOSE OF w-relat /* Os maiores Clientes do Faturamento */
DO:
...
&Scoped-define SELF-NAME fi-qtd-clientes
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL fi-qtd-clientes w-relat
ON LEAVE OF fi-qtd-clientes IN FRAME f-pg-par /* Quantidade de Clientes a Imprimir (10-1000) */
DO:
  IF INPUT FRAME {&FRAME-NAME} fi-qtd-clientes < 10 or
     INPUT FRAME {&FRAME-NAME} fi-qtd-clientes > 1000 THEN DO:
     MESSAGE "Esse parÉmetro deve estar entre 10 e 1000." VIEW-AS ALERT-BOX.
     ASSIGN fi-qtd-clientes:SCREEN-VALUE IN FRAME {&FRAME-NAME} = "1000".
     APPLY 'entry' TO fi-qtd-clientes IN FRAME {&FRAME-NAME}.
     RETURN NO-APPLY.
  END.
END.
...
{utp/ut9000.i "ESFT0008" "2.04.00.000"}
...
------------------------------------------------------------------------------*/
  ...
  DISPLAY fi-qtd-clientes tg-inc-prata tg-inc-topazio to-imp-param 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ENABLE fi-qtd-clientes tg-inc-prata tg-inc-topazio to-imp-param RECT-27 
         RECT-28 RECT-29 
      WITH FRAME f-pg-par IN WINDOW w-relat.
  ...
  DISPLAY fi-cod-estabel fi-nome-estabel fi-ini-dt-emissao fi-fin-dt-emissao 
          ...
          fi-ini-ge-codigo fi-fin-ge-codigo fi-dt-acumulo 
      WITH FRAME f-pg-sel IN WINDOW w-relat.
  ENABLE fi-cod-estabel fi-ini-dt-emissao fi-fin-dt-emissao fi-ini-cod-emitente 
         fi-fin-cod-emitente fi-ini-cod-rep fi-fin-cod-rep fi-ini-ge-codigo 
         fi-fin-ge-codigo fi-dt-acumulo IMAGE-1 IMAGE-2 IMAGE-3 IMAGE-4 IMAGE-6 
...
do on error undo, return error on stop  undo, return error:
    ...
    assign tt-param.usuario          = c-seg-usuario
           ...
           tt-param.dt-acumulo       = INPUT FRAME f-pg-sel fi-dt-acumulo 
           tt-param.inc-topazio      = INPUT FRAME f-pg-par tg-inc-topazio
           tt-param.inc-prata        = INPUT FRAME f-pg-par tg-inc-prata
           tt-param.qtd-clientes     = INPUT FRAME f-pg-par fi-qtd-clientes
    ...
    {include/i-rprun.i esrp/esft0008rp.p}
