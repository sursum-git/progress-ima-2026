...
*******************************************************************************/
{include/i-prgvrs.i Z01ES083 2.04.00.000}
...
DEFINE VARIABLE h_b01es083 AS HANDLE NO-UNDO.
DEFINE VARIABLE h_b02es083 AS HANDLE NO-UNDO.
...
DEFINE FRAME f-zoom
     bt-implantar AT ROW 15 COL 1.57
...
IF SESSION:DISPLAY-TYPE = "GUI":U THEN
  CREATE WINDOW w-pesquisa ASSIGN
         HIDDEN             = YES
         TITLE              = "Pesquisa de Etiquetas de Pano Cru"
         HEIGHT             = 17
         WIDTH              = 90
         MAX-HEIGHT         = 17
         MAX-WIDTH          = 90
...
ON END-ERROR OF w-pesquisa /* Pesquisa de Etiquetas de Pano Cru */
...
&ANALYZE-SUSPEND _UIB-CODE-BLOCK _CONTROL w-pesquisa w-pesquisa
ON WINDOW-CLOSE OF w-pesquisa /* Pesquisa de Etiquetas de Pano Cru */
DO:
  ...
  CASE adm-current-page: 
    ...
    WHEN 0 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             ...
             INPUT  'FOLDER-LABELS = ':U + 'C¢digo|Data Trans' + ',
                     FOLDER-TAB-TYPE = 1':U ,
             OUTPUT h_folder ).
    ...
    WHEN 1 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b01es083.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b01es083 ).
       RUN set-position IN h_b01es083 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.14 ) */

       /* Links to SmartBrowser h_b01es083. */
       RUN add-link IN adm-broker-hdl ( h_b01es083 , 'State':U , THIS-PROCEDURE ).

    END. /* Page 1 */

    WHEN 2 THEN DO:
       RUN init-object IN THIS-PROCEDURE (
             INPUT  'esbrw/b02es083.w':U ,
             INPUT  FRAME f-zoom:HANDLE ,
             INPUT  'Layout = ':U ,
             OUTPUT h_b02es083 ).
       RUN set-position IN h_b02es083 ( 2.50 , 2.00 ) NO-ERROR.
       /* Size in UIB:  ( 12.25 , 88.00 ) */

       /* Links to SmartBrowser h_b02es083. */
       RUN add-link IN adm-broker-hdl ( h_b02es083 , 'State':U , THIS-PROCEDURE ).
...
------------------------------------------------------------------------------*/
  ...
  {utp/ut9000.i "Z01ES083" "2.04.00.000"}
